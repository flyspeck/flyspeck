...
\begin{enumerate}\wasitemize 
\item \case{extreme~edge~\ref{f:ee}} For every $\v\in V$ and every $\w\in E(\v)$, the edge
$\{\v,\w\}\in E$ is $S$-extremal.
\item \case{flat~exists~\ref{f:fe}} For every $\v\in V$, at least one of $\v_1,\v_2,\v_3,\v_4$ is flat, or  $r+s\le 4$ and
$\norm{\u}{\w}=2$ for all $\{u,w\}\in E$.

%then $r+s\le 4$ and $\norm{\u}{\w}=2$ for all $\{u,w\}\in E$.
% $r+s=4$. %, and $\norm{\u}{\w}\in\{ 2,2\hm}$ for all $\{\u,\w\}\in E$.
\item \case{no~triple~flat~\ref{f:ntf}} For every $\v\in V$, at least one of $\v_0$, $\v_1$, and $\v_2$
is not flat.
\item \case{balance~\ref{f:b}} For every $\v\in V$, if $\v$ is flat and if $\{\u,\v\},\{\w,\v\}\in
E\setminus S$, then
\[ 
\norm{\u}{\v} = \norm{\w}{\v}.
\] 
\item \case{s~flat~\ref{f:sf}} For every $\v\in V$, if $\v_1$ and $\v_2$ are both flat, then 
\[ S\cap
\{\{\v_0,\v_1\},\{\v_1,\v_2\},\{\v_2,\v_3\}\} =
\emptyset.\] 
\item \case{flat~middle~\ref{f:fm}} For every $\v\in V$, if $\v_1$ and $\v_2$ are both flat, then
\[ 
\norm{\v_1}{\v_2} = 2.
\] 
\item \case{minimal~node~\ref{f:mn}} For every $\v\in V$, either $\normo{\v}=2$
  or there exists $\w\in E(\v)$ such that $\{\v,\w\}$ is $S$-minimal.
\item \case{minimal~node~flat~\ref{f:mmf}} For every $\v\in V$, if $\v$ is flat and if $\{\u,\v\}\in
E\setminus S$, then $\norm{\u}{\v}=2$ or $\normo{\v}=2$.
\item \case{flat~extremal~\ref{f:fext}} For every $\v\in V$, if $\v_1$ and $\v_2$
...
%\quad\text{ or }\quad\normo{\v_2}\in \{2,2\hm\}.
%\] 
\item \case{flat~count~\ref{f:fc}} There are at least three nonflat nodes of $V$.
\item \case{card~\ref{f:c}} %$k=\card(F)$,
Let      $s=\card(S)$ and $r=\card(E) - s = \card(F)-s$.  Then
\[ 0\le s \le 3\textand 3-s \le r \le 6 -
2s.\] 
\item \case{s~minimal~\ref{f:sm}} Either $r+2s = 6$, or every edge of $E$ is $S$-minimal.
\item \case{triangle~free~\ref{f:tf}} $\op{card}(F) \ne 3$.
\item \case{quadrilateral~free~\ref{f:qf}} $\op{card}(F) \ne 4$.
\item \case{pentagon~free~\ref{f:pf}} $\op{card}(F) \ne 5$.
\item \case{hexagon~free~\ref{f:hf}} $\op{card}(F) \ne 6$.
\end{enumerate}\wasitemize 
\end{lemma}
...
\begin{lemma}[card]\guid{FPITROS}\rating{50}\label{f:c}
...
\begin{lemma}[no~triple~flat]\guid{TESVAFW}\rating{80}\label{f:ntf}
...
\begin{lemma}[flat~count]\guid{GOKZLRP}\rating{40}\label{f:fc}
...
\begin{lemma}[s~flat]\guid{SDCCMGA}\rating{100}\label{f:sf}
...
\begin{lemma}[extreme~edge]\guid{BAWWPPB}\rating{900}\label{f:ee}
...
$\Delta = \Delta(4,4,4,a^2,b^2,c^2)$.  We may restrict $a$ further so that
$a\le 3.8$.
...
a \cc{2065952723}{This is a calculation of derivatives.} 
...
\begin{lemma}[minimal~node]\guid{PZFSEVR}\rating{200}\label{f:mn}
...
\begin{lemma}[minimal~node~flat]\guid{JLXMEJN}\rating{350}\label{f:mnf}
Every minimal fan $(V,E,F,S)$ has the feature \case{minimal~node
flat}.
...
\begin{lemma}[balance]\guid{TPKKQOL}\rating{350}\label{f:b}
...
\begin{lemma}[flat~middle]\guid{CFJSRQH}\rating{450}\label{f:fm}
...
\begin{lemma}[flat~extremal]\guid{OUCPLRI}\rating{1600}\label{f:fext}
...
\begin{lemma}[s~minimal]\guid{HJXLQOT}\rating{100} \label{f:sm}
...
\begin{lemma}[triangle~free]\guid{OMKYNLT}\rating{200}\label{lemma:triangle-free}\label{f:tf}
...
\begin{lemma}[flat~exists]\guid{CMBZAOZ}\rating{500}\label{lemma:flat-exists}\label{f:fe}
...
by a \cc{2986512815}{calc:cc:qua}, the function $\tau$ as a function of
...
\begin{lemma}[quadrilateral~free]\guid{JNTEFVP}\rating{250}\label{lemma:quadrilateral-free}\label{f:qf}
...
\begin{lemma}[pentagon~free]\guid{PQFYWHW}\rating{250}\label{lemma:pentagon-free}\label{f:pf}
...
\begin{lemma}[hexagon~free]\guid{GYQVFXJ}\rating{200}\label{lemma:hexagon-free}\label{f:hf}
