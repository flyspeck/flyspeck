...
This density is $\pi/\sqrt{18}$.\footnote{Research supported by NSF grant 
0804189.}
...
%10->4, tch, Oct 11, 2008 
thousands of executions of a multitude of independent
...
original source code\footnote{Ferguson does not use a labeling scheme.
As a result his computations are more difficult to recover.}.  To
...
floating point numbers and rounding modes directly from the point of view
of verification. The first is that
...
want.'' On the other hand, the \texttt{volatile} keyword\footnote{\texttt{volatile} is not
used in the original implementation.} is not defined by
the C language definition~\cite{Kernighan:1988:C}, meaning that a C
program using \texttt{volatile} is undefined by the
...
organize the computations in a unified framework.  

The main computational tasks as we see them are:

\begin{itemize} 
\item Inequality experimentation (C)
\item Inequality verification (C, C++)
\item Graph generation (Java)
\item Graph visualization (Java)
\item Linear program generation (Mathematica)
\item Linear program bounding (CPLEX)
\item Linear program bound verification (Not implemented)
\end{itemize} 

\noindent Each task is labeled with the language of the
original implementation.  The final task, rigorous checking
of the bounds returned by linear programming libraries by
checking the dual variables, had not been done before.
This task is discussed in detail in Section~\ref{sec:lp}.
The desired features for the new implementation were:
...
have a meaning apart from the particular compiler used\footnote{The
language is not perfectly defined.  For example, the \texttt{int} type
is left unspecified, and can be implemented as either fixed or
arbitrary precision without deviating for the definition.  It is
however a significant improvement over most languages.}.  It has an
...
external libraries, e.g. linear program solvers, written in languages
other than SML with relative ease.  It has an expressive module
system, and thus it is possible to write the code with respect to
abstract libraries and arithmetics. This allows us, for instance, to
use multiple independent implementations of interval arithmetic such
as MPFI~\cite{Revol:2005:MPFI}, as well as our own implementation.

We have made significant progress toward our goals. Almost all the
code has been completely rewritten~\cite{McLaughlin:2008:KeplerCode}
in SML, and is executable from a single command-line program.
The remainder of this section gives some details about the status
of the current implementation.

\paragraph{\emph{Inequalities}}
...
\paragraph{\emph{Graphs}} The graph generator
...
There are also a number of linear programs used in the original proof
that do not correspond directly to graphs.  These will be added to
...
interpret.  % spread over Hell's half acre 
Now all the computations
are executed from the same source, with organized output.  In addition
to giving us added confidence that the original computations were
sound, we have a fairly complete suite of software support for the
Flyspeck project. We are now in the process of organizing and
reevaluating the thousands of computations upon which the proof
depends.
