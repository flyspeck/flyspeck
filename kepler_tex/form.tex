...
$\Omega(v)=\Omega(\Lambda,v)$\indy{Notation}{ZZZomega@$\Omega$} around a
...
Instead of this single inequality, we prove a collection of inequalities of the form
\begin{equation}\label{eqn:gpart}
\gamma^j(X) + a_i \dih^j(X) + b_i^j y_1 + c_i^j (y_2+y_3+y_5+y_6) + d_i^j > 0, \text{ for } X \in D_i^j, \text{ and } i \in I. 
\end{equation}
The simplex $X$ has spine of length $y_1$ and other edges $y_k$.  Also, $\dih^j(X)$ is the dihedral angle along the spine.
