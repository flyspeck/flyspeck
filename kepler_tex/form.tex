...
\indy{Index}{Kepler conjecture} packing problem.
\indy{Index}{packing problem}
...
%cell\indy{Index}{Voronoi cell} $\Omega(v,\Lmambda)$\indy{Greek}{ZZzomega@$\Omega(D)$}
...
function $\sigma:\op{CentPack}\to\ring{R}$ is called the
  ...
  \indy{Index}{scoring function}\indy{Index}{score}
...
following interpretation:  $\dtet$\indy{Greek}{ZZdeltatet@$\dtet$} is
...
Let $\doct$\indy{Greek}{ZZdeltaoct@$\doct$} is the packing density of a
...
We define\indy{Index}{point}\indy{Index}{pt} the constant
...
contravening centered packing},\indy{Index}{contravening} if
...
contravening centered packings exist.\indy{Index}{centered
...
properties is said to be {\it tame}. All \indy{Index}{tame} tame
hypermaps have been classified. There are several thousand, up to
isomorphism.  The list appears in \cite{web}.  We refer to this
list as the {\it archival list}\indy{Index}{archival list of hypermaps}
of hypermaps.
...
\begin{lemma}\guid{TZNWDKL}\label{lemma:claim-fcc-graph}
...
\begin{lemma}\guid{EELXEXE}\label{lemma:claim-sigma}
...
$G_{hcp}$, $G_{fcc}$, and $G_{pent}$,\indy{Index}{pentahedral prism} we
...
\indy{Index}{vertex} vertices}. The term `vertex' will be reserved for
this technical usage.  

%A packing is said to be {\it
%\indy{Index}{saturated} saturated\/} if for every $x\in\ring{R}^3$,
...
\indy{Index}{truncation parameter ($t_0=1.255$)} to be the constant
...
A {\it quasi-regular\/} triangle\indy{Index}{quasi-regular triangle} is
...
A \indy{Index}{simplex} simplex\indy{Index}{simplex} is a set of four
vertices.
%The
...
$\{v,w\}$ of a quarter its {\it \indy{Index}{diagonal} diagonal\/}. When
...
\end{definition}
\indy{Index}{quarter!strict} \indy{Index}{quarter!upright}
\indy{Index}{quarter!flat} \indy{Index}{quasi-regular}
\indy{Index}{quasi-regular!triangle} \indy{Index}{quasi-regular!tetrahedron}
...
%\begin{definition}[overlap]\indy{Index}{overlap} def:overlap
...
called a {\it quartered \indy{Index}{octahedron} octahedron}, if the
...
An {\it adjacent pair\indy{Index}{adjacent pair}} is a tuple
...
diagonal\indy{Index}{conflicting diagonal}} if
...
$\op{conv}^0\{v,v_2,v_4\}$. \indy{Index}{interior position}
\end{definition}

\begin{definition}[isolated~pair]
A quarter is said to be {\it isolated}\indy{Index}{isolated} if it is
...
we say that a vertex $v$ $(\ne v_1,v_2)$ is an {\it \indy{Index}{anchor}
...
\begin{lemma}\guid{CXRHOVG} \label{lemma:diags-engulf}
...
triangles in this set will be called {\it barriers}.\indy{Index}{barrier}
...
\begin{enumerate}
    ...
    quasi-regular, or\indy{Index}{quasi-regular!triangle}
    \item The triangle is a face of a simplex in the $Q$-system.
\end{enumerate}
\end{definition}

\begin{lemma}\guid{KGMSVRZ}\label{lemma:barrier-no-overlap} %DCG p43.
 If $b,b'\in \CalB$, then
...
Tarski arithmetic\tarf{tri-eta-small} and that the length $|w_1-w_2|$ is
greater than $2t_0$ by Tarski arithmetic\tarfE{2t0-doesnt-pass-through}.
...
symmetric.\indy{Index}{obstructed}
\end{definition}
...
$v$.\indy{Index}{V-cell}\indy{Index}{VZ@$VC(v)$ ($V$-cell)}
%
\end{definition}
...
\begin{lemma}\guid{NACLMKY}\label{lemma:Q-divide}
...
%  \indy{Index}{centered packing}
%  \indy{Index}{DZS@$\op{CentPack}$}
%  \indy{Index}{DZ@$D(\Lambda,v)$}
%\end{definition}
%
...
%
%\begin{lemma}\guid{HKNAULJ} There is a $V$-cell $\op{VC}(\Lambda,v)$ attached to each
...
%\end{lemma}
%
%\begin{lemma}\guid{UQIVVYS} \label{lemma:vor}
...
\end{definition}\indy{Index}{context (of a diagonal)}
...
%\indy{Index}{admissible} %there exists $d>0$ such that for every
...
%\end{definition}
%
%\begin{lemma}\guid{DCRSXZT} If $\op{VCM}_1,\ldots,\op{VCM}_r$ are admissible,
...
functions.
   ...
   \end{equation}
%
 \indy{Greek}{ZZcamma@$\Gamma$}
\end{definition}

\begin{lemma}\guid{XZZTPWF}\label{lemma:gammavol}
...
$\sigma(v,S)$.
%
 \indy{Index}{compressed}
 \indy{Index}{decompressed}
\end{definition}
...
Suppose $S$ is an upright quarter.\indy{Index}{quarter!upright}
...
\begin{lemma}\guid{QPCPUXC}\label{lemma:A1-cancel}
...
%
 \indy{Index}{score}
 \indy{Greek}{ZZsigma@$\sigma(\Lambda,v)$}
\end{definition}

Recall from Definition~\ref{def:pt} the constant $\pt\approx
0.05537$.  This constant is called a point.\indy{Index}{point}

\begin{lemma}\guid{QWGNFAD}\label{lemma:8pt-compat}
