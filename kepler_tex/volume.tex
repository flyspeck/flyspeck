...
\begin{definition}[vol,~measurable,~null]
...
\begin{lemma}[null set]\label{lemma:null}
...
\item \case{disjoint union} If $X$ and $Y$ are measurable sets, and if $X\cap
...
\begin{definition}[ball] The open ball $B(\p,r)$ with center $\p$
and radius $r$ is the set
\begin{displaymath}
\{ \q\in\ring{R}^3 \mid \norm{\p}{\q} < r.\}
\end{displaymath}
The closed ball $\bar B(\p,r)$ is
\begin{displaymath}
\{ \q\in\ring{R}^3 \mid \norm{\p}{\q} \le r.\}
\end{displaymath}
...
A set $C$ is $r$-\newterm{radial} at center $\p$ if  $C\subset B(\p,r)$
and if
$\p + \u \in C$ implies
$\p + t \u \in C$ for all $t$ satisfying $0 < \normo{\u} t < r$.
A set $C$ is \newterm{eventually radial} at center $\p$ if $C\cap B(\p,r)$ is
...
\item a rectangle 
\begin{displaymath}
\op{RECT}(a,b)=\{\u \mid a_i < u_i < b_i,\text { for } i=1,2,3.\}.
\end{displaymath}
