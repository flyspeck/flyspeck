...
\begin{definition}[fan datum]  A fan datum of size $k$ is a pair $(\v,I)$ where
$\v:\{0,\ldots,k-1\}\to \BB$ and $I\subset \{0,\ldots,k-1\}$ such that
\begin{itemize}
...
\end{itemize}
The set of fan data of size $k$ is a subspace of the topological space  $\BB^k\times 2^k$.
\end{definition}

\begin{lemma}\guid{}\rating{}
If $(\v,I)$ is a fan datum, then
$$
(V(\v),E(\v),F(\v),G(\v,I))
$$
...
\claim{If $(\v,I)$ is a fan datum, then $$(V(\v),E(\v))$$
is a fan.}
\case{V},~\case{origin}:  The set $V(\v)$ is finite by Lemma~\ref{XX}.  It contains $\v_0$.  
$\orz\not\in \BB$.

\case{independence}: $0 < \arc(2,\hm,\hm)\le \arc_V(\orz,\{\v_i,\v_{i+1}\}) \le \arc(\hm,2,2) < \pi$.
Hence $\v_i$ and $\v_{i+1}$ are independent.

\case{intersection}:  XX finish.
\end{proof}

\begin{lemma}\guid{}\rating{}
The space of fan data of size $k$ is a compact metric space.
\end{lemma}

\begin{proof}  $\BB^k\times 2^k$ is a compact metric space and the constraints are all closed conditions.
\end{proof}
