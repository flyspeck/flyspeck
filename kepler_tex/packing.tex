...
\begin{lemma}\guid{NJIUTIU}\label{lemma:omega-uv}
...
\begin{lemma}\guid{TEZFFSK}\label{lemma:dk-uv}  
Let $V$ be a saturated packing.
Let $\bu=[\u_0;\ldots]\in \bV(3)$ be such that $\dimaff R(\bu)=3$.  Pick
$k\le 3$  such that $\norm{\omega_i(\bu)}{\u_0} < \sqrt2$ for all $i\le k$.
...
\begin{proof}
  ...
  By Lemma~\ref{lemma:h-omega}, $h(d_k\bu) <\sqrt2$.  By
...
simplices are tuned for detail.  The Voronoi cells
are coarsely tuned, suitable for rough hewing.  Delaunay simplices
articulate the foreground, while Voronoi cells fill the background.
...
\begin{lemma}[]\guid{QZKSYKG}\label{lemma:cell-in-rogers}
  Let $V$ be a saturated packing, let $\bu\in \bV(3)$, and $k\in
  \{0,1,2,3,4\}$.  Then $\cell(\bu,k)$ is contained in the union of
  $R(\bv)$, as $\bv$ runs over all $k$-rearrangements of $\bu$.
\end{lemma}

\begin{proof}  If $k=0$ or $k=1$, 
then by definition, $\cell(\bu,k)\subset R(\bu)$.
If  $2\le k\le 4$, then the result follows from the description of the cells,
the definition of $k$-rearrangement,
and Lemma~\ref{lemma:Rconv}.
\end{proof}

\begin{lemma}[]\guid{DDZUPHJ}\label{lemma:cell-disjoint}
...
The cases $k=0$ and $k=1$ are essentially trivial, and are left to the
reader.
\end{proof}
...
\begin{lemma}[]\guid{AJRIPQN}\label{lemma:marchal-partition}
Let $V$ be a saturated packing, let $\bu,\bv\in \bV(3)$, and let
