...
\card(F)$;
\end{description}
A local fan $(V,E,F)$ is \newterm{convex} if the following additional conditions hold.
\begin{description}
...
\begin{lemma}[]\guid{CIZMRRH}\rating{ZZ} Every convex local fan is
...
A \newterm{deformation} of a convex local fan $(V,E,F)$ over an interval
...
choice of $\v\in V$.  Call $\op{per}$ the \newterm{perimeter} of the convex local fan.
