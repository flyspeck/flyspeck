...
\includegraphics[scale=0.8]{alan-turing-223x300.jpg}
  \caption{Alan Turing~(image source~\cite{Bew})}
\end{figure}
...
%% computer use in Catalan ... Bulletin article, TAUNO METS... on
%% laptop drive. May directory.  eliminated in Steiger article on
%% laptop drive.  May directory.
...
\end{figure}
% image from
% http://physchem.ox.ac.uk/~doye/jon/structures/Morse/paper/node3.html
...
% Mandelbrot uses the phrase ``visual inspection supported by computer
% experiments'',
...
%% Anthony Phillips, ``Turning a surface inside out'', Scientific
%% American, May 1966, 112--120.

%% [Le] Silvio Levy. Making Waves: A Guide to the Ideas Behind
%% "Outside In". AK Peters, Wellesley, MA, 1995.
...
\label{fig:costa}
\end{figure}
% http://www.math.kobe-u.ac.jp/DIVERSIONS/g1costa.jpg graphics source,
% XX permissions needed.
...
\label{fig:double}
\end{figure}
% http://math.berkeley.edu/~hutching/pub/bubbles.html XX permissions
% needed.
...
% XX wiki graphic, permissions probably OK.


\subsection{kissing numbers} % (transient use section)
...
% http://www.cwi.nl/news/2011/siagoptimization-prize-kissing-number-research,
% graphics source, permissions needed.
...
\label{fig:e8}
\end{figure}
% http://en.wikipedia.org/wiki/File:Dynkin_diagram_E8.svg, wiki
% source, XX use probably ok.  [Insert graphic of $E_8$ Dynkin
% diagram.]
...
%Poincar\'e has drawn a distinction
...
%distinction between the algorithmically solvable and the rest.  
...
separate execution of the algorithm)~\cite[p.~4]{HPSH}.
% the {\it Poincar\'e
%  principle} holds for that algorithm.  
...
B\"ohme and Nipkow  %have tested the success rates of the
%sledgehammer tactic on a large test suite.  They 
...
\]
In other words, it is enough to prove that the ideal
$I=(q_1,\ldots,q_n)$ has no common zero.  For this, we may use
...
%worked out by a long line of mathematicians, notably Noether, van der
%Waerden, and Bourbaki.  Nevertheless, we may expect that the
%reimplementation of abstract algebra by computer may soon come to
%represent the single largest transformation of the subject since
%category theory.  I can imagine a future generation of textbooks in
%abstract algebra for math majors that are based on a formal
%implementation.
...
the concepts working together well''~\cite{gonMF}. % p14. Mod. Form.
%
...
operation $( * )$.  
%It is an exercise in group theory to check that
...
%of results in homot
%theory might infuse back into homotopy theory as well.
%, as with the
...
% http://people.pwf.cam.ac.uk/mg262/GanesalingamMdis.pdf The Language
% of Mathematics.
...
mathematics -- the language divested all the informal side-remarks.
...
mathematician.
%As a first step toward imitation, we need a computer
...
%\princ{1. ``Don't use manual procedures.''~\cite{}}
...
%Today I needed in HOL Light the $i$th component of a vector $\v\in
% \ring{R}^n$. Some languages $v.i$, or $v[i]$, but I had forgotten
% that it was $v\$i$.  How do I look it up.  How did I look it up last
% time?  How did other users look it up.
...
%structures produce the best code.~\cite{}} % Carette
...
% Carette et al.  http://www.cas.mcmaster.ca/~carette/publications/pepm28p-carette.pdf  (Gem of a paper.)
...
of presentation, fidelity to printed texts, and pedagogy.\footnote{%
  ...
  \cite{DijkST}} % Dijks, Scientific Thought
% To me, this is
%akin to asking digital watches to have hands, phones to have rotary
%dials, or digital cameras to make shutter sounds.  
...
%
% For example, I have estimated that the formalization of a single
...
%  acceptance.'' \cite{DijkST}} %  Dijks, Scientific Thought 
% To me, this is
%akin to asking digital watches to have hands, phones to have rotary
...
%
% http://www.cs.utexas.edu/users/EWD/ewd04xx/EWD447.PDF Dijkstra, Sci. Thought.
The most pressing concern is to 
%Let the research into formal proof construction focus on the most
%pressing issue as a separate concern: 
...
% The interaction should be a ``control interface,'' deterministic and guidable.
%Sudden changes in state are difficult for the user.
...
others %should be established, distinguished from the others by
only publishes reliable results.

%  Half the time, Auslander 
...
%  http://en.wikipedia.org/wiki/List_of_published_incomplete_proofs
% http://mathoverflow.net/questions/35468
%  http://mathoverflow.net/questions/879
% Plemelj ``Details are in the book The Riemann-Hilbert Problem by Anosov
...
% XX http://www.numericana.com/answer/polyhedra.htm graphic source,
% need permission.
% [ Graphic pseudorhombo]

%  Pont in overflow, Rokhlin in wiki.
%  Pont in Ioan Mackenzie James, History of Topology p 567.
%  http://mathoverflow.net/questions/879 , calc examples.
%  give graphic from overflow.
...
%  Lagrange http://mathdl.maa.org/images/upload_library/22/Ford/Grabiner3-18.pdf
