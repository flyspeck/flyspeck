...
follows.\xfootnote{Linear relaxations appear again later in the
...
in Calculation~\ref{calc:shorts}.\xfootnote{{\tt MathProg} implements a
...
\end{displaymath}
and\xfootnote{If $\{\v_1,\ldots,\v_4\}$ is a set of vectors such that
$y_i = \normo{\v_i}$ and $y_{ij} = \norm{\v_i}{\v_j}$, then
...
%  $g''(0)<0$.\xfootnote{The function $g(t)$ may fail to be
...
%\end{displaymath} 
%if\xfootnote{If $\{\v_1,\ldots,\v_4\}$ is a set of vectors such that
%  $y_i = \normo{\v_i}$ and $y_{ij} = \norm{\v_i}{\v_j}$, then
%  $\op{cross}(y_4,\ldots,y_{12}) = \norm{\v_2}{\v_4}$.}
%\begin{displaymath}
