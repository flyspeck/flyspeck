...
needed to develop a firm intuitive understanding of what is really going on in the proof.   How might we think about the sphere packing problem?  How might we think about the computations that enter into the solution?
...
$x\in\ring{R}^n$ that maximizes the objective function $x\cdot x$
...
where $c\in\ring{R}^n$ subject to a system of linear inequalities,
    ...
    A x\le b
    \end{equation} 
for some real-valued matrix $A$ and vector $b\in
...
the objective function $c\cdot x$ defines a preferred direction in
...
If $x$ is any feasible solution to the primal
If $y$ is any feasible solution to the dual (meaning that they satisfies
the constraints but are not necessarily optimal), then by
the inequality constraints, we have trivially that
    $$y\cdot b \ge y A x = c x.$$
...
and moreover $y^*\cdot b = c x^*$.  In summary, we can bound
...
Then we for any feasible $x$ to the primal problem
   $$
   c\cdot x = (\delta + y A) \cdot x \le \delta \cdot x + y b.
   $$
Using the a priori bounds on $x$, we get $\delta x\le \sum 
m|\delta_i|=D$.  Then an upper bound on the primal is the value
...
types of problems, we recast all the linear
programs in this book in terms of infeasibility.
When the maximum of $c\cdot x$ is less than $K$, then if
we add the constraint $c\dot x\ge K$ to the system of constraints
$A x\le b$, then we get an infeasible linear program.
...
with a system of inequalities in
...
over a compact domain $C\subset \ring{R}^n$.    A linear relaxtion
of the domain is a polyhedron defined by constraints $A x \le b$
such that the polyhedron contains the domain $C$.  That is,
$x\in C$ implies $A x \le b$.    A linear relaxation of the objective
function $f$ is a linear function $x\mapsto c\cdot x$ such that
$f(x) \le c\cdot x$ for all $x\in C$.  It then follows trivially
that the maximum of $f$ over $C$ is at most the value of 
the linear program that maximizes $c\cdot x$ such that $A x \le b$.
...
as the rolling hills in the countryside.  A helicopter,
...
rigorous proofs that each $x\in C$ satisfies each linear
constraint $A_i x\le b_i$.
This means that the maximum of $A_i x$ over $C$ must be at most $b_i$.
This again, is a nonlinear optimization problem on the domain $C$
with objective function $A_i x$.  For an arbitrary constraint $A_i x\le b$,
and complicated domain $C$, there is no reason to suspect that this
...
By partitioning the linear relaxation of the domain $C$ according
...
relaxations  bounds  are good enough to solve the sphere packing
problem.
...
spheres (Section~\ref{sec:13s}), we  introduced the Delaunay
