...
%\pdf{dart.pdf}{dart}{This symbol represents a dart.}
\begin{figure}[htb]
  \centering
...
\begin{definition}[path,~connected,~combinatorial~component]  A path from $x_0$ to $x_{k-1}$ is a list of darts $[x_0;p_1;\ldots;x_{k-1}]$ such that $x_{i+1} = h_i x_i$, where $h_i \in \{e,f,n,e^{-1},f^{-1},n^{-1},I\}$ 
for each $i$.   The contour path is injective if $x_i=x_j$ implies $i=j$. 
Write $x\sim y$ if there is a path from $x$ to $y$.  We
say that $x$ and $y$ lie in the same combinatorial component if $x\sim y$. (See Lemma~\ref{lemma:er}.  We write $\#c$ for the number of combinatorial components.  We say the hypermap is connected if $\#c=1$.
\end{definition}

\begin{lemma}\rating{50}\label{lemma:er} $\sim$ is an equivalence relation.
\end{lemma}

\begin{proof} This is elementary.
\end{proof}

If $p = [x_0;\ldots;x_{k-1}]$ is a path, we write  $p[x_i:x_j]$ for $i<j$
for the subpath $[x_{i+1};\ldots;x_j]$.  (The notation is ambiguous when the path is not injective.)  We write $x::p$ to prepend to a list
$[x;x_0;\ldots;x_{k-1}]$.  We write $\opat$ for the concatenation of lists:
$$
...
\begin{definition}[node,~face,~edge]  A node is an orbit  under
...
%\begin{figure}[htb]
%  \centering
%  \includegraphics[width=80mm]{noimage.eps}
...
$$\iota = \# f + \# e + \# n - \# D - 2\# \tangle{e,n,f}.$$
(A hypermap with null index is planar.)
\indy{Index}{index}
\indy{Index}{planar index}
\end{definition}

\begin{lemma}\guid{IUCLZYI}\rating{400}\tlabel{lemma:index} Let $x$ be a dart of a hypermap $(D,e,n,f)$. Let $(D',e',n',f')$ be the result of the face walkup $W$ at
...
where
   ...
   \end{cases}
   $$
and $\delta=0$ if $e x$ and $n x$ belong to the same connected component after the walkup $W$, and $\delta=1$ otherwise. Moreover, an $h$-walkup at an $h$-degenerate dart preserves the planar index.
\end{lemma}
...
at a dart $x$.  We have $\iota \ge \iota'$.
\end{lemma} 


%If $x$ is degenerate, then $\iota=\iota'$.
%If $W_h$ merges, then $\iota=\iota'$.
%If $x$ is nondegenerate and and the walkup
...
\begin{proof}  By triality symmetry, we can assume the walkup is an face walkup.  If $\delta=0$, the inequality is immediate.  If $\delta=1$, then $e x$ an $n x$ lie in different connected components after the walkup.  This implies that the walkup splits, and $\delta'=1$.  The result follows.
\end{proof}
...
\begin{proof}  An face walkup never decreases the index.  By a sequence
of face walkups we reach the empty hypermap, which has
index zero.
\end{proof}
...
For the double $W_e$, we refer to Figure~\ref{fig:doubleplain}.  If the original hypermap is plain, then the two darts marked $x$ are equal, as are the two darts marked $y$.  The new edge map swaps $x$ and $y$, but is  otherwise equal to $e$ on the reduced set of darts $D''$.  Swapping two darts has order two, as we plainly see.
\end{proof}
...
\begin{definition}[contour~path]  A contour path from $x_0$ to $x_{k-1}$ is a path  $[x_0;x_1;\ldots;x_{k-1}]$ such that $x_{i+1} = n^{-1} x_i$ or $f x_i$ for each $i<k$.  (That is, each step in the path is clockwise step around a node or a counterclockwise step around a face.)   If the contour path $[x_1;\ldots;x_{k-1}]$ is injective and  $x_0 = x_{k-1}$, then it is a contour loop.
\indy{Index}{contour path}\indy{Index}{path}
\indy{Index}{contour loop}\indy{Index}{loop}
\end{definition}

\begin{remark}  In Figure~\ref{fig:hypermap_ex}, a hypermap is constructed from a planar graph by drawing darts next to each angle.  With this representation, the darts along a contour path lie to the left of the corresponding planar graph edges.  For that reason, we draw figures of contour paths by drawing a topological path in the plane and adding shading to the left of the curve to indicate the region occupied by the darts.
\end{remark}
...
\begin{lemma}\rating{50}  If there is a contour path $[x_0;\ldots;x_{k-1}]$ from $x_0$ to $x_{k-1}$, then elements may be dropped from the list so that it becomes an injective contour path from $x_0$ to $x_{k-1}$.
\end{lemma}
...
\begin{proof} 
By being in the same orbit, the darts $x$ and $y$ are joined by a path, where each step is $z\mapsto h z$, for $h=e,n$, or $f$.  Eliminate $e$-steps through the relation $e\ocirc n\ocirc f = I$.   Replace each $n$-step with a sequence of $n^{-1}$-steps.  This gives the desired path.
\end{proof}


\begin{definition}[inverted~path,~complementary~path] 
If $p$ is a contour path from $x$ to $y$, let the inverted path $p^{-1}$ from $y$ to $x$ be the path that replaces each $h$ step $[u;h u]$ with $[h u;h^2 u;\ldots;u]$, for $h=f,n^{-1}$.
%

Consider a plain hypermap.  Let $p$ be a path that does not visit any node twice.  If $p=[x;y;\ldots;z]$ is a path, we form a complementary loop $p^c$ as follows. Replace each maximal sublist of $n^{-1}$-steps
$$
[u;n^{-1} u; \ldots; n^{-k} u]
$$
with the sublist
...
$$
Then join all the pieces in reverse order.
\end{definition}
...
\begin{figure}[htb]
  ...
  \caption{The complementary contour loop traces the remaining darts
   at the same nodes as the original contour loop.  The inverse path visits the same darts (among others) in the reverse order.}
...
\begin{proof}  We prove the contrapositive.  The successor of $n x$ on $L$ is $x$. Suppose $P=[n x;f n x;\ldots;n y;y]$.   Form contour path starting at $x$, then $n^{-1}$ steps to $L$, then follow $L$ to $y$, and on to $n x$.  Follow $P$ back to $n y$.  This is a M\"obius contour $x\ldots y\ldots n x\ldots n y$.

Suppose $P=[n x;x;\ldots;f^{-1} y;y]$.  Form a contour path
...
$$[x_0;\ldots;x_{k-1}],\quad x = x_{k-1}$$
such that $x_i$ does not lie on $L$ for all $i$ and such that $x_0$ lies at a node visited by $L$.
\end{lemma}
...
\begin{proof} Assume for a contradiction that the path $P$ exists. We can invert the path to get a contour path $P^{-1}$ from $x$ back to a dart $x'_0$ (not on $L$ but at a node visited by $L$).  In fact, Lemma~\ref{lemma:dart-interior}, shows that the interior points consist of $f$-orbits and $n$-orbits (away from nodes visited by $L$).  Thus an $f$-step from $x_i$ to $x_{i+1}$ can be replaced with a sequence of $f$ steps from $x_{i+1}$ to $x_i$.  Similarly and $n^{-1}$ step from $x_i$ to $x_{i+1}$ can be replaced with a sequence of $n^{-1}$ steps from $x_{i+1}$ to $x_i$.  When we reach a dart $x'_0$ on a node visited by $L$ we stop.  

There is an injective contour path $y_0,f y_0,\ldots,x$ that certifies that $x$ is interior.  Concatenate this with the inverted path from $x$ back to $x'_0$.  Then add a sequence of $n^{-1}$ steps to reach a dart $z$ on $L$.  Pass to a subsequence to get an injective contour path $y_0,\ldots,z$ that starts with an $f$ step and ends with an $n^{-1}$ step.  This is prohibited by Lemma~\ref{lemma:contour-path-type} 
\end{proof}
...
From a normal collection we can form a new hypermap.   A dart in the new set $D'$ of darts $D$ is a maximal path $[x;n^{-1} x; n^{-2} x;\ldots;n^{-k} x]$  of $n^{-1}$ steps appearing in some loop in $\cal L$. The map $f'$ takes the maximal path $[x;n^{-1}x;\ldots;y]$ to the maximal path (in the same contour loop) starting $[f y;\ldots]$. The map ${n'}^{-1}$ takes the maximal path  $[\ldots;y]$ to the maximal sequence (in some other contour loop) starting $[n^{-1}y;\ldots]$. Equivalently, $n'$ takes the maximal path $[x;\ldots]$ to the maximal path ending $[\ldots;n x]$. The map $e'$ is defined by $e'\ocirc n'\ocirc f' = I$.  
...
Intuitively, we can represent the quotient hypermap as a graph whose cycles under $f$ are precisely the contour loops in the normal family (Figure~\ref{fig:quot}).
...
In this subsection, we assume that $H = (D,e,n,f)$ is a hypermap with restrictive properties.
\begin{definition}[restricted]
A restricted hypermap is one with the following properties.
    \begin{enumerate}
...
Our aim is to prove that every restricted hypermap with at most $V$ darts is generated by a particular algorithm.  The proof is a long induction argument.  We begin by showing how to go from one partially constructed hypermap to another more fully constructed hypermap.  The hypermap $H$ represents the fully constructed hypermap and two quotients $H/{\cal L}$ and $H/{\cal M}$ represent the partially constructed hypermaps.  The algorithm involves the transition from the hypermap $H/{\cal L}$ to $H/{\cal M}$.  This represents one step of the algorithm.

This section describes the transition from $H/{\cal L}$ to $H/{\cal M}$.  The algorithm carries along various auxiliary data satisfying various assumptions, which we now enumerate.
...
$$  
is a subpath of $L$.
Let $y = f^{m+1} x$.  Let $S = S(x,{\cal L}) = \{f^i x \mid 1 \le i\le m\}$.   The set $S$ maps bijectively to a set of darts in the quotient, which we also denote by $S$. 
...
\begin{lemma}\guid{HQYMRTX}\rating{200}  There is a smallest $p\ge0$, such that $f^{p+1} y$ belongs to some loop of ${\cal L}$.  Set $z=f^{p+1} y$.    $z$ is on the contour loop $L$ and is not equal to $f^k x$, for $0 < k \le {m+1}$.
\end{lemma}

\begin{proof} 
For a contradiction, suppose $f^{p+1} y = f^k x$. Then also, $f^p y = f^{k-1} x$.  If $p>0$, then this goes against the minimality of $p$.  So $p=0$, and $y=f^{k-1} x = f^{m+1} x$.  Also, $k-1 < {m+1}$, which contradicts the minimality of ${m+1}$.  This proves the second claim of the lemma.  In particular, $z\not\in S$.

For a contradiction, assume that $z = f^{p+1} y$ lies on a different contour loop $L'$.  If $z$ lies in a true face, then $y$ and $x$ lie in the same true face, contrary to the choice of $x$.  So $z$ lies in a false face.  Let $z' = [\ldots;z_1]\in D'$ be the image of $z$ in $D'$.  Also, we have $z'\not\in S$.  By the definition of $S$-flag, we have $e'z'$ lies in a true face or $e'z'\in S$.  We consider two cases depending on which holds.

{\bf Case 1: $e'z'$ lies in a true face.}  The dart $f e x$ is interior, true, and lies at the same node as $x$.  By the definition of $S$-flag there is a path with true faces from $f e x$ to $e' z'$.  However, $e'z'$ is interior, and this contradicts Lemma~\ref{lemma:contour-interior-exterior}.

{\bf Case 2: $e'z'\in S$.}  Again $z$ is interior and lies at the same node as a dart in $L$, and this contradicts the same lemma.
\end{proof}


We make the following construction.  Let $L_1$ be the contour loop
$$
...
Let $L_2$ be the contour loop
$$
L[n^{-1}y:n z] \opat P^c[n z:n^{-1}y].
$$
which follows the contour loop $L$ from $n^{-1} y$ to $n z$, then complements the path of $L_1$ from $y$ to $z$, traveling instead from $n z$ to $n^{-1} y$.  
...
We pass from  $H/{\cal M}$ to $H/{\cal L}$ by a double walkup on all of the nodes coming from the nodes of $f y$, $f^2 y, \ldots, f^p y$, and then a double walkup on the edge from the node of $y$ to the node of $z$.  We pass in the other direction from $H/{\cal L}$ to $H/{\cal M}$ by adding an edge from the node of $y$ to the node of $z$ and inserting $p$ new nodes (of degree two) along it.
...
${\cal M}$ is a normal family, the noncanonical function $\varphi$ is an $S_{\cal M}$-flag, and the assumptions on the context (Remark~\ref{rem:enum-context}) are all satisfied in this new context (with $\phi$ instead of the canonical function, ${\cal M}$ instead of ${\cal L}$, etc.).
\end{lemma}

%\begin{note} % XX simplicity of quotient needed?
%Why do we need to bother with simplicity of the quotient?  Isn't it enough for the final hypermap $H$ to be simple?
%\end{note}
...
{\bf [normal]~}The new nodes visited on the subpath of $L_1$ from $y$ to $z$ do not contain any darts in any loop in ${\cal L}$.  (By the definition of normal family, every dart at a node or no dart at a node lies along some loop of ${\cal L}$.)  These new nodes contain two darts, with one dart along $L_1$ and the other along $L_2$.  

The paths $L_1$ and $L_2$ visit every dart visited by $L$.  They visit every dart at the nodes of $y,\ldots,z$.  Thus, if a loop visits a node, then every dart at that node is visited by some loop.

Next, we check that no dart is visited by two different loops.  By construction, the sets of darts are visited by $L_1$ and $L_2$ are disjoint and disjont from the darts visited by the loops of ${\cal L}\setminus \{L\}$.  The result follows.  

Finally, every loop visits at least two nodes.  This is true for $L_1$ and $L_2$ because they visit the nodes of $y$ and $z$.  It is true of the other loops because they belong to the normal family ${\cal L}$. It follows that we obtain a normal family ${\cal M}$  by eliminating $L$ and replacing it with $L_1$ and $L_2$.

{\bf [flag]~} We show that the function $\varphi$ is an $S_{\cal M}$-flag.  The edges of $H/{\cal L}$ can be identified with a subset of the edges of $H/{\cal M}$.   For this subset, the flag condition on edges is immediate.  Any other edge is one of the newly created edges, and they all share a dart with $S(x',{\cal M})$.  

If $F(L_2)$ is false, then the set of true faces is the same on $H/{\cal L}$ and $H/{\cal M}$.  In this case the proof is immediate.  If $F(L_2)$ is true, then we argue as follows.  Let $y'$ be the image of $y$ in $H'$.  Since the canonical function on $H'$ is a flag, the edge $\{y',e'y'\}$ meets a true face or $S$.  However, $y'\not\in S\subset F(L)$, by the simplicity of $H$.  $e'y'\not\in S \subset F(L)$, by the simplicity of $H'$.  The only remaining possibility is that $e'y'$ lies in a true face.  The dart $e'y'$ is naturally identified with the dart $e_{\cal M} y'$ on in $H/{\cal M}$, and hence a path exists from the true face $F(L_2)$ into another true face, and from there any true face may be reached.


{\bf [simple]~} To check simplicity of the quotient, we must check that each of the contour paths in ${\cal M}$ never returns to a node after leaving it.  This is true of ${\cal L}\setminus\{L\}$ by assumption and true of $L_1$ and $L_2$ by construction.  Note in particular, that the dart $z$ is not at the same node as $y$ (by the simplicity of $H$).
\end{proof}


We have various possibilities for $H/{\cal M} = (D_{\cal M},e_{\cal M},\ldots)$.
\begin{enumerate}
...
\item The canonical function is false on $F(L_1)$.  Then $\varphi$ is the canonical function.  By Lemma~\ref{lemma:phi}, all the context assumptions hold for the canonical function.  The iteration continues another step.
\item The canonical function is true on $F(L_1)$, but there exists another face $F(L_3)$ that is false.  In this case, the canonical function is a flag (with $S=\emptyset$).  Pick any $x\in F(L_3)$.  We have that $e_{\cal M} x$ lies in a true face.  Take $S=S(x,{\cal M})$.  The canonical function is a flag, hence an $S$-flag.  The iteration continues another step.
\end{enumerate}

The iteration must terminate, because each iteration constructs a quotient of $H$ with more darts than before.  The number of iterations is bounded by the size of the set of darts $D$ of $H$.

\subsection{algorithm}

To begin the process, we may take ${\cal L}$ to be the normal family of Example~\ref{ex:H2} with two contour loops, whose quotient hypermap is a polygon $H_{2k}$.  We may assume that we pick the initial contour loops on a face of maximal size.  The natural number $k$ is then an upper bound on size of a face.

In summary, we have described a process that by starting with a single polygon and then adding edges and nodes of degree two along the inserted edges, we obtain a restricted hypermap $H$.

We can describe the steps of the process in a way that does not make explicit reference to the hypermap $H$, nor to the normal family ${\cal L}$.   Let $V$ be a finite set that contains all the darts for all of the restricted hypermaps to be constructed..  For each $k=3,\ldots,M$, we give the steps to generate all  hypermaps whose faces have size at most $k$, with darts in $V$, and that satisfy all the properties listed at the beginning of Section~\ref{sec:face-insert}.  Since each face is assumed to contain at least $3$ darts, $M=V/3$ is an a priori upper bound on the number of faces.

We begin with the following initialization. We begin with the polygonal hypermap $H_{2k}$.  A flag $t$ marks one face  true and the other false.  We pick out the special darts $x$ on the false face, and pick $m<k$ and set $S=\{f^i x\mid 1\le i\le m\}$.

At each iteration, we have a finite list of quadruples $(H,S,\varphi,x')$, where $H$ is a simple hypermap, $\phi$ is an $S$-flag, $x'$ lies in a false face, and $S = \{f^i x\mid 1\le i\le m\}$ for some $m$.  The algorithm terminates when every face of every hypermap in the list is true.  At every step of the algorithm, one quadruple with some false face is removed and finitely many quadruples are added to the list.

At each interation the chosen $(H,S,\varphi,x')$ is modified in the following ways and each modification is placed back on the stack.  As we do not know what the set $S$ is without knowing the normal family ${\cal L}$, we generate all possible sets $S$.  Similarly, we do not know $p$ when ${\cal L}$ is not given, so we generate all possible $p$.  Whenever we reach a quadruple that is isomorphic to one previously considered, is is discarded as redundant. 

In summary, we have an algorithm for the construction of hypermaps.  The process must terminate, because the set $V$ is finite, so there are only finitely many quadruples (or quadruples up to isomorphism) that draw their dart sets from $V$.

\begin{lemma}\guid{BRGEFNH}\rating{2000}  Fix $k>0$ and $V>0$.
This process constructs in a finite number of steps all restricted hypermaps, up to isomorphism,  such that the dart set belongs to the finite set $V$.
\end{lemma}
...
the smallest $t>0$ for which $\{\orgn,v,v_t\}$ is a collinear set.  If it exists, let $I=\{t\mid 0\le t < t'\}$, otherwise let $I=[0,1]$.  For each $t\in I$, the blade $C_t^0$ is not a collinear set and is contained in a unique plane $P(t)$ through $\orgn$ and $v$.
