...
\claim{In a minimal counterexample $(V,E,F)$, if $\v\in V$ is a
  concave node, then both edges at $\v$ have length $3$.}  Indeed,%
...
\]
Once it is established that the edges at a concave node $\v$ have length $3$, it
...
$\arc(2,2,3)>\pi/2$, the edges bow away from the concave node, so
...
\Delta(2,2,2,y,3,t)^{1/2}\dfrac{d\,\op{soly}(2,2,2,y,3,t)}{dt} &= 
% for 3.2: \dfrac{4t(256 - 25t^2 + 25y^2)}{25(16 - t^2)} >0,
\dfrac{4 t (9-t^2 +y^2)}{16-t^2}>0,
\end{align}
when $t\le3$ and $y\ge 2$.  Recall that $\Delta>0$.
Hence, the region
...
finite \cc{9267276091. The triangle case is done in Mathematica.} of dimension at
most two.

\claim{In a minimal counterexample some node of the local fan
  $(V,E,F)$ is convex.}  Otherwise, the region that is complementary
to $U_F$ is convex.  The perimeter estimate for convex regions again
...
\begin{proof}
  ...
  Let $U$ be a topological component of $Y(V,E_{ctc})$.  Lemma~\ref{lemma:tauU'}
   ...
   either $D_U$ is a face that is not simple or consists of more than
   one face.  Either way, some node $\v$ lies in the interior to the
   $m_U$-gon.  Let $\u,\w$ be consecutive nodes around the $m_U$-gon.
   By a \cc{Mathematica derivative calculation 6621965370} the angles
...
equally spaced on further circles of constant latitude (Figure~\ref{fig:svdw}).  
Each circle
is rotated $\pi/4$ from the previous ring, so that the points
alternate.

\tikzfig{svdw}
{The Sch\"utte-van der Waerden contact graph.  Four edges that
belong to the standard graph but not the contact graph are shown in gray.}
{
[scale=0.004]
%Set the coordinates of the points:
\tikzstyle{every node}=[draw,shape=circle];
\path (45:400) node (P0) {};
\path (135:400)  node (P1) {};
\path (225:400) node (P2) {};
\path (315:400) node (P3) {};
\path (0:200) node (P4) {};
\path (90:200) node (P5) {};
\path (180:200) node (P6) {};
\path (270:200) node (P7) {};
\path(45:150) node (P8) {};
\path (135:150) node (P9) {};
\path (225:150) node (P10) {};
\path (315:150) node (P11) {}; 
\path (0,0) node (P12) {};
%Draw edges:
\draw
  (P12) -- (P8)
...
\begin{exer}[]\guid{SIZXIEV}\label{exer:radius}
Use \newterm{Gr\"obner} bases to show that the radius $r$ of the Sch\"utte-van der Waerden arrangement $V_{SvdW}$ is
the root of
\[
16 + 128 r^2 - 608 r^4 + 1024 r^6 - 872 r^8 + 384 r^{10} - 
 72 r^{12} + r^{16}
\]
that is greater than $2$:
\[
r \approx 2.091145444888708.
\]
\end{exer}

\begin{exer}[]\guid{IXNITUF}
Show that the standard fan $(V_{SvdW},E_{std})$ has four more edges than the
contact fan $(V_{SvdW},E_{std})$ as shown in Figure~\ref{fig:svdw}.
\end{exer}

For the rest of this section, $r$ denotes this fixed radius.  An
optimal solution to the Tammes problem for $k=13$ has radius at most $r$ and
can also be rescaled
so that the points form a packing $V$ on a sphere $S^2(r)$ of radius
$r$. 
 From now on, a solution to Tammes problem  refers to a set of
...
with this additional property an \newterm{equilibrated} arrangements.
...
\begin{exer}[]\guid{UOHDWRB}\label{exer:1-iso}
...
\begin{exer}\guid{FGAPIRA} 
Show that if $V$ is a solution of the Tammes problem such
that $(V,E_{std})$ is biconnected, then
\[
\sum_F \tau_T(V,E_{std},F) = 4\pi - 22 \sol_r\approx 1.7325.
\]
\end{exer}
...
\op{tgt}_T&=1.733,\\
d_T(r,s) &= \begin{cases}
0.088 (2-s) + 0.2365 (r+2s-4), & r + 2s > 3,\\
0, & r + 2s \le 3,\\
\end{cases}\\
d_T(k) &= d_T(k,0).
\end{align*}
