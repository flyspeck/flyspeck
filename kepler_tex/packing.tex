...
\begin{remark}\guid{ZHIQGGN}\label{remark:precise} The precise meaning of the
...
\begin{lemma}[]\guid{XYOFCGX}\rating{400}\label{lemma:sqrt2-close} 
...
Let $V$ be a saturated packing and let $\bu\in \bV(3)$.
Then  $X=\op{cell}(\bu,k)$ is measurable and eventually radial at each $\v\in V$.
Furthermore the cell $X$ is bounded away from every $v\in V\setminus X$, so that the solid angle of $X$ is zero, except at $\v\in V\cap X$.
\end{lemma}

\begin{definition}\guid{CODE}
...
Let $V$ be a saturated packing.  
\[
\sum_{k=0}^4 \sum_{\bu\in \bV(3)}  \op{solc}(\bu,k) = 4\pi.
\]
\end{lemma}
...
Then
\[
\sum_{k=2}^4 \sum_{~~~\bv=[\v_0;\v_1;\ldots]\in \bV(3)} \op{dihc}(\bv,k) = 2\pi.
\]
\end{lemma}

\begin{proof} The dihedral angle of each $4$-cell $\op{cell}([\u_0;\u_1;\u_2;\u_3],4)$ is the sum of the two corresponding Rogers simplices
...
A \newterm{critical edge} $\ee$ of a saturated packing $V$ is an ordered pair
...
\begin{displaymath} 
\gamma_L(X) \op{wt}(X) + \beta(\ee,X)\ge 0
\end{displaymath} 
...
\begin{lemma}[]\guid{TARJJUW}\rating{0}\label{lemma:poly-bounded} 
\oldrating{120}
