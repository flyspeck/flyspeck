...
Let $H$ be a restricted hypermap.  
Let $\op{ch}$ be any choice function that selects 
$(M,y)$ from any family ${\cal L}$ of contour loops
of $H$, where $M\in {\cal L}$ and $y$ is a dart visited by $M$.  Let $F$ be any face
of $H$.  Then associated with the choices $\op{ch}$ and $F$, 
...
\item If the contour loop $L$ in the transform $T{\cal H}_i=(H,{\cal
  ...
  contour loop of ${\cal M}$ that is not face specific, then ${\cal
    H}_{i+1}=(H,{\cal M},M,y,\varphi_{i+1})$, where $(M,y)$ is the choice
($\op{ch}$) from among the contour loops of ${\cal M}$ that are not face specific.
...
\item \claim{[The contour loop $L$ is face specific, but there exists
  ...
  $\varphi'_{i+1}$ is the canonical function.  
%The canonical function
%  is a flag, hence an $S(H,M,x)$-flag.  
${\cal H}_{i+1}$ is a marked
  hypermap.
\end{nomerate}
...
Choose $p,q$ such that $(m,p,q,r)\in C$, where $r$ is the cardinality of the face of $x$.  
Let $F$ be the face of $x$ in $H$.
It follows by construction, that every face $F'\ne F$ of $H$ is naturally
identified with a face of $R(H,x,m,p,q)$.
Say that a boolean-valued function $\psi$ on the set of faces of
$R(H,x,m,p,q)$ is an \newterm{extension} of 
...
The functions $f$ and $g$ give one iteration of the algorithm.  The powerset-valued
function $g$ is the heart of the algorithm.  It takes one partially constructed hypermap
and modifies it in various ways to construct further partially constructed hypermaps.
When the flag $\varphi$ permits, some hypermaps are also fully constructed.


\begin{definition}[g]  Let $g:X_2 \mapsto \powerset(X)$ be given as follows.
Let $r$ be the cardinality of the face $F$ of $x$.
 The subset $g(H,m,\varphi,x)\subset X$ is presented as a union of two sets:
\begin{displaymath}
   G_i = X_i \cap g(H,m,\varphi,x).
\end{displaymath}
 $H'\in G_1$ if and only 
\begin{itemize}
\item there exists
...
\end{itemize}
 $(H',m',\psi,x')\in G_2$ if and only if
\begin{itemize}
\item there exists $p,q$ with $(m,p,q,r)\in C$, such that $H'=R(H,x,m,p,q)$,
\item  $\psi$ is an extension of
$\varphi$, and 
\item  Let $F_1$ be the face of $x$ in $H'$.  One of the following two conditions hold:
\begin{itemize}
\item $\psi(F_1)$ is false;  $x' = x$; and  $p+m+1 \le m' \le r-1$.
\item $\psi(F_1)$ is true; there exists a false face in $H'$; $x'$ is the value of the choice
function on the union of false faces of $H'$; and $0 \le m' \le r-1$.
\end{itemize}
\end{itemize}
\end{definition}
...
$H/{\cal L}_0$ isomorphic to $H_{2d}$.  
Let ${\cal L}_i = (H,{\cal L}_i,L_i,x_i,\varphi_i)$ be the sequence of marked hypermaps
