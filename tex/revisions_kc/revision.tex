...
There are several gigabytes of log files available as a result of running linear programs in the original proof. 
In this section we present the formalization of the creation and bounding of these linear programs 
in the mechanical proof assistant Isabelle~\cite{nipkow:tutorial02}. 
A more detailed version of the material presented here can be found in~\cite{obua:phd}.

We rely on the availability of the archive of tame graphs. Each one (except the witnesses of the Kepler Conjecture) 
of the tame graphs within the archive represents a potential counter example to the Kepler Conjecture. 
Each such counter example has to obey certain constraints which 
in the original proof ultimately leads to the refutation of all potential counter examples via linear programming.


In order to make these constraints accessible in a formal environment, the collection of (almost) all of these
constraints forms the axiomatic base for an algebraic structure which we call a \emph{graph system}.
An instance of a graph system is a tame graph which obeys the constraints listed in the definition of a graph system.
In the current formalization we use not all of the constraints of the original proof but only those which can be employed 
without the application of branch and bound strategies. Our current notion of \emph{graph system} can best be seen
as a detailed formalization of what is called \emph{basic linear programs} in~\cite[sec. 23.3]{dcg}. 
Because we capture not all constraints of the original proof, we cannot hope for the refutation of all potential counter examples. Nevertheless we manage to refute most of them.

We represent tame graphs as \emph{hypermaps}. In our case, such a hypermap is just a collection of \emph{darts} together
with three permutations on darts, the edge, the face, and the node permutation. This representation greatly simplifies
...
Our approach for creating and solving the linear programs is summarized in Figure~\ref{fig:lpapproach}. For each tame graph,
we apply the axioms of a graph system to it. This results in a large Isabelle theorem which is a conjunction of
linear equalities and inequalities. We then normalize this conjunction such that it takes the shape of a matrix inequality
\begin{equation}
A * x \leq b.
\end{equation}
The entries of $A$ and $b$ are symbolic expressions which contain those symbols that have been used to 
define the axioms of a graph system, for example $\pi$. In order to apply linear programming, 
we need to approximate $A$ and $b$ by matrices which contain only numerical entries. We achieve this via a formalization of 
interval arithmetic in Isabelle, and arrive at numerical matrices $A'$, $A''$ and $b'$ such that we have the formally proven 
Isabelle theorems 
\begin{equation}
...
x' \leq x \leq x''
\end{equation}
holds. It is then possible to obtain a certificate from an external linear programming solver like GLPK (Gnu Linear Programming Kit) that allows us to formally prove \textsl{False} assuming $A * x \leq b$. The beauty of a certificate is 
...
Thus we were able to prove the inconsistency of 2565 of the graph systems, and failed on 206. 
This yields a success rate of about $92.5\%$. Future work is to extend the notion of graph system and the generation of 
the linear programs such that all constraints of the original proof are taken into account.


\begin{figure}
\caption{Refuting a Potential Counter Example to the Kepler Conjecture}
