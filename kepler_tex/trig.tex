...
\begin{lemma}[]\guid{SCEZKRH}\rz{1}\label{lemma:cos-sin}
\begin{displaymath}
\begin{array}{rll}
\sin(\pi/2 - x)  &= \cos(x),\\
\cos(\pi/2 - x)&=\sin(x).
...
\begin{lemma}[]\guid{FMGMALU}\rz{2}\label{lemma:sin-arccos}
\formal{sin\_acs\_t} 
  If $y\in[-1,1]$, then
...
\begin{lemma}[]\guid{OUIJTWY}\rz{1}\label{lemma:arccos-arctan}
\formal{acs\_atn2\_t}  
  If $y\in [-1,1]$, then
...
\begin{definition}[vector addition,~scalar multiplication]\guid{WHIAXXC}
...
\begin{lemma}[]\guid{CTCZHMR}\rz{ZZ}\label{lemma:delta-pos}
...
\begin{remark}[]\guid{LTCTBAN}\rating{ZZ}
...
is not collinear.\xendnote{The properties of the Cayley-Menger determinant for five points in $\ring{R}^3$ are developed further in 
\cite[tag~cm5-E]{LEG}.  }
\end{remark}
...
\begin{lemma}[]\guid{QRAAWFS}\rz{ZZ}\label{lemma:ups} Let
...
\begin{lemma}[law of sines]\guid{UKBAHKV}\rz{20}\label{lemma:los}
...
\begin{lemma}[]\guid{KVVWPNA}\rz{20}  
...
\begin{lemma}[]\guid{BKMUSOX}\rz{20}
\begin{displaymath}
...
\begin{definition}[dihedral angle]\guid{YMHELNF}\label{def:dih} When $\w_0\ne \w_1$,
...
\xfootnote{In three dimensionsal Eulidean space, the orthogonal
...
\begin{lemma}[Euler triangle]\guid{JLPSDHF}\rz{600}\label{lemma:euler}
...
\begin{lemma}[orthonormalization]\guid{QAUQIEC}\rz{ZZ}
\label{lemma:frame}
...
\begin{definition}[azim]\guid{UJBHGUX}
...
\begin{lemma}[]\guid{JBDNJJB}\rz{ZZ}\label{lemma:sim}
...
\begin{lemma}[spherical coordinates]\guid{XPHCPNY}\rz{60}\label{lemma:sph}
...
\begin{lemma}[]\guid{ISRTTNZ}\rz{100}\label{lemma:polar-sum}
