...
\begin{lemma}\guid{PUACSHX}\tlabel{lemma:r-r'}
...
By Tarski arithmetic\tarf{cm4}, 
the square of this determinant is given by a formula
$\op{CM}_4(x_{ij})$, which by Tarski arithmetic\tarf{cm4} is
...
\begin{lemma}\guid{TVPZSUN} \tlabel{lemma:rogers}
...
By Tarski arithmetic\tarf{rog-lemma}, we have that 
...
\begin{definition}[$\dtet$, $\doct$]  Define 
  ...
  $$
for $a<b<c$.  
\indy{Greek}{ZZdeltatet@$\dtet$}
\indy{Greek}{ZZdeltaoct@$\doct$}
\indy{Greek}{ZZdelta@$\delta$}
\end{definition}

\begin{lemma}\guid{COHJTUX}\tlabel{lemma:doct-calc}
...
\begin{lemma}\guid{DQSNVHJ}\tlabel{lemma:dtet-cal}
...
\begin{lemma}\guid{EOMDQTA}\tlabel{lemma:rog-doct}
...
\begin{lemma}\guid{RFZATZF}\tlabel{lemma:rog-tet}
...
By Tarski arithmetic\tarf{eta-root3}, the circumradius of a triangle
...
\begin{definition}[quoin]\label{def:quoin}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
...
from $v_0,v_1,v_2$ (given by Tarski arithmetic\tarf{mk-point}).
We define
...
\begin{lemma}\guid{QXQYHUU}\tlabel{lemma:quo-vol}
...
functions
  ...
  $$
(These implications are justified without pictures by Tarski arithmetic\tarf{BCDE}\tarf{BCEF}\tarf{ABCD}\tarf{ABCE}.)
...
\begin{lemma}\guid{OGDREBE}\tlabel{lemma:solquo}  The quoin
...
\begin{lemma}\guid{RMVTXBD}\tlabel{lemma:sovo:rog}  Let $R = \op{rog}^0(v_0,
...
%\subsection{caps}
%
%\begin{lemma}\guid{PVAAXAM}\tlabel{lemma:cap-rogers}
...
\begin{lemma}\guid{QHZUPHF}\tlabel{lemma:sovo:truncRog}
...
Tarski arithmetic\tarf{rog-exist}.  If $t< r$, set $R_t=\emptyset$ and
$q_t=v_2$.
...
Set
  ...
  \end{array}
  $$
We have the following relations by Tarski arithmetic\tarf{wedge-union}\tarf{rogers-ball}\tarf{rogers2}\tarf{rcone-ball}\tarf{rogers-FR}:
  $$
  \begin{array}{rll}
  ...
  \end{array}
  $$
Also, by Tarski arithmetic\tarf{rogers-rad},
...
%\begin{lemma}\guid{DHJNWOF}\label{lemma:truncRog:noQ}
...
\section{Composite}\tlabel{sec:tcc}
    %\oldlabel{4.10}
...
(The unique existence of $q_i$ is established by Tarski arithmetic\tarf{rog-exist}.)
...
\begin{definition}[plate,~PL]\tlabel{def:plate}
...
\begin{lemma}\guid{RHHKZBF}\label{lemma:plate}
...
\subsection{truncated simplex}

The formula of Lemma~\ref{lemma:plate} suggests the following
definition.

\begin{definition}[$\op{sv}_0$]\tlabel{def:sv0}
Let $\{v_0,v_1,v_2,v_3\}$ be a packing of four points in $\ring{R}^3$.
Set $h_i = |v_i-v_0|$.  If $(i,j,k)\in P_3$ is any permutation of $(1,2,3)$,
set 
$$Q_{(i,j,k)} = \op{quo}^0(v_0,v_i,v_j,v_k,t).$$
(Note that if $h_i\ge t$, then $Q_{(i,j,k)}$ is empty.)
Set 
$$
 A_0(h,t,\lambda) = 
\begin{cases}
 A(h,t,\lambda) & \text{if } h\le t,\\
  0 & \text{otherwise.}
\end{cases}
$$
Set 
$$
\begin{array}{lll}
\op{sv}_0(v_0,S,t,\lambda) &=
\sol(v_0,\op{conv}^0(S))\phi(t,t,\lambda) +
\sum_{(i,j,k)\in P_3} \op{sovo}(v_0,Q_{(i,j,k)},\lambda) +\\
&\op{azim}(v_0,v_i,v_j,v_k) A_0(h_i,t,\lambda).
\end{array}
$$
\end{definition}
...
\begin{lemma}\guid{XTXCNJR}\tlabel{lemma:sovo:CC} 
...
By Tarski arithmetic\tarf{CC}\tarf{CCbar}, we have 
$$
  \begin{array}{lll}
...
\begin{definition}[truncated~corner~cell,~TCC] We define the truncated corner cell
to be
$$
...
\begin{lemma}\guid{EGDTBGY}\label{lemma:tcc}  
...
\begin{lemma}\guid{AUCWGFA}\label{lemma:LL}
We have
  $$
...
We also have\FIXX{Is this a new Tarski arithmetic?}
  $$
...
\subsection{inverted truncated corner cell}\tlabel{sec:inverted}
...
\begin{lemma}\guid{RWVIUDM} Let $\op{sovo}^g(v_0,TCC,\lambda)$ be given
...
(Tarski arithmetic\tarf{rCCinvert-rad}).  Thus,
$$
\op{sovo}(v_0,L_1^-\cap L_2^-,\lambda) =
...
\begin{lemma}\guid{SPAHRBJ}\tlabel{lemma:2tcc} 
...
We have
 ...
 $$
It follows from Tarski arithmetic\tarf{2CCrad} that
...
The existence of such points is given by Tarski arithmetic\tarf{mk-point}.
Let $A=\op{aff}_+(v_0,\{q,v_1,v_2\})$ and
$A'=\op{aff}_+(v_0,\{q',v_1,v_2\})$.
Let $rc_i = \op{rcone}^0(v_0,v_i,y_i/(2\cos\psi_i))$.
By Tarski arithmetic\tarf{AA'}, we have the relations
$$
\begin{array}{lll}
...
By Tarski arithmetic\tarf{rcone2}, 
...
\FIXX{RECHECK.  Carried out originally by S. Ferguson.
http://www.math.pitt.edu/~thales/kepler98/interval/html/ineq4.html
I can't find it.  I think it is sam's file {\tt section\_a14\_9.c}.}
\end{proof}
...
%\begin{remark}\tlabel{remark:vor}\indy{Index}{vor}\indy{Index}{c-vor}\indy{Index}{score}
...
%$S=Sy_1,y_2,\ldots,y_6)$.  Let $R(a,b,c)$ denote a Rogers
%simplex. Set
%    \begin{equation}
...
%
\begin{definition}[fitted~crown,~$\FCR$~$\FC$,~$\FCinner$,~$\rogFC$]\label{def:fitted-crown}
...
\end{array}
$$
\indy{Index}{fitted crown}\indy{Index}{FC}\indy{Index}{FCR}
...
\begin{lemma}\guid{OVGJXBM}\label{lemma:sovo:CR} 
...
Then by Tarski arithmetic\tarf{RCFR}, we have
...
Set
    ...
    \tlabel{eqn:4.5}
    \end{array}
    \end{equation}\indy{Index}{anc@$\anc$}
...
Set
   ...
   \alpha\,\cro(y_1/2,t,\lambda)/(2\pi) +
    ...
    $$
    \indy{Greek}{zzkappa@$\kappa(y_1,y_2,y_3,y_5,y_6,\alpha,t,\lambda)$}
    \indy{Spec}{{\tt kappa\_dih\_h~y1~y2~y3~y5~y6~d}:\quad $\kappa(y_1,y_2,y_3,y_5,y_6,d,t_0,\lambda_{oct})$}
We are finally ready to state the main result about fitted crowns.
Assumption~\ref{eqn:q1q2} remains in force.

\begin{lemma}\guid{HFAVTZH}\label{lemma:sovo:FCR}
Let $\{v_0,v_1,w_1,w_2\}$ be a set of four points in $\ring{R}^3$.
...
\begin{lemma}\guid{ABGRLVH}  The solid angle of $\FCR(v_0,v_1,w_1,w_2,t,\lambda)$
is zero at $v_0$.
\end{lemma}
