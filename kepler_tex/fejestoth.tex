...
Suppose the hypermap of a centered packing with full contact has vertex of type $(p,q,0)$.  Then $(p,q)$ must be one of the following values:
$$
(0,3)~(1,3)~(2,2).
$$
\end{lemma}

\begin{proof} Let $\alpha_0 = \op{azim}(2,2,2,2,2,2)$.  The dihedral angle of a rhombus lies between $\beta_0 = \op{azim}(2,2,2,2.52,2,2)$ and
$\beta_1 = 2\op{azim}(2,2,2,2,2.52,2)$.  Thus,
$$
p\alpha_0 + q \beta_0 \le 2\pi \le p\alpha_0 + q\beta_1.
$$
The only three solutions to these inequalities among the natural numbers are those given.
\end{proof}
