...
To see that the polynomials $\ups$ and $\Delta$ are non-negative, volumes are not required.
Write $v_n-v_1=w_n$, then write out components for $w_2,w_3,w_4,w_5$:
$$
w_2 =(r_1,r_2,r_3),\quad w_3 = (s_1,s_2,s_3),\quad w_4= (t_1,t_2,t_3),
\quad w_5 = (u_1,u_2,u_3).
$$
Then set $x_{ij} = w_j\cdot w_j$ if $i=1$ and $x_{ij} = 
...
Then 
   ...
   $$
    $$
    0\le 4\det(v_2-v_1,v_3-v_1,v_4-v_1)^2 =\Delta(x_{ij}).
    $$
...
$x_{45}$ to view $\mu(x_{45})=R(x_{ij})$ as a function of $x_{45}$.
\name{cm5-quadratic}
...
where $r$ is the largest root of the quadratic function in $x_{45}$:
	$$
	R(x_{ij}) = 0,\quad x_{ij} = y_{ij}^2 \hbox{ if } (i,j)\ne (4,5).
	$$
