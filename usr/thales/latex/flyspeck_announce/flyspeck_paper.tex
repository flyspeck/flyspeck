...
is closely related to the other proof assistants in the HOL family and
...
own correctness~\cite{HaSelf}.  It is generally held by experts that it
is essentially impossible for the HOL Light proof assistant to create
a ``false'' theorem, except in extreme circumstances such as a user
who intentionally hacks the system with malicious intent.  A formal
proof in the HOL Light system is more reliable by orders of magnitude
than proofs published through the traditional process of peer review.
...
problem (the local annulus inequality) involving at most finitely many
balls and the Kepler conjecture.  This reduction appears as the
...
Isabelle must also be a proof in HOL Light (assuming the consistency of
...
logic.  We also note that the logics of  Isabelle/HOL and HOL Light  are
very closely related to each another.
...
packings, with at most $15$ points and with other properties. The
combinatorial structure of a contravening packing can be encoded as a
tame planar hypermap. The tame graph classification theorem implies
that there are finitely many tame planar hypermaps up to
isomorphism. For each such hypermap it is possible to generate a list
of inequalities which must be satisfied by a potential counterexample
associated with the given tame planar hypermap. Most of these
inequalities are nonlinear.
...
the original nonlinear inequalities but simply introduce new HOL Light
...
integer arithmetic. There are about 50,000 linear programs (after
...
to files that are then loaded by the formal verification procedure.
...
applied to proofs and proof automation ~\cite{LAR-Fly}, and a mechanism to
