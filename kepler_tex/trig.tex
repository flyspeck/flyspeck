...
\begin{lemma}\label{lemma:circle}\formal{SIN\_CIRCLE} 
   $$\sin^2(x) + \cos^2(x) = 1.$$
\end{lemma}
...
\begin{lemma}\label{lemma:sin-add}\formal{SIN\_ADD,\ COS\_ADD}
  $$\begin{array}{lll}
...
\begin{lemma}\label{lemma:cos-neg}\formal{COS\_NEG,\ SIN\_NEG}
...
\begin{lemma}\label{lemma:sin-pi2}\formal{SIN\_PI2}
    $\sin (\pi/2) = 1.$
\end{lemma}
...
\begin{lemma}\rating{1}\label{lemma:cos-sin}
    $$\cos(x) = \sin(\pi/2 - x).$$
\end{lemma}
...
\begin{lemma}\label{lemma:tan-add}\formal{TAN\_ADD}
    If $\cos(x)\ne 0$, $\cos(y)\ne 0$, and $\cos(x+y)\ne0$ then
...
\begin{lemma}\label{lemma:tan-pi4}\formal{TAN\_PI4}
    $$\tan(\pi/4) = 1.$$
\end{lemma}
...
\begin{lemma}\rating{2}\label{lemma:sin-arccos}\formal{sin\_acs\_t} 
If $y\in[-1,1]$, then
...
\begin{lemma}\rating{1}\label{lemma:arccos-arctan}\formal{acs\_atn2\_t}  
If $y\in [-1,1]$, then
    $$\arccos(y) = \pi/2 - \atn({ \sqrt{1-y^2}},{y}).$$
\end{lemma}

\begin{proved} The endpoints $y=\pm1$ can be checked directly.
...
There is a norm\index{Index}{norm}
$$|u| = \sqrt{u\cdot u}.$$
It satisfies $| s u | = |s| \, |u|$.  We have $|u|=0$  if and
only if $u=0$, the zero function.

\begin{lemma}[Cauchy-Schwartz inequality]\formal{Jordan/metric\_spaces.ml:cauchy\_schwartz}
    $$|u \cdot v| \le |u|\,|v|.$$
Equality holds exactly when $|v|u = \pm |u|v$.  Furthermore,
$u\cdot v = |u|\,|v|$ holds exactly when $|v| u = |u| v$.
\index{Index}{Cauchy-Schwartz inequality}
\end{lemma}
...
\begin{lemma}[triangle inequality]\formal{Jordan/metric\_spaces.ml:norm\_triangle}
  $$
  |u + v| \le |u| + |v |.
  $$
...
By tarski\tarf{tarski:cm4}, which interprets $\Delta$
...
\index{Index}{angle}\index{Index}{arc}\index{Index}{arclength}
\end{definition}
...
\end{lemma}\index{Index}{arc}\index{Index}{law of cosines} 
...
\begin{proved}  Both sides are non-negative.  It is enough to check
...
\begin{proved} These are elementary calculations.
+++\end{proved}
...
of the set $W$.
\index{Index}{lexicographic order}
\index{Index}{polar cycle}
...
\begin{lemma}\label{lemma:polar2}\rating{50}\formal{thetaij\_t}
...
\begin{lemma}\label{lemma:polar-sum}\rating{100}\formal{thetapq\_wind\_t}
...
azimuth angle is a polar coordinate of $(x,y)$:
    ...
    $$
\index{Index}{azimuth}\index{Index}{azim}\index{Index}{zenith}\index{Index}{latitude}
\index{Index}{longitude}
...
\index{Index}{frame}\index{Index}{orthogonal frame}
...
can be expressed in the form
   ...
   $$
where $u'\cdot e_3 = 0$.
\index{Index}{zenith}\index{Index}{orthonormal}
\end{lemma}
...
\begin{lemma}[cylindrical coordinates]\rating{80}
...
lemma.
\index{Index}{cylindrical coordinates}
\end{definition}

\begin{lemma}[spherical coordinates]\label{lemma:sph}\rating{60}\formal{spherical\_coord\_t}
...
\begin{lemma}\rating{50} The azimuth cycle $\sigma:W\to W$ on
