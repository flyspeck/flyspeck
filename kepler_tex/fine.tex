...
The previous chapter gives  constructions
...
the complement of a fan, then they are obviously disjoint.  This, in fact,
...
the lemma is the most widely used.  A smaller truncation parameter never
appears.  A larger parameter is used only when the parameter $t_0$
has been tried and found wanting.  The utility of this particular truncation
is that at this tight range,  the $V$-cell and Voronoi cell become
indistinguishable.
...
It is not at all evident that they are really subsets of the $V$-cell or even
...
syntactic sugar that allows us to swap the third and fourth entries
of the crown tuple by flipping a sign.  The fitted crown is
a composite region and  two of its primitive constituents are
Rogers simplices.   Sign flipping toggles between these two Rogers
...
definition of normal tuple, by showing that it captures every intersection
of a fitted crown with a blade fashioned from a barrier.
...
Say $w_2\ne u_1,u_2$.  If $\op{aff}_+(v_0,\{v,w_2\})$ meets $F$, then by Lemma~\ref{XX},
...
two quarters in the $Q$-system overlap. This is contrary to Lemma~\ref{XX}.

So $w_2\in\op{aff}_+(v_0,\{v,u_1,u_2\})$.  Then $\{v_0,v,w_2,u_i\}$ is a quarter, for
...
Our eventual aim is to confine fitted crowns, eventually showing that they are separate
from one another 
and that they cannot escape the confines of the $V$-cell (that is, separate from
all other $V$-cells).  To set up separation results, we need
lemmas that say that objects that appear to be close together 
are in fact separated
by some intervening obstacle.  The following lemma has just this form.  The hypothesis
is a closeness condition (a certain circumradius is small) and the conclusion is the
existence of an intervening anchor $w'$.
...
We are finally ready to establish that the fitted crown does not meet blades
formed by barriers.  That is the content of the following series of lemmas.
...
fan formed from barriers.  Since the set $\FC$ is not itself connected, we
work with a slighly larger connected set (called $B$ in the proof) that is connected
and that lies in $Y(v_0,V,E)$.  With this small adjustment, the proof is
straightforward.
...
Consider the special case that $S=\{v_0,u_1,u_2,u_3\}\in\CalQ(\Lambda)$.  Let 
$U=\op{aff}_+^0(v_0,T)$.  
...
$y\in U\cap \FC$.  By Lemma~\ref{lemma:delta-tri}, $\FC\subset U$.  Then
$v\in \op{aff}_+(v_0,T)$.  From this, we get $v\in T$.  Say $v=u_1$.
By tarski, it follows that $S$ is an upright quarter in the $Q$-system.
Then $\FC\subset \op{aff}_+^0(\{v_0,v\},\{w_1,w_2\})$, which is disjoint from
$\op{aff}_+(\{v_0,v\},\{u_j,u_k\})$.  So $y$ cannot be in both sets.  This
