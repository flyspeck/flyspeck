...
\formaldef{$\bV(k)$}{vor\_list}
Let $V$ be a saturated packing.
When $k=0,1,2,3$, let $ \bV(k)$ be the set of lists $\bu=[\u_0;\ldots;\u_k]$ of length $k+1$, with $ \u_i\in V$, 
such that
...
{\it length} of the list $\bu$ (which is $k+1$).\footnote{By convention aa $d$-simplex is
presented as a $d+1$-tuple.
  Because of this shift by $1$, the notation $\trunc{\bu}{j}$ also differs by
  the same shift.}
...
$\bu=[\u_0;\ldots]\in \bV(k)$
for some $k$.  Define points $\omega_j=\omega_j(V,\bu)\in\ring{R}^3$ by recursion over $j$.
\begin{eqnarray*}
...
\claim{The point $\omega(\bu)$ exists when $\bu\in V(k)$.}
...
The point $\omega_k$ depends on $\bu$ through its projection to $\trunc{\bu}{k}$ so that
\[
