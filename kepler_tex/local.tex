...
A nonreflexive local fan $(V,E,F)$ is \fullterm{generic}{generic fan}  if for every $\{\v,\w\}\in E$
and every $\u\in V$, 
\[ 
C\{\v,\w\}\cap C^0_-\{\u\} = \emptyset.
\] 
A nonreflexive local fan is \fullterm{circular}{circular fan}  if there exists $\u\in V$ and
$\{\v,\w\}\in E$ such that
\[ 
C^0\{\v,\w\}\cap C^0_-\{\u\}\ne \emptyset.
\] 
A nonreflexive local fan is \fullterm{lunar}{lunar fan} with pole $\{\v,\w\} \subset V$ if it is not
...
Next consider a deformation of a nonreflexive local fan.  The following 
...
\end{align*}
$(V',E',F')$ is called the \fullterm{polar}{polar!fan} of $(V,E,F)$.
...
Let $(V,E,F)$ be a nonreflexive local fan.  Recall that $\hm = 1.26$.  Set
\begin{align*}
\rho_0(y) &= 1 + \dfrac{\sol_0}{\pi} \cdot
    \dfrac{y-2}{2\hm-2} = 1 + \dfrac{\sol_0}{2}(1 - L(y/2))\\
...
\item 
\[
2\le a_{i,j}\le \stab \text{ and } a_{i,j}\le
  b_{i,j}, \text{ for all }  i,j\in I \text{ such that } i\ne j.
\]  
\item
  Also, 
\[0 = a_{i,i}\le b_{i,i} \text{ and }
  b_{i,i+1}\le \stab.
\]  
\item
...
\begin{remark}\guid{XUHUBYO}
  ...
  $V=\{\v_i\mid i\in I\}$ of a local fan, indexed such that
$\rho_{V,E,F}\v_i = \v_{i+1}$.   The constants $a_{i,j}$ and
  $b_{i,j}$ prescribe the lower and upper bounds on the edges and
...
$d(s)$, $I(s)$, $a_{i,j}(s)$, and so forth for the associated
parameters.  

\begin{example} The constants in the conclusions of the main estimate
  can be packaged into stable constraint systems.  For example, the standard
  main estimate for $k=6$ gives the constraint system $d=0.712$, $J=\emptyset$,
...
The condition \case{nonparallel} follows from the estimates based on stability.
\[
...
Suppose that
\[
d(s)\le 0.9 \text{ and } \tau^*(s,\v)\le 0.
\]
Then $\sol(V_\v,E_\v,F_\v) < \pi$.
...
systems that includes $\smain$.   To obtain the main estimate by induction by slicing fans
into pieces, the constraint systems must be compatible. We impose the
following coherence conditions.

\begin{definition}[coherence]\guid{YCFMXRF}
A set $S$ of stable constraint systems is \newterm{coherent}, if the following conditions
hold.
\begin{enumerate}
\item \case{coherent slice}
...
\item For $t=s',s''$,
\[
a_{p,q}(s) \in \leftclosed a_{p,q}(t),b_{p,q}(t)\rightclosed.
\]
%%!! Removed b_{p,q}(s) in interval on Jan 23, 2012.
%
...
\begin{lemma}\guid{MTUWLUN}
Let $S$ be a coherent set of constraint systems.  Let $s',s''\in S$ be constraint systems 
...
%and the sums over the singleton sets $J(s')$ and $J(s'')$ are nonnegative.
%Hence
%\[
...
%set $J(s)$ is identified with the disjoint union of $J(s')$ and $J(s'')$ so that
%the sum over $J(s)$ breaks into corresponding sums over
%$J(s')$ and $J(s'')$.
\end{proof}
...
construct an explicit finite coherent set $S$ of constraint systems that
includes the set $\smain$ of constraint systems appearing in the main
...
\begin{definition}[minimal counterexample]\guid{WNUGMSE}
  Let $S$ be a coherent set of constraint systems.  We say that
...
%is negative.
\item If $s'\in S$ is any constraint system such that $k(s')<k(s)$, then the
...
Let $S$ be a coherent set of constraint systems.
...
\begin{proof}
  ...
  minimum $a_{i,j}(s)$, then we can find induction systems
...
\item $J(s')\subset J(s)$,
\item $d(s')\ge d(s)$.
\item $a_{i,j}(s)=a_{i,j}(s')$ and $b_{i,j}(s)=b_{i,j}(s')$ for all $i,j\in I$.
% Added Jan 23, 2012.
...
The largest value that arises in our calculations will be $d(s)=0.712$
...
systems, and let $s\in S$ with $k=k(s)$.
...
Let $S$ be a coherent set of constraint systems, let $s\in S$, and let
$i\in I(s)$.
...
and isometrically moves the set $W$ by a rotation of the plane $A$ about the line
...
We saturate $S$ by adding constaint systems for
all possible ways of  slicing systems $s\in \smain$
...
additional constraint systems (up to symmetry), one corresponding to a
...
For these smaller constraint systems, the constants $d$ have been
...
we add a constraint system $s'$ to $S$, where $s'$ has
...
then the system $s'$ satisfies \eqref{eqn:stab}.

In general, whenever we add any system to $S$, we recursively add
...
(with diagonals greater than $\stab$) into a single constraint system
...
By taking a common tightening of constraint systems, we have reduced
the calculation for pentagons ($k=5$) to a single system $s$.  By
...
a_{i,j}(s)=\stab.
\end{equation}  
We may assume that our constraint system
has this property.
