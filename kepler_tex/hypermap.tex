...
\subsection{marked-up hypermap}\label{sec:face-insert}
...
If $L$ lies in a single face $F$ of $H$, then let $S=F$.
Otherwise,
...
\begin{definition}[marked-up hypermap]\label{def:data}
  A tuple $(H,{\cal L},L,x,\varphi)$ is a \newterm{marked-up hypermap} if
...
\item $x\in D$ and lies in the loop $L$.
...
of $x$ in $F(L)$. Moreover, the dart $x'$ has the form $[\ldots;x]$.
\item The flag set $S$ of $(H,L,x)$ maps bijectively to a set
of darts in the quotient, which is also denoted $S$.
\item  $\varphi$ is an $S$-flag, where $S$ is the flag set of $(H,L,x)$.
\end{enumerate}
\end{definition}
...
Let $(H,{\cal L},L,x,\varphi)$ be a marked-up hypermap with flag set $S=S(H,L,x)$.
Assume that $L$ meets at least two faces.
Let $m\ge0$ be given by $m = \card(S) -1$.  Set $y = f^{m+1} x$.  
...
The first entry $H$ of the tuple does not change.  Let $m = \card(S(H,L,x))-1$.
...
function. The flag set $S(H,L_1,x)$  contains the proper
subset $S(H,L,x)$.
...
If $(H,{\cal L},L,x,\varphi)$ be a marked-up hypermap such that $L$
meets at least two faces of $H$,  then
$(H,{\cal M},L_1,x,\varphi')$ (as constructed above)
is also a marked-up hypermap.
\end{lemma}
...
\subsection{algorithm with quotients}
...
There is a sequence of marked-up hypermaps
...
gives a marked-up hypermap.  The iteration
...
\subsection{algorithm without quotients}
