...
\item  {\bf [half-space]~} The topological component $U_F$ is the intersection of the open
...
$$
\item {\bf [diagonal]~}  If $x,y\in F$ are distinct, with corresponding vertices $v(x),v(y)\in V$, then
...
plane $\{\orz,v(z),v(f z)\}$.  Without loss of generality, assume that $z=x$, 
...
\end{summary}
%In this example, the hypermap of the cyclic fan  is isomorphic to $H_{2k}$, where $k=\op{card}(F)$.  This, in fact, becomes one of the defining properties of a cyclic fan.
...
The unit normal $e$ satisfies $(v_0\times v_1)\cdot e > 0$.    
This inequality is the base case of an induction.
...
\begin{lemma}[slicing]\guid{EJRCFJD}\rating{ZZ}  Let $(V,E,F)$ be a cyclic fan with hypermap $H$.  Pick $v,w\in V$. For each $u\in \{v,w\}$, assume that $u$ is independent of all elements of $V\setminus\{u\}$.    Assume that $C^0\{v,w\}\subset \Wdart(x)$ for all darts $x\in F$.  Then
\begin{itemize}
...
\begin{proof}
Let $E' = E\cup \{\{v,w\}\}$.
Claim: $(V,E')$ is a fan.  (Compare Lemma~\ref{lemma:add-edge}.)
Except for the intersection property, all of the properties follow trivially from the fact that $(V,E)$ is a fan and
...
{\bf [fan:1]:} The set $V(t)$ is the image of $V$ under $\varphi(\cdot,t)$, and is therefore finite and nonempty.  The map $\varphi(\cdot,t)$ is invertible, so that $V(t)$ is in bijection with $V$.  In particular, the cardinality does not depend on $t$.

{\bf [fan:2]:} The radial spherical coordinate is nonzero for every element of $V(t)$.  Hence $V(t)$ does not contain $0$.

{\bf [fan:3]:} For $\{v,\rho v\}\in E$, the angle 
...
{\bf [fan:4]:} The points $V(t)$ are contained in the union of two half-planes $A_1,A_t$.  The deformation is the identity on $A_1$ and a $A_0\to A_t$ on the second half-plane.  These bijections preserve the incidence relations of balds of the cones $C(\e)$.  

{\bf [cyclic:1]:} The combinatorial properties of the hypermap do not depend on $t$.  In particular, the hypermap has face $F(t)$ and the hypermap is isomorphic to $H_{2k}$.

{\bf [cyclic:2]:} The azimuth angle $\op{azim}(x(\varphi(u,t)))$ is fixed for $u\ne v,w$ and is decreasing in $t$ for $u\in \{v,w\}$.  Hence, the upper bound on the angle is preserved.

{\bf [cyclic:3]:} $V(t)\subset A_0\cup A_t$, where $A_0$ and $A_t$ are the
...
{\bf [fan:1]:} The set $V(t)$ is the image of $V$ under $\varphi(\cdot,t)$, and is therefore finite and nonempty. The lower bound on $\op{arc}_V$ implies that the cardinality of $V(t)$ is independent of $t$.

{\bf [fan:2]:} The radial coordinate of each element of $V(t)$ is nonzero.  Hence $V(t)$ does not contain $0$.
...
{\bf [fan:4]:} If the intersection property fails:
$$
C(\e) \cap C(\e') \ne C(\e \cap \e'),
$$
...
{\bf [cyclic:1]:} Let $\sigma(t,v)$ be the azimuth cycle on $E(t)(v)$.  It is equivariant:  $\sigma(t,v) (\varphi(w,t)) = \varphi(\sigma(v,w),t)$.   (This follows from the intersection property~(\ref{eqn:inter}).)  Thus, the combinatorial properties of the hypermap do not depend on $t$.  In particular, the hypermap has face $F(t)$ and the hypermap is isomorphic to $H_{2k}$.

{\bf [cyclic:2]:} The upper bound $\pi$ on the azimuth angle is preserved by the flatness condition.

{\bf [cyclic:3]:} Finally, it remains to be seen that 
\begin{equation}\label{eqn:includeVW}
V(t)\subset \bWdart(x(v(t)))
\end{equation} 
for all $v\in V$ and $t\in I$.  If not, there is a largest time $0\le t_0\in I$ for which inclusion~(\ref{eqn:includeVW}) holds for every $v\in V$.  Assume that
$$
u(t)\in \bWdart(x(v(t)))
$$ 
