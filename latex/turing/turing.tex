...
\title{Mathematical Reflections in the Age of the Turing Machine}
...
statistics''~\cite{Pi2011}.  Without it, we would be bereft of Shor's
...
models of climate change. Without it, entire fields of engineering
from Control Engineering to Operations Research would close their
doors.  Applications fuel the economy:  in 2011 rankings, the first five of the
``10 best jobs'' are math or computer related: software engineer,
mathematician, actuary, statistician, and computer systems analyst~\cite{CC11}.
\footnote{Yet the popular media still portrays the mathematician as one
who writes formulas on windows and mirrors in a crazed frenzy, rather than as a
...
time''~\cite{Aus}.  Euclid gave us a method, but even he erred in the
...
center, must intersect.  The concept that is needed to repair the
gap in Euclid's reasoning is the completeness of the real numbers.
This defect in Euclidean geometry was not remedied until
...
% 2002).'' Math Overflow.
%Lickorish ``proved'' that a covering space of a punctured plane
%cannot have handles.  But this isn't so.~\cite{XX need citation, Kuperberg}

Theorems that are calculations or enumerations are especially prone to
error.  Feynman laments, ``I have mathematically proven to myself
so many things that aren't true. 
%I'm lousy at proving things -- I always make a mistake 
\dots And I don't notice in the morass of things that something, a
little limit or sign, goes wrong.
% So I always have to check with calculations; and
\dots I'm very poor at calculations -- I always get the wrong answer''
\cite[p.~885]{XX Selected papers of Richard Feynman, By R. Feynman,
  L. Brown} Pontryagin and Rokhlin erred in computing stable homotopy
groups of spheres.  Little's tables of knots from 1885 contains
duplicate entries that went undetected until 1974.  In enumerative
geometry, in 1848, Steiner counted $7776$ plane conics tangent to $5$
general plane conics, when there are actually only $3264$.
