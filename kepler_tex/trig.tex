...
set
	$$
	   ...
	   \op{conv}^0 S &= \op{aff}^0_+\, (\emptyset,S).\\
           \end{array}
        $$
\indy{Notation}{conv(S)}
\indy{Notation}{conv0@$\op{conv}^0 S$}
...
With the notation, $a = v_1\cdot v_2$, $b = u\cdot v_1$, $c= u\cdot v_2$,  and
$$
\Delta(1,1,1,2-2a,2-2b,2-2c) = 4 (1-a^2-b^2-c^2+2 a b c)
$$ 
and
$p = 1+a+b+c$.    In terms of $\phi,\alpha,\theta$, calculemus (let us calculate!)
$$
...
There is a cyclic permutation $\sigma:W\to W$ which sends
...
and all $0\le i \le j < n$,
   ...
   \theta(p,\sigma^j(p)).
   $$
Moreover, if there exist $p,q\in W$ such that $\theta(p)\ne\theta(q)$,
  $$
...
Say that $W$ is cyclic with respect to $(v,w)$ if
