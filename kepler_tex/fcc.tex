...
from a cubic lattice, with a ball at each extreme point of each cube and adding
a ball at the center of each face of each cube.  The name `face-centered cubic' comes from this construction.  The edge of each cube is
...
superimposed on the cubic picture.  There is one tetrahedron at each extreme point
of the cube, extending to the centers of the three adjacent faces.
An octahedron sits the center of the cube, an extreme point of the octahedron
...
of the volume
of the part within the tetrahedron of the unit balls (at the four extreme points)
...
Define a \newterm{hexagonal layer} to be a translate of the two-dimensional
...
Let $V$ be a packing built as a sequence of close-packed
...
that $V$ does not contain any balls whose local
...
Now assume that a packing $V$ contains some ball
...
Once a packing $V$ contains a single hexagonal layer, the
...
Start with an arbitrary lattice $V$ in which every point has
...
Let $V$ be the set of centers of a collection of unit disks.
...
assume that $0\in V$ is the center of our Voronoi cell.  
...
There is a \newterm{Delaunay triangulation} for infinite sets $V$
of points in the plane (or more generally in $n$-dimensions).  
Assume that $V$ is the set of centers of packing so that
distances between points of $V$ are at least two.  
A enlargement of the packing
$V\subset V'$ increases density.  In issues
of density, it does no harm to assume that the packing is \newterm{saturated}; that is, it is not a proper subset of another packing.  A saturated
...
contains no points of $V$ in its interior.   Each Delaunay
triangle has
