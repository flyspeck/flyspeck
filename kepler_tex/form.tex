...
B(x,r)$.\index{ZZdelta@$\delta(\Lambda,x,r)$}
...
\begin{definition}[Voronoi~cell,~$\Omega$]\label{def:voronoi}\index{Voronoi cell} Voronoi cell
$\Omega(v)=\Omega(\Lambda,v)$\index{ZZzomega@$\Omega(v)$} around a
...
Chapters~\ref{sec:construction} through \ref{sec:scoring} introduce
the basic definitions.  Chapter~\ref{sec:scoring} gives a proof of
Claim~\ref{claim-A}. Chapter~\ref{sec:local-opt} proves
Claim~\ref{claim-F}. Chapters~\ref{sec:fine} through \ref{sec:fb}
present the fundamental estimates. Chapters~\ref{XX}
through \ref{sec:proof-classification} give a proof of
Claim~\ref{claim-C}. Chapters~\ref{sec:startame} through
\ref{sec:weight} give a proof of Claim~\ref{claim-B}.
Chapters~\ref{XX} through \ref{XX} give
...
%\begin{definition}[overlap]\index{overlap} def:overlap
...
\begin{definition}[context,~$c(Q,\Lambda)$] \label{def:context}
