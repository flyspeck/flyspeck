...
Then $I=\op{conv}\{w,w'\}$ does not meet $C=\op{conv}\{v_0,v_1,v_2,v_3\}$.
...
\begin{proved}  By Lemma~\ref{tarski:small-edge-conv0}, if the sets meet, they
...
By the definition of small face and Lemma~\ref{tarski:small-eta}, 
each of these two faces has at most
one edge of length greater than $2.51$.  

Suppose that the set $W$ of two points in $S$ 
...
Let $W' = \{v_0,v_1,v_2,v_3\}\setminus W$ be the
edge opposite the shared edge.  We can decrease $|w-w'|$, contracting
along $W'$ until the length of $W'$ is $2$.  The assumptions now allow
us to label the vertices so that $W'=\{v_1,v_2\}$, and so
that
...
$w_1=v_1$ and $w_2=v_2$.  
Assume that $S_W$ and $S_V\setminus\{v_i\}$ is small for $i=1,\ldots,4$.
Let $C_V=\op{conv}(S_V)$ and $C_W=\op{conv}(S_W)$.
Assume that $C_V\cap C_W\ne \op{conv}\{v_1,v_2\}$.
...
From $\op{quadp}(w,(v_0,v_1,v_2,v_3))$ and Lemma~\ref{XX--to here--}, we may assue that
$$
