...
international conference on the formal proofs and the Flyspeck project
at Institute of Math (VAST) in Hanoi in 2009 transformed the project
...
project might actually set the current record in terms of lines of code
in a verification project.
...
The formal proof of the main statement in the Kepler conjecture can be
executed in about five hours on a 2 GHz CPU.  We encourage readers of
this article to make an independent verification of this main
statement on their computers.  This main statement reduces the Kepler
conjecture to three computationally intensive subclaims that are
described in Section~\ref{sec:statement}.  Two of these subclaims can
be checked in less than one day each.  The third and most difficult of
these subclaims takes about 5000 CPU hours to verify.
...
is closely related to the other proof assistants in the HOL family,
which are based on a simply polymorphically-typed lambda calculus. The
type system, the term constructors, and the theorem constructors of
HOL Light are contained in a small kernel of about 300 lines of code.
The design of the OCaml type and module systems prevent the creation
of theorems, except by the axioms and the given rules of inference.
For an overview of the rules of inference, see \cite{harrison2009hol}.
The logic is classical.  There are three mathematical axioms: the
axiom of infinity (positing the existence of a function that is
one-to-one but not onto), the axiom of extensionality (which states
that two functions are equal if their outputs agree on every input),
and the axiom of choice.  HOL Light has a mechanism that permits the
user to extend the system with new constants and new types.
...
implementation in code is free of bugs.  This includes a formal
verification of its own correctness.  It is widely held by experts
that it is essentially impossible for the HOL Light proof assistant to
create a ``false'' theorem, except in extreme circumstances such as a
user who intentionally hacks the system with malicious intent.  A
formal proof in the HOL Light system is more reliable by orders of
magnitude than proofs published through the traditional process of
peer review.

One feature that makes HOL Light particularly suitable for the Flyspeck
project is its large libraries of formal results for real and complex analysis.
This includes libraries for multivariate (gauge) integration, differential calculus,
transcendental functions, and point-set topology on $\ring{R}^n$.
...
scripting language.  Isabelle/HOL supports a form computational
reflection (which is used in the Flyspeck project) that allows
executable terms to be exported as SML and executed, with the results
...
hexagaon-close packing and the face-centered cubic packing also
...
\item The proof of the Kepler conjecture relies on nearly a thousand
  ...
  conjunction of these nonlinear inequalities.  See Section~\ref{sec:ni}.
...
described in \cite[\textsection 5]{tankink2013formal}.
...
Section~\ref{sec:ab}) provides the link between a local optimization
problem involving at most finitely many balls and the Kepler conjecture.
This reduction appears as the following formal result:
...
$\{\p\in\ring{R}^3 \mid 2\le \normo{\p} \le 2.52\}$.  As this set is
...
packing $V$ can be stated in the following form:
...
repeat here shows that it is enough to consider $V$ with at most $16$
...
consider tame planar hypermaps to be essentially plane graphs with
certain restrictive properties.  The nodes of the graph are in
bijection with $V$.)  A computer is used to enumerate all of the
finitely many tame plane graphs up to isomorphism.  This reduces the
possible counterexamples $V$ to an explicit finite family of cases.

For each explicitly given tame planar hypermap $H$, we may consider all
contravening packings $V$ associated with $H$.  By definition, these
are counterexamples to Equation~(\ref{eqn:la}).  We express the
...
techniques to show that the linear programs are infeasible.  This
implies that the corresponding nonlinear inequalities are
inconsistent, so that the contravening $V$ cannot exist.  Eliminating
all possible counterexamples, by running through all tame planar
hypermaps, we conclude that the Kepler conjecture must hold.
...
\subsection{appendices to the blueprint}\label{sec:ab}
...
rectangle of dimension $k$ to an end rectangle of dimension $k-1$.
...
days.  Identical results were obtained through in these repeated calculations.
...
In informal terms, it asserts that every tame plane graph is isomorphic to a
graph appearing in the archive. 
...
recast the original Java program for the enumeration of tame plane in
HOL. The result is a set of graphs called \verb!TameEnum!.  In
...
components in the plane of a Jordan curve), a final outer one and a
non-final inner one, where a {\it final} face means that the algorithm
is not permitted to make further modifications of the face.  If a
graph contains a non-final face, it can be subdivided into a final
face and any number of non-final ones.  Because a face can be
subdivided in many ways, this process defines a forest of graphs. The
leaves are final graphs.  The formalization defines an executable
function \verb!next_plane!  that maps a graph to the list of successor
graphs reachable by subdividing one non-final face.  The set of plane
graphs, denoted \verb!PlaneGraphs! in HOL, is defined to be the set of
final graphs reachable from a seed graph in finitely many steps.
%To facilitate a fair enumeration, it is
%parameterized by an upper bound on the size of final faces in the
%generated graphs.

In the Isabelle/HOL part of the proof, plane graphs are represented as an
$n$-tuple of data including a list of faces. Faces are represented
as lists of vertices, and vertices are represented by an integer index.
A function $fgraph$ strips the $n$-tuple down to the list of faces.
...
directly).  The HOL predicate is called \verb!tame!.
...
The formalization defines a function \verb!next_tame! from a graph to a
list of graphs. It is a restricted version of \verb!next_plane! where two
kinds of graphs are removed: non-final ones that cannot lead to tame
graphs and final ones that are not tame. For computational reasons,
the tameness check here is approximate: no tame graphs are removed,
but non-tame graphs may be produced. This is unproblematic: in the
worst case a fake counterexample to the Kepler conjecture is produced,
which is eliminated at a later stage of the proof,
but we do not miss any real ones.

Each step \verb!next_tame! is executable. The exhaustive enumeration of all final graphs reachable from a
seed graph via \verb!next_tame! yields a set, denoted \verb!TameEnum!.
...
The archive that came with the original proof contained (for historic
...
application in mind~\cite{McLaughlin:2006:IJCAR}, this tool has not been maintained.  A
...
explict list (the archive) of graphs.  All of the HOL Light terms
...
packing can be encoded with a tame planar hypermap. The tame
graph classification theorem implies that there are finitely many
tame planar hypermaps up to isomorphism. For each such hypermap it is possible to generate a
...
packing associated with the given tame planar hypermap. The process of
construction and solution of linear programs is repeated for all tame
planar hypermaps and it is shown that no contravening packings (and hence
...
\]
With the modified dual solution, the sum of the constraints, weighted
by the coefficients of the dual vector, is $0 \le -0.2974$. This
contradiction shows that our original system of inequalities is not
consistent.
...
The article \cite{adams2014flyspecking} describes the steps involved in the
auditing this formal proof.  The proofs scripts must be executed to
...
between formal proof systems~\cite{obua:import}, [XXMcLaughlin],
[XXAdams]; the refactoring of formal proofs [XXAdams]; machine learning
applied to proofs and proof automation ~\cite{LAR-Fly}; a mechanism to
execute trustworthy external arithmetic from
Isabelle/HOL~\cite{obua:phd},~\cite{ObuaN}; the verification of linear
programs [XX]; and the verification of nonlinear inequalities [XX].
