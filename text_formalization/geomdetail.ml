...
let conv0_2 = new_definition ` conv0_2 s = { x | ? a b t. ~ ( a =b)/\a IN s
/\ b IN s /\ &0 < t /\
t < &1 /\ x = t %a + (&1-t)%b}`;;
...
( ? x y. x IN d /\ y IN d /\ d3 x y > &2 * t0 ))`;;

let strict_qua2 = new_definition ` strict_qua2 d (ch:real^3 -> bool ) s = (
quarter d s /\ CARD ch = 2 /\ ch SUBSET d
/\ ( ? x y. ch x /\ ch y /\ d3 x y > &2 * t0 ) )`;;

(* second time *)
let quartered_oct = new_definition `quartered_oct ( v:real^3 , w:real^3 ,
v1:real^3 , v2:real^3 , v3:real^3 , v4:real^3 ) s =
(~( (v = w)\/ ( v= v1) \/ ( v= v2) \/ ( v=v3) \/ ( v= v4) \/ ( w= v1) \/ (
w= v2) \/ ( w=v3) \/ ( w= v4) \/( v1= v2) \/ ( v1=v3) \/ ( v1= v4)\/( v2=v3)
\/ ( v2= v4)\/(v3 = v4))
/\ strict_qua2 { v , w , v1 , v2 } { v , w } s
/\ strict_qua2 { v , w , v2 , v3 } { v , w } s
/\ strict_qua2 { v , w , v3 , v4 } { v , w } s
/\ strict_qua2 { v , w , v4 , v1 } { v , w } s
/\ ( let s1 = { v , w , v1 , v2 } in let s2 = { v , w , v2 , v3 } in
let s3 = { v , w , v3 , v4 } in let s4 = { v , w , v4 , v1 } in
( conv0 s1 INTER conv0 s2 = {} /\ conv0 s2 INTER conv0 s3 = {} /\
conv0 s3 INTER conv0 s4 = {} /\ conv0 s4 INTER conv0 s1 = {} )) )`;;

let adjacent_pai = new_definition ` adjacent_pai v v1 v2 v3 v4 s = (
strict_qua2 { v , v1 , v3 , v2 } { v1 , v3 } s
/\ strict_qua2 { v , v1 , v3 , v4 } { v1 , v3 } s /\
( conv0 { v , v1 , v3 , v2 } INTER conv0 { v , v1 , v3 , v4 } = {} ) )`;;

let conflicting_dia = new_definition ` conflicting_dia v v1 v3 v2 v4 s = (
adjacent_pai v v1 v3 v2 v4 s
/\ adjacent_pai v v2 v4 v1 v3 s )`;;

let interior_pos = new_definition `interior_pos v v1 v3 v2 v4 s = (
conflicting_dia v v1 v3 v2 v4 s
/\ ~( conv0_2 { v1, v3 } INTER conv0 { v , v2 , v4 } = {} ))`;;

let anchor = new_definition ` anchor (v:real^3) v1 v2 = ( d3 v1 v2 <= sqrt (
&8 ) /\
...
(* note: this definition is only correct with sets s with CARD s =2 *)
let simp_conv0_2 = new_axiom `( ! v0 v1. conv0_2 { v0, v1 } = { x | ? t. &0
< t /\ t < &1 /\ x = t % v0 + (&1 - t ) % v1 } ) `;;
...
( ! v0 v1. conv0_2 { v0, v1 } = { x | ? t. &0 < t /\ t < &1 /\ x = t % v0 +
(&1 - t ) % v1 } )`;;

let convex3 = new_axiom ` convex {t | ?a b c.
&0 <= a /\ &0 <= b /\ &0 <= c /\ a + b + c = &1 /\ t = a % v1 + b % v2 + c %
v3}`;;

(* needs barrier Q_SYS *)

let chuachungminh = new_axiom `! bar. bar IN barrier s /\
...
let voronoi_trg = new_definition `voronoi_trg v S = { x | !w. ((S w) /\
~(w=v))
==> (dist ( x , v ) < dist ( x , w )) }`;;
...
(*
let border = new_definition ` border s = { x | ! ep. ep > &0 /\ ( ? a b. ~
(b IN s ) /\
dist (b, x) < ep /\ a IN s /\ dist (a, x) < ep ) }`;;
*)

(*
let packing = new_definition `packing (s:real^3 -> bool) = (! x y. s x /\ s
y /\ ( ~(x=y))
==> dist ( x, y) >= &2 ) `;;
*)
...
let trg_sub_se = prove ( `A SUBSET B <=> (!x. x IN A ==> x IN B )`,
SET_TAC[] );;
...
let trg_in_union = prove (` x IN ( A UNION B ) <=> x IN A \/ x IN B `,
SET_TAC[]);;

let not_in_set3 = prove ( `~ ( x IN { z | A z /\ B z /\ C z } ) <=> ~ A x \/
~ B x \/ ~ C x `,
SET_TAC[]);;

let trg_d3_sym = prove ( ` d3 x y = d3 y x `, SIMP_TAC[d3; DIST_SYM]);;

let affine_hull_e = prove (`affine hull {} = {}`, REWRITE_TAC[AFFINE_EMPTY;
AFFINE_HULL_EQ]);;

let wlog = MESON[]` (! a b. ( P a b = P b a ) /\ ( Q a b \/ Q b a ) ) ==>
((? a b . P a b ) <=> ( ? a b. P a b
/\ Q a b ))`;;

let wlog_real = MESON[REAL_ARITH `( ! a b:real. a <= b \/ b <= a )`] ` (!
a:real b:real . P a b = P b a ) ==> ((? a:real b . P a b ) <=> ( ? a b. P a
b
/\ a <= b ))`;;

let dkdx = MESON[REAL_ARITH ` ! a b:real. a <= b \/ b <= a `]`! P. (!a b u v
m n p . P a b u v m n p <=> P b a v u m n p)
==> ((?a b u v m n p. P a b u v m n p) <=> (?a b u v m n p. P a b u v m n p
/\ a <= b))`;;
...
&0 <= a /\ &0 <= b /\ &0 <= c /\ a + b + c = &1 /\ t = a % v1 + b % v2 + c %
v3}`, REPEAT GEN_TAC THEN REWRITE_TAC[ IN_ELIM_THM] THEN EXISTS_TAC ` &1 `
THEN EXISTS_TAC ` &0 ` THEN EXISTS_TAC ` &0 ` THEN REWRITE_TAC[ VECTOR_ARITH
` &1 % v1 + &0 % v2 + &0 % v3 = v1 `] THEN REAL_ARITH_TAC);;

let v3_in_convex3 = prove (`! v1 v2 v3. v3 IN
{t | ?a b c. &0 <= a /\ &0 <= b /\ &0 <= c /\ a + b + c = &1 /\ t = a % v1 +
b % v2 + c % v3}`, PURE_ONCE_REWRITE_TAC [ VECTOR_ARITH ` a % v1 + b % v2 +
c % v3 = c % v3 + a % v1 + b % v2`] THEN REWRITE_TAC[ SET_RULE ` {t | ?a b
c. &0 <= a /\ &0 <= b /\ &0 <= c /\ t = c % v3 + a % v1 + b % v2} =
{t | ? c a b. &0 <= c /\ &0 <= a /\ &0 <= b /\ t = c % v3 + a % v1 + b %
v2}`] THEN REWRITE_TAC[v1_in_convex3]
THEN
...
let v1v2v3_in_convex3 = prove (` ! v1 v2 v3 . v1 IN {t | ?a b c. &0 <= a /\
&0 <= b /\ &0 <= c /\
a + b + c = &1 /\ t = a % v1 + b % v2 + c % v3}
/\ v2 IN {t | ?a b c. &0 <= a /\ &0 <= b /\ &0 <= c /\ a + b + c = &1 /\ t =
a % v1 + b % v2 + c % v3}
/\ v3 IN {t | ?a b c. &0 <= a /\ &0 <= b /\ &0 <= c /\ a + b + c = &1 /\ t =
a % v1 + b % v2 + c % v3}`,
REPEAT GEN_TAC THEN
REWRITE_TAC[v1_in_convex3] THEN CONJ_TAC THEN
REWRITE_TAC[ SET_RULE ` {t | ?a b c. &0 <= a /\ &0 <= b /\ &0 <= c /\ a + b
+ c = &1 /\
t = a % v1 + b % v2 + c % v3}
= {t | ?b a c. &0 <= b /\ &0 <= a /\ &0 <= c /\ a + b + c = &1 /\ t = a % v1
+ b % v2 + c % v3} `]
THEN PURE_ONCE_REWRITE_TAC[VECTOR_ARITH` a % v1 + b % v2 + c % v3 = b % v2 +
a % v1 + c % v3 `] THEN
PURE_ONCE_REWRITE_TAC[REAL_RING ` a + b + c = b + a + c`] THEN
REWRITE_TAC[v1_in_convex3] THEN PURE_ONCE_REWRITE_TAC [ VECTOR_ARITH ` a %
v1 + b % v2 + c % v3 =
c % v3 + a % v1 + b % v2`]
THEN REWRITE_TAC[ SET_RULE ` {t | ?a b c. &0 <= a /\ &0 <= b /\ &0 <= c /\ a
+ b + c = &1 /\ t = c % v3 + a % v1 + b % v2} =
{t | ? c a b. &0 <= c /\ &0 <= a /\ &0 <= b /\ a + b + c = &1 /\ t = c % v3
+ a % v1 + b % v2}`]
THEN ONCE_REWRITE_TAC [ REAL_FIELD ` a + b + c = c + a + b ` ] THEN
REWRITE_TAC[v1_in_convex3]);;



let INTERS_SUBSET = SET_RULE `! P t0. P t0 ==> INTERS { t| P t } SUBSET
t0`;;


(* interactive proof:: *)

g `! t v1 v2 v3. convex t /\
{v1, v2, v3} SUBSET t
==> ( ! a b c. &0 <= a /\ &0 <= b /\ &0 <= c /\ a + b + c = &1 ==>
a % v1 + b % v2 + c % v3 IN t )`;;

e (REPEAT GEN_TAC THEN REWRITE_TAC[convex] THEN REWRITE_TAC [ TAUT ` a/\b
==> c <=>
a==> b ==> c `] THEN
REWRITE_TAC [SET_RULE ` {v1, v2, v3} SUBSET a <=> v1 IN a /\ v2 IN a /\ v3
IN a`] THEN
REPEAT DISCH_TAC THEN
REPEAT GEN_TAC THEN
REWRITE_TAC[ REAL_ARITH ` &0 <= a <=> &0 = a \/ &0 < a`] THEN
MATCH_MP_TAC (MESON[]` (a ==> c )/\(b ==> c ) ==> ( a\/b ==> c)`) THEN
CONJ_TAC THEN
SIMP_TAC [] THEN
SIMP_TAC[ REAL_ARITH ` &0 = a <=> a = &0 `] THEN
REWRITE_TAC[REAL_ARITH ` b = &0 \/ &0 < b <=> &0 <= b`] THEN
REWRITE_TAC [ REAL_ARITH ` &0 + a = a `; VECTOR_ARITH ` &0 % x + y = y `]
THEN
DISCH_TAC THEN
REPLICATE_TAC 2 (FIRST_X_ASSUM MP_TAC) THEN
REWRITE_TAC [ TAUT ` e /\ f /\ t ==> a ==> c <=> a /\ e ==> f ==> t ==> c`]
THEN
DISCH_TAC THEN
ASM_REWRITE_TAC[] );;
...
e (REWRITE_TAC[MESON[]` &0 < a /\ v1 IN t
==> v2 IN t
==> v3 IN t
==> &0 <= b
==> &0 <= c
...
e (REWRITE_TAC[ MESON[]`v1 IN t /\ v2 IN t /\ v3 IN t
==> &0 < a
==> &0 <= b
==> &0 <= c
==> a + b + c = &1
...
let sdf = REAL_FIELD ` &0 < a /\ &0 <= b /\ &0 <= c /\ a + b + c = &1 <=>
&0 < a /\
&0 <= b /\ &0 <= a /\
a = (a + b) * a / (a + b) /\
b = (a + b) * b / (a + b) /\
...
&0 <= a / (a + b) /\
&0 <= b / (a + b) /\
a / (a + b) + b / (a + b) = &1 /\ (a / ( a+ b) ) % v1 + ( b / ( a + b))% v2
IN t /\
&0 <= b /\
&0 <= a /\
a = (a + b) * a / (a + b) /\
...
let sap = MESON[VECTOR_MUL_RCANCEL] ` &0 < a /\
&0 <= a / (a + b) /\
&0 <= b / (a + b) /\
a / (a + b) + b / (a + b) = &1 /\
...
&0 <= c /\
(a + b) + c = &1
==> ((a + b) * a / (a + b)) % v1 + ((a + b) * b / (a + b)) % v2 + c % v3 IN
t `;;

e (REWRITE_TAC[sap]);;

e (REWRITE_TAC[VECTOR_ARITH ` ((a + b) * a / (a + b)) % v1 + ((a + b) * b /
(a + b)) % v2 + c % v3 = (a + b) % ( a / (a + b) % v1 + b / (a + b) % v2 ) +
c % v3 `]);;

e (MESON_TAC[]);;

let minconvex3 = top_thm();;
...
let xxx = SET_RULE ` ( ! x. x IN A ==> x IN B ) ==> A SUBSET B`;;

e (MATCH_MP_TAC xxx );;

e (REWRITE_TAC[IN_ELIM_THM]);;

e (REWRITE_TAC[IN_INTERS]);;

let xxx = MESON[] `( !x. (?a b c.
&0 <= a /\
&0 <= b /\
&0 <= c /\
a + b + c = &1 /\
...
e (REWRITE_TAC[ IN_ELIM_THM ; TAUT ` a ==> b ==> c <=> a /\ b ==> c `]);;

e (MESON_TAC[minconvex3]);;

let convex3_in_inters = top_thm();;

g ` ! v1 v2 v3. conv_trg {v1 , v2 ,v3} = {t | ?a b c. &0 <= a /\ &0 <= b /\
&0 <= c /\ a + b + c = &1
/\ t = a % v1 + b % v2 + c % v3}`;;

e ( REPEAT GEN_TAC THEN
REWRITE_TAC[conv; hull] THEN REPEAT GEN_TAC THEN
REWRITE_TAC[ SET_RULE` a = b <=> a SUBSET b /\ b SUBSET a `] THEN CONJ_TAC
THEN
...
==> ( voronoi2 v0 s SUBSET X UNION Z UNION VC v0 s )`;;

e (DISCH_TAC THEN SIMP_TAC[trg_sub_se] THEN SIMP_TAC [ trg_in_union ]
THEN REWRITE_TAC[ MESON[] `( !x. x IN voronoi2 v0 s ==> x IN X \/ x IN Z \/
x IN VC v0 s )
<=> ( !x. x IN voronoi2 v0 s ==> x IN Z \/ x IN VC v0 s \/ x IN X )`] THEN
SIMP_TAC [ MESON[] ` A ==> B \/ C <=> A /\ ~ B ==> C`] THEN
ASM_REWRITE_TAC[voronoi2 ; VC ] THEN REWRITE_TAC[ not_in_set3] THEN
REWRITE_TAC [IN_ELIM_THM] THEN
REWRITE_TAC[trg_d3_sym]THEN REWRITE_TAC[ MESON[] ` (( A /\ B ) /\ C ) /\ (
~B \/ D \/ E ) <=>
(( A /\ B ) /\ C ) /\ ( D \/ E )`] THEN SIMP_TAC[ MESON[] ` ((A/\B) /\ C)
<=> A /\ B /\ C`]
THEN REWRITE_TAC [d3 ; voronoi_trg;IN_ELIM_THM] THEN
REWRITE_TAC [ MESON[ IN ; EQ_SYM_EQ ; DIST_SYM] `(!w. s w /\ ~(w = v0) ==>
dist (x,v0) < dist (x,w))
<=> (!y. y IN s /\ ~(v0 = y) ==> dist (v0,x) < dist (x,y)) ` ] THEN
REWRITE_TAC[ MESON[] ` A /\ B /\ C /\ ( D \/ ~A) <=> A /\ B /\ C /\ D` ;
DIST_SYM ; EQ_SYM_EQ ]
THEN REWRITE_TAC[obstructed] THEN
REWRITE_TAC[MESON[] `
~(x IN UNIONS {aff_ge {v0} {v1, v2} | {v0, v1, v2} IN barrier s}) /\
(?bar. bar IN barrier s /\ ~(conv0_2 {v0, x} INTER conv_trg bar = {}))
<=> (?bar. bar IN barrier s /\ ~(conv0_2 {v0, x} INTER conv_trg bar = {}) /\
~(x IN UNIONS {aff_ge {v0} {v1, v2} | {v0, v1, v2} IN barrier s}))`]
THEN GEN_TAC THEN REWRITE_TAC[TAUT `( a /\ b /\ c ==> d) = ( c ==> ( a /\ b)
==> d )`]
...
THEN REWRITE_TAC[ MESON[] ` ( ? x . p x ) /\ ng <=> ( ? x. p x /\ ng )`]
THEN
REWRITE_TAC[MESON [IN ; EQ_SYM_EQ ; DIST_SYM ] ` ( !y. (y IN s /\ ~(v0 = y)
==> dist (v0,x) < dist (x,y))) <=>
( !y. ( s y /\ ~(y = v0) ==> dist (x,v0) < dist (x,y)) ) `] THEN
PURE_ONCE_REWRITE_TAC[SET_RULE ` (!y. s y /\ ~(y = v0) ==> dist (x,v0) <
dist (x,y)) <=> ( x IN { x | !y. s y /\ ~(y = v0) ==> dist (x,v0) < dist
(x,y)} )`]
THEN REWRITE_TAC [ MESON[voronoi_trg] ` ( x IN {x | !y. s y /\ ~(y = v0) ==>
dist (x,v0) < dist (x,y)} )
...
PURE_ONCE_REWRITE_TAC[ MESON[voronoi2] `{x | x IN voronoi_trg v0 s /\ d3 x
v0 < &2} = voronoi2 v0 s `]
THEN REWRITE_TAC [ TAUT ` (( a /\ b /\ c)/\d)/\e <=> a /\ b /\ c/\d/\e `]
THEN
...
~(x IN UNIONS {aff_ge {v0} {v1, v2} | {v0, v1, v2} IN barrier s}) /\
{v0} UNION {v1, v2, v3} IN Q_SYS s) `] THEN
REWRITE_TAC[ SET_RULE` ~ (a INTER b ={}) <=> (? i j. i IN a /\ j IN b /\ i =
j)`] THEN
PURE_ONCE_REWRITE_TAC[SET_RULE ` ~ ( a INTER b = {}) <=> ( ? x y. x IN a /\
y IN b /\ x = y)`] THEN
REWRITE_TAC[simp_conv0_2; simpl_conv3] THEN
REWRITE_TAC[IN_ELIM_THM] THEN
...
PURE_ONCE_REWRITE_TAC[MESON[ REAL_FIELD ` &0 < &1 - t ==> ( ! a . a = (&1 -
t ) * ( a / ( &1 - t )))`]`
&0 < &1 - t <=> &0 < &1 - t /\ --t = (&1 - t) * ( --t ) / (&1 - t) /\ a =
(&1 - t) * a / (&1 - t)
...
&0 <= c /\
a + b + c = &1 /\
(&1 - t) % x = ((&1 - t) * --t / (&1 - t)) % v0 + ((&1 - t) * a / (&1 - t))
% v1 + ((&1 - t) * b / (&1 - t)) % v2 +((&1 - t) * c / (&1 - t)) % v3)`]
THEN
REWRITE_TAC[VECTOR_ARITH` ((&1 - t) * --t / (&1 - t)) % v0 +
((&1 - t) * a / (&1 - t)) % v1 +
((&1 - t) * b / (&1 - t)) % v2 +
((&1 - t) * c / (&1 - t)) % v3
= (&1 - t ) % ( (--t / (&1 - t)) % v0 + (a / (&1 - t)) % v1 + (b / (&1 - t))
% v2 + (c / (&1 - t)) % v3)`] THEN
PURE_ONCE_REWRITE_TAC[REAL_ARITH` &0 < a <=> &0 < a /\ ~ ( a = &0 )`] THEN
REWRITE_TAC[MESON[MESON[VECTOR_MUL_LCANCEL_IMP]` ~ (( &1 - t) = &0 ) /\ (&1
- t) % x =
(&1 - t) %
(--t / (&1 - t) % v0 +
a / (&1 - t) % v1 +
...
REWRITE_TAC[ MESON[REAL_LT_IMP_NZ ; REAL_FIELD ` ~ ( a = &0 ) ==> ( ! b. b =
a * b / a )`]`(&0 < &1 - t /\
--t = (&1 - t) * --t / (&1 - t) /\
a = (&1 - t) * a / (&1 - t) /\
b = (&1 - t) * b / (&1 - t) /\
c = (&1 - t) * c / (&1 - t)) <=> (&0 < &1 - t )`] THEN
PURE_ONCE_REWRITE_TAC [ MESON[REAL_ARITH ` (! a . &0 < a ==> &0 <= a)` ;
REAL_LE_DIV ] `
...
PURE_ONCE_REWRITE_TAC[ MESON[ REAL_FIELD ` a + b + c = &1 /\ &0 < &1 - t ==>
--t / (&1 - t) +
a / (&1 - t) +
b / (&1 - t) +
c / (&1 - t) = &1 `]
...
{v0} UNION {v1, v2, v3} IN Q_SYS s))`] THEN
DISCH_TAC THEN
MATCH_MP_TAC (MESON[REAL_ARITH ` ! a b. a <= b \/ b <= a `] ` ( ! (a:real) b
(v1:real^3) (v2:real^3). Q a b v1 v2 = Q b a v2 v1 ) /\
((? (a:real) (b:real) (v1:real^3) (v2:real^3). Q a b v1 v2 ) /\ (? (a:real)
b (v1:real^3) (v2:real^3). a <= b /\ Q a b v1 v2) ==> c) ==> ((? a b
(v1:real^3) (v2:real^3). Q a b v1 v2) ==> c ) `) THEN
CONJ_TAC THEN REPEAT GEN_TAC THEN
REWRITE_TAC [ MESON[]` (quasi_tri {v2, v1, v3} s \/ (?v4. {v2, v1, v3} UNION
{v4} IN Q_SYS s)) /\
&0 < t /\
&0 < &1 - t /\
&0 <= b /\
...
--t / (&1 - t) + b / (&1 - t) + a / (&1 - t) + c / (&1 - t) = &1 )/\ nnn` ]
THEN
REWRITE_TAC[ MESON[]` ( a <=> b ) <=> ( a ==> b ) /\ ( b==> a )`] THEN
REWRITE_TAC[]);;
...
b / (&1 - t) % v2 +
c / (&1 - t) % v3 `] THEN
REWRITE_TAC [ SET_RULE ` ! v1 v2 (v3:real^3 ). { v1, v2 , v3} = { v2 , v1 ,
v3}`] THEN
FIRST_X_ASSUM MP_TAC THEN MESON_TAC[] THEN
DISCH_TAC THEN ONCE_REWRITE_TAC[ REAL_ARITH ` ! a b (c:real). (a + b + c =
&1 /\
...
a / (&1 - t) % v1 +
c / (&1 - t) % v3` ] THEN
ONCE_REWRITE_TAC [ SET_RULE ` ! v1 v2 (v3:real^3). {v1 , v2 ,v3} = { v2, v1,
v3}`] THEN
REWRITE_TAC[ MESON[ SET_RULE ` { a , b, c} = { b, a , c}`]` {v1, v0, v2} IN
barrier s
...
ONCE_REWRITE_TAC[ SET_RULE `! v1 v2 (v3:real^3). { v1, v2, v3} = {v2, v1
,v3} `] THEN
ONCE_REWRITE_TAC[MESON [SET_RULE ` ! a b c. { a, b, c} = {b, a, c}` ]`( ! a
b c s. {a, b, c} IN
barrier s <=> {b, a, c } IN barrier s) `] THEN FIRST_X_ASSUM MP_TAC THEN
MESON_TAC[] );;
...
MATCH_MP_TAC (MESON[ REAL_ARITH ` ! a (b:real). a <= b \/ b <= a `;
REAL_ARITH`! a b c:real. a <= b /\ b <= c ==> a <= c`] ` ( !(a:real) b
(c:real) (v1:real^3) (v2:real^3) (v3:real^3) t.
Q a b c v1 v2 v3 t <=> Q c b a v3 v2 v1 t ) /\ ((? (a:real) b (c:real)
(v1:real^3) (v2:real^3) (v3:real^3) t.
a <= b /\ a <= c /\ Q a b c v1 v2 v3 t ) ==> last ) ==> ((? (a:real) b
(c:real) (v1:real^3) (v2:real^3) (v3:real^3) t.
...
c / (&1 - t) % v3 +
b / (&1 - t) % v2 +
a / (&1 - t) % v1 ` ;MESON[SET_RULE ` ! a b c. { a, b, c} = {c, b, a}` ]`( !
a b c s. {a, b, c} IN
barrier s <=> {c, b, a } IN barrier s) ` ; SET_RULE `! v1 v2 (v3:real^3). {
v1, v2, v3} = {v3, v2 ,v1} `]);;
...
e (REWRITE_TAC[ MESON[REAL_FIELD ` &0 = a ==> a / b = &0 `; VECTOR_ARITH`! v
w. &0 % v + w = w `] `
&0 = a /\
a <= b /\
a <= c /\
...
e (REWRITE_TAC[ MESON[ SET_RULE ` {v0} UNION {v1, v2, v3} = {v0, v2, v3}
UNION {v1 } ` ;
barrier ] ` (&0 = a /\
x = --t / (&1 - t) % v0 + b / (&1 - t) % v2 + c / (&1 - t) % v3) /\
~(v0 IN {v1, v2, v3}) /\
...
e (ONCE_REWRITE_TAC[ MESON[ MESON[barrier; SET_RULE ` {v0, v2, v3} UNION
{v1} IN Q_SYS s ==> {v0, v2, v3} IN {{v1, v2, v3} | quasi_tri {v1, v2, v3} s
\/
(?v4. {v1, v2, v3} UNION {v4} IN Q_SYS s)} `]` {v0, v2, v3} UNION {v1} IN
Q_SYS s ==> {v0, v2, v3} IN barrier s `]
` {v0, v2, v3} UNION {v1} IN Q_SYS s <=>{v0, v2, v3} UNION {v1} IN Q_SYS s
...
e (REWRITE_TAC[MESON[]` (?t. (?v0 v1 v2. {v0, v1, v2} IN barrier s /\ t =
aff_ge {v0} {v1, v2}) /\
x IN t) <=>
...
e (REWRITE_TAC[ MESON []`&0 < a /\ a <= b /\ a <= c /\ last <=> ( &0 < a /\
a <= b /\ a <= c ) /\ last`;
REAL_ARITH ` &0 < a /\ a <= b /\ a <= c <=> a <= b /\ a <= c /\ &0 < a /\ &0
< b /\ &0 < c `]);;
...
e (REWRITE_TAC[ MESON[REAL_LT_DIV] ` (&0 < t /\ &0 < a /\ &0 < b /\ &0 < c
/\ &0 < &1 - t)
...
b / (&1 - t) % v2 +
c / (&1 - t) % v3 + --t / (&1 - t) % v0 `;
REAL_ARITH ` --t / (&1 - t) + a / (&1 - t) + b / (&1 - t) + c / (&1 - t) =
&1 <=>
--t / (&1 - t) + a / (&1 - t) + b / (&1 - t) + c / (&1 - t) = &1 /\
...
e (SET_TAC[]);;
 (* no sub goal *)
let lemmaeight_1 = top_thm();;


(* let lemma8_1 = prove (` ( center_pac s v0 /\
Z = UNIONS { aff_ge {v0} {v1, v2} | {v0, v1, v2} IN barrier s} /\
X = UNIONS {aff_gt {v0} {v1, v2, v3} INTER
aff_le {v1, v2, v3} {v0} INTER
voronoi2 v0 s | {v0, v1, v2, v3} IN Q_SYS s} )
==> ( voronoi2 v0 s SUBSET X UNION Z UNION VC v0 s )`,
DISCH_TAC THEN SIMP_TAC[trg_sub_se] THEN SIMP_TAC [ trg_in_union ]
THEN REWRITE_TAC[ MESON[] `( !x. x IN voronoi2 v0 s ==> x IN X \/ x IN Z \/
x IN VC v0 s )
<=> ( !x. x IN voronoi2 v0 s ==> x IN Z \/ x IN VC v0 s \/ x IN X )`] THEN
SIMP_TAC [ MESON[] ` A ==> B \/ C <=> A /\ ~ B ==> C`] THEN
...
<=> {v0, v1, v2} IN barrier s `] THEN
FIRST_X_ASSUM MP_TAC THEN MESON_TAC[] THEN
DISCH_TAC THEN ONCE_REWRITE_TAC[REAL_ARITH ` !a b c.
a + b + c = &1 /\
--t / (&1 - t) + a / (&1 - t) + b / (&1 - t) + c / (&1 - t) = &1 <=>
b + a + c = &1 /\
--t / (&1 - t) + b / (&1 - t) + a / (&1 - t) + c / (&1 - t) = &1 `;
...
a <= b /\ a <= c /\ Q a b c v1 v2 v3 t ) ==> last ) ==> ((? (a:real) b
(c:real) (v1:real^3) (v2:real^3) (v3:real^3) t.
a <= b /\ Q a b c v1 v2 v3 t ) ==> last ) `) THEN
CONJ_TAC THEN REPEAT GEN_TAC THEN
MESON_TAC[REAL_ARITH ` !a b c.
a + b + c = &1 /\
--t / (&1 - t) + a / (&1 - t) + b / (&1 - t) + c / (&1 - t) = &1 <=>
c + b + a = &1 /\
--t / (&1 - t) + c / (&1 - t) + b / (&1 - t) + a / (&1 - t) = &1 `;
...
a b c s. {a, b, c} IN
barrier s <=> {c, b, a } IN barrier s) ` ; SET_RULE `! v1 v2 (v3:real^3). {
v1, v2, v3} = {v3, v2 ,v1} `] THEN
REWRITE_TAC[ MESON[ REAL_ARITH ` ! a. &0 <= a <=> &0 = a \/ &0 < a `] `
a <= b /\
a <= c /\
(quasi_tri {v1, v2, v3} s \/ (?v4. {v1, v2, v3} UNION {v4} IN Q_SYS s)) /\
&0 < t /\
...
&0 < &1 - t /\
ccc `] THEN
REWRITE_TAC[ MESON[REAL_FIELD ` &0 = a ==> a / b = &0 `; VECTOR_ARITH`! v w.
&0 % v + w = w `] `
&0 = a /\
a <= b /\
a <= c /\
...
x =
--t / (&1 - t) % v0 + &0 % v1 + b / (&1 - t) % v2 + c / (&1 - t) % v3
<=> x = --t / (&1 - t) % v0 + b / (&1 - t) % v2 + c / (&1 - t) % v3 `] THEN
REWRITE_TAC[ MESON[ SET_RULE ` {v0} UNION {v1, v2, v3} = {v0, v2, v3} UNION
{v1 } ` ;
barrier ] ` (&0 = a /\
x = --t / (&1 - t) % v0 + b / (&1 - t) % v2 + c / (&1 - t) % v3) /\
~(v0 IN {v1, v2, v3}) /\
...
~(x IN UNIONS {aff_ge {v0} {v1, v2} | {v0, v1, v2} IN barrier s}) /\
{v0} UNION {v1, v2, v3} IN Q_SYS s`] THEN
ONCE_REWRITE_TAC[ MESON[ MESON[barrier; SET_RULE ` {v0, v2, v3} UNION {v1}
IN Q_SYS s ==> {v0, v2, v3} IN {{v1, v2, v3} | quasi_tri {v1, v2, v3} s \/
(?v4. {v1, v2, v3} UNION {v4} IN Q_SYS s)} `]` {v0, v2, v3} UNION {v1} IN
Q_SYS s ==> {v0, v2, v3} IN barrier s `]
` {v0, v2, v3} UNION {v1} IN Q_SYS s <=>{v0, v2, v3} UNION {v1} IN Q_SYS s
/\ {v0, v2, v3} IN barrier s `] THEN
REWRITE_TAC[MESON[] ` --t / (&1 - t) <= &0 /\
&0 <= a / (&1 - t) /\
&0 <= b / (&1 - t) /\
&0 <= c / (&1 - t) /\
(a + b + c = &1 /\
...
{v0, v2, v3} IN barrier s /\
x = --t / (&1 - t) % v0 + b / (&1 - t) % v2 + c / (&1 - t) % v3) /\
~(x IN UNIONS {aff_ge {v0} {v1, v2} | {v0, v1, v2} IN barrier s}) `] THEN
REWRITE_TAC[ IN_UNIONS; IN_ELIM_THM] THEN
REWRITE_TAC[MESON[]` (?t. (?v0 v1 v2. {v0, v1, v2} IN barrier s /\ t =
aff_ge {v0} {v1, v2}) /\
x IN t) <=>
(?v0 v1 v2. {v0, v1, v2} IN barrier s /\ x IN aff_ge {v0} {v1, v2}) `] THEN
REWRITE_TAC[simp_def; IN_ELIM_THM] THEN
REWRITE_TAC[ MESON[]`
(a + b + c = &1 /\
--t / (&1 - t) + a / (&1 - t) + b / (&1 - t) + c / (&1 - t) = &1) /\
(&1 - t) % x =
...
&0 <= w /\
u + v + w = &1 /\
x = u % v0 + v % v1 + w % v2)) <=> F `] THEN
REWRITE_TAC[ MESON []`&0 < a /\ a <= b /\ a <= c /\ last <=> ( &0 < a /\ a
<= b /\ a <= c ) /\ last`;
REAL_ARITH ` &0 < a /\ a <= b /\ a <= c <=> a <= b /\ a <= c /\ &0 < a /\ &0
< b /\ &0 < c `] THEN
REWRITE_TAC[ MESON[] ` (a <= b /\ a <= c /\ &0 < a /\ &0 < b /\ &0 < c) /\
(quasi_tri {v1, v2, v3} s \/
(?v4. {v1, v2, v3} UNION {v4} IN Q_SYS s /\ {v1, v2, v3} IN barrier s)) /\
&0 < t /\
&0 < &1 - t /\ last
...
( &0 < t /\ &0 < a /\ &0 < b /\ &0 < c /\
&0 < &1 - t ) /\ last `] THEN
REWRITE_TAC[ MESON[REAL_LT_DIV] ` (&0 < t /\ &0 < a /\ &0 < b /\ &0 < c /\
&0 < &1 - t)
<=> (&0 < t /\ &0 < a /\ &0 < b /\ &0 < c /\ &0 < &1 - t /\
&0 < a / ( &1 - t ) /\
&0 < b / ( &1 - t ) /\
...
&1 <=>
--t / (&1 - t) + a / (&1 - t) + b / (&1 - t) + c / (&1 - t) = &1 /\
a / (&1 - t) + b / (&1 - t) + c / (&1 - t) + ( --t / (&1 - t)) = &1 `] THEN
ONCE_REWRITE_TAC[ MESON[] `(?t. (?v1 v2 v3 v0 s.
{v0, v1, v2, v3} IN Q_SYS s /\
t =
{t | ?x y z w.
&0 < y /\
...
a + b + c + d = &1 /\
t = a % v1 + b % v2 + c % v3 + d % v0} INTER
{x | x IN voronoi2 v0 s}) `] THEN
REWRITE_TAC[ MESON[SET_RULE` { a } UNION { f, b , c } = {a, f, b, c}`]
` {v0} UNION {v1, v2, v3} IN Q_SYS s <=> {v0, v1, v2, v3} IN Q_SYS s `] THEN
SET_TAC[]);;
 *)
