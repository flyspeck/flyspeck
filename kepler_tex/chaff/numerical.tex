...
% XX (insert) See Figure~\ref{fig:optimal-circles}.
...
Later in 1947, within minutes\footnote{Danztig himself described his
  ...
  programs\dots.  Thus I learned about \newterm{Farkas' Lemma}, and
  about \newterm{Duality} for the first time''~\cite{Dan91}.} of first
...
program defined by the same data, but in \newterm{dual} form:
\begin{equation}
\min_y {y b}
\end{equation}
such that
...
\end{equation}
is a \newterm{certificate} that the system \eqref{eqn:empty} is infeasible.  Indeed,
...
%A certificate may be found by solving the given linear programming
%problem \eqref{eqn:ci}.  
...
A \newterm{certificate} of the assembly problem \eqref{def:lap}
...
%format \eqref{eqn:lp1} \eqref{eqn:lp2}.
