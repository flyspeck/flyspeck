...
%\begin{lemma}\tlabel{tarski:flat-Q}\usage{ lemma:voronoi-truncation-over-Q  }
...
\begin{lemma}\guid{FRCXQKB} \tlabel{tarski:qrtet-pair-pass}\usage{DCG-[4.22]{ lemma:qrtet-pair-pass}}
%{Lemma 1.4}
...
\begin{lemma}\guid{VAXNRNE} \tlabel{tarski:pass-makes-quarter}\usage{DCG-[4.34]{ tarski:pass-makes-quarter}}
%{Lemma 1.9}
...
\begin{lemma}\guid{LOKDUSU}\tlabel{tarski:sqrt2-unobstructed}\usage{DCG-[5.20]{ lemma:sqrt2-unobstructed}}\usage{DCG-[5.20]{ lemma:sqrt2-unobstructed}} %DCG p48, L.5.20.
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
...
% OLD VERSION:
%\begin{lemma}\guid{TLKHAUY} \tlabel{tarski:sqrt2-unobstructed} %DCG p48, L.5.20.
...
\begin{lemma}\guid{FFSXOWD}\tlabel{tarski:unobstr-t0}\usage{DCG-[5.22]{ lemma:unobstr-t0}}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that the pairwise distances are at least $2$.  Assume
that 
$$
...
\begin{lemma}\guid{ZODWCKJ} \tlabel{tarski:v-interior}\usage{DCG-[4.16]{ lemma:v-interior}}
...
\begin{lemma}\guid{VSMPQYO} \tlabel{tarski:chi}\usage{DCG-[5.15]{ lemma:chi}}
Let $S=\{v_1,\ldots,v_4\}$ be a set of four points
in $\ring{R}^3$.  
...
\begin{lemma}\guid{IAALCFJ} \tlabel{tarski:at-most-one-negative}
%proclaim{Lemma 2.1}
Let $S=\{v_1,v_2,v_3,v_4\}$ be a set of four points in $\ring{R}^3$.
Assume that 
   $$
...
\begin{lemma}\guid{NJBVVWG}\tlabel{tarski:sqrt2-chi+}\usage{DCG-[5.18]{ lemma:sqrt2-chi+}}
%
...
(XX add details.)  This falls within the first case of the lemma.

%% XX MOVE THIS BACK TO FINE.TEX
...
\newpage
\begin{lemma}\guid{DVLHHMF}\tlabel{tarski:prev}\usage{DCG-[9.7]{ lemma:prev}}
...
\begin{proof} The relation $\epsilon$ takes incompotable on
...
Then there exists $i\in\{1,2\}$ such that one of the two occurs:
 \begin{itemize}
  \item $|w_1-w_2|\le 2.51$, $|v_i-w_j|\le 2.51$, for $j=1,2$.  Also,
     $$
     \op{aff}_+(\{v_0,v_i,w_j\},w_k) = \op{aff}_+(\{v_0,v_i,w_j\},p_j),
     $$
   for some matching $\{j,k\}=\{1,2\}$.
  \item $\rad_V\{v_0,v_i,w_1,w_2\}<\eta(2,2.51,|v_0-v_i|)$.  Also,
     $$
     \op{aff}_+(\{v_0,v_i,w_j\},w_k) = \op{aff}_+(\{v_0,v_i,w_j\},p_j),
     $$
for both matchings $\{j,k\} = \{1,2\}$.
 \end{itemize}
\end{lemma}

\begin{proof}
Let $x$ be a point in the intersection of $\op{rog}^0(v_0,w_1,v_1,p_1,b_1)$ 
with $\op{rog}^0(v_0,w_2,v_2,p_2,b_2)$.  Up to renaming indices, 
we may assume that
$\epsilon(x,\{v_0,w_1,w_2\},v_0,u)$ when $u=w_2$ but not when $u=w_1$.  
Let $S=\{v_0,v_1,w_1,w_2\}$.

We choose the index $i=1$ in the conclusion of the lemma.

XXX MESSED UP XXD.

We have $|w_2-v_1|\le 2.51$. 
Otherwise $\rad_V(S) \ge \eta(2,2.51,|v_1-v_0|)$ and
the orientation of $S$ at $w_2$ is positive.  In this case, we have
$\epsilon(x,\{v_0,w_1,w_2\},v_0,w_1)$ contrary to hypothesis.  
...
\begin{lemma}\guid{BIQFYVG}\tlabel{tarski:back}\usage{DCG-[5.31]{ lemma:back}} 
\FIXX{Revisit}
...
%% XX Where is tarski:v-interior-alt used?

% constant was 2.6 in DCG.
\begin{lemma}\usage{DCG-[4.15]{ lemma:v-interior-alt}}\guid{GEWKDPE} \tlabel{tarski:v-interior-alt}
...
\begin{lemma}\guid{HTYDGWI}  \tlabel{tarski:skew-quad}\usage{DCG-[4.30]{ lemma:skew-quad}}
%\proclaim{Lemma 1.6}
...
\begin{lemma}\guid{XLHACRX} \tlabel{tarski:oct-t0}\usage{DCG-[4.31]{ lemma:oct}}
%\proclaim{Lemma 1.7}
...
\begin{lemma}\guid{ZTFABJM}\tlabel{tarski:gc}\usage{DCG-[10.12]{ lemma:gc}}
...
\begin{lemma}\guid{VTIVSIF}\tlabel{tarski:enclosed}\usage{DCG-[10.13]{ lemma:enclosed}} % {Lemma 2.2}
...
\begin{lemma}\guid{YWPHYZU} \tlabel{tarski:double-face}\usage{DCG-[4.32]{ lemma:double-face}}
%proclaim{Lemma 1.8}
...
\begin{lemma}\guid{PYURAKS} \tlabel{tarski:single-enclosed}\usage{DCG-[4.33]{ lemma:single-enclosed}}
%proclaim{Lemma 1.11}
...
\begin{lemma}\guid{BYUOUXO} \tlabel{tarski:qrtet-over}\usage{DCG-[4.35]{ lemma:qrtet-over}}
...
\begin{lemma}\guid{ZYMHOWN} \tlabel{tarski:oct-over}\usage{DCG-[4.36]{ lemma:oct-over}}
...
\begin{lemma}\guid{OOHHKOI} \tlabel{tarski:adj-over}\usage{DCG-[4.37]{ lemma:adj-over}}
...
\begin{lemma}\guid{VXMZWGM}\tlabel{tarski:iso-over}\usage{DCG-[4.39]{ lemma:iso-over}}
...
Assume that $\{v_0,u,v\}$ are not collinear.
...
%\begin{lemma}\tlabel{tarski:4.12.1}
%%    \oldlabel{4.12.1}
...
Let $\{v_0,v,w\}$ be a set of three points in $\ring{R}^3$.  Let $t,\lambda,\lambda'>0$.  If $\op{rcone}(v_0,v,\cosarc(|v-v_0|,t,\lambda))$ meets
