...
\end{definition}
\indy{Notation}{0@$'$ (polar local fan $(V',E',F')$)}%
\indy{Notation}{V@$V'$ (polar node set)}%
\indy{Notation}{E@$E'$ (polar edge set)}%
...
\indy{Notation}{3@$\sim$ (equal up to positive scalar)}%
...
We slice torsors as we did earlier with local fans (Figure~\ref{fig:slice-torsor}).
...
2\le a_{i,j}(s)\le \stab \text{ and } a_{i,j}(s)\le
  b_{i,j}(s), \text{ for all } s\in S \text{ and all } i,j\in I(s) \text{ such that } i\ne j.
\]  
  Also, 
...
If $\{i,j\}\in J(s)$, then $\leftclosed
  a_{i,j}(s),b_{i,j}(s)\rightclosed=\leftclosed\sqrt{8},\stab\rightclosed$.
...
\indy{Notation}{ZZsigma@$\sigma=\pm1$}%
