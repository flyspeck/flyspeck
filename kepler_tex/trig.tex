...
\begin{definition}[$\atn$]\guid{CODE}\label{def:atn}
\begin{displaymath}
...
\begin{definition}[arccos]\guid{CODE}\label{def:arccos}\formal{acs,\ ACS\_COS,\ COS\_ACS}
  By the inverse function theorem of \hyperref[back:analysis]{real
...
\begin{definition}[$\ring{R}^N$,~vector]\guid{CODE}
...
\begin{definition}[vector addition,~scalar multiplication]\guid{CODE}
...
\begin{definition}[dot product]\guid{CODE}\label{def:dot}
There is a \newterm{dot product}
\begin{displaymath}
...
\begin{definition}[norm]\guid{CODE}\label{def:norm}
...
\begin{definition}[affine hull]\guid{CODE}
A set $A\subset\ring{R}^N$ is \newterm{affine}, if for
every $\v,\w\in A$ and every $t \in \ring{R}$, 
\begin{displaymath}
  t \v + (1-t) \w \in A.
\end{displaymath}
The \newterm{affine hull} of a set $S\subset\ring{R}^N$ is the smallest affine set
containing $S$. 
...
\begin{definition}[affine]\guid{CODE}\label{def:aff} 
...
\begin{definition}[convex hull]\guid{CODE} 
...
\begin{definition}[cone]\guid{CODE}
...
%\begin{definition}[Voronoi cell $\Omega$]\guid{CODE}
...
\begin{definition}[line,~collinear,~parallel]\guid{CODE}
...
\begin{definition}[plane, half plane, coplanar]\guid{CODE}\label{def:plane}	
...
\begin{definition}[half space]\guid{CODE} 
A set
...
%% WW repeated DEF.
\begin{definition}[$\Delta$]\guid{CODE}\label{def:delta}
...
\begin{remark}[Cayley-Menger determinant]\guid{CODE}
...
\begin{definition}[angle,\ arclength]\guid{CODE}\label{def:angle}
...
% \begin{definition}[arc length]\guid{CODE} 
...
\begin{definition}[arc]\guid{CODE}\label{def:arc}
Define
...
\begin{definition}[$\ups$]\guid{CODE}\label{def:ups}
...
\begin{definition}[cross product]\guid{CODE}\label{def:cross}   
...
\begin{definition}[dihedral angle]\guid{CODE}\label{def:dih} When $\w_0\ne \w_1$,
...
\begin{definition}[frame,~positive,~adapted]\guid{CODE}
...
\begin{definition}[azim]\guid{CODE}
...
%
%\begin{definition}[spherical coordinates]\guid{CODE}
...
%\begin{definition}[azimuth]\guid{CODE}\label{def:azimuth}
...
\begin{definition}[spherical coordinates]\guid{CODE}\label{def:sph}
...
%\indy{Index}{angle!zenith}%
%
%\begin{definition}[polar coordinate]\guid{CODE}\label{def:polar}
...
\begin{definition}[cyclic set]\guid{CODE} 
...
\begin{definition}[azimuth cycle]\guid{CODE}
