...
A fan is a geometric object that bridges the pass between sphere packings and hypermaps.  A fan is a particular geometric realization of a hypermap.  It is closely related to the notion of planar graph, but relates more directly to sphere packings.   The main result of this chapter gives a simple version of the Euler formula, which implies that the hypermap of a fan is planar.  The proofs have been carefully organized to avoid any use of the Jordan curve theorem.
\indy{Index}{fan}
\indy{Index}{hypermap}
\indy{Index}{planar}
\indy{Index}{packing}
...
\begin{lemma}\guid{XOHLED}\rating{XX}  Let $(V,E)$ be a fan.
...
Write $v(x)\in V$ for the vertex corresponding to a dart $x\in D$.  The lemma identifies $V$ with the set of nodes of $D$.
...
\item If $0 < t \le 1$, and $C^0(t)$ meets $X$, then $\{v,v_1\}\in E$, $t=1$,  and $C^0(1) = C^0(\{v,v_1\})$.
...
is less than $\pi$ at $w$, which means that for any $t$ for which  $C^0(t)$ meets $w$ 
there is a smaller $t'''<t$ for which $C^0(t''')$ meets a blade at $w$.
...
This lemma has several significant corollaries.  The corollaries all hold under the assumptions of the lemma: $(V,E)$ is a fan such that $\op{azim}(x)<\pi$ for all darts $x$ in its fan.  The proof of the lemma appears after the corollaries.
...
\begin{lemma}\guid{DWFBRQY}\rating{XX} Lemma~\ref{lemma:face} holds under the additional assumption that $N(V,E) = 0$.
\end{lemma}
...
\begin{lemma}\guid{ZSZIUQE}\rating{XX} 
Assume that $N(V,E)>0$.  Let $E'=E\cup \{\{v,w\}\}$, as above.  Then
$N(V,E')<N(V,E)$.
\end{lemma}
...
be to use the modified fan $(V,E')$ to show that the conclusions of the Lemma~\ref{lemma:face} hold for $(V,E)$ as well.  Each conclusion will be established for $(V,E)$ in turn through a series of lemmas.  The result will contradict the assumption that
...
\begin{lemma}\guid{TXFBALB}\rating{XX}  The area conclusion (Lemma~\ref{lemma:face}.iii) holds for every minimal counterexample $(V,E)$.
\end{lemma}
...
\begin{lemma}\guid{GGZWYRM}\rating{XX}  Let $(V,E)$ be a minimal counterexample.  For any dart $x$ on the face $F$ and dart $y\in F$ that is not adjacent to $x$ under the face map, 
$$
...
\begin{proof}
By the assumed minimality of the counterexample, it is enough to consider the merged face $F$.  
...
\begin{lemma}\guid{HYUAZSE}\rating{XX} Let $(V,E)$ be a minimal counterexample.  The half-space conclusion  (Lemma~\ref{lemma:face}.ii) holds for $(V,E)$.
\end{lemma}
\indy{Index}{component!topological}
...
to $U$. The intersection of the two half-spaces, by the minimality of $(V,E)$,
...
\begin{itemize} 
\item $(V,E)$ is a fan.
\item Its hypermap $H=\op{hyp}(V,E)$ is isomorphic to $H_{2k}$ for some $k\ge 3$.
...
\begin{lemma}\guid{PGSQVBL}\rating{XX}  Let $(V,E,F)$ be a cyclic fan.  Let $x$ be a dart in the hypermap of $(V,E,F)$.  If for given $v,w\in V$  the triple $\{0,v,w\}$ is not collinear, then $C(\{v,w\}) \subset \bWdart(x)$.
\end{lemma}
\indy{Index}{fan!cyclic}
...
\begin{lemma}\guid{WRGCVDR}\rating{XX}  Let $(V,E,F)$ be a cyclic fan with hypermap $(D,e,n,f)$.  Then there exists a unique cyclic permutation $\phi:V\to V$ such that
\begin{itemize}
...
\begin{lemma}\guid{CIZMRRH}\rating{XX} Let $(V,E,F)$ be a cyclic fan.  Then $(V,E)$ is
generic, lunar, or circular.  Moreover, these three conditions are mutually exclusive.
\end{lemma}
\indy{Index}{fan!cyclic}
...
\begin{lemma}[monotonicity]\guid{EGHNAVX}\rating{XX}  Let $(V,E,F)$ be a cyclic fan with hypermap $(D,e,n,f)$.  Let $k=\card(F)$; $x=(v_0,v_1)\in F$;  $v_r = \phi^r v_0$; $\alpha(r) = \op{azim}(0,v_0,v_1,v_r)$;  $x_r = f^r x = (v_r,v_{r+1})\in F\subset D$, for all $r$.  
...
\begin{lemma}[internal blades]\guid{YOLCBTG}\rating{XX}  Let $(V,E,F)$ be a cyclic fan.  Pick a dart $x=(v_0,v_1)\in V$.  Set $v_i = \phi^i v_0$ and $x_i = \phi^i x_i$.  Assume that there are four darts $(y_1,y_2,y_3,y_4)$, $y_j = x_{i(j)}$, with
...
\begin{lemma}[slicing a cyclic fan]\guid{EJRCFJD}\rating{XX}  Let $(V,E,F)$ be a cyclic fan with hypermap $H$.  Let $v,w\in V$ be such that $\{0,v,w\}$ is not collinear.  Assume that $C^0\{v,w\}\subset W(x)$ for all darts $x$ in the fan of $(V,E,F)$. Then
\begin{itemize}
...
\begin{lemma}\guid{HZIYFIZ}\rating{XX} Let $(V,E,F)$ be a lunar cylic fan with poles $v_0,v_r$.  In the deformation described above, the triple $(V(t),E(t),F(t))$ is a lunar cyclic fan for all $0\le t< 1$.
\end{lemma}
...
\begin{lemma}\guid{JASDHOQ}\rating{XX} Let $I = \{t\mid 0\le t\le t_1\}$ or $I=\{t\mid 0\le t < t_1\}$.  Let $t\mapsto (V(t),E(t),F(t))$, for $t\in I$, be a deformation of a cyclic fan.  Suppose that 
\begin{itemize} 
...
\begin{lemma}\guid{CZZHBLI}\rating{XX}\label{lemma:webster}  
...
\begin{lemma}\guid{AMHFNXP}\rating{XX} Let $C$ be a bounded polyhedron with $0$ as an interior point.  Let $(V_C,E_C)$ be the associated fan.  There is a bijection between the facets of $C$ and the topological components of $Y(V_C,E_C)$ given by 
$$
...
\begin{lemma}\guid{BSXAQBQ}\rating{XX} Let $C$ be a bounded polyhedron with $0$ as an interior point.  Let $\op{hyp}(V_C,E_C)$ be the associated hypermap.  For every dart $x$ in the hypermap, $\op{azim}(x) < \pi$.
\end{lemma}
\indy{Index}{polyhedron}
