...
// This file is obsolete.  It has been replaced by machine generated cfsqp code.
...
int trialcount = 300;
double eps = 1.0e-6;
double sqrt2() { return  sqrt(2.0); }
double sqrt8() { return  sqrt(8.0); }
...
double hplus() { return 1.3254; }



double marchal_quartic (double r) {
...
double d6 = dih_y(y6, y1, y5, y3, y4, y2);
 return 8.0*((Kmar/(2.0*pi()))*(d1+d2+d3+d4+d5+d6) - Kmar - 
	     (Mmar/pi() )*(d1*marchal_quartic(y1/2.0)+d2*marchal_quartic(y2/2.0)+d3*marchal_quartic(y3/2.0)+
			d4*marchal_quartic(y4/2.0)+d5*marchal_quartic(y5/2.0)+d6*marchal_quartic(y6/2.0)));
}
...
double vol3M(double y1,double y2,double y3) {
  ...
  return (2.0*Kmar/pi())*(sol1+sol2+sol3) -
    (4.0*Mmar/pi())*2.0*(d1*marchal_quartic(y1/2.0)+d2*marchal_quartic(y2/2.0)+d3*marchal_quartic(y3/2.0));
}
...
double gamma3Lwt(double y1,double y2,double y3) {
  return gamma3L(y1,y2,y3)/wtcount3_y(y1,y2,y3);
}
...
double gamma23L(double y1,double y2,double y3,double y4,double y5,double y6) {
  ...
  double cell2 = dihrest*gamma2Ldivalpha(y1/2.0);
...
void t21a(int numargs,int whichFn,double* y, double* ret,void*) {
  ...
  //*ret = (wtcount6_y(y[0],y[1],y[2],y[3],y[4],y[5])==1 ? 0.0 : gamma4L(y[0],y[1],y[2],y[3],y[4],y[5]));
...
Minimizer m38h(double* mdata) {
  ...
  double xmax[6]={y1max,2*hminus(),2*hminus(),y4hmax,2*hminus(),2*hminus()};
	Minimizer M(trialcount,6,0,xmin,xmax);
	M.func = t38h;
	return M;
}
