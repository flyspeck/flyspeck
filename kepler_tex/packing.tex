...
\begin{definition}[$\bV$]\guid{NOPZSEH} 
  \formaldef{$\bV(k)$}{vor\_list} Let $V$ be a saturated packing.
...
\begin{lemma}\guid{CODE}
  ...
  \bV(k)$, for some $k\le3$.  Then $h(\bu)\le
...
\formaldef{$i$-cell}{mcell}
\formaldef{$\xi$}{mxi}%
\indy{Index}{cell}%
\indy{Index}{Marchal cell}%
Let $V$ be a saturated packing.  Let $\bu=[\u_0;\ldots;\u_3]\in \bV(3)$.
When $\norm{\omega(\bu)}{\u_0}\ge\sqrt2$, define $\xi(\u)$ to be the unique 
point in 
\[
\bigcup_{i=0}^2 \op{conv}\{\omega(d_i\bu),\omega(d_{i+1}\bu)\}
\]
at distance $\sqrt2$ from $\u_0$.  Let $a=a(\bu)=\cos(h(\trunc{\bu}{1})/\sqrt2)$.
%\begin{displaymath} 
...
%$\sqrt2$ from $ \u_0$.  
%\end{definition}
A set $\cell(\bu,i)\subset\ring{R}^3$ is associated with each $\bu$ and $i=0,1,2,3,4$.  
...
\begin{displaymath} 
\cell(\bu,1) = (R(\bu) \cap B(\u_0,\sqrt2))\setminus \op{rcone}^0(\u_0,\u_1,a).
\end{displaymath}
...
\begin{eqnarray*} 
\cell(\bu,2) &=& 
R(\bu) \cap \op{rcone}(\u_0,\u_1,a)\cap \op{aff}_+(\{\u_0,\u_1\},\{\xi(\u),\omega(\u)\}).
...
\noindent
\claim{$[\p\not\in W(\u_0,\u_1,\u_2,\xi(\bu))]$.} In this
case, $\p\in \cell(\bu,2)$.

\noindent
\claim{$[\p\in W(\u_0,\u_1,\u_2,\xi(\bu))]$.} In this
case, $\p\in \cell(\bu,3)$.
\end{proof}
...
The intersection of $V$ with a cell $\op{cell}(\bu,k)$ is the set $\{\u_0,\ldots,\u_{k-1}\}$.
...
\begin{proof}  Indeed, the cells partition $\ring{R}^3$, and by Lemma~\ref{XX} the sum of the
azimuth angles is $2\pi$.
\end{proof}
