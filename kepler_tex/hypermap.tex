...
$Y$, whose union is all of $Y$, then them
...
disjoint.  It follows directly from the definitions that
   $$C(\orgn,e)\subset \op{aff}_+^0(\{\orgn,v\},w).$$
\end{proof}
...
circle centered at $\orgn$) from $p$ to a point on $X=X(\orgn,V,E)$.
...
$\phi,\epsilon$ that give 
   $$
   \begin{array}{rll}
   V(x_t,\phi)&\subset Y(\orgn,V_t,E_t)\\
...
$s>0$ for which $\varphi(s)$ fails, where 
 $\varphi(t)$ be the statement that $x_t$ and $x'_t$ lead
...
from $V(x_t,\phi)$ to $V(x'_t,\phi)$.  This path avoids 
$X(\orgn,V_{t'},E_{t'})$ and connects $V(x_{t'},\phi)$ to
$V(x'_{t'},\phi)$ for all $t'$ sufficiently close to $t$.
...
constructing a curve that connects $V(x_s,\phi)$ to $V(x'_s,\phi)$.
...
$W(F,0.1)$ as $F$ runs over faces of the hypermap.
