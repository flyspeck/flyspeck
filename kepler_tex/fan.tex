...
\begin{displaymath}C^0\{\v,\w\}=C^0_+(\{\v,\w\}) = \op{aff}^0_+(\{\orz\},\{\v,\w\}).\end{displaymath}
...
a \newterm{fan\/} if the following properties hold.
 ...
 \begin{displaymath}C(\ee )\cap C(\ee ') = C(\ee \cap \ee ').\end{displaymath}
    \end{nomerate}
When $\ee\in E$, call $C^0(\ee)$ or $C(\ee)$ a \newterm{blade\/} of the fan.
\end{definition}
\indy{Index}{fan}%
...
%a \newterm{retro-fan\/} if the following conditions hold.
%    \begin{itemize}
...
%        \begin{displaymath}C^0(\orz,\ee) \cap C^0(\orz,\ee')\ne\emptyset\ \Rightarrow (\ee = \ee').\end{displaymath}
%    \item Sets $\v,\v'\in V$ give
%     \begin{displaymath}\op{aff}^0_+(\orz,\v) = \op{aff}^0_+(\orz,\v')\ \Rightarrow (\v=\v').\end{displaymath}
...
\begin{displaymath}\{\{\v,\w\}\mid \w\in E(\v)\}\end{displaymath} is the set of edges at node $\v$.
...
\item \case{independence} \claim{If the property \case{intersection} is known, then to prove the independence of $\u$ and $\v$, it is enough to show the strict form of the triangle inequality:}
...
\arc_V(\orz,\{\u,\v\}) < \pi.
\end{displaymath}
\item \case{intersection} The intersection property is generally the most difficult to verify in practice.  Some geometrical reasoning based on additional facts about $(V,E)$ is generally required to verify the intersection property.   If $(V,E)$ is a fan, then the intersection property is inherited by subsets of $V$ and $E$.  Also, note  that
\begin{displaymath}
...
two subsets $D_1,D_2$:
    ...
    \end{array}
    \end{displaymath}
Darts in $D_2$ are said to be \newterm{isolated}; and darts in $D_1$ are \newterm{non-isolated}.
%
\indy{Index}{dart}%
...
meeting both.
That is,
        \begin{displaymath}(n^k e x = e n^\ell x)\Rightarrow (n^\ell x = x).\end{displaymath}
...
of a topological space \newterm{``topological components''} and the connected
components of a hypermap \newterm{``combinatorial components''}.
\end{remark}
...
In particular, \begin{displaymath}S^2 = \{ \v \mid \normo{ \v} = 1\},\end{displaymath} the unit sphere in
...
A fan $(V,E)$ is \newterm{fully surrounded}, if $\op{azim}(x)<\pi$ for all darts $x$ in the
hypermap of $(V,E)$.
\end{definition}
...
Let $(V,E)$ be a fan with hypermap $(D,e,n,f)$.   Assume that $(V,E)$ is fully surrounded.  Then for every face $F$ of the hypermap, there exists a topological component $U$ of $Y(V,E)$ such that for every $x\in F$, the dart $x$ leads into $U$. 
\end{lemma}
\indy{Index}{connected!topological component}%
\indy{Index}{hypermap}%
\indy{Index}{dart}%
...
The node set is unchanged.  It remains finite and nonempty, and it does not contain $\orz$.  The property \case{independence} of $E'$ follow from the corresponding property of $E$
...
Let $(V,E)$ be a fan with hypermap $(D,e,n,f)$. Let $Y=Y(V,E)$. Assume that $(V,E)$ is fully surrounded.   For every topological component $U$ of $Y$, there is a dart $x\in D$ that leads into $U$.
\end{lemma}
\indy{Index}{connected!topological component}%
\indy{Index}{fan}%
\indy{Index}{hypermap}%
...
Let $(V,E)$ be a fan with hypermap $(D,e,n,f)$. 
Let $Y=Y(V,E)$.
Assume that $(V,E)$ is fully surrounded. Fix a face $F$ of cardinality three, fix
...
\begin{displaymath}A^0_+(i)=\op{aff}_+^0(\{\orz,\v(x_{i+1}),\v(x_{i+2})\},\v(x_i)),\quad i=0,1,2.\end{displaymath}
...
\begin{definition}[conforming~faces]
Let $(V,E)$  be a fan with hypermap $(D,e,n,f)$.    The fan is \newterm{conforming} if the
...
\item \case{half-space} For every face $F$, the topological component $U_F$ is the intersection of the open
...
\end{displaymath}
\item \case{diagonal}  For every face $F$, if $x,y\in F$ are distinct, with corresponding nodes $\v(x),\v(y)\in V$, then
...
\begin{proof} By \case{half-space}, $U_F$ is the intersection of half-spaces.
\end{proof}
...
There is a well-defined continuous (in fact, locally constant) function from $Y$ onto $[D]$ given as follows.  For $\p\in Y$, choose $F$ such that $\p\in U_F$ and send $\p$ to the class of $F$ in $[D]$.  By property \case{bijection}, this map is well-defined. This map extends continuously to $C^0(\ee)$, for $\ee\in E$, by sending $\p\in C^0(\ee)$ to the the combinatorial component of $D$ containing the edge $\{x,y\}$ of the hypermap that corresponds with $\ee$.  The domain
\begin{displaymath}
Y\cup \bigcup C^0(\ee)
\end{displaymath}
...
This section proves the existence of many conforming fans.
The existence proof breaks into a series of small lemmas.  The primary  method to prove the existence of conforming fans is an induction on the following invariant of a fan $(V,E)$.  
...
A fan $(V,E)$ is said to be \newterm{minimally non conforming} if the following conditions hold.
\begin{itemize}
\item $(V,E)$ is fully surrounded.
\item $(V,E)$ is not conforming.
\item $N(V,E)>0$.
\item If $(V,E')$ is any other fully surrounded fan on the same vertex set $V$, and if
$N(V,E') < N(V,E)$, then $(V,E')$ is conforming.
\end{itemize}
\end{definition}
...
%Among all such counterexamples with fixed node set $V$,  pick
...
Assume that $N(V,E)>0$.  Let $E'=E\cup \{\{\v,\w\}\}$, be the reduction data as above.  Assume that $F(x')$  is a triangle. Then $N(V,E')<N(V,E)$.
\end{lemma}
...
\begin{displaymath}N(V,E) - N(V,E') = ((k+1)-3) ~~-~~ ((k-3) + (3-3)) = 1 >0,\end{displaymath}
where $k$ is the cardinality of $F(y')$.
\end{proof}


\begin{lemma}\guid{OBHTHCD}\rating{ZZ}
The property \case{bijection} of conforming fans  holds for any minimally non conforming fan:
$F\mapsto U_F$ is a bijection
\end{lemma}

\begin{proof}  Let $(V,E)$ be a conforming fan.    
Choose reduction data for $(V,E)$.  In particular,
...
\begin{lemma}\guid{TXFBALB}\rating{ZZ}  Let $(V,E)$ be any minimally non conforming fan.  Then it has property \case{solid angle} of conforming fans.
\end{lemma}

\begin{proof}  Choose reduction data
for $(V,E)$.
Every topological component of $Y=Y(V,E)$
except $U=U_F$ is already a topological component of $Y'$ and the conclusion
...
\begin{lemma}\guid{GGZWYRM}\rating{ZZ}  Let $(V,E)$ be any minimally non conforming fan.  For any dart $x$ on any face $F$ and dart $z\in F$ that is not adjacent to $x$ under the face map, 
\begin{displaymath}
...
\begin{proof}  
Choose reduction data for $(V,E)$, the face $F$, and dart $x$.
By excluding trivial cases of the proof,  the set $F$ and the dart $x\in F$ can be the choices used to construct $E'$.
...
\begin{lemma}\guid{HYUAZSE}\rating{ZZ}   Let $(V,E)$ be a minimally non conforming fan.  Then  property \case{half-space} of conforming fans holds for $(V,E)$.
\end{lemma}
\indy{Index}{component!topological}%

\begin{proof}
Choose reduction data for $(V,E)$.
...
%Let $(V,E)$ be any minimally non conforming fan.  Then a triangulation of $(V,E)$ exists.
%\end{lemma}
%
...
\begin{proof}
Suppose for a contradiction that a fully surrounded fan $(V,\ldots)$ exists that is not conforming.  Among all fully surrounded conforming fans $(V,\ldots)$ on the same vertex, pick one $(V,E)$ that minimizes $N(V,E)$.  
In fact, $N(V,E)>0$, for otherwise $(V,E)$ is conforming by Lemma~\ref{lemma:N=0}.

This
is a minimally non conforming fan.  The preceding lemmas show that a minimally non conforming fan satisfies all of the properties of a conforming fan.  This is contrary to the definition of minimally non conforming.  This contradiction gives the proof.
\end{proof}
...
An  \newterm{affine set} $A\subset\ring{R}^n$ is a set of the form
\begin{displaymath}
A=\op{aff}(S).
\end{displaymath}
The
...
Let $P$ be a convex set.  A \newterm{face} of $P$ is a convex set $F$ such that the conditions
\begin{displaymath}
...
The term \newterm{face} occurs in this book with two meanings: the face of a hypermap and the face of a convex set.  The two contexts are sufficiently different that we hope to avoid misunderstanding.  Graphs, hypermaps, fans, and polyhedra all have \newterm{edges}.  Fans and hypermaps have \newterm{nodes}, while polygons and polyhedra have \newterm{vertices}.
\end{remark}
...
\begin{definition}[polyhedron,~vertex]  A \newterm{polyhedron} is the intersection of
a finite number of closed half-spaces in $\ring{R}^n$.  An extreme point of a polyhedron is also called a \newterm{vertex}.
\end{definition}
