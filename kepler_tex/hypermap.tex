...
\begin{note} %XX
The algorithm and its proof seem needlessly complicated.  A entire
true face is constructed at once in the algorithm.  A different approach
would be to construct each face bit by bit with more primitive operations,
one reverse double wallkup at a time.  With such an approach there would
be less to justify at each step.
\end{note}
