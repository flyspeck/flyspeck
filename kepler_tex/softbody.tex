...
\begin{definition}[\id{CVERTEX},~\id{IVERTEX},~\id{CFACE},~\id{FACE}]
We define the following numbers and sets:
\begin{enumerate}
\item $\id{CVERTEX} = \card(V).$
\item \id{CFACE} is the number of faces in the hypermap.
...
from the set of natural numbers \id{IVERTEX}.
\item $\id{FACE}  = \{0,\ldots,\id{CFACE}-1\}$.  We identify \id{FACE} with
...
\id{CVERTEX}  that takes some value in $\{13,14,15\}$, because this much
holds for all contravening packings.  For any given contravening
packing $V$, its data file provides the exact value of \id{CVERTEX}.
...
\begin{definition}[\id{DART},~\id{EDART}]
Every face
...
\begin{definition}[\id{DEDGE},~\id{EDGE}]
...
\begin{definition}[standard~face,~\id{STANDARD},~\id{ITRIANGLE},~\id{IQUAD},
~\id{IPENT},~\id{IHEX}]
...
The set \id{STANDARD} is the disjoint union of
\id{ITRIANGLE}, \id{IQUAD}, \id{IPENT}, \id{IHEX} according to the number of
...
We define a map from extended fans to decorated
hypermaps.
