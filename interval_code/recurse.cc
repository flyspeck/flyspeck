...
/*
It depends on regression, and I want to remove the dependency
of recurse.cc on regression so that I can use autogenerated library.
...
int f206A(int depth,double xx[6],double zz[6],cellOption options) {
  ...
  const Function F = 
    ( (s.mode==numerical_data::strategy::n1) ? regression::num1   :
      ((s.mode ==numerical_data::strategy::n1m) ? regression::num1 * mone  :
       ((s.mode == numerical_data::strategy::n2m) ? regression::num2 * mone :
	(assert(s.mode == numerical_data::strategy::merge),
	 regression::num2 * malpha * mone + 
	 regression::num1 * alpha) ) ) );
