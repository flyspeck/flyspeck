...
\begin{definition}[quotient~data]\label{def:data}
  A tuple $(H,{\cal L},L,x',\varphi)$ is a \newterm{quotient data} if
...
in the quotient is false.
\item  $x'\in F(L)$ is a quotient dart such that $e'x'$ lies in
a true face (with respect to $\varphi$).  
\item The set $S$ (defined below) maps bijectively to a set
of darts in the quotient, which is also denoted $S$.
\item  $\phi$ is an $S$-flag.
\end{enumerate}
The set $S\subset D$ is defined as follows.
Write $x' = [\ldots;x]$ with $x\in D$.  Define $m\ge0$ to be the largest $m$ 
such that 
\begin{displaymath}
...
is a subpath of $L$.  
%Set $y = f^{m+1} x$.  
Set $S = S(x,{\cal L}) = \{f^i x \mid 1 \le i\le m\}$.
\end{definition}
...
Let $(H,{\cal L},L,x',\varphi)$ be a quotient data.  
Let $x' = [\ldots,x]$ with $x\in D$.  Let $m\ge0$ be the constant 
in the definition of quotient data.  Set $y = f^{m+1} x$.  
...
With this lemma in place, starting from from one quotient
data $(H,{\cal L},L,x',\varphi)$, we construct a new quotient
data $(H,{\cal M},L_1,x'',\varphi')$
The first entry $H$ of the tuple does not change.  Let $m\ge 0$
and dart $x$ be as given in the definition of quotient data.
Let $p$, $y$, and $z$ be as given in Lemma~\ref{lemma:yz}.
...
the quotient data of the iteration is preserved.
...
is a quotient data in the sense of Definition~\ref{def:data}.
\end{lemma}

\begin{proof} The proof can be organized into three parts,
establishing that ${\cal M}$ is normal, that $\varphi$ is an $S_{\cal M}$-flag,
...
gives quotient data.  The iteration
