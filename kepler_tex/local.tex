...
\end{definition}
\indy{Index}{straight (node of a fan)}
...
%% XX need to generalize so that straight  can become unstraight.
...
%  if $\v$ is straight, then $\v(t)$ is straight for all sufficiently small
%  $t$.  
...
\begin{proof}[proof \eqref{lemma:fan-open-lunar}~\eqref{lemma:fan-open-generic}] 
  ...
  \case{lunar}: The poles $\{\v,\w\}$ of the lunar fan are not straight
...
\begin{definition}[torsor,adjacent]
  ...
  %\]
  An
  \newterm{isomorphism of torsors} is a bijection that respects the action.
...
\item \case{coherent slice}
 ...
 there exist $s',s''\in S$  %$m'\in I(s')$, $m''\in I(s'')$,
