...
\begin{definition}[permutation] A \newterm{permutation} $f$ on a set
...
\begin{remark}[planar graphs as hypermaps]\tlabel{rem:hypermap} A hypermap is an abstraction of
the concept of 
...
\begin{lemma}[equal equivalences]\rating{50}\label{lemma:er} %\guid{QLPBIKV}
...
\begin{remark}[dart universe]\label{rem:dart-universe}
...
\begin{remark}[Eulerian relation]\label{rem:Euler}   
...
\begin{remark}[contour path illustration] Figure~\ref{fig:hypermap_ex}
...
\begin{remark}[M\"obius strip] Heuristically, a M\"obius contour is a 
...
\begin{lemma}[quotient bijection,~$\F$]\label{lemma:quotient-bijection}
  ...
  corresponding face $\F({\cal L},L)$ of $H/{\cal L}$ is the
...
\begin{lemma}[nodal fixed point]\label{lemma:nfp}
...
\begin{lemma}[quotient node]\label{lemma:quotient-node}
...
\begin{definition}[flag]\label{def:flag} 
  ...
  on $H$ is a boolean-valued function on the set of faces that
...
steps.  The dart set of such a contour loop is a face of $H$.  Based
on this observation, we make the following definition.

\begin{definition}[face specific]
  A contour loop $L$ in a hypermap is \newterm{face specific} 
if its
...
lie in the same face $\F({\cal L},L)$ of $H/{\cal L}$.  (The existence of $q$ is trivial
when $L$ is face specific.)
...
\begin{proof} 
%We break the proof into two cases, depending on whether $L$ is face specific.
%
%\claim{[$L$ is face specific.]}  In this case $S=\emptyset$,  $m=0$, $p=0$, $k=1$,
...
%
%\claim{[$L$ is not face specific.]} In this case,
...
Let $(H,{\cal L},L,x,\varphi)$ be a
marked hypermap, where $H$ is restricted. Assume that $L$ is not face specific.
Let $m,p,q$ be the natural numbers and let $x,y,z$ be the darts given by
Definition~\ref{def:yz}.  Let $r = \op{card}(\F({\cal L},L))$.  Then
\begin{displaymath}
...
y$, for otherwise the subpath $P=[x;f x;\ldots;f^{m+1}x]$ gives a face
specific contour loop, which is contrary to the assumption that $L$ is
not face specific.  Also, $y\ne z$, for otherwise the face of $y$ is
...
number has this property.  Also, $x'$ and $y'$ belong to the face $\F({\cal L},L)$.
...
face $\F({\cal L},L)$.  By the simplicity of $H'$, it follows that $y' =
...
$S'$.  However, $y'\not\in S'\subset \F({\cal L},L)$, by the simplicity of $H$.  Also,
$e'y'\not\in S'$, by the previous paragraph.
The dart $y'$ lies in the false face $\F({\cal L},L)$.  The only
...
The dart $u''=[n^{-1}y;\ldots;u]$ of $H''$ lies in the face $\F({\cal M},L_2)$.
Also, 
\begin{displaymath}
...
true.)  Hence a path exists from the true face $\F({\cal M},L_2)$ into another
...
%XX background digraph, vertex, edge, sink, path.

\begin{definition}[digraph,~vertex,~edge,~sink,~path]
A \newterm{digraph} is an ordered pair $(V,E)$ where $V$ is any set, and
$E$ is a set of ordered pairs of vertices.  
An element of $V$ is called a \newterm{vertex}.
An element of $E$ is called a \newterm{directed edge}.
If $(v,w)\in E$, then $v$ is the \newterm{head} and $w$ is the \newterm{tail}
of the directed edge.
A vertex $v$ is a \newterm{sink} if it is not the head of any directed edge.
A path $P=[v_0;v_1;\ldots;v_{k-1}]$ is a list of vertices, such that $(v_i,v_{i+1})\in E$
for all $i<k-1$.
\end{definition}

\begin{definition}[digraph of a hypermap]
Let $H$ be a restricted hypermap.  Form a directed graph as follows.
The vertex set $V$ of the graph is the set of all marked hypermaps 
$(H,{\cal L},L,x,\varphi)$ such that $L$ is not face-specific.  
Let $T$ be the transform on $V$.  Write $T{\cal H} = (H,{\cal M},L_1,x,\psi)$.
The set of tails of directed edges with head ${\cal H}$ is as follows:
\begin{itemize}
\item If every contour loop of ${\cal M}$ is face-specific, then ${\cal H}$ is a sink.
\item If $M$ is not face-specific, 
then there is a single tail $T{\cal H}$.
\item If $M$ is face-specific, but not every loop in ${\cal M}$ is face specific,
then the tails are $(H,{\cal M},M,y,\psi)$, where $M$ is a 
non-face-specific loop in ${\cal M}$ and $y$ is a dart visited by $M$ such
that $y$ is followed by an $f$-step:  $M = \lp{y;f y;\ldots}$.
\end{itemize}
\end{definition}

\begin{lemma}[digraph sink]
...
a sink, and if $T{\cal H} =(H,{\cal M},L_1,x,\psi)$ is its transform, then
$H$ is naturally isomorphic to $H/{\cal M}$, under the map that sends
a dart $y$ of $H$ to the unit path $[y]$.
\end{lemma}

\begin{proof}  Each transform increases the number of darts visited by the
normal family.  Each step in the path makes one transform.  The number of
darts visited by a normal family is bounded by $\#D$.  This bounds the length
of a path.

The condition on a sink is that every contour loop of ${\cal M}$ is face-specific.
That is, the canonical function on ${\cal M}$ is true on every face.
...
loop is face-specific: $M = \lp{u;f u;\ldots}$.
...
%at least one non-face-specific loop, returning a dart $x'$ of $H/{\cal L}$
%in a false face (with respect to the canonical boolean function).
%Given a normal family ${\cal L}$ (with a non-face-specific loop),  write
% $x' = [\ldots;x]$ and let $L$ be the the non-face-specific contour loop of ${\cal L}$ that
...
%
%
%\begin{lemma}[sequence]\label{lemma:sequence}  
...
%  marked hypermaps ${\cal H}_i = (H,{\cal L}_i,L_i,x_i,\varphi_i)$ for
...
%\item \claim{[The contour loop $L$ is face specific, but not all contour loops in
%     ${\cal M}$ are face specific.]}  In this
...
relies to a lesser degree on the structural details of the marked
...
hypermaps that we have constructed.  The following lemma allows us
to replace every hypermap $H/{\cal L}_{i+1}$ in the sequence Lemma~\ref{lemma:sequence} with 
an isomorphic hypermap obtained by reverse double walkup transformations
on $H/{\cal L}_i$.  


\begin{lemma}[walkup transform]
Let ${\cal H} =(H,{\cal L},L,x,\varphi)$ be a marked hypermap,
  with $H$ restricted.  Assume that $L$ is not face specific, and let 
$(H,{\cal{M}},M,x,\psi)$ be 
...
gives a domain $D$ that will contain all the darts of all the
hypermaps that will be constructed.  In practice, we take $D$ to be a
finite subset of the set of natural numbers. The set of darts for some
of the hypermaps may be a proper subset of $D$.
...
The following constructions depend on $D$ and $d$, although the
...
face.  Set
\begin{displaymath}
A = \{(H,m,\varphi,x) \mid 0\le m \le d-1\} \subset X_2.
\end{displaymath}
\end{definition}
...
\begin{definition}[extension]  
  Let $(H,m,\varphi,x)\in X_2$.  Choose $p,q$ such that $(m,p,q,r)\in
  ...
  $RDW(H,x,m,p,q)$.  Say that a boolean-valued function $\psi$ on the
...
$\varphi$ permits, some hypermaps are also fully constructed.


\begin{definition}[g] Let $g:X_2 \mapsto \powerset(X)$ be given as
  follows.  Let $r$ be the cardinality of the face $F$ of $x$.  The
  subset $g(H,m,\varphi,x)\subset X$ is presented as a union of two
  sets:
\begin{displaymath}
   Y_i = X_i \cap g(H,m,\varphi,x).
\end{displaymath}
...
and 
\item $\varphi(F')$ is true for all $F'\ne F$.
\end{itemize}
 $(H',m',\psi,x')\in Y_2$ if and only if
\begin{itemize}
\item there exists $p,q$ with $(m,p,q,r)\in C$, such that $H'=RDW(H,x,m,p,q)$,
\item  $\psi$ is an extension of
$\varphi$, and 
\item Let $F_1$ be the face of $x$ in $H'$.  One of the following two
  conditions hold:
\begin{itemize}
\item $\psi(F_1)$ is false;  $x' = x$; and  $p+m+1 \le m' < r$.
\item $\psi(F_1)$ is true; there exists a false face in $H'$; $x'$ is
...
most $\card(D)$ darts and such that the largest face has size  $d$
...
\begin{definition}[related]
A marked hypermap $(H,{\cal L},L,x,\varphi)$ is said to be \newterm{related} to
an element $(H',m',\varphi,x')\in X_2$ if the following conditions hold:
\begin{itemize}
\item $H/{\cal L}$ is isomorphic to $H'$ by some isomorphism $G$.
\item The image of $x$ in $H/{\cal L}$ maps to $x'$ under $G$.
\item The pull back of $\varphi'$ under $G$ is the boolean function on
faces of $H/{\cal L}$ associated with $\varphi$.
\item $m = \card(S(H,L,x))$.
\end{itemize}
\end{definition}

\begin{lemma}[related seeds]\label{lemma:related-seed}
  Let $H$ be a restricted hypermap with digraph $(V,E)$.  Assume that
  $H$ has at most $D$ darts and that every face has size at most $d$.
  There exist a marked hypermap in $V$ and a tuple
  $(H',m',\varphi,x')\in \op{fix}_2$ that are related.
\end{lemma}

\begin{proof}
\end{proof}

\begin{lemma}[related edges]\label{lemma:related-edge}
Let $H$ be a restricted hypermap with digraph $(V,E)$.
Assume that $H$ has at most $D$ darts and that every face has size
at most $d$.   Assume that the marked hypermap $v\in V$ is not a sink.
Assume that $v$ is related to an element in $\op{fix}_2$.
Then there exists a directed edge with head $v$, and an element $u$ in $\op{fix}_2$
such that the tail of the edge is related to $u$.
\end{lemma}

\begin{proof}
\end{proof}

\begin{lemma}[related sink]\label{lemma:related-sink}
Let $H$ be a restricted hypermap with digraph $(V,E)$.
Assume that $H$ has at most $D$ darts and that every face has size
at most $d$.   Then some sink in the digraph is related to some element of
$\op{fix}_2$.
\end{lemma}
...
\begin{proof}  Turn to the proof of Theorem~\ref{lemma:XX}.
Let $(H,{\cal L},L,x,\varphi)$ be a sink that is related to some
tuple $(H',m,\varphi',y')\in \op{fix}_2$.  By Lemma~\ref{XX}, $H$ is
isomorphic to the 
quotient $H/{\cal M}$, where $(H,{\cal M},\ldots)$ is the transform of
the sink.   This quotient is isomorphic to $\op{RDW}(H/{\cal L},x',m,p,q)$,
where $x'$, $m$, $p$, $q$ are given by Definition~\ref{def:yz}.
By the relatedness of the sink, this is isomorphic to
$H''=\op{RDW}(H',x',m,p,q)$.  By Lemma~\ref{XX}, $(m,p,q,r)\in C$, where
$r=\op{card}(\F({\cal L},L))$.  Under the isomorphism, $r$ is the cardinality
of the face of $y$ in $H'$.   By the definition of $f$ and $g$,  $H''\in \op{fix}_1$.
\end{proof}
...
%    L}_i,L_i,x_i,\varphi_i)$ be the sequence of marked hypermaps
