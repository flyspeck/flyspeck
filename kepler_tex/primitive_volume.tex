...
$$
  [ST]-[R] = ([SC]-[WFR]) - [Q].
$$
That is, as we can see from Figure~\ref{def:quoin}, the region
...
We write $\op{sovoQ}(a,b,c,\lambda)$ for the value of
...
is zero, so that
$$
\op{sovoQ}(a,b,c) = \lambda_v \op{quovol}(a,b,c).
$$
%% XX This is an underused function.  Eliminate.
...
Then
   $$
   \op{sovo}(v_0,R'\cap B(v_0,t)) = 
   d [(1-\cos\psi)\phi(t,t,\lambda)+A(h,t,\lambda)]
   -s \phi(t,t,\lambda) + \op{sovoQ}(a,b,t),
   $$
...
% XX ADD external references for intermediate results,
%% such as lemma:sovo:rog.
...
(The unique existence of $q_i$ is established by Lemma~\showref{XX}.)
...
\ref{lemma:sovo:truncRog:noQ}, we have
$$
\begin{array}{lll}
  ...
  \op{sovo}(R_i\cap B) &= \dih(R_i) [(1-\cos\psi)\phi(t,t,\lambda)+A(y/2,t,\lambda)]\\
   &\quad -(\dih(R_i) (1-\cos\psi) - \sol(R_i)) \phi(t,t,\lambda) + \op{sovoQ}(a,b,t),
\end{array}
$$
...
The existence of such points is given by Lemma~\showref{XX}.
%% XX New Tarski?
...
We have by Lemmas~\showref{XX} and \showref{XXX}, that
$$
\begin{array}{lll}
