...
\footnote{An alternative proof uses the open cover of compact ball $\bar B(\p,r)$ by the sets $\bar B(\p,r)\setminus V$ and $B(\v,1)$, for $\v\in V$. By compactness, the cover is necessarily finite.}
\end{proof}
...
Given a packing $ V$, \cite{Rogers:1958:Packing} gives a partition of Euclidean space into
simplices with extreme points in $ V$.  He used
...
some point of $V$.  The set $V(\p,2)$ is finite by Lemma~\ref{lemma:V-finite}.  Hence, $\p$ is at least as close to some
...
$A(\u_0,\u_i)$ is a face of the polyhedron $A_+(\u_0,\u_i)$.  Thus, if the ``open convex hull'' 
\begin{displaymath}
...
\begin{displaymath}
N = \left\{k\in\ring{N}\mid j\le k\le 3, ~~\Omega(V,\bu) = \bigcup_{\bv \in \bV(k),~\trunc{\bv}{j}=\bu}
...
\claim{Such an index $k$ exists.}  Indeed, the definition of points $\omega(\trunc{\bu}{i})$
depends on $\bu$ only through the sets $\Omega(V,\trunc{\bu}{j})$.  Hence, $R(\u)\ne R(\v)$
implies that the two sequences $\Omega(V,*)$ must differ at some index.
...
$X-\q$ denotes the translate of a set $X$ by $-\q$, and $(\perp)$ is the orthogonality relation.
\end{itemize}
\end{lemma}
...
\claim{The lemma holds when $k=0$.}  Indeed, $\Omega(V,\u_0)$ is an open set,
so that its affine hull is $\ring{R}^3$.  This is the first conclusion.  The other conclusions reduce
to  trivial facts: $\dimaff\ring{R}^3 = 3$; $\dimaff\{\u_0\}=0$; $\ring{R}^3\cap \{\u_0\} = \{\u_0\}$; and $\ring{R}^3\perp \{\orz\}$.

Assume the induction hypothesis for $k$.  We may assume that $k<3$, for otherwise
there is nothing further to prove.
Let $\bu\in \bV(k+1)$.
Let $\bv = \trunc{\bu}{k}\in \bV(k)$.  Let $\q$ be the circumcenter of (the point set of) $\bv$.  Write $A_j = \cap_{i\le j} A(\u_0,\u_i)$;  $B_j = \affhull(\Omega(V,\trunc{\bu}{j}))$;
$C_j = \affhull\{\u_0,\ldots,\u_j\}$.%; $S_j = \{\u_0,\ldots,\u_j\}$.
...
\claim{We claim that $B_{k+1} = A_{k+1}$.}  Indeed, by definition, $B_{k+1}\subset A_{k+1}\subset A_k$.
Also,
\begin{displaymath}
\dimaff B_{k+1} = 3 - (k+1) \le \dimaff{A_{k+1}} \le \dimaff A_k = 3 - k.
\end{displaymath}
Hence, by general background on affine sets,  if $A_{k+1}\ne A_k$, then $B_{k+1}=A_{k+1}$
follows.  Suppose for a contradiction that $A_k = A_{k+1}$.  Then
$\Omega(V,\bv) \subset \Omega(V,\bu) = \Omega(V,\bv)\cap A(\u_0,\u_{k+1}) \subset \Omega(V,\bv)$,
so that $B_k = B_{k+1}$.  This contradicts the defining conditions of $\bV(k+1)$.

\claim{We claim that $A_{k+1}\cap C_{k+1} = \{\q_{k+1}\}$, where $\q_{k+1}$ is the
circumradius of $S_{k+1}$.}  Indeed, by the definition of $A_{k+1}$, any point in this affine set
is equidistant from every point of $S_{k+1}$.  By the definition of $C_{k+1}$, the point
lies in the affine hull of $S_{k+1}$.  This uniquely characterizes the circumcenter.
...
\begin{lemma}\rating{400}\label{lemma:sqrt2-close}
...
\claim{[$k=2$].} In this case, let $\w'$ be the projection of $\w$ to the plane containing
$\p, \u_0, \u_1, \u_2$.  Under orthogonal projection, the angles remain obtuse:
\begin{displaymath}
...
\begin{lemma}\guid{XNHPWAB}\rating{100}\label{lemma:v2} 
Let $\bu=[\u_0;\ldots;\u_k]\in \bV(k)$.
Assume that $h(\trunc{\bu}{k})<\sqrt2$.
%\begin{displaymath}
%\norm{\omega(\trunc{\bu}{j}) }{  \u_0} < \sqrt2.
%\end{displaymath}
Then 
\begin{description}
\item[(circumcenter)]  $\omega(\trunc{\bu}{k})$ is the circumcenter of the
set $S =\{ \u_0,\ldots, \u_k\}$.
\item[(convex hull)]  The circumcenter of $S$ lies in the convex
hull of $S$.
\item[(distinctness)]  The points $\omega(\trunc{\bu}{j})$, for
...
Now assume $j>0$.  The circumcenter $\p=\p_{j}$ of the set $S_{j}$ is
the point on the plane $\op{aff}(\Omega(V,S_{j}))$ closest to
$\p_{j-1}=\omega(\trunc{\bu}{j-1})$.

\claim{The point $\p$ lies in $\Omega(V,S_{j})$.}   
This follows from Lemma~\ref{lemma:sqrt2-close}.
...
hull. Then there is $i<j$ such that $\op{aff}(S')$ separates $\p$ from
$ \u_i$, where $S'=S_j\setminus\{i\}$.  Let $\p'$ be the circumcenter
of $S'$, and let $\v= \u_i$.  Then for $k\ne i$ and $0\le k\le j$, it
follows that
\begin{eqnarray*}
\norm{ \u_k}{\p'} &=& (\norm{ \u_k}{\p}^2 - \norm{\p}{\p'}^2)^{1/2} \\
&\ge& \norm{ \u_k}{\p} - \norm{\p}{\p'} \\
