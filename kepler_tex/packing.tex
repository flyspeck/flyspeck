...
that $\p\in \Omega'\setminus\Omega(V,\v)$.  That is, $\p$ is closer to some
$\w\in V\setminus V(\v,4)$ than to $\v$.  By the triangle inequality,
\begin{displaymath}
\norm{\v}{\w} \le \norm{\v}{\p} + \norm{\w}{\p} <2 \norm{\v}{\p} < 4.
\end{displaymath}
...
of closed half-spaces is a polyedron.
\end{proof}
...
In this section, a single saturated packing $V$ will be considered.
Since it is fixed, it will be dropped from the notation.  

%Let $\Omega(V, \u)$  be the Voronoi cell centered at $ \u$:
%\begin{displaymath}
%\Omega(V, \u) = \{\p \mid  \norm{\p}{ \u} 
%\le \norm{\p}{\v},\  \u\in V\}.
%\end{displaymath}
If $W\subset V$, %(or if $W$ is an ordered tuple of elements of $ V$), 
then the intersection of the family is  $\Omega(V,W)$:
\begin{displaymath}\Omega(V,W) = \bigcap \{\Omega(V, \u)\mid \u\in W
\}.\end{displaymath}
Note that an intersection of Voronoi cells can be written in various equivalent forms:
\begin{displaymath}
\Omega(V,\v)\cap \Omega(V,\u) = \Omega(V,\v)\cap A_+(\u,\v) 
...
\begin{array}{lll}
\Omega(V,[]) &= \ring{R}^3,\\
\Omega(V,\u_0::\bu) &= \Omega(V,\u_0)\cap \Omega(V,\bu).\\
\end{array}
\end{displaymath}
%Call $\Omega(V,W)$ the $W$-face of $\Omega(V, \u)$ when $ \u\in W$.


As general notation, an underscore  indicates a list of points in $\ring{R}^3$.
When $\bu=[\u_0;\ldots;\u_k]$ and $j\le k$, write $\bu[j] = 
[\u_0;\ldots;\u_j]$.  Let $ V(k)$ be the set of lists of length $k+1$:
$\bu=[\u_0;\ldots;\u_k]$, with $ \u_i\in V$ such that
\begin{equation}\label{eqn:omega-dim}
\dimaff(\Omega(V,\bu[j])) = 3-j
\end{equation}
for all $0<j\le k$,
where   $\dimaff(X)$ denotes the affine dimension of $X$ (Definition~\ref{def:affine}).
\indy{Notation}{dimaff@$\dimaff$}%
In particular, $V$ can be identified with $V(0)$ under the natural bijection $\v\mapsto[\v]$, 
 and $V(1)$ is the set
of lists $[\u;\v]$ of distinct elements such that the Voronoi cells at $ \u$ and $\v$
have a common facet, and so forth.  Beware of the notation: 
 the index $k$ is the codimension of $\Omega(V,\bu)$,
when $\bu\in V(k)$; it is not the length of the list $\bu$ (which is $k+1$).



\begin{lemma}  Let $\bu\in V(k)$.  Then $\Omega(V,\bu)$ is a face of $\Omega(V,\bu[0])$.
\end{lemma}
...
\begin{lemma} 
Let $\bu\in V(k)$.   Then $F$ is a facet of $\Omega(V,\bu)$ if and only if
there exists $\v\in V(k+1)$ such that $F = \Omega(V,\bv)$ and $\v[k] = \bu$.
\end{lemma}
...
Let \begin{displaymath}\omega( \bu[0]) = \bu[0],\end{displaymath} and
let $\omega( \bu[j+1])$ be the closest point on $\Omega(V, \bu[j+1])$ to
$\omega( \bu[j])$.  The point is defined whenever $\Omega(V, \bu[j+1])$ is
nonempty.  The set $\Omega(V, \bu[j+1])$ is convex and compact.  Thus,
by real analysis, the point $\omega( \bu[j+1])$ exists uniquely.
\indy{Notation}{zzomega@$\omega$ (extreme points of Rogers's simplex)}%
\end{definition}

\begin{definition}[R]
For $\bu\in V(k)$, let 
\begin{displaymath}R(j,\bu) = \op{conv}\{\omega( \bu[j]), \omega(
\bu[j+1]),\ldots,\omega( \bu[k])\}.\end{displaymath} Set
$R(\bu)=R(0,\bu)$.  \indy{Notation}{R@$R$ (Rogers's simplex)}%
\end{definition}
...
\begin{displaymath}\Omega(V,\bu[0]) = 
\bigcup\, \{ R(\bv) \mid \bu[0]=\bv[0],~\bv\in  V(3)\}.
\end{displaymath}
By Lemma~\ref{lemma:webster}, the relative boundary of the bounded
polyhedron $\Omega(V,\bu[0])$ is the union of its facets.  The
polyhedron can be partition into the cones over these facets.
\begin{displaymath}\Omega(V,\bu[0]) = \bigcup\, \left\{
  ~\op{conv}(\{\omega(\bu[0])\}\cup \Omega(V,\bu[1]))   
   \mid \bu[1]\in  V(1)
   \right\}.
\end{displaymath}
It is enough to show that
\begin{displaymath}
\Omega(V,\bu[1]) = \bigcup\, \{ R(1,\bv) \mid \bv[1]=\bu[1],~\bv\in  V(3)\}.
\end{displaymath}
After successively partitioning each facet into cones over facets of
facets, it is enough to show that
\begin{displaymath}\Omega(V,\bu[3]) = \bigcup\,\{R(3,\bv)\mid\bv[3]=\bu[3],~\bv\in V(3)\}.\end{displaymath}
...
\end{displaymath}
By induction, $\Omega(V, [\u_0;\ldots;\u_k]) = \Omega(V,[\v_0;\ldots;\v_k])$,
but
...
%% XX circumradius is not defined!
\begin{definition}[h]
%Let $h(\bu[i]) = \norm{\omega(\bu[i])}{ \u_0}$.  
...
Then 
\begin{nomerate}
\item \case{circumcenter} $\omega(\bu[j])$ is the circumcenter of the
set $W_j =\{ \u_0,\ldots, \u_j\}$.
\item \case{convex hull} The circumcenter of $W_j$ lies in the convex
hull of $W_j$.
\item \case{distinctness} The points $\omega(\bu[j])$, for
...
\begin{proof} 
Since the sets $\Omega(V,\bu[j])$ satisfy \eqn{eqn:omega-dim}, it
...
and $\v_1\}$ lies in $\Omega(V,\bv[1])$.  By the distinctness
conclusion of the same lemma, some neighborhood of this midpoint in
the bisecting plane of $\{\v_0,\v_1\}$ lies in $\Omega(V,\bv[1])$.
Thus, $\dimaff(\Omega(V,\bv[1]))=2$.  Continue in this fashion to show
that
\begin{displaymath}
\dimaff(\Omega(V,\bv[j]))=3-j.\end{displaymath}
\end{proof}

Let $\op{Sym}(k+1)$ be the group of all permutations on $k+1$ letters.
Let $\bu = ( \u_0,\ldots, \u_k)\in V(k)$.  For any permutation
$\pi\in\op{Sym}(k+1)$, let $\pi_*(\bu)\in V(k)$ be given by
\begin{displaymath}
\pi_*(\bu)_i =  \u_{\pi i},
\end{displaymath}   
where $\v_i$ denotes the $i$th element of a list $\v$.
...
By construction, the simplices $R(\bu)$ in Rogers's partition are
...
Lemma~\ref{lemma:13-14} are all due to him.  \indy{Index}{Marchal,
C.}%
...
\u_3)\in V(3)$.  \hfill\break\smallskip \case{The $0$-cell} of $\bu$
is
\begin{displaymath}
...
\text{where } R_2 &= \{\p \in R(\bu)\cap \Omega(V,\bu[1]) \mid \norm{ \u_0}{\p}=\norm{ \u_1}{\p} =\sqrt2\}.
\end{array}
\end{displaymath}
\bigskip
