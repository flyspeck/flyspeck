...
of~\cite{Hales:2006:DCG}, if it is obtained by mechanically replacing
...
$n=\#(\Lambda\setminus\{0\})$ be the cardinality. The proof of
...
when $v_1$ and the circumcenter of $S$ lie on the same side of (resp. on, opposite side of)  the
...
the desired bound (\ref{eqn:pos}) holds on $T$.  
...
%Lemma~\ref{lemma:4fold} shows it is
...
This step of the proof has been formally proved in
Isabelle~\cite{Paulson:1994:Isabelle} by Nipkow and
Bauer~\cite{Nipkow:2005:Tame}\footnote{The Isabelle proof was originally
designed for the Kepler conjecutre classification. That proof was
modified by Nipkow and McLaughlin to check the Dodecahedral
constraints as well. The modified Isabelle proof is found
...
% Modified to use the Isabelle definition of tameness
...
% STM: This is exactly what's done in the code
...
\subsubsection{Vertex cardinality}
...
%XX
In our hypermap systems, $\Phi$ is a list of around 5000 constraints.
These are generated from a list of about 100 \emph{parameterized constraints}. 
The parameterized constraints are specialized to particular hypermaps.  For
example, a parameterized constraint $c(F)$ might relate the volume, solid angle and
azimuth angles of any triangular face. Given a hypermap $H$, $c(F)$ 
becomes a constraint on every triangular face of $H$, 
yielding dozens of actual constraints specialized to $H$.

For each tame Voronoi hypermap $H$, the generic constraints determine
a well-defined set of constraints $\Phi_{dod}(H)$. The hypermap
...
Dodecahedral conjecture that has been carried out by computer. This
...
$\optt{sol},\ldots$ that satisfies the predicates $\Phi_{dod}$. The
notation has been set up in a way that suggests the assignment.
