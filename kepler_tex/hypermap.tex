...
\begin{definition}[path,~list,~sublist,~visit,~dart~set] 
...
%not injective.)  
The infix operator $::$ prepends an element $x$ to a list $[x_0;\ldots]$:
\begin{displaymath}
x::[x_0;\ldots] = [x;x_0;\ldots].
\end{displaymath}
...
\begin{lemma}[quotient face,~$\F$]\label{lemma:quotient-bijection}
...
\begin{definition}[canonical function] Let $H$ be a hypermap with
...
%\begin{definition}[canonically true]
...
\begin{lemma}[flag quotient]\label{lemma:flag-set-quotient}
...
\begin{remark}[canonical compatibility]
...
\begin{definition}[digraph,~vertex,~edge,~head,~tail,~sink,~path]
...
\begin{lemma}[digraph sink]\label{lemma:digraph-sink}
...
\begin{lemma}[walkup-digraph]\label{lemma:RDW}
...
\begin{lemma}[correlated seed]\label{lemma:correlated-seed}
...
\begin{lemma}[correlated edge]\label{lemma:correlated-edge}
...
\begin{lemma}[correlated sink]\label{lemma:correlated-sink}
Let $H$ be a restricted $(\card(\Omega),d)$-hypermap with digraph $(V,E)$.
