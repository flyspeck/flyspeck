...
\begin{lemma}\label{lemma:orbit-connect}
...
\begin{lemma}\label{lemma:min-path}\rating{400}
...
\begin{proof}  The minimal vertex path between $\phi$ and $\phi'$ has $r=1$.
...
\begin{lemma}\label{lemma:convex-hyper}\rating{300}  
...
\begin{lemma}\label{tarski:subvertex} (XX Move to Tarski)  
...
and the right-hand side is at least $2\arc(\orgn,v,w)+\arc(\orgn,v_1,v_2)$,
...
\subsection{containment of cones} %DCG  12.11, p 136
    \oldlabel{4.12}
...
\begin{lemma}\label{tarski:cone-meet} (XX Move to Tarski)
...
$$
where $\epsilon(u) = \cosarc(|u|,1.255,\lambda)$ and $\lambda=1.6$.
\end{lemma}
...
\arc_V(\orgn,p,v') \ge \sum \arc_V(\orgn,v(\phi_i),v(\phi_{i+1})\ge
  r \arc(2.51,2.51,3.07) > r (1.316).
$$
Hence, $r=1$.
...
$$
where $\epsilon(u,h) = \cosarc(|u|,1.255,h)$ and $\lambda=1.815$.
\end{lemma}
...
V(x,\epsilon(v,\lambda))\cap V(x',\epsilon(v',1.255)) = \emptyset,
$$
where $\epsilon(u,h) = \cosarc(|u|,1.255,h)$ and $\lambda=1.9$.
\end{lemma}
...
follows from tarski\tarf{tarski:cones-meet}
