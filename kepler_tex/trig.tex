...
\footformal{DIFF\_SIN\, DIFF\_COS}
  ...
  \sin(x) = x - x^3/3! + x^5/5! \cdots.
    \indy{Index}{cos}\indy{Index}{cosine}\indy{Index}{sin}\indy{Index}{sine}
    \end{equation}
...
We call elements of $\ring{R}^N$ {\it vectors}.  There is a
...
\begin{lemma}\guid{JJKJALK}[Cauchy-Schwartz inequality]\formal{Jordan/metric\_spaces.ml:cauchy\_schwartz}
    $$|u \cdot v| \le \normo{u}\,\normo{v}.$$
Equality holds exactly when $\normo{v}u = \pm \normo{u}v$.  Furthermore,
$u\cdot v = \normo{u}\,\normo{v}$ holds exactly when $\normo{v} u = \normo{u} v$.
\indy{Index}{Cauchy-Schwartz inequality}
\end{lemma}
...
\indy{Index}{angle}\indy{Index}{arc}\indy{Index}{arclength}
\end{definition}
...
\end{lemma}\indy{Index}{arc}\indy{Index}{law of cosines} 
...
\begin{lemma}\guid{RLXWSTK}[spherical law of cosines]\label{lemma:sloc}\rating{100}
...
\begin{lemma}\guid{NLVWBBW}[spherical law of cosines - second form]\rating{80}
...
\begin{lemma}\guid{OJEKOJF} \label{lemma:dihform}\rating{80} Let $v_0,v_1,v_2,v_3$ 
...
\FIXX{I'm not sure this is needed any more.}
...
\begin{lemma}\guid{ISTYLPH}\label{lemma:beta-cone}\rating{100}
...
\begin{lemma}\guid{JLPSDHF}[Euler triangle]\label{lemma:euler}\rating{300}
Let $v_0,v_1,v_2,v_3$ be points in $\ring{R}^3$. 
Let 
...
\begin{lemma}\guid{KEITDWB}\rating{80}\label{lemma:sph-tri-ineq}
...
\begin{lemma}\guid{FGNMPAV}\rating{40}\label{lemma:sph-tri-multi}
...
\begin{lemma}[Lexell] Fix two points on a unit sphere $v_1,v_2$ that
are not antipodal.
...
\begin{lemma}\guid{PDPFQUK}\label{lemma:polar2}\rating{50}\formal{thetaij\_t}
...
azimuth angle is a polar coordinate of $(x,y)$:
    ...
    $$
\indy{Index}{azimuth}\indy{Index}{azim}\indy{Index}{zenith}\indy{Index}{latitude}
...
\indy{Index}{frame}\indy{Index}{orthogonal frame}
...
can be expressed in the form
   ...
   $$
where $u'\cdot e_3 = 0$.
\indy{Index}{zenith}\indy{Index}{orthonormal}
\end{lemma}
...
\begin{lemma}\guid{EYFCXPP}[cylindrical coordinates]\rating{80}
...
\begin{lemma}\guid{YBXRVTS}\label{lemma:polar-gen}\rating{60}
Let $\{v,w,u\}$ be a set of three points in $\ring{R}^3$
...
\begin{lemma}\guid{NLOFMTR}\rating{50} The azimuth cycle $\sigma:W\to W$ on
...
\begin{lemma}\guid{YVREJIS}\rating{40} Let $(v,w)$ be an ordered pair of points in $\ring{R}^3$,
...
\begin{lemma}\guid{ULEKUUB}\rating{60} 
...
\begin{lemma}\guid{QQZKTXU}\label{lemma:dih-azim}\rating{100}
