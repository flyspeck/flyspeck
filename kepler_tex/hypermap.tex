...
\end{definition}
\indy{Notation}{1@$\sim$ (equivalence relation)}
...
The \newterm{planar index} of a hypermap is
\[ \iota = \# f + \# e + \# n - \# D - 2\,\#
c.\] 
...
equivalence class.
\end{definition}
\indy{Notation}{1@$\lp{\wild}$ (cyclic list)}
...
\begin{example}[illustration]\label{ex:graph-gen}  
  ...
  under the face map.  The edges give the edges of a plane
  graph representing the subquotient.  The polygons that are fully shaded
  are true, together with the face {\it at infinity}.  
  ...
  the three edges through darts $f'x'$, $f'^{2} x'$, and $f'^{3} x'$ meet
  a true face, and these three edges meet $S'$.)
%% XX recheck
\end{example}
