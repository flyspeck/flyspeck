...
The upper bound of $9$ on this parameter is hardwired; it can be made smaller
but not larger than this (except by editing the source code).

\subsubsection{faceCardMax=8}

For every hypermap $H$ in $C$, the cardinality of every face is at most
this value.  This parameter is currently frozen at $8$.  It cannot be reconfigured,
except by editing the file graph\_generator/src/Constants.java.
...
faceCountMaxAtExceptionalVertex.  The other three parameters are functions:
...
\subsubsection{squanderFace\wild =squanderTarget}
...
that are not in the range $n=0,\ldots,\text{faceCardMax}$ are ignored.

\subsubsection{excessTCount\wild =squanderTarget}
...
\begin{verbatim}
squanderTarget=1500
faceCountMaxAtExceptionalVertex=6
squanderFace3=0
squanderFace4=200
squanderFace6=700
excessTCount5=800
...
\[
p=0,\ldots,\text{faceCountMaxAtExceptionalVertex}-1
\]
are ignored. 


\subsubsection{squanderVertex\wild\wild=squanderTarget}
...
\begin{verbatim}
squanderTarget=1600
squanderVertex21= 800
squanderVertex22= 400
squanderVertex23=1200
\end{verbatim}
...
$2b(2,1)  = 1600=\op{tgt}$.
\end{example}
Properties declarations where $p$ or $q$ is greater than faceCountMax are ignored.
