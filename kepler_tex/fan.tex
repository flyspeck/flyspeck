...
Then
\begin{itemize}
\item The set $\{\orz,v,w(t)\}$ is not collinear for any $t\in[0,1]$.
...
that $\{\orz,v,v_0\}$ is not collinear.  By continuity, $\{\orz,v,w(t)\}$
is not collinear when $t$ is positive and sufficiently small.  
...
Let $I\subset\leftopen0,1\rightclosed$ be an interval that contains $(0,\epsilon)$ for $\epsilon$ sufficiently small and such that $\{0,v,w(t)\}$ is not collinear for all $t\in I$.
...
Now assume for a contradiction that $\{0,v,w(t)\}$ is collinear for some $t\in\leftopen0,\rightclosed1$.  Let $b\in I$ be the least such constant.  
Pick $0<a<b$.  Then
$\{\orz,v(a),v(b),v\}$ lie in a unique plane $P$.  Since all $v(t)$
are collinear,  $v(t)\in P$ all $t\in I$.  In particular $v_0\in C^0(a)\cap X$,
