...
$\op{aff}_+(p,\{v\})$ in a point, giving a spherical triangle with sides $a,b,c$.  By the spherical law of cosines, the angle $\gamma$ of the spherical triangle is given in terms of the edges as
$$
...
The simplices $R(\bl)$ in Rogers's partition  are compatible with the Voronoi
...
$4$-cells combine multiple simplices.

\begin{lemma}\guid{RVFXXBU}\rating{400}  The $0,\ldots,4$-cells of all simplices $R(\bl)$, for $\bl\in \Lam(3)$
...
Finally,  every point $p$ in $\ring{R}^3$ belongs to a cell.  In fact, by Rogers's
...
\begin{conjecture}[Marchal]\guid{PHNFUXP}\rating{XX}\label{conj:m1} For any packing $\Lam$, and
any $\lam_0\in\Lam$, 
$$
...
\begin{conjecture}\guid{BJERBNU}\rating{XX}\label{conj:L12} For any packing $\Lam$, and
any $\lam_0\in\Lam$, 
...
\begin{corollary}\guid{RDWKARC}\rating{XX}\label{cor:CE} If the Kepler conjecture is false,
...
\begin{lemma}\guid{TARJJUW}\rating{XX}\label{lemma:poly-bounded} 
Fix $2\le r\le R$.
