...
\begin{proof} The explicit enumeration of hypermaps with tame
  ...
  FCC and HCP.  The remaining six must be eliminated.  One of these cases
is eliminated with a geometrical argument and the other five cases are eliminated
by linear programming.

One case with a hexagonal cannot be realized geometrically as a contact fan
(Figure~\ref{fig:fthex}).  Indeed, the perimeter of a hexagon with sides $\pi/3$
is $2\pi$.  However, $2\pi$ is a strict upper bound on the perimeter of a convex
...
\path ( 400,0) node (P0) {};
\path ( 200,346) node (P1) {};
\path ( -200,346) node (P2) {};
\path ( -400,0) node (P3) {};
\path ( -200,-346) node (P4) {};
\path ( 200,-346) node (P5) {};
\path ( 195,-99) node (P6) {};
\path ( 11,-236) node (P7) {};
\path ( -186,-165) node (P8) {};
\path ( -217,58) node (P9) {};
\path ( -31,218) node (P10) {};
\path ( 181,128) node (P11) {};
%Draw edges:
\draw
  (P0) -- (P5)
  (P0) -- (P1)
...
%\]
\end{enumerate}\wasitemize 
By a linear programming \cc{See linear programming glpk directory for the code},
