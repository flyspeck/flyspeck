...
\end{equation}
Substitute the formula \eqn{eqn:sigma} into \eqn{eqn:convS} and  use the bijection of
...
\setcounter{enumi}{-1}
\item %[~0]
The $0$-cell of $\bu$ is defined to be empty unless $\sqrt2 < h(\bu)$.
...
\bigskip
\item
The $1$-cell of $\bu$ is defined to be empty unless $\sqrt2 < h(\bu)$.
...
empty unless $h(\trunc{\bu}{1})<\sqrt2< h(\bu)$.  If this inequality holds, then
the $2$-cell is
(with $a$ as above)
\begin{align*} 
\cell(\bu,2) &= 
...
%XX Replace $\sol_0/\pi =\Delta_1$ to simplify formulas.
...
\begin{proof}  %This is a \cc{HJKDESR}{}. 
This inequality is not used in the proof of the Kepler conjecture and its proof is omitted.
...
&\le 8 m_1 - 8 m_2 \sum_{\u\in V\setminus \{\u_0\} } M(h([\u_0;\u]))\\
...
$r^2$.  % XX give reference to C*r^2 estimate in volume chapter.
Thus,
\[  
...
\begin{theorem}[cell cluster inequality]\guid{OXLZLEZ}\rz{1500} 
\label{lemma:cluster}
...
\begin{lemma*}\guid{BJERBNU}\rz{0}\label{conj:L12} 
...
follows from an easy \cc{4198769118}{This inequality gives the nonoverlap of disks.}
that
...
bounded.  By a \cc{8055810915}{The nonoverlap of disks}
