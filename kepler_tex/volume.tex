...
\begin{summary}
  ...
  To keep the presentation as simple as possible surface integrals
  have been avoided.  As an elementary substitute for surface
  integration on a sphere, this book has systematically replaced
...
primitive regions, is enumerated in Definition~\ref{def:primitive}.  These
...
%\begin{definition}[$\equiv$]\label{def:equiv}
...
\begin{lemma}[measurable]\label{lemma:measurable}
...
\begin{nomerate}%[Volume]
\item \case{non-negative} The volume is defined for every measurable set.  It is
...
\begin{definition}[ball]\guid{CODE}
...
\begin{definition}[radial]\guid{CODE}
A set $C$ is $r$-\newterm{radial} at center $\v$ if  $C\subset B(\v,r)$
and if
$\v + \u \in C$ implies
...
\begin{lemma}[radial intersection]\guid{KODOBRF}\rating{60}  If $C$ and $C'$ are  $r$-radial
at $\v$, then $C\cap C'$ is also $r$-radial at
$\v$.
\end{lemma}

\begin{lemma}[eventually radial]\guid{CODE}
If $C$ is $r$-radial for some $r>0$ then it is eventually radial.
\end{lemma}

\begin{lemma}[radial scale]\guid{PUACSHX}\tlabel{lemma:r-r'}\oldrating{140}
...
\begin{lemma}[solid angle]\guid{CODE}
...
\begin{definition}[wedge]\guid{CODE}
Assume that $\v_0\ne \v_1$ and that
...
\begin{definition}[lune]\guid{CODE}
The set $\op{aff}^0_+(\{\v_0,\v_1\},\{\v_2,\v_3\})$ was defined
in Definition~\ref{def:aff}.  Call it a \newterm{lune}.  It is the intersection
...
%For angles that are larger than $\pi$,   a wedge $W^0(\v_0,\v_1,\w_1,\w_2)$ is used instead of the lune.  
\indy{Index}{lune}%
...
\begin{lemma}[lune~wedge]\guid{VICUATE} Let $\{\v_0,\v_1,\v_2,\v_3\}$ be a set of four points
...
\begin{definition}[solid triangle]\guid{CODE}
...
\begin{definition}[conic cap]\guid{CODE}
...
\begin{definition}[rcone]\guid{CODE}\tlabel{def:p:rcone}
...
\begin{definition}[frustum, FR]\guid{CODE}
 The frustum
...
\begin{definition}[tetrahedron]\guid{CODE}
...
\begin{remark}[quadratic solids]\guid{CODE}
...
\begin{definition}[primitive]\guid{CODE}\label{def:primitive} 
...
\item \rating{250} The frustum $\op{FR}(\v_0,\v_1,h,a)$ (with $h'=0$) 
has volume
\begin{displaymath}
\pi (t^2-h^2) h/3,\quad h = t a.
\end{displaymath}
\item \rating{250} The conic cap $\op{CAP}(\v_0,\v_1,r,a)$
has volume
\begin{displaymath}
2\pi(1-a) r^3/3.
\end{displaymath}
\item\rating{250} The ball $B(\v_0,r)$ has volume
% with a wedge
...
%The next lemma considers the simple example of the solid angle of a lune.
%A classical calculation of the volume of a solid triangle runs the argument in
%the other direction
%and derives the volume of a solid triangle from the solid angle of a lune.
%%\subsection{lune}  
%
%\begin{lemma}[lune solid angle]\guid{WFFVGVF}
%Let $A=\op{aff}_+^0(\{\v_0,\v_1\},\{\v_2,\v_3\})$ be a lune.  
%It is eventually radial at $\v_0$ and has a solid angle given by
% $\sol(\v_0,A) =
...
%&= 
%\sol(\op{TRI}(\v_0,\{\v_1,\v_2,\v_3\})) + \sol(\op{TRI}(\v_0,\{\v_1',\v_2,\v_3\})) \\
%&=
%2\dih_V(\{\v_0,\v_1\},\{\v_2,\v_3\}).
%\end{array}
%\end{displaymath}
...
%\end{proof}
%
%\begin{lemma}[wedge solid angle]\guid{CZOQFNL}\tlabel{lemma:wedge:sol} 
...
\begin{definition}[orthosimplex]\guid{CODE}\label{def:orth}  Let
...
%
%\begin{definition}[orth]\guid{CODE}
% \tlabel{def:orth} Let
...
%\end{definition}
%
%%\begin{definition}[orthosimplex, orth]\guid{CODE}
% \tlabel{def:ortho}
...
%\begin{definition}[circumradius, $\eta$]\guid{CODE}\label{def:etaV}
...
%\end{definition}
%
%\begin{definition}[$abc$ parameter]\guid{CODE}\tlabel{def:abc}
