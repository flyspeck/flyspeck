...
\begin{lemma}[]\guid{AURSIPD} \label{lemma:straight-count}  Let $s$ be an unadorned SCS with $k(s)>3$, and let $\v\in \MM_s$ be generic.   
...
\begin{lemma}[lune]\guid{RRCWNSJ}\label{lemma:lune-main}
...
By Lemma~\ref{lemma:lunar-main}, $\v$ is generic.
By Lemma~\ref{lemma:exists-2}, there exists an edge $\{i,j\}$ such that $\norm{\v_i}{\v_j} = 2$.
...
the deformation IMJ implies that the edge $\{0,1\}$ is a short a possible, length $2$.  
...
We claim that $\v$ is not lunar: because of the triangle inequality there can be no pole:
\[
...
Let $s$ be an SCS with $k(s)=4$ in which every edge is std or \prop, with at most two non-standard edges.
...
Assume every edge is std or \prop.
Let $\v\in \MM_s$.  
...
We consider that case that there is a path from $i-1$ to $i+1$ involving at most one non-standard edge.
...
Let $s$ be an SCS with $k(s)=4$, and $J(s)=\emptyset$.
Assume every edge is std or \prop, with at most two non-standard edges.  
Let $\v\in \MM_s$.  
Assume that $\norm{\v_i}{\v_j}> a_{ij}(s)$ for every diagonal.
Then every edge is extremal (that is $2$ or $2h_0$ for standard edges and $2h_0$ or $\stab$ for non-standard edges).
\end{lemma}
...
Let $s$ be an SCS with $k(s)=4$, and $J(s)=\emptyset$.
Assume every edge is std or \prop, with at most one non-standard edge.  
Let $\v\in \MM_s$.  
Assume that $\norm{\v_i}{\v_j}> a_{ij}(s)$ for every diagonal.
