...
$V$ is a packing, $(V,E)$ is a biconnected graph, and $V\subset
...
Equation~\ref{eqn:drs}), one of the two faces $F_1$ or $F_2$ is a
...
two or more concave nodes.  Let $k'$ be the number of concave and
convex nodes.  Calculations~\cite[cc:lft]{hales:2009:nonlinear}%
%% CALC XX Calculation 
...
\begin{lemma} Let $V\in \CalV$.  Then there exists $V'\in \CalV$ with
...
\]
\item Let $E'_{cut}$ be the set of all pairs $\{\u,\v\}\subset V'$
  such that $2.52\le\norm{\u}{\v} <\sqrt8$.  Set $E'_{agg} =
  E'_{ctc}\cup E'_{cut}$.  Then $(V',E'_{agg})$ is a fan.
\item $\op{hyp}(V',E'_{agg})$ is biconnected.
\end{itemize}
\end{lemma}

\begin{proof}
  Begin with the contact fan $(V,E_{ctc})$.  Let $E_{cut}$ be the set
  ...
  remains in $\CalV$ and no new edges $E_{ctc}$ are formed.
...
\claim{We claim that every face has at least three darts.}  Otherwise,
if some face has only two darts, then because of the no double join
condition, the two edges $\{x, e x\}$ and $\{ e^{-1} f^{-1} x, f^{-1}
...
x$ and $x$ lie at the same node.  The constant $m$ can be viewed as a
...
\begin{proof} Assume to the contrary that either the face is not
  ...
  there is some node $\v$ interior to the $m_U$-gon.  The azimuth
...
\begin{proof} 
  ...
  follows that the set $D_U$ is either simple with at most $8$ darts,
  or a nonsimple face $F$ with $8$ darts and $m_U=6$.

  One case remains: $8$ darts and $m_U=6$.  Suppose this occurs in a
  contravening fan.  This arrangement involves $7$ nodes: the six
  nodes counted by $m_U$ and the node in the center of the hexagonal
  arrangement.  As there are $12$ nodes in all, there are five
  additional nodes.  Each of these five nodes meets a non-triangular
  region.  The total weight is then at least \arrayqed
\begin{eqnarray*}
d(8,0) + d(5,0) &>& \op{tgt}, \text{ or }\\
d(8,0) + 2 d(4,0) &>& \op{tgt}\qedhere
\end{eqnarray*}
\end{proof}
...
\begin{remark}\label{rem:local-same} %XX move
  Localize the fan $(V,E)$ along the face $F$, so that without loss of
  generality the packing $V$ consists of nodes that meet the face $F$.
  The assumption of biconnectedness implies that $\tau(V,E,F)$ takes
  the same value, before and after localization.
\end{remark}
...
further use for the old functions.  Recall that $\op{tgt}=1.541$.
\indy{Index}{tame}%
...
%
  ...
  of non-negative real numbers. A weight assignment is a {\it contact}
...
\begin{theorem}\guid{ZXZSVPH} The contact hypermap of a 
  packing $V\in \CalV$ is a tame contact hypermap.
\end{theorem}
\indy{Index}{hypermap!tame}%
...
\end{displaymath}
\indy{Index}{weight!total}%
Let $\v$ be a node of type $(p,q,0)$.  Let $A$ be the set of faces
that meet the node $\v$. Then
\begin{displaymath}
\sum_{F\in A}\tau(V,E_{ctc},F) > d(4)~q.
\end{displaymath}
...
There are some obvious linear
  programming constraints:
\begin{itemize}
\item The angles around each node sum to $2\pi$.
\item Each angle of a triangle is $\alpha_3$.
\item Each angle of each rhombus is between $\alpha_4$ and $\beta_4$.
...
\end{displaymath}
\end{itemize}
These inequalities eliminate all but the FCC and HCP hypermaps.
\end{proof}
%% CALC XX Calculation
