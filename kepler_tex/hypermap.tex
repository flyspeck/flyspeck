...
\begin{lemma}\guid{IUCLZYI}\rating{400}\tlabel{lemma:index} Let $x$ be a dart of a hypermap $(D,e,n,f)$. Let $(D',e',n',f')$ be the result of the edge walkup $W_e$ at
...
\begin{itemize}
    ...
    in true faces.
    \item Let $T$ be the union of $S$ with the set of all darts in the true faces.  Then $T$ meets the edge of every dart in every false face.
    \end{itemize}
...
\begin{definition}[canonical boolean function] Let $H/{\cal L}$ be a quotient hypermap with
normal family $\cal L$.  The {\it canonical boolean function}
$\varphi$ on the set of faces of the quotient is the function that
...
Let $H=(D,e,n,f)$ be a hypermap with normal family $\cal L$, quotient $H/{\cal L}$.  We assume that the quotient is simple.  We assume that the canonical boolean function on $H'=H/{\cal L} = (D',e',n',f')$ is a flag.  Choose $F'$ be a false face of the quotient, corresponding to a contour loop $L$ of ${\cal L}$ and choose $x'\in F'$.  By the definition of $\emptyset$-flag, it  lies on an edge that meets a true face.   $x' = [\ldots,x]$.  As the face $F'$ is false in the canonical flag, there is a smallest $i>0$ such that $f^i x$ is followed by a $n^{-1}$ step in a loop of ${\cal L}$. Let $y = f^i x$. Consider the darts $f^{j+1} y$.  There is a smallest $j\ge0$, such that $f^{j+1} y$ is a dart in some path of ${\cal L}$.  Set $z=f^{j+1} y$.
