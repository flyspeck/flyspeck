...
\begin{lemma}[localization]\guid{LVDUCXU}\rating{ZZ}  Let $(V,E)$ be any fan, and let $F$ be a face of its hypermap.
Let 
$$
\begin{array}{rll}
V' &= \{v \mid \exists~w.~(v,w)\in F\}.\\
E' &= \{\{v,w\} \mid (v,w)\in F\}.
\end{array}
$$
Then $(V',E',F)$ is a cyclic fan.  Moreover, $\op{azim}(x)$ is the same
...
\item The perimeter is $P(V,E,F) =2\pi$.
\end{itemize}
\end{lemma}
...
\item $V \cap \op{aff}_+(\{0,v\},\rho^{-1} v) = \{w,\rho w,\ldots v\}$; and
\item $P(V,E,F) = 2\pi$.
\end{itemize}
\end{lemma}
...
\begin{lemma}[generic geometry]\guid{EGHNAVX}\rating{ZZ}  Let $(V,E,F)$ be a generic cyclic fan with hypermap $(D,e,n,f)$.  Fix $v_0\in V$.  For all $r$, set $v_r = \rho^r v_0$ and $\beta(r) = \op{azim}(0,v_0,v_1,v_r)$.
...
\begin{lemma}[slicing]\guid{EJRCFJD}\rating{ZZ}  Let $(V,E,F)$ be a cyclic fan with hypermap $H$.  Let $v,w\in V$ be independent.  Assume that $C^0\{v,w\}\subset \Wdart(x)$ for all darts $x$ in the fan of $(V,E,F)$. Then
\begin{itemize}
