...
Let $\CalQ=\CalQ(\Lambda)$ be the set of simplices
in the $Q$-system.  
...
Voronoi cell coincide:
   $$B(v_0,t_0)\cap \op{VC}(v_0) \equiv B(v_0,t_0)\cap \Omega(v_0).$$
...
as various auxiliary sets $FC$, $\FCinner$, $FC_{rog}$ in
...
\begin{enumerate}  
 ...
 $|u-w|\ge 2.77$,
    ...
    $\eta_V(v_0,u,w)$ or $\eta_V(v,u,w)$ is
    $\ge\sqrt2$.
    \label{enum:wedge2}
\end{enumerate}
\end{itemize}
...
\begin{lemma}  Let $\Lambda$ be a sphere packing with crown tuple
...
$\{v_0,v\}$ is always be an upright diagonal: $|v-v_0|<\sqrt8$.
$(v_0,v,w,w')$ is  a crown tuple.  
Set $R_w=\op{rog}^0(v_0,w,v,p,\eta_{V0}(v,v_0))$,
...
let $S'=(v_0,v,w,w')$ be a crown tuple in a packing $\Lambda$.
Then
$\FC(S')$ does not meet $F=\op{cone}(v_0,\{u_1,u_2\})$.
\end{lemma}

\begin{proof}  By tarski\tarf{tarski:eps-bigd-}, 
...
\begin{proof}
We have separated $\FCinner(v,W)$ and $R_w$ from $A$.
Together they form $\FC(v,W)$.
\end{proof}

XX Can $A$ cut between $R_w$ and $\FCinner$?
...
Let $y\in \FC(v,W)$ be the point in the convex hull of $T$ on
the segment from $v_0$ to $x$.  Fix $i$ so that $y\in T(i)$. If
$v=u_i$, then each point $y$ of $T(i)$ is closer to $v$ than to
$v_0$.  But each point of $\FC(v,W)$ is closer to $v_0$ than to
...
construction that $y\in \FC(v,W) \subset V(v_0)$.  (There are
...
that $v$ is not enclosed over $S$.  The set $\FC(v,W)$ has
a star convexity with respect to the ray from $v_0$ through $v$.
Thus, if $\FC(v,W)$ intersects the convex hull of
$T$ at $y$, then $\FC(v,W)$ intersects the cone over a face
...
in the $\CalQ(v_0)$-system.  Then $\FC(v,W)$ is
a subset of $\op{VC}(v_0)$.
\end{lemma}

\begin{proof}
We begin by showing that $\FCinner(v,W)\subset\op{VC}(v_0)$.
Suppose to the contrary, that a point $x$ in the interior of
$\FCinner$ lies in $\op{VC}(w)$, with $w\ne0$.  Then $x$ is closer
to $w$ than to  $v_0$.  Thus, $\eta_V(v_0,v,w)<\eta_{V0}(v,v_0)$, and $w$
is an anchor of $\{v_0,v\}$.  The face $E_w$ in the construction
$\FCinner(v,W)$ prevents this from happening.
...
the construction crown tuples, we have that $R_w$ must
...
\eta(|v-v_0|/2)$, then a point of $R_w$ cannot be closer to $u$ than
to $v_0$.
\end{proof}
...
\begin{lemma}  The sets $\FC(v,W)$ do not overlap one another.
\end{lemma}

\begin{proof}
This is clear for two sets around the same vertex $v$.  Consider
the sets $\FC(u,W(u))$ and $\FC(v,W(v))$ at $u$ and $v$.

To treat the points in $\FCinner(u,W(u))$ and $\FCinner(v,W(v))$, we
...
$\FCinner(u,W(u))$ from $\FCinner(v,W(v))$.

Next we separate points in $\FCinner(u,W(u))$ from points of
...
$\FCinner(u,W(u))$ and $R_w^{(v)}$, so that the sets are disjoint.
...
$\FC(v,W(u))$ is inconsistent with Lemma~\ref{lemma:delta-tri}.)
...
We consider various types of simplices $A$, $B$, $C$, $D$, formed by vertices in $\Lambda$.
  The edge lengths of
...
$\FC(v,W)$ and simplex of type $\SCp$, when the upright
...
obvious case where the simplex of type $\SCp$ and the crown tuple are both
...
$\sigma_R(\Lambda,v)$, noting that $\op{VC}(v,Q) 
= \op{VC}_{R}(\Lambda,v)$,
\end{proof}
