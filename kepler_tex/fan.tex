...
\begin{lemma}[]\guid{XOHLED}\rating{ZZ}  
...
\begin{remark}\guid{WCXASPV}\label{rem:fan}\rating{30}
\begin{itemize}
...
\begin{definition}[affine~set,~affine~hull,~affine~dimension,~affinely~independent,~hyperplane]\guid{AJXXAWK}\label{def:affine}
...
\begin{displaymath}
C(\ee) \cap C(\ee') = \{\orz\}\cup \{t \p \mid \p\in \op{conv}(\ee) \cap \op{conv}(\ee') \text{ and } t \ge 0\}.
\end{displaymath}
...
Thus,
\begin{displaymath}
C(\ee)\cap C(\ee') = \{\orz\}\cup \{t \p \mid \p\in \op{conv}(\ee\cap \ee')\} = C(\ee \cap \ee').
\end{displaymath}
