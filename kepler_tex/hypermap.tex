...
\begin{definition}[facial]
A contour loop $L$ in a hypermap is \newterm{facial} if its dart set is a face of $H$.
\end{definition}
...
\begin{proof} 
  ...
  {m+1}$, which forces  $L$ to be facial, which is
...
\case{flag-2} \claim{Each edge meets $S$ or a true face.}  Indeed, if the
...
$A_i$, whenever the contour loop $L$ is not facial.
\item $A_{i+1}$ has the form $(H,{\cal M},M,y,\phi_{i+1})$, where $(H,{\cal M},L_1,x,\phi)$ is
the the transform of $A_i$, whenever the $L_1$ is facial, and there exists some other contour loop of ${\cal M}$ that is not facial.
...
\begin{nomerate}
\item 
\claim{[Every contour loop of ${\cal M}$ is facial.]}   In this case,  by
Lemma~\ref{lemma:all-dart}, the quotient map $H\to H/{\cal M}$ is an
isomorphism.  Set $k= i+1$. The sequence terminates.
\item 
\claim{[The contour loop $L$ is not facial.]}  In this case,  from the inductive hypothesis that $\varphi_i$ is
...
\item
\claim{[The contour loop $L$ is facial, but there exists another $M\in {\cal M}$ that is not facial.]}
