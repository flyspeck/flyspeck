...
for every $x\in F$.  The result follows.
...
From the  induction hypothesis $\v_0\in \Wdart(\v_{r-1})$
...
\end{equation}
When $t=0$, we have $\theta_j(0)=\beta(j)=0$,
for $j=1,\ldots,r$.  By continuity, when $t$ is sufficiently small, 
$\theta_r(t)-\theta_{r-1}(t)$ is near zero, so that \eqref{eqn:theta} gives
$\theta_r(t)\ge \theta_{r-1}(t)$.  The induction hypothesis $\v_{r-1}\in \Wdart(\v_0)$
gives $\theta_{r-1}(t)\ge0$.  Hence $\theta_r(t)\ge0$.  
...
\]
This is precisely the desired relation $\v_r\in \Wdart(\v_0)$,
...
$C^0\{\v,\w\}\subset \Wdarto(x)$ for all $x\in F$.
...
% $C^0\{\v_{i(1)},\v_{i(3)}\} \subset \Wdarto(x)$ for all $x\in F$.
\end{lemma}
...
consider is $\nd(x)=\v$.  For all $\p\in C^0\cap \Wdart(x)$,
\[ 
...
imply $\p\in \Wdarto(x)$, as desired.  The case $\nd(x)=\w$ is similar.
...
half-spaces.  Since $\Wdart(F,\rho^{-1}\v)$ is contained in one of
the two half-spaces, the condition $\v,\rho\v\in
\Wdart(F,\rho^{-1}\v)$ fails, and $(V,E,F)$ is not a nonreflexive local
