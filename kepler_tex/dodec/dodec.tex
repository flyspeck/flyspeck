...
\begin{figure}[htb]
  \begin{center}
    \includegraphics[scale=0.50]{images/dode.pdf}
   \end{center}
...
\begin{figure}[htb]
  \begin{center}
    \includegraphics[scale=0.5]{images/icos.pdf}
  \end{center}
...
\begin{figure}[htb]
  \begin{center}
    \includegraphics[scale=0.18]{images/counter.pdf}
  \end{center}
...
% Its proof is used in the classification of Quad types, to don't delete unless you are sure the proof can avoid this.
% XX
\label{def:tame8}
...
Indeed, it is a direct consequence of the inequalities of \cite{McLaughlin:2008:KeplerCode} that if $U_1,\ldots,U_n$, for $n=p+q+r\ge 6$, are the standard components of the faces around the node, then 
  ...
  $$
%The same property holds in the solution of the sphere packing
%problem~\cite[Lemma~21.11]{Hales:2006:DCG}.  
...
% then $(H,\Phi)$ itself is infeasible.\FIWW{Perhaps there is
