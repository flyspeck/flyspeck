...
If $\{\v_1,\v_2,\v_3\}\subset V$ is a set of cardinality $3$ that satisfies 
 $\norm{\v_1,\v_2}\le 2\hm$,
then $C^0\{\v_1,\v_2\}\cap   C^0\{\v_3\} = \emptyset$.
If $ \{\v_1,\v_2,\v_3,\v_4\}\subset V$ is a set of cardinality $4$ that satisfies
$\norm{\v_1,\v_2}\le 2\hm$
  and  $\norm{\v_3,\v_4}\le 2\hm$,
 then $C^0\{\v_1,\v_2\} \cap C^0\{\v_3,\v_4\} = \emptyset$.
\end{lemma}
...
\begin{lemma}[compactness of special fans]\guid{}\rating{ZZ}\label{lemma:cyclic-compact}
...
Assume that there is a constant $0<c_3<c_4<\pi$ such that $c_3\le\arc_V(\orz,\{\v_{i,j},\v_{\ell,j}\})\le c_4$ 
for all $j$ and all $i\ne \ell$.
Then for each $i$, the sequence $\{\v_{i,j}\}_j$ has a convergent subsequence.  Furthermore, 
for any convergent sequence converging to $\v_i$, write
$$V = \{\v_i\mid i=0,\ldots,k-1\},$$
$$E = \{\{\v_{i},\v_{\ell}\}\mid \{\v_{i,0},\v_{\ell,0}\}\in E_0\},\quad\text{and}$$ 
$$F = \{(\v_{i,j},\v_{\ell,j})\mid (\v_{i,0},\v_{\ell,0})\in F_0\}.$$
Then $(V,E,F)$ is a generic cyclic fan.

XX not finished. (Add more hypotheses as needed.)
\end{lemma}

\begin{proof}  Each of the properties of a cyclic fan can be verified in turn.

\case{V}:  This set is finite and nonempty by construction.

\case{origin}: The norms $\normo{\v_{i,j}}$ are bounded away from $0$.  Hence $\orgn\not\in V$.

\case{intersection}: XX

\case{independence},~\case{generic}:  The angles $\arc_V(\orz,\{\v_i,\v_\ell\})$ are bounded between $0$ and $\pi$.
In particular, they are independent and the fan is generic.
