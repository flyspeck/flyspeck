...
If $x_{ij} = \norm{v_i}{v_j}$ for some points $\{v_1,v_2,v_3\}\subset\ring{R}^3$,  then
$\ups(x_{ij})\ge 0$.
If $x_{ij} = \norm{v_i}{v_j}$ for some points $\{v_1,v_2,v_3,v_4\}\subset\ring{R}^3$,  then
$\Delta(x_{ij})\ge 0$.
If $x_{ij} = \norm{v_i}{v_j}$ for some points $\{v_1,v_2,v_3,v_4,v_5\}\subset\ring{R}^3$,  then
$R(x_{ij})= 0$.
\end{lemma}
...
$S$ is a packing if $\norm{v}{w} < 2$, for $v,w\in S$ implies that $v=w$.
\end{definition}
...
%three points $v_1,\ldots,v_3$ in $\ring{R}^3$, with $\norm{v_i}{v_j}^2 = x_{ij}$,
...
$$
where $x_{ij}=\norm{v_i}{v_j}^2$.
Then there exists a unique point $v_0$ such that $\norm{v_0}{v_i}^2 = a_{0i}$, for $i=1,2,3$.
In fact, 
  $$
...
For example,
$$\begin{array}{lll}
 \norm{v_0}{v_1}^2 &= \normo{t_2 (v_2-v_1) + t_3 (v_3-v_1)}^2 = t_2^2 x_{12} + t_2 t_3 (x_{13}+x_{12}-x_{23})  + t_3^2 x_{13} \\ &=
...
Assume that
	$$
	\begin{array}{rlrlrl}
		2\le\norm{v_1}{v_2}&\le 2.52, &2\le\norm{v_1}{v_3}&\le 2.2, &2\le\norm{v_2}{v_3}&\le 2.2\\
	\end{array}
	$$
...
Assume that 
  $$
  2\le\norm{v_0}{v_1}\le 2.51,\quad 2.45\le\norm{v_1}{v_2}\le 2.51,
  \quad 2.77\le \norm{v_0}{v_2}\le \sqrt8.
  $$
...
%four points $v_1,\ldots,v_4$ in $\ring{R}^3$, with $\norm{v_i}{v_j}^2 = x_{ij}$,
...
\begin{itemize}
\item  $v_1\ne v_2$, and
\item $\norm{v_i}{v_j}^2 = x_{ij}$ for $1\le i < j \le 4$  $(i,j)\ne (34)$.
\end{itemize} 
\end{definition}
\end{tarski}
%<<<<<
...
Then $\norm{v_3}{v_4}^2$ is a root of the polynomial $\mu_\Delta(x)$. 
\end{lemma}
...
If $v_3$ and $v_4$ lie in the same half-plane of the line through $v_1,v_2$, then 
 $\norm{v_3}{v_4}^2$ is the smaller root of $\mu_\Delta$.
\end{lemma}
...
Assume that
	$$
	\begin{array}{lll}
	\norm{v_1}{v_2}&\ge m_{12},\\
	\norm{v_2}{v_3}&\ge m_{23},\\
	\norm{v_3}{v_4}&\ge m_{34},\\
	\norm{v_4}{v_1}&\ge m_{14},\\
	\norm{v_1}{v_3}&< M_{13},\\
	\norm{v_2}{v_4}&\le M_{24},\\
	\end{array}
	$$  
...
which $\op{conv}(e)$ meets $\op{conv}(e')$.  Set $\norm{v_i}{v_j}=x_{ij}$. Contract $\norm{v_1}{v_3}$
...
We may therefore contract $\norm{v_1}{v_3}$ until $\norm{v_2}{v_4}=M_{24}$.  Then we use
...
\begin{itemize}
	\item $2\le \norm{u}{v}$ for every distinct $u,v\in S$.
          \item $\norm{v_1}{w_2}\ge \sqrt{8}$.
		\item $ \norm{v_1}{v_2} \le 3.2$.
	\item $\norm{w_1}{w_2}\le \sqrt{8}$.
...
%Let $S=\{v_1,v_2,v_3,v_4\}$ be a set of four points in $\ring{R}^3$.
%Assume that the distanes $y_{ij}=\norm{v_i}{v_j}$ are given by
% $$
...
until $\norm{v_3}{v_1} = M_{13}$ and $\norm{v_3}{v_2}=M_{23}$.   Next pivot $v_1$, increasing $\norm{v_1}{v_2}$
...
%$v_3$ at the same time to preserve the constraints on $\norm{v_3}{v_1}$ and $\norm{v_3}{v_2}$. 
By the assumptions on the constants $M_*$ and $m$, this is increasing in $\norm{v_3}{v_4}$.  The conditions
...
$v_1,v_2,v_3,v_3'$ form a planar quadrilateral with diagonals $\norm{v_1}{v_2}=2m$ and $\norm{v_3}{v_3'} \ge 2m_{34}$.
...
of the distances $y_{ij} = \norm{v_i}{v_j}$.  It is known that
 $$
...
$\norm{v_3}{v_4}\le M$, which would force the pivot to stop when $\norm{v_3}{v_4}=M$.
...
distance $\norm{v_3}{v_4}$ is increasing or decreasing under a pivot $v_3$
...
We have the approximation
  $$
  \norm{(v_3(t)}{v_4}^2 \approx \norm{(v_3+t n)}{v_4}^2 
  \approx \norm{v_3}{v_4}^2 + 2 t n \cdot (v_3-v_4).
  $$
Thus, the sign of the derivative of $\norm{v_3(t)}{v_4}^2$ is determined by the
...
that 
  $$
    \norm{v_1}{v_0},\ \norm{v_2}{v_0}, \norm{v_3}{v_0}\le 2.51.\quad 
    \norm{v_1}{v_2}\le 3.2,\quad \norm{v_1}{v_3}\ge 2.51.
   $$
Then $R^0=\op{rcone}^0(v_0,v_3,\norm{v_3}{v_0}/2.51)$ does not meet
...
Pivot so that $\norm{v_1}{v_2}=3.2$, $\norm{v_3}{v_1}=2.51$, $\norm{v_3}{v_2}=2$.
Let $\cos\psi  =\norm{v_3}{v_0}/2.51$ and $\theta=\arc_V(v_0,v_2,v_3)$. 
...
Assume that
   $$
   \norm{v_2}{v_0}\le 2.23,\quad \norm{v_3}{v_0}\le 2.23,\quad 2.77\le \norm{v_2}{v_3}\le\sqrt8.
   $$
Assume $\norm{v_1}{v_i}\le 2.51$, for $i=0,2,3$.
Then $R^0=\op{rcone}^0(v_0,v_1,\norm{v_1}{v_0}/2.77)$ does not meet
...
along the axis $\{v_0,v_3\}$, until $\norm{v_1}{v_2}=2$.
Set $\theta = \arc_V(v_0,v_1,v_3)$.
Define $\psi$ by $\cos\psi = \norm{v_1}{v_0}/2.77$.
...
\begin{proved}
Pivot until
$\norm{v_1}{v_3}=3.2$. Then use the calculation\footnote{\calc{193836552}} %A1
...
Set $y_{ij}=\norm{v_i}{v_j}$ and $x_{ij} =y_{ij}^2$.  In particular, $y_{01}=y$.
...
Then $\norm{v_4}{v_5}^2$ is a root of the polynomial 
$\mu_V[\{v_1,v_2,v_3\},\{v_4,v_5\}]$. 
\end{lemma}

\begin{proved}
Let $x_{ij}=\norm{v_i}{v_j}^2$.  By Lemma~\ref{tarski:cayley-menger-pos},
$$
...
If $v_4$ and $v_5$ lie in the same half-space of the plane through $v_1,v_2,v_3$, then 
 $\norm{v_4}{v_5}^2$ is the smaller root of $\mu_V[\{v_1,v_2,v_3\},\{v_4,v_5\}]$.
\end{lemma}
...
Pivot in a constraint satisfying way until $\norm{w_i}{v_j}=2$, for
...
$r=\eta_V(v_1,v_2,v_3)$. The Pythagorean theorem gives
   $$
   4 = \norm{v_1}{w_2}^2 = \norm{v_1}{p}^2 + \norm{p}{w_2}^2 = r^2 + \frac{\norm{w_1}{w_2}^2}4 \le r^2 + 2.
   $$
...
Set $y_{ij}^2 = x_{ij} = \norm{v_i}{v_j}^2$.
	...
	\item 
		$$
		\begin{array}{rlrlrl}
		\norm{v_1}{v_5} &\le M_{15},\\
		\norm{v_2}{v_3} &\le M_{23}, &\norm{v_2}{v_4} &\le M_{24}, & \norm{v_3}{v_4}&\le M_{34},\\
		\norm{v_1}{v_2} &\ge m_{12},&\norm{v_1}{v_3} &\ge m_{13},& \norm{v_1}{v_4}&\ge m_{14},\\
		\norm{v_5}{v_2} &\ge m_{25},&\norm{v_5}{v_3} &\ge m_{35},& \norm{v_5}{v_4}&\ge m_{45}.\\
		\end{array}
		$$
	\end{itemize}
\end{lemma}
...
to hold and that is non-increasing in the length $\norm{v_1}{v_5}$.  By the conditions of the
...
that $\CalE = \norm{v_1}{v_5}$.  Thus, we reach the contradiction
	$$
	\CalE = \norm{v_1}{v_5} \le M_{15} < \CalE.
	$$
\swallowed\end{proved}
\end{tarski}
%<<<<<
...
Assume that
	...
	\begin{array}{lll}
	\norm{v_2}{v_3}, \norm{v_3}{v_4}, \norm{v_2}{v_4}\le 2.52.\\
	\norm{v_1}{v_5}\le \sqrt8.
	\end{array}
	$$
Assume that $\op{conv}\{v_1,v_5\}$ meets $\op{conv}\{v_2,v_3,v_4\}$.
Then $\norm{w}{v}\le 2.2$ for $w\in\{v_1,v_5\}$ and $v\in\{v_2,v_3,v_4\}$.
\end{lemma}

\begin{proved}
This is a corollary of Lemma~\tref{tarski:line-tri} (if say $\norm{v_1}{v_2}\ge2.2$).
%Let the diagonal edge be $\{v_0,v_0'\}$ and the vertices of the
%face be $\{v_1,v_2,v_3\}$.  If $\norm{v_i}{v_0}>2.2$ or $\norm{v_i}{v_0'}>2.2$
%for some $i>0$, then constraint preserving deformations give the
%contradiction
%$$\norm{v_0}{v_0'}\ge{\CalE}(2,2,2,2.51,2.51,2.51,2,2,2.2) > \sqr8.$$
\swallowed\end{proved}
\end{tarski}
%<<<<<
...
Assume  that $\norm{v_0}{v_i}\le 2.51$ for $i=1,2,3,4$ and
$\norm{v_1}{v_3},\norm{v_2}{v_4}\le 2.51$. Then the intersection of
...
contradiction
    $$2.51 < \CalE(2,2,2,2.51,2.51,2.51,2,2,2) \le \norm{v_1}{v_3}\le 2.51.$$
...
\begin{itemize}
  \item For every distinct $u,v\in S$, we have $\norm{u}{v}\ge 2$.
    \item $\op{conv}\{v_1,v_5\}$ meets $\op{conv}\{v_2,v_3,v_4\}$.
  \item $$
    \begin{array}{lll}
      \norm{v_1}{v_2}\ge m_{12}, &\norm{v_1}{v_3}\ge m_{13}, &\norm{v_1}{v_4}\ge m_{14}\\
      \norm{v_3}{v_4}\le M_{34}, &\norm{v_2}{v_4}\le M_{24}, &\norm{v_2}{v_3}\le M_{23}\\
      \norm{v_2}{v_5}\ge m_{25}, &\norm{v_3}{v_5}\ge m_{35}, &\norm{v_4}{v_5}\ge m_{45}\\
      \norm{v_1}{v_5}\le M_{15},\\
      \end{array}
    $$
\end{itemize}
...
\begin{itemize}
  ...
  \item $2\le \norm{u}{v_0} \le 2.51$, for $u=v_1,w,v,v_2$.
    \item $\norm{v}{v_1}\ge 3.2$,  $\norm{v}{v_2}\ge 2$,
    \item $\norm{w}{v_1}\ge2$, $\norm{w}{v_2}\ge2$,
    \item   $\norm{v}{w}\le2$, $2\le \norm{v_1}{v_2}\le 3.2$.
\end{itemize}
...
The function
$$
   g_1(v_0,v_1,v_2,v,w) = \sum_{u = v_0,v,w}\sum_{i=1}^2 \norm{v_i}{u}
$$
...
\end{itemize}
In the second case, the figure is rigid, with $\norm{v}{w}$, $\norm{w}{v_0}$ and $\norm{v_0}{v}$ attaining
their upper bounds and $\norm{v_i}{u}$ attaining their lower bounds.  By direct calculation of
...
$\norm{w}{v}=2$, for otherwise $g$ is not maximal (pivot $v_1$).  Similarly, $\norm{v}{v_0}=2.51$,
$\norm{v_2}{v}=2$, and $\norm{v_1}{v}=3.2$.
...
%$\norm{v_2}{v}$ is not $2$ (pivot $v_2$ around $\{v_0,w\}$ toward $v$), if
%$\norm{v_1}{v}$ is not $3.2$ (pivot $v_1$ around $\{v_0,w\}$ toward $v$), if
%$\norm{w}{v}$ is not $2$ (pivot $w$ around $\{v_1,v_2\}$ away from $v$),
%or $v$ is not $2.51$ (pivot $v$ and $w$ simultaneously preserving
%$\norm{w}{v}$ around $\{v_1,v_2\}$).  Therefore, we may assume without
...
$f_h(\norm{v}{u})$, where $f_h(x)=\sqrt{x^2-h^2}$.
Note that
$$
2\le \norm{v_2}{w}\le \norm{v_2}{p}+\norm{p}{w}=f_h(\norm{v_2}{v})+f_h(\norm{v}{w}) = 2 f_h(2) = 2\sqrt{4-h^2}.
$$
...
Assume $2.51\le\norm{w}{v_0}\le\sqr8$, $\norm{v_1}{v_3}\le\sqr8$.
Assume that $\norm{u}{v}\le 2.51$, with $u=v_1,v_2,v_3$,
$v=v_0,w$.
Assume that $\op{conv}\{v_1,v_3\}$ meets $\op{conv}\{v_0,w,v_2\}$.
Then
$\min(\norm{v_1}{v_2},\norm{v_2}{v_3})\le2.51$. Furthermore, if the minimum is
$2.51$, then $\norm{v_1}{v_2}=\norm{v_2}{v_3}=2.51$.
\end{lemma}


\begin{proved}
Assume that $\norm{v_1}{v_2}\ge 2.51$ and $\norm{v_2}{v_3}\ge2.51$.

We have $\norm{v_1}{v_3}=\sqrt8$.  Otherwise,
$\norm{v_1}{v_3}=r<\sqrt8$, and the configuration does not exist by
exist by the calculation
  $$
  E(2.51,2,2,\sqrt8,2.51,2.51,2.51,2,2; r).
  $$
We have $\norm{v_0}{w}=\sqrt8$, for otherwise $\norm{v_0}{w}=r<\sqrt8$ and
$$
   E(2.51,2,2,r,2.51,2.51,2.51,2,2,\sqrt8).
$$
We have $\norm{w}{v_3}=2$, for otherwise,
$\norm{w}{v_3}=2+s$, with $s>0$, and  
the configuration does not exist:
  $$
  E(2.51,2,2+s,\sqrt8,2.51,2.51,2.51,2,2;\sqrt8). 
  $$
Similarly, $\norm{w}{v_i}=\norm{v_0}{v_i}=2$, for $i=1,3$.

Thus, $(v_0,v_1,w,v_3)$ is a square.
 We may also assume, without loss of generality, that
$\norm{w}{v_2}=\norm{v_2}{v_0}=2.51$. This forces $\norm{v_2}{v_i}=2.51$, for $i=1,3$.
...
Assume that $\norm{v_1}{v_3}\le \sqrt8$ and $\norm{v_0}{v_2}\le\sqrt8$.
Assume that $\op{conv}\{v_1,v_3\}$ meets $\op{conv}\{v_0,v_2,v_4\}$.
Then $\norm{v_1}{v_4}< 3.2$.
\end{lemma}

\begin{proved} Assume for a contradiction that $\norm{v_1}{v_4}\ge 3.2$.
By
...
%    Suppose $\norm{v}{v_0}\le\sqrt8$.  Suppose $\eta(v_0,v_2,v_3)<\sqrt2$.
%    Then 
%    $\op{rcone}^0(v_0,v,\norm{v}{v_0}/\sqrt8)$ does not meet 
%    $C=\op{cone}(v_0,\{v_2,v_3\})$.
%\end{lemma}
%
%\begin{proved}
...
%edge constraint $\norm{v}{v_0}\le\sqrt8$ forces the closest point in $C$ to
%lie in $\op{conv}\{v_0,v_2\}\cup\op{conv}\{v_0,v_3\}$.  Since
%$\norm{v_2}{v_0},\norm{v_3}{v_0}\le\sqrt8$, this closest point has distance at least
...
%moving $v$ toward $C$ (preserving $\norm{v}{v_0}$), we may assume that
%$\norm{v}{v_2}=\norm{v}{v_3}=2$.  Stretching the edge $\{v_2,v_3\}$, we may
...
%preserving the circumradius and increasing the lengths $\norm{v}{v_2}$ and
%$\norm{v}{v_3}$.  By moving $v$ again toward $C$, we may assume without
%loss of generality that $\norm{v_2}{v_0}=\norm{v_3}{v_0}=2$ and $\norm{v_2}{v_3}=\sqrt8$. We
%have reduced to a one-parameter family of arrangements, parametrized
%by $\norm{v}{v_0}$. We observe that the disk $D$ is
%tangent to the segment $\op{conv}\{v_2,v_3\}$ at its midpoint, no matter what
%the value of $\norm{v}{v_0}$ is.  Thus, in the extremal case, the open disk
...
If $p$  lies outside $C$, then the
edge constraint $\norm{v_1}{v_0}\le 2.51$ forces the closest point in $C$ to
lie in $\op{conv}\{v_0,v_2\}\cup\op{conv}\{v_0,v_3\}$.  Since
$\norm{v_2}{v_0},\norm{v_3}{v_0}\le\sqrt8$, this closest point has distance at least
...
$\norm{v_1}{v_2}>2.51$, the longest edge must not have $v_2$ as an endpoint,
so the longest edge is $\{v_0,v_3\}$.
Lemma~\tref{tarski:pass-makes-quarter} forces one of $\norm{v_1}{v_2}$ or
$\norm{v_1'}{v_2}$ to be at most $2.51$. But these are both equal to
$\norm{v_1}{v_2}>2.51$, a contradiction.
\swallowed\end{proved}
\end{tarski}
%<<<<<
...
Assume that 
$$
   \norm{v_1}{v_2}\le 2.51,\quad \norm{v_2}{v_3}\le 2.51,\quad \norm{v_1}{v_3}<\sqrt8.
$$
Assume that $B(v_0,\sqrt2)$ meets $T=\op{conv}\{v_1,v_2,v_3\}$ at $x$.
Then $\norm{v_0}{v_i}\le 2.51$, for $i=1,2,3$.
\end{lemma}
...
length less than $\sqrt8$. If $\norm{v_1}{v_3}\le2.51$, then
then Lemma~\tref{tarski:qrtet-pair-pass} implies the result.
Assume that $\norm{v_1}{v_3}>2.51$.
By Lemma~\tref{tarski:pass-makes-quarter},  
either $\norm{v_0}{v_2}\le2.51$ or $\norm{v_0'}{v_2}\le 2.51$.  Given that
$v_0'$ is the mirror image of $v_0$, these distances are equal;
hence $\norm{v_0}{v_2}\le 2.51$.  By Lemma~\tref{tarski:pass-anchor},
$\norm{v_0}{v_1},\norm{v_0}{v_3}\le 2.51$.
\swallowed\end{proved}
\end{tarski}
%<<<<<
...
By
Lemma~\tref{tarski:2t0-doesnt-pass-through}, we have $\norm{v_1}{v_3}>2.51$.
...
Let $\{v,v_1,\ldots,v_4\}$ be a packing of five points.  Assume
	$$ \norm{v_i}{v_j} \le \sqrt8, \text{ for } i\ne j.$$
...
We pivot points in the figure in a constraint preserving way until 
	$$
	\norm{v_i}{v_j} = \sqrt8,\text{ for } 1 \le i < j \le 4.
	$$
Re-indexing the vectors $v_i$ if necessary, we then pivot $v$ until
	$$
	\norm{v}{v_i} = 2, \text{ for } i=1,2,3.
	$$
By Lemma~\tref{tarski:cm5-small}, $x_{45}=\norm{v}{v_4}^2$ is the smaller root $r$ of the quadratic
polynomial
...
%
%We have $\norm{w}{v_i}\ge 2$ and $\norm{v_1}{v_2}<\sqrt8$.  Hence $\arc_V(w,v_1,v_3)<\pi/2$,
...
five distinct points.  Suppose that $\norm{v_i}{v_j}\le
...
Pivot $v_4$ in a constraint satisfying way to decrease $\norm{v_5}{v_4}$.
If $\norm{v_1}{v_4}<\sqrt8$, then Lemma~\tref{tarski:v-interior} % not skinny
gives the
result.
...
If $\norm{v_1}{v_2}\le \sqrt{8}$, the result follows from
Lemma~\tref{tarski:pass-makes-quarter}. 

Assume that a figure exists with $\norm{v_1}{v_2}>\sqrt{8}$. Suppose for a
contradiction that $\norm{v}{w}>2.51$.    Pivot $v_1$ around $\{v_0,v_2\}$ until
$\norm{v}{v_1}=2.51$ and $v_2$ around $\{v_0,v_1\}$ until $\norm{v}{v_2}=2.51$.  Rescale
$w$ so that $\norm{w}{v_0}=\sqrt{8}$. Set $x = \norm{v_1}{v_2}$. If, through constraint
preserving deformations, $w$ is not deformed into the plane of 
$\{v_0,v_2,v_1\}$,
then we are left with the one-dimensional family $\norm{w'}{v_0}=\norm{w'}{w}=2$, for
$w'=v_2,v_1$, $\norm{v}{w}=\norm{v}{v_0}=\norm{v_1}{v}=\norm{v_2}{v}=2.51$, depending on  $x$. This
...
contain the center of the circle, because $\norm{w}{v_0}$ is less than the
diameter.
%
Reflect $v$ through $P$ to get $v'$.  Then $\norm{v}{v'}< 2x_0$. Swapping
$v_1$ and $v_2$ as necessary, we may assume that 
  $$w\in\op{cone}(v_0,\{v,v',v_2\}).$$  
The desired bound $\norm{v}{w}\le 2.51$ now follows from
  $$
...
Assume that
  ...
  \norm{v_2}{v_3}&\le 2.51,& \norm{v_3}{v_4}&\le 2.51,  &\norm{v_2}{v_4}&\le 2.51,\\
  \norm{v_1}{v_2}\ge 2.51,\quad &\norm{v_1}{v_3}\ge 2.51,&\norm{v_1}{v_4}&\le 2.51\\
  \norm{v_1}{w}&\le \sqrt8,&\norm{v_1}{v_2}&\le \sqrt8,&\norm{v_1}{v_3}&\le\sqrt8.\\
  \end{array}
$$
...
Let $x_{ij} = \norm{v_i}{v_j}^2$, for $1\le i < j\le 3$.
...
such that
   $\norm{p'}{v_i} = r$, for $i=1,2,3$, provided by Lemma~\ref{tarski:rog-exist}.
...
Let $S=\{v_0,v_1,v_2,v_3,v_4\}$ be a packing of five points.
Assume that
  $$2\le \norm{v_1}{v}\le 2.51,\quad \text{ for } v = v_2,v_3,v_4.$$
Assume that
  $$2.51 < \norm{v_1}{v_0} < \sqrt8,\quad \norm{v_3}{v_0}\le 2.51,
   \quad \norm{v_2}{v_4}\le2.51.$$
Assume that $\norm{v_0}{v_2},\norm{v_0}{v_4}<\sqrt8$ and that
...
\begin{proved}
  We calculate
  $$
  \normo{v'}^2-\normo{v}^2 = (t_1+2t_2+2t_3)(a'^2-a^2) + t_2 (t_2+2t_3)(b'^2-b^2)
    +t_3^2 (c'^2-c^2)\ge0.
  $$
\swallowed\end{proved}
\end{tarski}
...
gives
  $$(p'-p)\cdot v_i=0.$$
Then $$\norm{p'}{p}^2 = \sum_it_i (p'-p)\cdot v_i = 0.$$  
Thus, $p'=p$.
\swallowed\end{proved}
\end{tarski}
%<<<<<
...
Let $S=\{v_1,\ldots,v_4\}$ be a set of four points
in $\ring{R}^3$.  
Let $x_{ij}=\norm{v_i}{v_j}^2$, for $1\le i< j\le 4$.
...
Assume
 $$
 2.51\le \norm{v_2}{v_4}\le \sqrt8,\quad \norm{v_2}{v_3}\le 2.51,\quad \norm{v_3}{v_4}\le 2.51.
 $$
Assume that the $v_1$ has negative orientation
along $S$. Then
   $$
   \norm{v_1}{v_i}\le 2.51, \quad i=2,3,4.
   $$
\end{lemma}

\begin{proved}  
Assume for a contradiciton that $\norm{v_1}{v_i}> 2.51$ for some $i$.
...
packing of four points.  Assume that 
  $$
  \norm{v_i}{v_j}\le 2.51,\quad 1\le i < j \le 3,
  %\quad \norm{v_0}{v_k},\quad 1\le k\le 3.
  $$
 If $v_0$
has negative orientation along
$S$, then
  $$
  2 \le \norm{v_0}{v_i } < 2.51, \text{ for } i=1,2,3.
  $$
\end{lemma}
...
Assume that 
$$
\norm{v_i}{v_j}<\sqrt8,\quad 1\le i < j \le 3.
$$
  Suppose that
...
packing of four points.  Assume that
  $$
   \norm{v_i}{v_j}\le 2.51, \quad\text{ for } i\ne j.
  $$
Let $S=\{v_1,v_2,v_3\}$.
Suppose that
...
Then
  $$
  2 \le \norm{v}{v_i } < 2.51, \text{ for } i=1,2,3.
  $$
\end{lemma}
...
packing of four points. Suppose
  ...
  \norm{v_1}{v_2}\le 2.51,\quad 
  \norm{v_2}{v_3}\le 2.51,\quad
  \norm{v_1}{v_3}\le \sqrt{8}.
  $$
Suppose that
$\op{conv}(S\setminus \{v\})$ meets $\Omega(v,S)$.
Then
  $$
  2 \le \norm{v}{v_i } \le 2.51, \text{ for } i=1,2,3.
  $$
\end{lemma}
...
Assume that
   $$
   \norm{v_0}{v_1} < \sqrt8,\quad \norm{v_0}{v_2} < \sqrt8,\quad \norm{v_1}{v_2} < \sqrt8.
   $$
...
Assume that 
  $$
  \norm{v_i}{v_j}< \sqrt{8}, \text{ for } 0\le i < j \le 2.
  $$
Assume that at most one of $\norm{v_i}{v_j}$ is greater than $2.51$.
Then
  $$\norm{w}{v_i}\le 2.51,\text{ for } i=0,1,2.$$
...
the orientation of $w$ in $S$ is negative.  By Lemma~\tref{tarski:vor-bar-quad}, $\norm{w}{v_i}\le 2.51$, for
$i=0,1,2.$.
...
%we have
%  $$\norm{w}{v_i}\le 2.51,\text{ for } i=1,2,3.$$
%
...
\begin{itemize}
  ...
  \item $F'=\{u,v,w\}$ with $\norm{u}{v} \le 2.51$, $\norm{v}{w}\le 2.51$,
        $\norm{u}{w}\le\sqrt8$. Also, $\norm{w_0}{u}>2.51$ for some $u\in F'$.
\end{itemize}
...
nonpositive, then 
   $$\norm{w_0}{u}\le 2.51,\quad\text{ for all } u\in F'$$
...
on $\op{conv}\{v_0,w,v\}$.
Since $\norm{w}{w'}> 2.51$, we see that the orientation of each face
...
to $S$.  Suppose that 
  $$
  2 \le \norm{v_i }{ v_j } < \sqrt8, \quad 0\le i < j \le 3.
  $$
...
Let $S\cup\{u,v,w\}$ be a finite packing.  Suppose that $\norm{u}{v}<\sqrt8$.
...
Also, $\norm{u}{v}<\sqrt8$ implies $(u+v)/2\in\Phi(S,v,u)$.
...
Since $\norm{v_2}{v_3}<\sqrt8$, the midpoint of $\{v_2,v_3\}$ is closer
...
$\{v_1,u_2\}$, are
less than $\sqrt8$. Suppose that $2.51 < \norm{v_1}{v_0}$.
Suppose that at most one of lengths $\norm{v_0}{u_1}$,
$\norm{v_0}{u_2}$, $\norm{u_1}{u_2}$ is greater than $2.51$.
%
Set $y=\norm{v_1}{v_0}$ and $b=\eta(2,2.51,y)$.
...
If $\norm{v_1}{u_1} > 2.51$, then we get the contradiction (by Lemma~\tref{tarski:eta-rad})
...
Suppose that 
   ...
   \begin{array}{rlrlrll}
   \norm{v_0}{u_1}&<\sqrt8,& \norm{v_0}{u_2}&<\sqrt8,& \norm{u_1}{u_2}&<\sqrt8,\\
   \norm{w}{v_0}&\le 2.51, &2.51&<\norm{v_0}{v_1}<\sqrt8.\\
   \norm{v_1}{w}&\le 2.51.
   \end{array}
   $$
Suppose that at most one of $\norm{v_0}{u_1},\norm{v_0}{u_2},\norm{u_1}{u_2}$ is
greater than $2.51$.
%
Set $y=\norm{v_1}{v_0}$ and $b=\eta(2,2.51,y)$.
...
Moreover, one of the following holds:
  ...
  $\norm{v_1}{u_1}\le 2.51$, and $\norm{v_1}{u_2}\le 2.51$.
  \item  We have $2.51<\norm{u_1}{u_2}<\sqrt8$, and 
...
If $2.51 < \norm{u_1}{u_2}$, then by hypothesis $\norm{v_0}{u_1},\norm{v_0}{u_2}\le 2.51$.  Also, by
Lemma~\tref{tarski:EE}, we have $\norm{v_1}{u_1},\norm{v_1}{u_2}\le 2.51$.  This falls into the second case
of the conclusion.  If $\norm{u_1}{u_2}\le 2.51$, then by Lemma~\tref{tarski:EE} % 2^3 2.51^3 2^3
we have $\norm{v_0}{u_1},\norm{v_0}{u_2}\le 2.51$.  Also, $\norm{v_1}{u_1},\norm{v_1}{u_2}\le 2.51$.  Again,
...
which implies (by Lemma~\tref{tarski:EE}) that $\norm{u_1}{u_2}>2.51$.  Thus, we are in the third case of the conclusion.



 
%By Lemma~\ref{tarski:consec-anchors}, we have $2.51<\norm{u_1}{u_2}$.
%The hypotheses of the lemma give $\norm{v_0}{u_1},\norm{v_0}{u_2}\le 2.51$.
% Moreover, the
%estimates $\norm{v_1}{u_1}\le 2.51$ and $\norm{v_1}{u_2}\le 2.51$ hold
...
condition holds.
If $\op{rad}_V\{v_0,v_1,w,u_1\} < \eta(\norm{v}{v_0},2,2.51)$ and $u_1\in \op{aff}_+(\{v_0,v_1,w\},p)$, then
%that if $\rad_V\{v_0,v_1,w,u_1\} < \eta(\norm{v}{v_0},2,2.51)$, 
%and if $R\subset \op{aff}_+(\{v_0,v_1\},\{w,u_1\})$,
%then there exists
%$w'\in\op{aff}_+^0(\{v_0,v\},\{w,u_1\})$ such that 
  \begin{enumerate}
  \item $w'\in\op{aff}_+^0(\{v_0,v_1\},\{w,u_1\})$ 
  \item $\rad_V\{v_0,v_1,w,w'\} \ge \eta(\norm{v_0}{v_1},2,2.51)$, 
  %\item $2\le \norm{u'}{w'}$, for $u'\in\{v_0,v_1,w,u_1,u_2\}$, and
  \item $\norm{w'}{v_1}\le 2.51$, $\norm{w'}{v_0}\le 2.51$, $\norm{w'}{w}\ge 2.77$.
  \end{enumerate}
\end{definition}
...
Then one of the following holds:
  ...
  \item $\norm{v}{u}\le 2.51$, for $v=v_0,v_1$, and $u=u_1,u_2$.  Also,
...
This implies that $\norm{u_1}{v_1},\norm{u_1}{w_1},\norm{u_1}{v_0}\le 2.51$ (by Lemma~\ref{tarski:neg-orient-quad}).  
In particular,
...
From $\norm{w'}{w}\ge 2.77$ and $\rad_V\{v_0,v_1,w,w'\} \ge b$,
...
The second disjunct is not possible by Lemma~\tref{tarski:EE} (since $\norm{v_1}{w'}\le 2.51$).
...
% $\norm{u}{w'}\le 2.51$.  In particular $S'=\{v_0,v,w',u\}$ is
...
Assume that $2.51<\norm{v_0}{v}<\sqrt8$, $2.51<\norm{w}{w'}$, and
  $$
  \norm{v'}{w''} \le 2.51, \text{ for } v'=v_0,v \text{ and } w''=w,w',u.
  $$
Assume $\rad_V\{v_0,v,w,w'\}\ge b$, where $b=\eta(2,2.51,\norm{v}{v_0})$
...
Let $x'$ be the point associated with $x$ in the definition of $\Phi$.  It satisfies
 $\norm{x'}{u}=\norm{x'}{v_0}$.  By the definition of $\Phi$,
$\norm{x'}{u} \le \norm{x'}{w}$.  The orientation of $w'$ in $S'$ is positive by Lemma~\tref{tarski:neg-orient-quad}.
Since $\rad_V(S')\ge\eta(2,2.51,\norm{v}{v_0})$, we have
$\norm{x'}{u}=\norm{x'}{v_0} \le \norm{x}{v},\norm{x}{w'}$.  Thus, $x'$ is at least as close to $u$
...
$\norm{u}{w'}\le 2.51$ by Lemma~\tref{tarski:cm5-small}.
...
Let $S=\{v_0,v_1,v_1',w_1,w_2\}$ be a packing of five points.
  ...
  2.51 \le \norm{v_0}{v_1} < \sqrt8,\\
  2.51\le \norm{v_0}{v_1'} < \sqrt8,\\
  \norm{v}{w}\le 2.51, \text{ for } v=v_0,v_1, w=w_1,w_2.\\
  \end{array}
  $$
Assume $\{v_0,v_1,w_1,w_2\}$ is not coplanar.  
%Assume that
%$$\op{rad}_V\{v_0,v_1,w_1,w_2\} > \eta(\norm{v_0}{v_1},2,2.51).$$
Let $$
   \begin{array}{lll}
   b' &= \norm{v_0}{v'_1}/(2\eta(\norm{v_0}{v'_1},2,2.51))\\
   b &= \norm{v_0}{v_1}/(2\eta(\norm{v_0}{v_1},2,2.51)).
...
By Lemma~\tref{tarski:eta-rad},  we have $\eta(\norm{v_0}{v_1},2,2.51) \le \rad_V S'$.  Hence $\op{rog}^0\subset \Phi(\{v_1,w_1,v_1'\},v_0,w_1)$.
...
Let $S=\{v_0,v_1,v_2,w_1,w_2\}$ be a packing of five points.
  Assume that
  $$\begin{array}{lll}
  2.51 \le \norm{v_0}{v_i} < \sqrt8,\quad i=1,2\\
  \norm{v_i}{w_i}\le 2.51, \quad i=1,2.\\
  \norm{v_0}{w_i}\le 2.51,\quad i=1,2.
  \end{array}
  $$
...
%  \begin{itemize}
%  \item $\norm{w_1}{w_2}>2.51$,
%  \item $\rad_V\{v_0,v_i,w_1,w_2\} < \eta(2,2.51,\norm{v_0}{v_i})$, and
...
%   \item $\norm{u_i }{ v_0} \le 2.51$, 
%   \item $\norm{u_i }{ v_i} \le 2.51$, and
%   \item $\norm{u_i }{ w} \le 2.51$, for some $w\in\{w_1,w_2\}$.
%  \end{itemize}
Assume that $\op{rog}^0(v_0,w_1,v_1,p_1,b_1)$ meets
  $\op{rog}^0(v_0,w_2,v_2,p_2,b_2)$.
Then there exists a matching $\{i,j\}=\{1,2\}$ 
such that $\norm{v_i}{w_j}\le 2.51$ and one of the following holds:
\begin{itemize}
     \item $\norm{w_1}{w_2}\le 2.51$ and $\op{rog}^0(v_0,w_j,v_j,p_j,b_j)$ meets
           $\op{aff}_+(v_0,\{v_i,w_i\})$.
       \item $\norm{w_1}{w_2}\le 2.51$ and 
     $$
     \op{aff}_+(\{v_0,v_i,w_i\},w_j) = \op{aff}_+(\{v_0,v_i,w_i\},p_i);
     $$
              \item $\norm{w_1}{w_2}> 2.51$, $\Phi(\{v_0,v_1,w_1,w_2\},v_0,w_j)$ meets
                $\op{rog}^0(v_0,w_i,\ldots)$, 
             $\rad_V\{v_0,v_i,w_1,w_2\}<\eta(2,2.51,\norm{v_0}{v_i})$, and
    $$
     \op{aff}_+(\{v_0,v_i,w_i\},w_j) = \op{aff}_+(\{v_0,v_i,w_i\},p_i).
     $$
\end{itemize}
...
If $\norm{w_1}{w_2}> 2.51$, then the orientation of $w_2$ in $S$ is positive (Lemma~\tref{tarski:neg-orient-quad}).
We have $\rad_V(S) < \eta(2,2.51,\norm{v_1}{v_0})$, for otherwise
...
five points.  Suppose that the pairwise
distances are at most $2.51$ for distinct $u,v\in S$, with
$\{u,v\}\ne \{v_1,v_3\}, \{v_2,v_4\}$.  Suppose that $\norm{v_0}{v_1}\le 2.3$.
Suppose that $v_1\in\op{cone}(v_0,\{v_2,v_3,v_4\})$.
Then $\norm{v_1}{v_3}< 2.51$.  
\end{lemma}
...
Suppose that $\norm{v_0}{v_i}\le 2.51$, for $i=1,2,3,4$.
Suppose $\op{quadp}(v_0,(v_1,v_2,v_3,v_4))$.
Suppose that
  $$\norm{v_1}{v_3}<\sqrt8,\quad \norm{v_2}{v_4} <\sqrt8.$$
Then $\norm{v_i}{v_j}\le 2.51$, for $(i,j)=(1,2),(2,3),(3,4),(4,1)$.
\end{lemma}

%\begin{lem}\guid{XMSECYQ}
%Suppose that there exist four nonzero vertices $v_1,\ldots,v_4$ of
%height at most $2.51$ (that is, $\norm{v_i}{v_0}\le 2.51$) forming a skew
...
The calculation
   ...
   $$
gives $\norm{v_1}{v_2}\le 2.51$. The other inequalities are similar.
\swallowed\end{proved}
\end{tarski}
%<<<<<
...
Suppose that
  $$\norm{v_1}{v_3}<\sqrt8,\quad \norm{v_2}{v_4} <\sqrt8.$$
Assume $w\in\op{quadc}(v_0,(v_1,v_2,v_3,v_4))$ and that $\norm{w}{v_0}<\sqrt8$.
Then $\norm{w}{v_i}\le 2.51$, for $i=1,2,3,4$.
\end{lemma}


\begin{proved}
If, say, $w\in \op{cone}(v_0,\{v_1,v_2,v_3\})$, then
$\norm{w}{v_1}$, $\norm{w}{v_3}\le 2.51$ (Lemma~\tref{tarski:pass-anchor}).
 ...
 \item $\op{quadp}(v',(v_1,v_2,v_3,v_4))$.
  ...
  \item For i=1,\ldots,4 \ (indices \hbox{mod } 4),
   $$\begin{array}{lll}
    ...
    2\le&\norm{v_i}{v_{i+2}},\\
    h_i\le &\norm{v}{v_i}, \\
    2\le &\norm{v}{v'}\le \ell, \hbox{ for every } v\in S 
    \end{array}
    $$
\end{itemize}
...
together with the additional constraints
    ...
    \begin{array}{lll}
    &\norm{w_i}{w_{i+1}}=k\\
    &\norm{w'}{w_i}=2, \hbox { for } i=1,\ldots,4,\\
    &\norm{w'}{w}=\ell \hbox { for } w\in S'.
    \end{array}
    $$
\end{lemma}
...
Suppose that $\norm{u}{v}\le 2.51$ for
  ...
  (v_1,v_2), (v_2,v_3), (v_3,v_4), (v_4,v_1).$$
Suppose that
$\norm{v_5}{v_i}\ge 2.51$ for $i\in\{2,3,4\}$.  Then
$\norm{v_0}{v_5} > 2.51$.
\end{lemma}
...
We apply Lemma~\tref{tarski:gc} to assume
    $$\norm{v_i}{v_{i+1}}=2.51,\quad \norm{v_i}{v_0}=2, \quad \norm{v}{v_0}=2.51,$$
for $i=1,\ldots,4$. Reindexing and perturbing $v_5$ as necessary, we
may assume that $2\le \norm{v_1}{v_5}\le2.51$ and $\norm{v_i}{v_5}\ge2.51$, for
...
Suppose that $\norm{u}{v}\le 2.51$, for all $u,v\in S$, 
except
...
%proclaim{Lemma 1.8}
Let $\{v_0,v_1,v_2,v_3,v_4,v_5\}$ be a packing of six points.
Assume that $\norm{v_0}{v_5}<\sqrt8$ and that
  $$\norm{u}{v} \le 2.51,\quad u=v_0,v_5,\quad v = v_1,v_2$$
...
\begin{proved}
Suppose the figure exists with $\norm{v_3}{v_4}\le\sqr8$. Label vertices
...
$v_4$ around the axis $\{v_1,v_2\}$ until
    $\norm{v_5}{v_0}=\sqr8$.  Lemma~\tref{tarski:cross} shows that the incidence constraints are preserved.
...
Assume that  $\norm{v_1}{v_2}\le\sqrt8$,
$\norm{v_1}{v_3}\le 2.51$, and $\norm{v_2}{v_3}\le 2.51$.
Assume that $w,w'\in\op{cone}(v_0,\{v_1,v_2,v_3\})$.
Assume that $\norm{v_0}{w}\le\sqrt8$.  Then $\norm{v_0}{w'} > \sqrt8$.
\end{lemma}
...
%For a contradiction, assume $\norm{v_0}{w'}\le\sqrt8$.  By
%Lemma~\tref{tarski:qrtet-pair-pass}, we have $\norm{v_1}{v_2}\ge 2.51$.
%By Lemma~\tref{--},\FIWW{$\CalE$}
%we have $\norm{v_i}{u}\le 2.51$, for
...
\begin{itemize}
  \item $\norm{v_i}{v_j}<\sqrt8$, for
$0\le i< j \le 2$.  
   \item If $2.51 < \norm{v_i}{v_j}$ and $2.51 < \norm{v_j}{v_k}$ ,
...
$\op{conv}\{w,w'\}$ meets $\op{conv}(S)$.  Suppose $\norm{w}{w'}<\sqrt8$.  Then
...
$\norm{w}{w'} < \sqrt8$.  Suppose that each triple $\{v_i,v_j,v_k\}$
...
\item $\norm{u}{v}>2.51$ if $W_i = \{u,v\}$;
\item $\norm{u}{v}\le 2.51$ if $u\in S$, $v\in W_2$.
\end{itemize}
%Then $2.51 \le \norm{w}{w'}$ and
%  $$
%  \norm{u}{v_i}\le 2.51,\quad u=w,w',\quad i=0,1,2,3.
%  $$
%Furthermore, there is a reindexing $\{w_0,w_1,w_2,w_3\} =\{v_0,v_1,v_2,v_3\}$
%such that
%  $$
%  \norm{w_i}{w_j}\le 2.51,\quad (i,j)=(1,2),(2,3),(3,4),(4,1),
%  $$
%and $2.51 < \norm{w_1}{w_3}, \norm{w_2}{w_4}$.
\end{lemma}
...
edge opposite the shared edge.  We can decrease $\norm{w}{w'}$, contracting
...
until $\norm{v_0}{v_1}=\sqrt8$ and $\norm{v_1}{v_3}=2.51$.
Similarly, we move $v_2$  until $\norm{v_0}{v_2}=\sqrt8$ and $\norm{v_3}{v_2}=2.51$.

We arrive at a rigid figure that satisfies all the constraints.
Pick coordinates and compute $\norm{w}{w'} > \sqrt8$, a contradiction.
\swallowed\end{proved}
\end{tarski}
%<<<<<
...
to both of these faces, we get that 
   ...
   \norm{u}{v_i}\le 2.51,\quad u=w,w',\quad i=0,1,2,3.
   $$
We have $\norm{w}{w'}\ge 2.51$, by Lemma~\tref{tarski:EE}.
%
...
%
%By Lemma~\tref{tarski:skew-quad}, we have that $\norm{v_2}{v_3}\le 2.51$.
...
we have 
   $$
   \begin{array}{lll}
   \norm{v'_0}{v'_1}\le 2.51,\quad \norm{v'_1}{v'_2}\le 2.51,\quad 2.51 < \norm{v'_0}{v'_2}<\sqrt8,\\
   \norm{w'_0}{w'_1}\le 2.51,\quad \norm{w'_1}{w'_2}\le 2.51,\quad 2.51 < \norm{w'_0}{w'_2}<\sqrt8.
   \end{array}
   $$
...
By Lemma~\tref{tarski:EE}, we have $\norm{v_0}{v_1}>2.51$.  
...
In the first case, we get that $\norm{w_1}{w_2}>2.51$, $\norm{w_0}{w_1},\norm{w_0}{w_2}\le 2.51$ (by Lemma~\ref{tarski:EE}).
...
determined pair $\{w_i,w_j\}$ such that $\norm{w_i}{w_j}>2.51$.
...
with $\norm{v_a}{v_b}>2.51$ and $\norm{v_c}{v_d}>2.51$. 
(or $\norm{v_2}{v_4},\norm{v_1}{v_3}>2.51$, after reindexing). The conditions of
...
and $\norm{w_1}{w_3}>2.51$.
\item B:  $\op{conv}\{v_3,v_4\}$ meets $C_W$ and $\norm{v_3}{v_4}>2.51$.
\end{itemize}
\end{lemma}
...
$\norm{w_1}{w_3}>2.51$ follows from Lemma~\tref{tarski:EE}.
...
we are in situation B.  The estimate $\norm{v_3}{v_4}>2.51$ follows
...
$B$.  If $\norm{v_1}{v_3}, \norm{v_2}{v_1} > 2.51$, then $B'$ follows.
If $\norm{v_1}{v_2}, \norm{v_3}{v_4} > 2.51$, then $C'$ follows.
...
up to reindexing.
If $A$, then $\op{conv}\{w_1,w_3\}$ meets $\{v_2,v_3,v_4\}$ and $\norm{w_1}{w_3}>2.51$.  In particular, $\op{conv}\{w_1,w_3,w_4\}$ meets $\op{conv}\{v_1,v_2,v_3,v_4\}$.  This was treated in case (2).
...
between points in $S_V\cup S_W$ are at least $2$.  Assume that
  $$
  \norm{v_i}{v_j}\le 2.51,\quad \norm{w_i }{w_j}\le 2.51,\quad 1\le i<j\le 4.
  $$
...
between points in $S_V\cup S_W$ are at least $2$.  Assume that
  $$
  \norm{v_i}{v_j}\le 2.51\quad 1\le i<j\le 4.
  $$
...
Moreover, we have
  $$
  \begin{array}{lll}
  &\norm{w_1}{w_4}>2.51,\quad \\
  &\norm{w_j}{v_i}\le 2.2,\quad i=2,3,4;\ j=1,4\\
...
$\norm{v_i}{v_j}>2.51$, which is contrary to hypothesis.  So $n\le 5$.
...
That lemma gives that $\norm{w_1}{w_4}>2.51$ and that
...
If, furthermore, 
\begin{itemize}
\item $2\le \norm{u}{v_i}\le 2.51$, for $u=w,w'$ and $i=0,1,2,3$,
\item $2\le \norm{v_i}{v_{i+1}}\le 2.51$, for $i=0,1,2,3$ (with $v_4=v_0$), and
\item $2.51<\norm{w}{w'}<\sqrt8$, 
\end{itemize}
then we say that 
...
diagonal. Assume that $\norm{v_1}{v_3}<\sqrt8$.\indy{Index}{conflicting
diagonals}
...
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that the set is not coplanar.
Let $\eta_V(v_0,v_1,v_2)\le b\le c$.  Let $y=\norm{v_1}{v_0}$.
Then
$$
...
from $v_0,v_1,v_2$.  (The unique existence of this point is given
by Lemma~\ref{tarski:rog-exist}.)   Let $y = \norm{v_1}{v_0}$.
$$
\begin{array}{lll}
...
consider the figure as a function of $y_4=\norm{w_1}{w_2}$. When $y_4$ is
...
Let  
$\mu = 3.2 - 1.255 = 1.945$.  Let $y = \norm{v_1}{v_0}$ and $y'=\norm{v_1'}{v_0}$.
Set $\psi = \arc(y,1.255,\mu)$.
Assume that $\norm{v_1}{v'_1}\ge 3.2$ and that $y,y'\le 2.51$.
Then 
   $$
...
%    \oldlabel{4.12.1}
Let $\{v_0,v_1,v_2,v_3\}$ be a packing of four points.
Assume that $\norm{v_1}{v_2}\le 3.2$.  Let $w_1,w_2$ be points (not necessarily
...
%%    \oldlabel{4.12.1}
%Let $v$ be a concave vertex with $\norm{v}{\orgn}\ge2.2$. The truncated
...
to the lower bound on $\norm{v_3}{w_i}$ and keep it at $3.2$ in
...
$$\op{rcone}(v_0,v,\cosarc(\norm{v}{v_0},t,\lambda)) \text{ meets }
\op{rcone}^0(v_0,w,\cosarc(\norm{v}{v_0},t,\lambda')),$$ then
$\norm{v}{w} < \lambda+\lambda'$.
\end{lemma}

\begin{proved} Pull $v$ away from $w$ keeping $\normo{v}$ fixed
