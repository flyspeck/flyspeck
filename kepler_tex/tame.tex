...
1/(2\arctan(5,\sqrt{2})$.)
%
 \indy{Index}{target}\indy{Index}{tgt@$\op{tgt}=14.8$}
...
\begin{lemma}\guid{JGTDEBU} The hypermap $H$ satisfies Conditions~\ref{XX}-\ref{XX} 
of tameness.
...
\begin{lemma}\guid{RZMJAKJ} \label{lemma:0.55:bis} %proclaim{Lemma 5.3}
...
\begin{lemma}\guid{CDTETAT} \label{lemma:0.8638}
...
\end{lemma}
 %
 \indy{Index}{ZZZZ1.153@$1.153$}
 \indy{Index}{ZZZZ0.8638@$0.8638$}

\begin{lemma}\guid{HWWSYOM} \label{lemma:excess-1:bis}
Let $(H,\azim,\flat,\sigma)$ be a formally contravening hypermap.
...
\begin{lemma}\guid{IBNPQMU} Let $(H,\azim,\flat,\sigma)$ be a formally contravening
...
\begin{lemma}\guid{MQGCDGZ}  Let $(H,\azim,\flat,\sigma)$ be a formally contravening
...
\begin{lemma}\guid{MWNYTZP} Let $V$ be any set of nodes of type $(5,0,0)$ in a
...
%
%\begin{lemma}\guid{YLHITTK}  Under these conditions, for every $v\in V$,
%$p_v>1$.
%\end{lemma}
%
%\begin{proof}
...
%
%
%\begin{lemma}\guid{NGXNFWS} Under these same conditions, for every $v\in V$,
%$p_v>2$.
%\end{lemma}
%
%\begin{proof}
...
\begin{lemma}\guid{IVCPPEN} Let $V$ be a separated set of nodes.  For every
...
\begin{lemma}\guid{NQVCEWW}  Property \ref{definition:admissible:excess}  of
...
\begin{lemma}\guid{TCYONLW} Let $(\Lambda,v_0)$ be a contravening centered packing.  There
...
\indy{Index}{aggregate hypermap}
\end{definition}

\begin{lemma}\guid{QKRXKKQ}\tlabel{lemma:agg-hyper}
...
\begin{lemma}\guid{PHRBWRW} The bound $tri(v)>2$ holds if $v$ is a node
...
node of a flat quarter.  In fact, by Tarski arithmetic\tarfE{node},
...
%% Table simplified...  The entry \pqr(7,0,0) with 14.76 is relevant here.
...
%
 \indy{Index}{type (of a node)}
 \indy{Greek}{ZZtauLP@$\tlp{\pqr{(p,q,0)}}$}
...
$2\sqrt2$ by Tarski arithmetic\tarfE{2t0-doesnt-pass-through}.  If an
