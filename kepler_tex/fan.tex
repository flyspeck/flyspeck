...
\begin{lemma}\guid{TXFBALB}\rating{XX} The area formula holds.
\end{lemma}
...
\begin{lemma}\guid{GGZWYRM}\rating{XX}  For any dart $x$ on the face $F$, and dart $y\in F$ that is not adjacent to $x$ under the face map, 
$$
...
\begin{lemma}\guid{HYUAZSE}\rating{XX} The given intersection of open half-spaces is equal
...
\begin{lemma}\guid{HZIYFIZ}\rating{XX} Let $(V,E,F)$ be a lunar cylic fan with poles $v_0,v_r$.  In the particular deformation as described, the triple $(V(t),E(t),F(t))$ is a lunar cyclic fan for all $0\le t< 1$.
\end{lemma}
...
\begin{lemma}\guid{BSXAQBQ}\rating{XX} Let $C$ be a bounded polyhedron with $0$ as an interior point.  Let $\op{hyper}(V_C,E_C)$ be the associated hypermap.  For every dart $x$ in the hypermap, $\op{azim}(x) < \pi$.
\end{lemma}
\indy{Index}{polyhedron}
