...
Sum the inequality~(\ref{eqn:mfe}) over all cells in a large ball $B(p,r)$ to get an
...
Let $E_c(X)\subset E(X)$ be the set of critical edges of $X$.  If $X$ is any cell such that $E_c(X)$ is nonempty, let the weight $\omega(X)$ of $X$  
be $1/\op{card}(E_c(X))$.
\end{definition}
\indy{Index}{critical!edge}
\indy{Notation}{E@$E_c$ (critical edges)}
...
\indy{Notation}{Zcamma@$\gamma$!$\gamma_L$ (supercell)}
$$
\begin{array}{lll}
...
&=\sum_{X\subset B(p,r)}\gamma_L(X)\sum_{e \in E_c(X)}\omega(X) + O(r^2)\\
&=\sum_{e\subset B(p,r)}\sum_{X\mid e \in E_c(X)}\gamma_L(X)\omega(X) + O(r^2)\\
