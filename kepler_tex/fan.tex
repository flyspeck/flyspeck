...
the intersection of $\Wdart(x)$ with $H^0(0)$.  The sets $C^0(t)$ belong to
...
\item $f(v,w) = (w,\rho w)$ for all $(v,w)\in D$.
\end{itemize}
\end{lemma}
...
$$
P=P(V,E,F) = \sum_{i=0}^{k-1} \arc_V(0,\{\rho^i v,\rho^{i+1} v\}), 
$$
where  $k=\op{card}(F)$.
...
that $\phi^{-1} v \in A$.  In fact, by the definition of fan, 
$$\phi^{-1}v \in \op{aff}^0_+(\{0,v\},u) \cup \op{aff}^0_+(\{0,v\},w).$$
Suppose that $\phi^{-1} v$ lies in the first term of the union.  If $u\ne \phi^{-1}v$, then the assumptions of the lemma
are met for $\{u,\phi^{-1} v,v\}$, giving the conclusion $\alpha(\phi^{-1} v)=\pi$.  Repeating, we obtain a chain
$$
\pi=\alpha(v) = \alpha(\phi^{-1} v) = \cdots,
$$
with $v,\phi^{-1}v,\ldots\in A$.  Another chain can be constructed in the other direction $v,\phi v,\ldots$.
...
\begin{proof}  Let $v$ be any element of  $V$ and pick $u,\phi u\in V$ such that $C^0(\{u,\phi u\})$ meets $\op{aff}^0_-(0,v)$.  Apply
Lemma~\ref{lemma:A} to $\{u,v,\phi u\}$ to conclude that $\alpha(v)=\pi$, and that there is a plane $A$ containing
$\{0,v,u,\phi u, \phi v,\phi^{-1} v\}$.    Iterate along the chain to find that $V\subset A$.

The arcs making up the perimeter all lie in a common plane.  The permutation $\phi$ may be identified with the azimuth cycle around a line orthogonal to the plane.  The azimuth angles sum to $2\pi$ by Lemma~\ref{lemma:2pi-sum}.
\end{proof}

\begin{lemma}\guid{HKIRPEP}\rating{ZZ}
Let $(V,E,F)$ be a lunar fan with dependent vectors $v,w\in V$.  Let $k=\op{card}(F)$.  Write $w=\phi^r v$, with $0<r$ minimal.
Then
\begin{itemize}
\item $\alpha(u) = \pi$, for all $u\in V\setminus \{v,w\}$.
\item $0< \alpha(v) = \alpha(w)\le pi$.
\item $V\cap \op{aff}_+(\{0,v\},\phi v) = \{v,\phi v,\ldots, w\}$.
\item $V \cap \op{aff}_+(\{0,v\},phi^{-1} v) = \{v,\phi^{-1} v,\ldots w\}$.
\item $P(V,E,F) = 2\pi$.
\end{itemize}
\end{lemma}
...
that $\{u,v,w,\phi u,\phi^{-1} u\}$ belongs to a plane $A$.  Repeatedly apply 
Lemma~\ref{lemma:A}, as described above, to find that the plane (in fact half-plane)
is the same for $v,\phi v,\ldots,w$.  Similarly, all the vectors $w,\phi w,\ldots,v$ belong to a second half-plane.   

The angles $\alpha(v)$ and $\alpha(w)$ are both equal to the dihedral angle between these two half-planes.  In particular, $\alpha(v)=\alpha(w)\le\pi$.
...
\begin{lemma}[monotonicity]\guid{EGHNAVX}\rating{ZZ}  Let $(V,E,F)$ be a generic cyclic fan with hypermap $(D,e,n,f)$.  Let $k=\card(F)$; $x=(v_0,v_1)\in F$;  $v_r = \phi^r v_0$; $\beta(r) = \op{azim}(0,v_0,v_1,v_r)$;  $x_r = f^r x = (v_r,v_{r+1})\in F\subset D$, for all $r$.  
...
%\end{itemize}
%
Then for every $x=(v_0,v_1)\in F$, the following conclusions hold.
\begin{itemize}
\item $0=\beta(1)\le \beta(2)\le \cdots\le \beta(k-1)\le\pi$.
\item If for some $j\ge 2$, equality holds: $\beta(j)=\beta(j-1)$, then 
\begin{itemize}
  \item {\it either} $v_{j-1}\in C^0\{v_0,v_j\}$ and $\op{azim}(x_i)=\pi$, for $i=0,\ldots,j-1$;
  \item {\it or} $v_j\in C^0\{v_{j-1},v_0\}$ and $\op{azim}(x_i)=\pi$, for $i=j,\ldots,k-1$.
\end{itemize}
\end{itemize}
\end{lemma}
...
%
For the first claim one may assume by induction that $0\le \beta(1)\le\cdots\le \beta(i)$.  Pick spherical coordinates for which $\phi_0=0$, $\theta_1=0$, $\theta_i=\beta(i)$, for all $i$.  Write the condition
$$
v_0\in \bWdart(x_i)
$$
as a determinant:
...
$$
\det(v_0,v_i,v_{i+1})= r_0r_ir_{i+1}\sin\phi_i\sin\phi_{i+1}\sin(\theta_{i+1}-\theta_i)\ge0.
$$
...
Assume $\beta(j)=\beta(j-1)$.  This implies that $\{0,v_0,v_{j-1},v_j\}$ are coplanar.  By genericity, any pair of vectors in $V$ is independent.  This gives
$$
\op{aff}^0_+(\{0,v_0\},v_{j-1}) = \op{aff}^0_+(\{0,v_0\},v_{j}).
$$
By elementary real arithmetic,
$$
v_{j-1}\in C^0\{v_0,v_j\},\quad\text{or}\quad v_{j-1}\in C^0\{v_0,v_j\}.
$$
Consider the case $v_{j-1}\in C^0\{v_0,v_j\}$.    In this case, $\beta(i)=\theta_i=0$, for $i=0,\ldots,j$.  The points $\{0,v_0,\ldots,v_j\}$ are then coplanar and $\op{azim}(x_i)=0$ for $i=0,\ldots,j-1$.  This gives the second conclusion. The other case $v_j\in C\{v_0,v_{j-1}\}$ follows by similar arguments.
\end{proof}
...
%Pick a dart $x=(v_0,v_1)\in V$.  Set $v_i = \phi^i v_0$.  Assume that there are four darts $(y_1,y_2,y_3,y_4)$, $y_j = x_{i(j)}$, with
...
$$
0 < \op{azim}(0,v,\phi v,p) < \op{azim}(0,v,\phi v,\phi^{-1} v).
$$  
This implies $p\in \Wdart(x)$ as desired.  The case $v(x)=w$ is similar.

Now assume that $u=v(x)\ne v,w$.  We have the following facts (by Lemma~\ref{lemma:A}):
$$
\begin{array}{rll}
v,w\in \bar W &= \{p \mid 0 \le \op{azim}(0,u,v,p) \le \op{azim}(0,u,v,w)\}.\\
C^0 &\subset \bar W\\
...
%$$
Also, by Lemma~\ref{lemma:A},
$$
\op{aff}\{0,u,v\}\cap C^0 \subset \op{aff}\{0,u,v\}\cap \op{aff}\{0,v,w\} = \op{aff}\{0,v\}.
$$
However, the right-hand side is disjoint from $C^0$.  Thus, $C^0$ is disjoint from $\op{aff}\{0,u,v\}$ and is similarly disjoint from $\op{aff}\{0,u,w\}$.  The result follows.
\end{proof}
...
\begin{lemma}[slicing a cyclic fan]\guid{EJRCFJD}\rating{ZZ}  Let $(V,E,F)$ be a cyclic fan with hypermap $H$.  Let $v,w\in V$ be independent.  Assume that $C^0\{v,w\}\subset \Wdart(x)$ for all darts $x$ in the fan of $(V,E,F)$. Then
\begin{itemize}
...
In the definition of a deformation, it is not required that $(V(t),E(t),F(t))$ is a cyclic fan for all $t\in I$, although this this will usually be the case.  Let $H=(D,e,n,f)$ be the hypermap of $(V(0),E(0),F(0))$.  Extend $\rho:V\to V$ to $\rho(t):V(t)\to V(t)$; extend $x=(v,w)\in D$ to $x(t)=(v(t),w(t))$; extend $\op{azim}(x)$, for $x\in D$,  to $\op{azim}(x(t))$; extend $\bWdart(x)$ to $\bWdart(x(t))$, and so forth. 
...
A cyclic fan $(V,E,F)$ is determined by $V$ and the permutation $\rho:V\to V$:  $E=\{\{v,\rho v\}\mid v\in V\}$ and $F = \{(v,\rho v)\mid v\in V\}$.
