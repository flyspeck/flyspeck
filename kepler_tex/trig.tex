...
\begin{proved}
   ...
   On this interval, $\sin$
    is non-\hyperref[lemma:sin-pi2]{negative}.  By \hyperref[back:analysis]{real analysis}, it is enough to check that the squares of the two non-negative numbers are equal.  It then an arithmetic consequence of the circle identity
...
expressed in terms of the constants $x_{ij}$:
...
Under the domain restrictions of the definition 
$$
2 a b = (\mp a+b+c)(a \mp b \pm c) \pm (a^2 + b^2 - c^2) \ge \pm  (a^2 + b^2 - c^2)
$$
so that the argument of $\arccos$ always falls within its domain.
...
Let $V=\{\p_1,\ldots,\p_k\}$ be a finite set of
...
$\p\in V$ to the next larger element with respect to this order,
...
checked that
$$
   \sigma'(T \p) = T (\sigma \p),\quad \text{ for } \p\in V. 
$$
...
and all $0\le i \le j < n$,
   ...
   \theta(\p,\sigma^j(\p)).
   $$
Moreover, if there exist $\p,\q\in V$ such that $\theta(\p)\ne\theta(\q)$,
  $$
  \sum_{i=0}^{n-1} \theta(\sigma^{i}\p,\sigma^{i+1} \p) = 2\pi.
  $$
(If $\theta(\p)=\theta(\q)$ for all $\p,\q\in V$, then all the
summands are zero.)
\end{lemma}
\indy{Notation}{ZZsigma@$\sigma$ (permutation)}%
