...
A computation of $\sin^2\alpha$ and the remaining terms in the same way gives
   ...
   \end{array}
   $$
The result follows by \hyperref[back:real]{real} arithmetic.
\end{proof}
...
the \newterm{orthonormal frame} for $(\v,\w,\u)$.
\end{definition}
\indy{Index}{frame}%
...
The final two lemmas of this subsection are applications of cylindrical coordinates.

\begin{lemma}\guid{JBDNJJB}\rating{ZZ}\label{lemma:sim}
Write $x\sim y$ when there exists $t>0$ such that $x= t y$. 
Then 
$$\sin(\op{azim}(\orz,\u,\v,\w))\sim (\u \times \v)\cdot \w.$$
\end{lemma}
...
\end{array}
$$
Write $\w= r\cos\theta\, \e_1 + r\sin\theta \, \e_2 + h\, \e_3$ in cylindrical coordinates, where $\theta = \op{azim}(\orz,\u,\v,\w)$.
Then
$$
\begin{array}{rll}
...
\begin{proof}  For simplicity,  take our base point $\v=\orz$.
Let $\v_i' = (\w\cdot \w) \v_i - (\w\cdot \v_i) \w$.  
From the assumptions, $\v_1'\ne 0$.  Set $\e_1 = \v_1'/\normo{\v'_1}$.  Choose a unit vector
$\e_2$ so that $(\e_1\times \e_2)\cdot\w>0$ and $\e_1\cdot \e_2 = \w\cdot \e_2=0$.
...
Call $\theta$ the \newterm{azimuth} angle and 
$\phi$ the \newterm{zenith} angle of $(x,y,z)$.  The azimuth angle is also 
known as the \newterm{longitude}.  The zenith angle is also known as the \newterm{latitude}. 
The azimuth angle is a polar coordinate of $(x,y)$:
    $$
...
Write
   ...
   $$
for some integers $k_{pq}$ that satisf $0\le \theta(\v,\w) < 2\pi$.
Then for all $\v\in V$
...
Say that $V$ is cyclic with respect to $(\v,\w)$ if
the following two conditions hold.
\begin{itemize}
\item $\p = \q + h (\w-\v)$, with $\p,\q\in V$ and $h\in \ring{R}$
implies that $\p=\q$.  
\item  The line through $\v$ and $\w$ does not meet $V$.
\end{itemize}
\end{definition}
...
$\sigma:V\to V$ by $f\sigma(\p) =\sigma'f(\p)$.
Call $\sigma$ the \newterm{azimuth cycle}
on $V$ with respect to $(\v,\w)$.
\indy{Index}{azimuth cycle} %
\indy{Index}{orthonormal frame}%
\indy{Index}{polar cycle}%
...
a cyclic set $V$ with respect to $(\v,\w)$ does not depend
...
Let $(\v,\w)$ be an ordered pair of points in $\ring{R}^3$,
with $\v\ne \w$.  Let $V$ be a finite set in $\ring{R}^3$ of
cardinality $n$ that
...
and all $0\le i \le j < n$,
   ...
   $$
Moreover, if there exists $\q\in V$ such that 
$\op{azim}(\v,\w,\p,\q)\ne0$,
then
  $$
  \sum_{i=0}^{n-1} \op{azim}(\v,\w,\sigma^ip,\sigma^{i+1}\p) = 2\pi.
  $$
(If $\op{azim}(\v,\w,\p,\q)=0$ for all $\q\in V$, then all the
summands are zero.)
\end{lemma}
\indy{Notation}{azim}%
\indy{Index}{azimuth}%
