...
two permutations $e,n$, leaving $f$ to be derived from $e n f = I_D$; however,  the
...
path is \newterm{injective} if the condition $x_i=x_j$ implies $i=j$. 
...
same orbit of the group generated by $S$.  The following two simple lemmas provide
a substitute for group theory.
...
$P$. This claim gives the first conclusion of the lemma. 
...
A walkup deletes a dart from a hypermap and reforms the
edge, node, and face maps to produce a hypermap on the reduced set of
darts.  Walkups come in three varieties: edge walkups, face walkups, and
node walkups.
...
is to produce a M\"obius contour in the hypermap  (Lemma~\ref{lemma:no-mobius}).
