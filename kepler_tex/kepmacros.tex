...
%% debug:
\def\rating#1{\if\debug t{{\textsc {[rating={\ensuremath {#1}}].\ }}}\else{}\fi}
\def\formal#1{\if\debug t{{\tt [formal:#1].\ }}\else{}\fi}
\def\footformal#1{\if\debug t{\footnote{\formal{#1}}}\else{}\fi}
\def\usage#1{\if\debug t{\symbolfootnote[1]{#1}}\else{}\fi}
\def\move#1#2{\begin{#1} See \ref{#2}.\end{#1}}
\def\FIXX#1{\if\debug t{\footnote{\tt [#1]}}\else{}\fi}
\def\page#1#2{#2 {\it [dcgp.#1]}}
% first entry Lemma or Sec of DCG, second entry page.
\def\dcg#1#2{{\if\debug t{{\tt{[DCG-#1]}}\index{References}{ZC{#2 #1}@{DCG-#1}|page{#2}}}\else{}\fi}}
\def\tlabel#1{\label{#1}\if\debug t{{\tt [#1].\ }%
