...
\begin{definition}[$\Omega$ reprise]\guid{BBDTRGC} 
  ...
  V$, %(or if $W$ is an ordered tuple of elements of $ V$),
...
\item\claim{We claim $\omega(\bu)\in\op{conv}(S)$.}  Otherwise, there
...
\begin{proof}
  Enumerate $F_1,\ldots,F_k$ the distinct facets of $P$, and for each
  one pick a defining equation
\[
F_k = P\cap \{\p \mid \u_i \cdot \p = b_i\},
...
projected face, and pull the points $\w_j$ back to points on $F$.
...
which are computed in Chapter~\ref{chapter:volume} in terms of
dihedral angles.  Set
\[
...
\] 
Thus, the function is convex $\beta$.  By convexity, the minimum area
...
follows: \indy{Index}{polyhedron!convex}%
...
of the previous lemma.  Extend the packing to a the weak saturation
