...
\begin{figure}
\caption{In this figure, segments represent geodesic arcs on the unit sphere.  The shaded region is a subregion whose boundary is not a simple polygon.
...
\item all edges $\{v,w\}\subset U$ of an anchored simplex except those on a special simplex, whenever the upright diagonal is an unerased loop.
\item all edges $\{v,w\}\subset U$ such that $\|v-w\|\le\sqrt8$, where $\{v,w\}$ does not cross any other edge in previous two items.
\end{itemize}
These edges do not cross.  The projection of this set of edges to the
