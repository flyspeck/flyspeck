...
let x_fan= new_definition`x_fan (x,V,E)={v | ?e. (e IN E)/\(v IN aff_ge {x} e)}`;;

let y_fan= new_definition`y_fan (x,V,E)={v:real^3 | ?e. (e IN E )/\(~(v IN aff_ge {x} e))}`;;
...
let rw_dart_fan=prove(`!x:real^3 (V:real^3->bool) (E:(real^3->bool)->bool) v:real^3 u:real^3 h:real.
FAN(x,V,E)/\ {v,u} IN E/\ &0 <h /\  h< pi/ &2
...
(!h:real. &0 <h /\  h< pi/ &2
==> connected(rw_dart_fan x V E ((x:real^3),(v:real^3),(u:real^3),sigma_fan x V E v u) (cos(h))))`,
MESON_TAC[continuous_set_fan;rw_dart_avoids_fan;rw_dart_fan]);;
...
THEN RESA_TAC
THEN MP_TAC(ISPECL[`x:real^3 `;`(V:real^3->bool) `;
`(E:(real^3->bool)->bool)`;` w:real^3`;` u:real^3`]remark1_fan)
THEN RESA_TAC
...
ASM_REWRITE_TAC[]
THEN MP_TAC(ISPECL[ `x:real^3`;`v:real^3`;`u:real^3`;`w:real^3`;`c:real`;`&0`]open_vector_angle_fan)
THEN RESA_TAC
THEN MP_TAC(ISPECL[ `x:real^3`;`v:real^3`;`u:real^3`;`w:real^3`;`c:real`;`pi`]open_vector_angle_fan)
THEN RESA_TAC
THEN REWRITE_TAC[DE_MORGAN_THM;]
...
top_goal();;
map dest_var (variables(concl compact_aff_ge_ballnorm_fan));;
let tm1 = snd(top_goal()) and tm2 = concl compact_aff_ge_ballnorm_fan;;
tm1 = tm2;;
p();;
aconv tm1 tm2;;
compact_aff_ge_ballnorm_fan;;
