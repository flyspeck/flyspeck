...
%%XX Fix this, remove the lambda, check the identitites...
(Rating: 80) To see that these polynomials are non-negative, volumes are not required.
When $n=3$, write $v_2-v_1=(r_1,r_2,r_3)$ and $v_3-v_1=(s_1,s_2,s_3)$.
Then 
   $$
   0 \le (r_2s_3-s_2r_3)^2 + (r_1s_3-s_1r_3)^2 + (r_1s_2-s_1r_2)^2 = 
   4 \ups(x_{12},x_{13},x_{23}),
   $$
as we see by substituting $x_{12}=r_1^2+r_2^2+r_3^2$,
$x_{13}=s_1^2+s_2^2+s_3^2$, $x_{23} = x_{12}+x_{13}-2 (r_1s_1+r_2s_2+r_3s_3)$.
When $n=4$, write also $v_4-v_1=(t_1,t_2,t_3)$.
Then expanding both sides.
    $$
    0\le\det(v_2-v_1,v_3-v_1,v_4-v_1)^2 = \lambda \Delta(x_{ij}).
    $$
Similarly, when $n=5$, write also $v_5-v_1 = (u_1,u_2,u_3)$.  Then
    $$
    0 = CM_5(x_{ij}).
    $$
