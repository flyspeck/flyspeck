...
proof requires three other estimates that are similar in nature to the
main estimate.  To give a uniform proof of the main estimate and
...
\begin{definition}[profile,~basic~profile]\guid{ZKDPFPW}\label{def:profile}
A \newterm{profile} $p$ is a four-tuple $(\phi_p,\psi_p,c_p,d_p)$,
where $\phi_p$ is a predicate on $\ring{N}^2$,
$\psi_p$ is a predicate on $\ring{N}$, $c_p\in\ring{R}$, 
and $d_p$ is a real-valued function on
\[
\{ (r,s)\in\ring{N}^2 \mid \phi_p(r,s) \ \ \land\ \ 
     0\le s \le 3\ \ \land\ \  3-s \le r \le 6-2s\}.
\]
The following four profiles are called \newterm{basic profiles}:
\begin{enumerate}
\item $\psi_p(t) = (t=0)$, $\phi_p(r,s)=\op{true}$, 
   $c_p=2h_0$, $d_p = d$.
\item $\psi_p(t) = (t=1)$, $\phi_p(r,s)=(r+2s\le5)$,
   $c_p =2h_0$, $d_p(r,s) = d(r-1,s+1)$.
\item $\psi_p(t)=(t=0)$, $\phi_p(r,s)=(s>0 \lor r\ge 5)$,
  $c_p=\sqrt8$, 
 \[
 d_p(r,s) = 
  \begin{cases} 0.128 (2-s) + 0.44 (r+2s-4)  & (r,s)\ne (6,0)\\
       0.91 & (r,s) = (6,0).
  \end{cases}
 \]
\item $\psi_p(t)=(t=1)$, $\phi_p(r,s) = (r+2s\le 4)$, $c_p=\sqrt8$,
  $d_p(4,0) = 0.4773$, $d_p(2,1) = 0.3493$.
\end{enumerate}
\end{definition}
...
\smallskip\noindent{\case{general flat node}}
...
\smallskip\noindent{\case{single flat node}\hfill}
...
\smallskip\noindent{\case{adjacent flat nodes}}
