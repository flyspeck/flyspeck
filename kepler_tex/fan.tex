...
\item \case{independence} If $\{\v,\w\} \in E$, then $\v$ and $\w$
are independent.
...
%
\item The property \case{independence} implies that the graph has no
loops: $\{\v,\v\}\not\in E$.
%
...
\begin{nomerate} 
\item \case{V} If $V$ is defined as a subset of or as the image of a
...
\item \case{independence} \claim{If the property \case{intersection}
is known, then to prove the independence of $\u$ and $\v$, it is
...
imply linear independence.  The inequality~(\ref{eqn:strict-triangle})
is equivalent to
\begin{displaymath}
\arc_V(\orz,\{\u,\v\}) < \pi.
\end{displaymath}
...
such that $\v$ and $\w(t)$ are independent for all $t\in I$.
...
Now assume for a contradiction that $\v$ and $\w(t)$ are dependent for
...
Let $(V,E)$ be a fan and let $\v,\w\in V$ be independent.  Suppose
...
\case{independence} of $E'$ follow from the corresponding property
of $E$ and the assumed independence for $\{\v,\w\}$.
...
node $\v = \v(x)$ such that for all sufficiently small positive
...
the hypermap such that $\v=\v(x)$ and $\w=\v(y)$.  (That is,
...
\begin{displaymath}A^0_+(i)=\op{aff}_+^0(\{\orz,\v(x_{i+1}),\v(x_{i+2})\},\v(x_i)),\quad
...
\begin{proof} The intersection of two half-spaces, $A^0_+(1)\cap
...
small.  This implies that $\v(y)$ lies in the intersection of the
closed half-spaces $A_+(i)$.  As previously established, this
intersection is the disjoint union of $U_F$ and $X'$.  As $\v(y)\in
X$, which does not meet $U_F$, it follows that $\v(y)\in X'$.  The set
$X'$ is the disjoint union of the rays $C\{\v(x_i)\}$ and the three
blades $C^0(i)$.  These blades do not meet $V$, hence $\v(y)=\v(x_i)$
...
$\op{aff}_+^0(\{\orz,\v(x),\v({f x})\},\v(f^2 x))$, as $x$ runs over
$F$.
...
with corresponding nodes $\v(x),\v(y)\in V$, then $\v(x)$ and
$\v(y)$ are independent.  Moreover, either $x,y$ are adjacent under
the face map, or $C^0\{\v(x),\v(y)\}\subset U_F$.  {\it That is, the
...
%   component) assume that $\{\orz,\v(x),\v(y)\}$ is not a collinear
%   set.  For each blade $C^0(\ee)$ of the fan that meets
%   $C=C^0\{\v(x),\v(y)\}$ pick one of the two endpoints of $\ee$.
...
This section proves the existence of many conforming fans.  The
...
that $C^0\{\v,\w\}\subset U_F$, where $\v = \v(x)$ and $\w =
\v(y)$. Form a new fan $(V,E')$ on the same node set with $E' =
...
% assume that $\v(x)\in e$.  Let $E''' = E \cup \{e\, \{\v,\w\}\}$.
...
half-space with bounding plane $\{\orz,\v(z),\v(f z)\}$.} Indeed,
...
Fans and hypermaps have \newterm{nodes}, while polygons and
polyhedra have \newterm{vertices}.
\end{remark}
...
and $\w$ are dependent.  As $\orz\not\in \op{conv}(\ee)$, some
dependency has the form $s \v = t \w$, for some $s, t>0$.  By
