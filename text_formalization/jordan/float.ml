...
let interval_eps =
  new_definition(
                   `interval_eps x f eps = ((abs (x-f)) <= eps)`);;
...
let dest_interval intv =
   ...
   let _ = assert(h3 = `interval_eps`) in
   (a,f,ex);;
...
let INTERVAL_ADD = prove(
   `!x f ex y g ey. interval_eps x f ex /\ interval_eps y g ey ==>
...
let INTERVAL_NEG = prove(
  `!x f ex. interval_eps x f ex = interval_eps (--. x) (--. f) ex`,
(REWRITE_TAC[interval_eps;REAL_ABS_NEG;REAL_ARITH `!x y. -- x -. (-- y) = --. (x -. y)`]));;
...
let INTERVAL_SUB = prove(
   `!x f ex y g ey. interval_eps x f ex /\ interval_eps y g ey ==> interval_eps (x -. y) (f -. g) (ex +. ey)`,
...
let INTERVAL_NUM = prove(
  `!n. (interval_eps(&.n) (float(&:n) (&:0)) (float  (&: 0) (&:0)))`,
(REWRITE_TAC[interval_eps;float;TWOPOW_POS;pow;REAL_MUL_RID;INT_NUM_REAL;REAL_SUB_REFL;REAL_ABS_0;REAL_LE_REFL]));;

let INTERVAL_CENTER = prove(
  `!x f ex g. (interval_eps x f ex) ==> (interval_eps x g (abs(f-g)+.ex))`,
((REWRITE_TAC[interval_eps]))
THEN (DISCH_ALL_TAC)
...
let INTERVAL_WIDTH = prove(
  `!x f ex ex'. (ex <=. ex') ==> (interval_eps x f ex) ==> (interval_eps x f ex')`,
((REWRITE_TAC[interval_eps]))
THEN (DISCH_ALL_TAC)
THEN ((H_VAL2 IWRITE_REAL_LE_RHS (HYP_INT 1) (HYP_INT 0)))
THEN ((ASM_REWRITE_TAC[])));;
...
let INTERVAL_MAX = prove(
  `!x f ex. interval_eps x f ex ==> (x <=. f+.ex)`,
(REWRITE_TAC[interval_eps]) THEN REAL_ARITH_TAC);;

let INTERVAL_MIN = prove(
  `!x f ex. interval_eps x f ex ==> (f-. ex <=. x)`,
(REWRITE_TAC[interval_eps]) THEN REAL_ARITH_TAC);;

let INTERVAL_ABS_MIN = prove(
  `!x f ex. interval_eps x f ex ==> (abs(f)-. ex <=. abs(x))`,
  (REWRITE_TAC[interval_eps] THEN REAL_ARITH_TAC)
);;

let INTERVAL_ABS_MAX = prove(
  `!x f ex. interval_eps x f ex ==> (abs(x) <=. abs(f)+. ex)`,
  (REWRITE_TAC[interval_eps] THEN REAL_ARITH_TAC)
);;
...
EVERY[
  ...
  DISCH_ALL_TAC;
  REWRITE_TAC[interval_eps;float;TWOPOW_NEG;INT_NUM_REAL;REAL_POW_1;REAL_MUL_LID];
...
let INTERVAL_EPS_0 = prove(
  `!x f n. (interval_eps x f (float (&:0) n)) ==> (x = f)`,
EVERY[
  REWRITE_TAC[interval_eps;float;int_of_num_th;REAL_MUL_LZERO];
  REAL_ARITH_TAC]);;
...
let INTERVAL_DIV = prove(`!x f ex y g ey h ez.
  (((interval_eps x f ex) /\ (interval_eps y g ey) /\ (ey <. (abs g)) /\
...
let INTERVAL_ABSV = prove(`!x f ex. interval_eps x f ex ==> (interval_eps (abs x) (abs f) ex)`,
EVERY[
  REWRITE_TAC[interval_eps];
  DISCH_ALL_TAC;
...
let INTERVAL_SSQRT = prove(`!x f ex u ey ez v. (interval_eps x f ex) /\ (interval_eps (u*.u) f ey) /\
  (ex +.ey <=. ez*.(v+.u)) /\ (v*.v <=. f-.ex) /\ (&.0 <. u) /\ (&.0 <=. v) ==>
  (interval_eps (ssqrt x) u ez)`,
EVERY[
  DISCH_ALL_TAC;
  ...
  H (REWRITE_RULE[interval_eps]) (HYP_INT 12);
  H (ONCE_REWRITE_RULE[interval_eps]) (HYP_INT 14);
...
(*test*) add_test("INTERVAL_CLEAN_CONV",
  ...
  let concval = concl testval in
        (length hypval = 0) &&
        concval =
     `interval_eps (&1 + &1) (float (&:3) (&:4) + float (&:2) (--: (&:3)))
     (float (&:1) (&:2) * float (&:3) (--: (&:2))) =
     interval_eps (&1 + &1) (float (&:386) (--: (&:3))) (float (&:3) (&:0))`
                  );;
...
let ABS_TO_INTERVAL = prove(
  `!c u k. (abs (c - u) <=. k) ==> (!f g ex ey.((interval_eps u f ex) /\ (interval_eps k g ey) ==> (interval_eps c f (g+.ey+.ex))))`,
   EVERY[
   REWRITE_TAC[interval_eps];
   DISCH_ALL_TAC;
   REPEAT GEN_TAC;
...
add_test("INTERVAL_OF_NUM",
   ...
   `interval_eps (&3) (float (&:3) (&:0)) (float (&:0) (&:0))`));;
...
add_test("INTERVAL_TO_LESS_CONV",
  let thm1 = ASSUME
   `interval_eps (#0.1) (float (&:1) (--: (&:1))) (float (&:1) (--: (&:2)))` in
  let thm2 = ASSUME `interval_eps (#7) (float (&:4) (&:1)) (float (&:1) (&:0))` in
...
let INTERVAL_MINMAX = prove(`!x f ex.
   ((f -. ex) <= x) /\ (x <=. (f +. ex)) ==> (interval_eps x f ex)`,
   EVERY[REPEAT GEN_TAC;
   REWRITE_TAC[interval_eps;ABS_BOUNDS];
   REAL_ARITH_TAC]);;
...
let INTERVAL_OF_TWOPOW = prove(
   `!n. interval_eps (twopow n) (float (&:1) n) (float (&:0) (&:0))`,
   REWRITE_TAC[interval_eps;float;int_of_num_th] THEN
   REAL_ARITH_TAC
   );;
...
let rec INTERVAL_OF_TERM bprec tm =
  ...
  (* treat sqrt : instantiate INTERVAL_SSQRT *)
    ...
    let thm2 = prove(hyp2,REWRITE_TAC[interval_eps] THEN
