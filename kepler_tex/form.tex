...
The first section of this paper gives the top level structure of
the proof of the Kepler conjecture.  The next two sections
describe the fundamental decompositions of space that are needed
in the proof. The first decomposition, which is called the
$Q$-system, is a collection of simplices that do not overlap. This
decomposition was originally inspired by the Delaunay
decomposition of space.  The other decomposition, which is called
the $V$-cell decomposition, is closely related to the Voronoi
decomposition of space.  In the following section, these two
decompositions of space are combined into geometrical objects
called {\it centered packings}. The centered packing is the
fundamental geometrical object in the proof of the Kepler
conjecture.

The final section of this paper describes a particular nonlinear
function  on the set of all centered packings, called the scoring
function.  The Kepler conjecture reduces to an optimization
problem involving this nonlinear function on the set of all
centered packings. This is an optimization problem in a finite
number of variables. The subsequent papers (Papers~\ref{part:iii}
-- \ref{part:tame}) solve that optimization problem.
...
We assume basic facts about measure, summarized in
Appendix~\ref{chap:measure}.  Let $B(x,r)$ denote the open ball in
...
If $\Lambda$ is a saturated packing, then there is an object
$D(v,\Lambda)$ constructed around each vertex $v\in\Lambda$.  The
objects $D(v,\Lambda)$ are called {\it centered packings}, and the
set of all centered packings is precisely $\op{DS}$.
\index{centered packing} Section~\ref{sec:cell-ds} shows that the
data in a centered packing are sufficient to determine a Voronoi
cell\index{Voronoi cell} $\Omega(D)$\index{ZZzomega@$\Omega(D)$}
for each $D\in\op{DS}$. The same section shows that the Voronoi
...
there is a centered packing $D(v,\Lambda)\in\op{DS}$ such that the
...
The maximum of $\sigma$ on the set $\op{DS}$ of all centered
...
wish to show that there is no centered packing $D$ with value
$\sigma(D)
> 8\,\pt$.  We reason by contradiction, assuming the existence of
such a centered packing.  With this in mind, we call $D$ a {\it
contravening centered packing},\index{contravening} if
    $$\sigma(D)\ge 8\,\pt - \epsilon_0, \quad \text{with }\epsilon_0 = 10^{-12}.$$
...
To each contravening centered packing $D$, we associate a hypermap
$G(D)$.   Assume, for the moment, the existence of this hypermap.
...
Let $D$ be a contravening centered packing. Then its hypermap
$G(D)$ is tame.
...
$D$ is a contravening centered packing, then its hypermap $G(D)$
is isomorphic to $G_{hcp}$ or $G_{fcc}$.
\end{lemma}

\begin{proof} Assume that $D$ is a contravening centered packing.  Then its hypermap is tame, and consequently appears on the
...
centered packing $D\in \op{DS}$ that has $\sigma(D)>8\,\pt$. By
definition, this is a contravening centered packing. By
Lemma~\ref{lemma:claim-fcc-graph}, its hypermap is isomorphic to
$G_{hcp}$ or $G_{fcc}$.  By Claim~\ref{claim-F}, $\sigma(D)\le
8\,\pt$, in contradiction with $\sigma(D)>8\,\pt$.
\end{proof}
...
following.  Let $D$ be a contravening centered packing with
hypermap $G(D)$. We assume that the hypermap $G(D)$ is not
isomorphic to $G_{fcc}$, $G_{hcp}$, $G_{pent}$ and then prove that
$D$ is not contravening. This is a case-by-case argument, based on
...
present the fundamental estimates. \Chaps~\ref{sec:def-and-class}
...
\Chaps~\ref{sec:linearprogram} through \ref{sec:branchbound} give
...
saturated packing $\Lambda$ with $0\in\Lambda$. Let $\op{DS}$ be
...
definition of $\op{DS}$ so that it involved only finitely many
...
\begin{lemma} There is a $V$-cell $\op{VC}(D)$ attached to each
...
Voronoi cell at $v$.  In fact,
  $$\Omega(D(v,\Lambda)) = v + \Omega(v,\Lambda).$$
...
$2t_0$ from $v$.  From $v$ and a centered packing $D(v,\Lambda)$
...
origin, the $V$-cell $\op{VC}(D)$ at the origin, the Voronoi cell
$\Omega(D)$ at the origin, and so forth, because these structures
...
set $\op{DS}$ that was introduced in \Chap~\ref{sec:compact}.  In
fact, we construct a  function $A_0$ on the set $\op{DS}$ such
that for each saturated packing $\Lambda$ and each $v\in\Lambda$,
the value of the function $A$ at $v$ is a value in the range of
the function $A_0$ on $\op{DS}$. In this way, we are able to
translate the fcc-compatibility of $A$ into an extremal property
of the function $A_0$ on the set $\op{DS}$.
...
is a canonically associated centered packing $D(v,\Lambda)$. The
negligible function $A:\Lambda\to\ring{R}$ that we define is a
composite
  \begin{equation}
  A = A_0\circ D(\cdot,\Lambda)
  :\Lambda\to \op{DS}\to \ring{R},\quad v\mapsto D(v,\Lambda)\mapsto
  A_0(D(v,\Lambda)),
  \label{eqn:A}
  \end{equation}
where $A_0:\op{DS}\to\ring{R}$ is as defined by
...
attached to $D=D(v,\Lambda)$. Let $\CalQ_0(D)$ be this set of
...
The function $A_0$ will be defined to be a function on $\op{DS}$
of the form
  \begin{equation}
  A_0(D) = -\op{vol}\,(\Omega(D)) + \op{vol}\,(\op{VCM}(D)) +
   \sum_{Q\in\CalQ_0(D)} A_1(Q,c(Q,D),0),
   \label{eqn:A1}
   \end{equation}
for some bounded measurable subset $\op{VCM}(D)$ of $\op{VC}(D)$.
...
$\op{DS}$ to subsets of $\ring{R}^3$ is {\it admissible} if
\index{admissible} there exists $d>0$ such that for every
$D\in\op{DS}$,
    \begin{itemize}
        \item $\op{VCM}(D)$ is measurable,
        \item $\op{VCM}(D)\subset B(0,d)$, and
        \item $\op{VCM}(D)\cap |Q| = \op{VC}(D)\cap |Q|$ for all
        $Q\in\CalQ_0(D)$.
    \end{itemize}
\end{definition}

\begin{lemma} If $\op{VCM}_1,\ldots,\op{VCM}_r$ are admissible,
then so is $$D\mapsto \op{VCM}_1(D)\cup\cdots\cup \op{VCM}_r(D).$$
\end{lemma}
...
\Chap~\ref{chapter:measure_second}.  The notation $\op{volan}$
...
The solid angle of a simple at a vertex $v$ is defined in
\chap~\ref{chapter:measure_second}.  It is denoted $\sol(S,v)$.
...
are bounded by Claim~\ref{negbounds}.  The set
    $$\op{VC}(S,v) = \op{VCM}(D(v,\Lambda))\cap |S|$$
...
\begin{proof}   Set $\op{VCM}(v,\Lambda) = v + \op{VCM}(D(v,\Lambda))$.
