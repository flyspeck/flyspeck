...
Assume $D$ is a contravening centered packing and
    ...
    \pi(D,\rho,\phi).
    $$
Assume further that if $\op{trans}(\rho,\rho')$, then\FIXX{XX}.
    If
    $$\pi(D,\rho,\phi),$$
    then
...
\FIXX{
\begin{itemize}
\item Relate tame linear systems back to centered packings.
\item Define $v(D,\alpha)$, etc. on centered packings.
\item Move relations back up to where the subscript is defined.
\item Use the definition of $R$ to check the axioms (A).
\end{itemize}
}
...
\begin{lemma}\guid{KOAZGYC}  Let $X$ be a set.  Let $R$ be a relation on
