...
Given a packing $V$, we define $\bV(k)$ to be certain lists of points in $V$.
...
\indy{Notation}{zzomega@$\omega$ (extreme points of Rogers's
  simplex)}%

\begin{definition}[R,~Rogers's simplex]
...
Consider the following statement of a natural number $k$: Either  $k>3$ or
\begin{displaymath}
\Omega(V,\bu) = \bigcup
...
\claim{This statement holds by induction on $k\ge j$.}  Indeed, the base case
$k=j$ is 
\begin{displaymath}
\Omega(V,\bu) = \op{conv}(\Omega(V,\bu)),
\end{displaymath}
which holds by basic facts about the convexity for a polyhedron $\Omega(V,\bu)$.
Assume the induction hypothesis holds for $k$, and consider the statement for
$k+1$.  We may assume that $k<3$ to avoid trivial cases.
Then
\begin{displaymath}
\begin{array}{lll}
&\phantom{=}\bigcup 
\left\{\op{conv}(O_{k+1} \cup\Omega(V,\bv))
\mid
\bv \in \bV(k+1),~\trunc{\bv}{j}=\bu\right\}\vspace{6pt}\\
&=\bigcup 
\left\{\op{conv}(O_{k} \cup\op{conv}(\{\omega(\trunc{\bv}{k})\}\cup\Omega(V,\bv)))
\mid
\bv \in \bV(k+1),~\trunc{\bv}{j}=\bu\right\}\vspace{6pt}\\
&=\bigcup 
\left\{\bigcup\left\{
...
\mid
\bv \in \bV(k+1),~\trunc{\bv}{k}=\bw\right\}\mid \bw\in V(k),~\trunc{\bw}{j}=\bu\right\}\vspace{6pt}\\
&=\bigcup 
\left\{
\op{conv}(O_{k} \cup \Omega(V,\bw)    
\mid \bw\in V(k),~\trunc{\bw}{j}=\bu\right\}\vspace{6pt}\\
&=\Omega(V,\bu).
\end{array}
\end{displaymath}
The induction hypothesis is used in the last step.  This proves the induction.

Now specialize this statement to $j=0$ and $k=3$.  The set $\Omega(V,\bv)$
reduces to $\{\omega(\bv)\}$  and the convex hull becomes
\begin{displaymath}
\op{conv}(O_{k+1}\cup \Omega(V,\bv)) = R(\bv),
\end{displaymath}
when $\bv\in \bV(3)$.
The statement reduces to the cover
\begin{displaymath}
...
If $\bu=[\u_0;\u_0;\ldots;\u_k]\in \bV(k)$, let $h(\trunc{\bu}{j})$ be the
