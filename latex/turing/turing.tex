...
translations\footnote{My Flyspeck project seeks to give a formal proof
  of the Kepler conjecture~\cite{Hales:2005:DSP}. This logical
  framework technology gives me hope that an automated tool may
  assemble the different parts of my project -- which is now scattered
  between different proof assistants.} can be constructed between
...
This box presents a rather trivial example of
proof by resolution, to deduce the easy theorem asserting that every infinite
set has a member.


The example will use the following notation.  Let $\emptyset$ be a
constant representing the empty set and constant $c$ representing a
given infinite set. We use three unary predicates $i$, $f$, $e$ that
have interpretations
\[
i(X)~~\text{``$X$ is infinite''}, 
\quad f(X)~~\text{``$X$ is finite''}, 
\quad e(X)~~\text{``$X$ is empty''}.
\]
There will be one binary predicate $(\in)$ for set membership.
Write $u(X)\in X$ when there exists $u\in X$.
We wish to prove $i(c) \Rightarrow (\exists z. z\in c)$ by resolution. 

To argue by contradiction,
we introduce the
hypothesis $i(c)$ and the negated conclusion $\lnot (t\in c)$ as axioms.
Here are the axioms that we introduce for the deduction.  The axioms have
been preprocessed so that each one is a disjunction of literals.
...
7.& $\lnot e(U) \lor \lnot e(V) \lor \lnot i(U) \lor i(V)$~~&Weak uniqueness of empty set.\\
\end{tabular}
\bigskip
...
By choice, we may pick  $x_1\in P_1$ and $x_2\in P_2$; and by construction
\[
