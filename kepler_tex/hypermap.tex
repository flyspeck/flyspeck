...
is isomorphic to $H_{2n}$ of Example~\ref{ex:H2k}.  (This
...
is {\it concave}.  (We also use the term non-concave for angles at most $\pi$.)
\index{convex}\index{concave}\index{non-concave}
%
\end{definition}
...
\begin{definition} A sequence $\phi_0,\phi_1,\ldots,\phi_r$ is a path in $U$ from $\phi_0$ to $\phi_r$, if (setting $v_i = v(\phi_i)$),
\begin{itemize}
\item $v_i\ne \orgn$,
...
\begin{lemma}\rating{400}
For every path in $U$, there is a vertex-path with the same structured
endpoints that has no greater length
such that the intermediate points $\phi_i$ 
on that vertex path occur at concave darts and $\alpha_i\ge\pi$.
...
that every path with arc-length at most $L$ and constant $a$ has length at
most $r(a,L)$.  (We can find a constant $C>0$ such that every
arc-length between concave darts is at least $C$, so that the path contains at most $L/C$ such steps.  Thus, we can take $r(a,L) = 2 a + L/C$.)

Thus, among paths of arc-length at most $L$, from $\phi$ to $\phi'$,
...
We repeat this construction as long as $a>0$.  Eventually all intermediate points of the path lie at concave darts.  Among all paths whose intermediate points lie at concave darts there is one of shortest length.  (If such a path visits the same dart twice it creates a ``loop'' which will cannot be part of a minimal-length path.)  Thus, a minimal length path exists.  If any of the intermediate path angles $\alpha_i$ are less than $\pi$, we can reason as above, using the triangle inequality to produce a shorter path.
Thus, on the minimal path the intermediate path angles satisfy $\alpha_i\ge \pi$ and each intermediate point is a concave dart.
\end{proof}


\begin{lemma}  Let $(\orgn,V,E)$ be a fan. Let $U$ be a component of $Y(\orgn,V,E)$.  Let $\phi_0,\ldots,\phi_r$ be a minimal vertex path from $\phi_0$ to $\phi_r$.  Let $v=v(\phi_0)$ and $v'=v(\phi_r)$, with $v,v'\in V$ and $\{v,v'\}\not\in E$.  Then $v$ is visible from $v'$ if and only if $r=1$. 
\end{lemma}
...
Conversely, if $v$ is visible from $v'$, then $\phi_0,\phi_r$ is a vertex path from $\phi_0$ to $\phi_r$.  By the spherical triangle inequality, it has minimal length.  By the case of equality in the spherical triangle inequality, it is the unique minimal vertex path.  So $r=1$.
\end{proof}
