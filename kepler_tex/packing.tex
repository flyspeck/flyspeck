...
\begin{summary}
  ...
  sharp.  The Marchal decomposition leads to a sharp bound
  $\pi/\sqrt{18}$ on the density of sphere packing in three
...
\end{align*}
\end{definition}
\indy{Notation}{ZZZmega@$\Omega(V,W)$ (intersection of Voronoi cells)}%
...
\end{definition}
\indy{Notation}{V@$\bV(k)$ (set of suitable lists of length $k+1$ in $V$)}%
\indy{Notaton}{vz@$\bu$ (list of members of a packing)}%
...
\begin{proof} This follows directly from the definition of face on
  page \pageref{def:page}.  The set $\Omega(V,\bu)$ is
an intersection of the convex sets $\Omega(V,\u_i)$ and
is thefore convex.  Also,  $\Omega(V,\bu)$ is the intersection of
...
\p = \sum^3_{j={i+1}} \frac{s_j}{1-s_i} \omega_j(\bu),\quad
\p' = \sum^3_{j={i+1}} \frac{t_j}{1-t_i} \omega_j(\bv)
\]
...
$\p = \sum u_i \w_i\in W$ and let $\q\in\ring{R}^n$ be the image of
$\p\in W$.  Then
\[  
...
\item\claim{We claim $\omega(\bu)\in\op{conv}(S)$.}  Otherwise, pick
...
if $k=3$, then $\op{equi}(S,r)$ is a set of $2$ points.
...
\begin{definition}[edge]\guid{WYORUNK}
  \formaldef{$E(X)$}{edgeX} \indy{Notation}{EX@$E(X)$}%
...
Currently, although no proof of Marchal's conjecture has been published,
its status may soon change.  This book proves a variant of Marchal's
conjecture.

\begin{theorem}\guid{QFUXEXR}\label{theorem:mk1} 
  % formalization must follow the statements in pack_concl.hl, 250
  % each estimate.
  Marchal's conjecture~\eqref{conj:m1} and Marchal's
...
the function creates a large difference in the difficulty of the
conjectures.
...
In light of Lemma~\ref{lemma:deltabound}, inequality~\ref{enq:L12}
...
\begin{definition}[$\BB$]\guid{WTKURHK} 
  ...
  the closed ball of radius $r$.  \indy{Notation}{BB@$\BB$}
\end{definition}
...
\[
b_1 \sqrt{1-t^2}/t = b_0/\normo{\v}
\]
is contained in the projected face, where $b_1$ is the distance from
...
corresponding to the facet $i$.  
By Lemma~\ref{lemma:edge-bi}, the number of edges of the facet $i$
is $k_i$.  By Lemma~\ref{lemma:ngon}, the solid
