...
A packing of balls in this article is identified with its set of centers.  We adopt the convention that balls in a packing have unit radius.  Formally, a \newterm{packing} is a set $V\subset\ring{R}^3$ for which $2\le\norm{\v}{\w}$ for every $\v,\w\in V$.  Call a nonempty packing $V$ in $\ring{R}^3$ in which every $\v\in V$ has distance $2$ from twelve other $\w\in V$ a \fullterm{packing with full contact}{packing!full contact}.
...
We follow the general approach described in \cite{dsp}.  We use a number
of definitions given there.  In particular, we have the following.
