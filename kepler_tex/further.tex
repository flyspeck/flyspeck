...
Earlier chapters were written in a ``formal blueprint'' style.  Rather complete proofs
...
proof by traditional mathematical standards, but not quite as detailed as earlier chapters.
...
\begin{proof}
By Lemma~\ref{lemma:XXD}, we may replace $V$ with a new set in $\CalV$ if necessary
so that $(V,E)$ is a biconnected fan, where
$E\setminus E_{ctc}$ is  the set of pairs $\{\u,\v\}\subset V'$
...
\]
By biconnectedness of $(V,E)$, each term $\tau(V,E,F)$ is the same before and after
...
and $r(F) = \card(F)-s(F)$.
Thus, the claim follows from the main estimate (Lemma~\ref{lemma:main-estimate-12}).
