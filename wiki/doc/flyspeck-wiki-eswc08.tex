...
\begin{abstract}
  ...
  They are particularly appropriate for % e-science, 
  \ednote{Why e-science?  is this language standard?  I don't think it is in the US.}
  scientific and mathematical collaboration.
  In previous work we described the \textit{OMDoc} semantic markup language, 
  an ontology for mathematical knowledge, and a semantic wiki
  based thereon\cite{OMDoc,previous papers}.  
  We are now evaluating these technologies in concrete application
  scenarios.  

  \ednote{why aren't the paragraphs showing up in the abstract?}
  In this paper we evaluate the applicability of our infrastructure 
  to mathematical knowledge management by focusing on 
  \textit{the Flyspeck project\cite{Flyspeck:definition}}, a formalization of 
  Thomas Hales' proof of the Kepler Conjecture.  
  The Flyspeck project is an ideal target for the semantic wiki framework:  
  It is a relatively large project built upon a large number of smaller subprojects that must be
  organized and easily accessible by a diverse audience. 

  After describing the Flyspeck project and its requirements in detail, 
  we present our progress on a prototype in the general Semantic MediaWiki\cite{SemanticMediaWiki}.
  Finally, we establish a roadmap on how to improve our mathematics-specific semantic wiki
  SWiM to better meet the needs of the Flyspeck collaborators.
\end{abstract}
