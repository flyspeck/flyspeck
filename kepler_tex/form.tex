...
\indy{Notation}{ZZLamda@$\Lambda(p,r)$}
\indy{Notation}{ZZLamda@$\Lambda^*(p,r)$}
...
$\Omega(v)=\Omega(\Lambda,v)$\indy{Notation}{ZZzomega@$\Omega(v)$} around a
...
The nonlinear inequalities for four blades are found at~\cite[cc:4bl:QITNPEA]{hales:2009:nonlinear}.  Let $q\in\{1,2,3,4\}$ be the number of quarters along the spine.  Between two blades there is a quarter $Q$, a $4$-cell $F$, or a $2$-$3$-cell combination $T$.  Let $*$ represent $T$ or $F$. We make a table of possibilities when $q\le 2$, listing the possibilities for the types of cells between blades.  We say the blade is small if its nonspinal edges have length at most $2h_-$.  We mark a blade $x$ if it is large, and $o$ if it is small. The blades bounding a quarter are small.

XX


In the case $q\le 2$, we may assume that all the cells around the spine are $4$-cells.  Otherwise, we argue as follows.  Call a blade small if its two edges that are not spinal have length at most $2h_-$.  If there is a group $B$ of $2$- and $3$-cells along two small blades, then a calculation gives
$$
\Gamma(Z) > \sum_{X\in B}\gamma_L(X) w(X) -    2(0.0057) > 0
$$
If there is a group $B$ of $2$- and $3$-cells and there is a blade that is not small, then $3$-cells can be combined with quarters as in inequality~\ref{eqn:34}.  In this situation, each $4$-cell has a blade that is not small, and gives contribution $>0.0057$, which compensates for a quarter adjacent to it.

When $q=2$, the first row of coefficients in the table apply to the situation where the two quarters are adjacent, and the blade not next to the quarters has an edge longer than $2h_-$.  (There is only one such long edge adjacent to the spline.)  The second row of coefficients in the table for $q=2$ apply to the situation when all of the edges (except the spine) of all blades have length at most $2h_-$.




The next part of the argument uses the following table of constants:
\begin{equation}
\begin{matrix}  q&a_q&b_q\\
...
%Let $q\in\{1,\ldots,4\}$ be the number of quarters.
A quarter satisfies the inequality, for $q\in\{1,\ldots,4\}$:
$$
\gamma_L(X) > a_q + b_q\, \op{azim}(X).
$$
For $q\ne4$, define $a_q'$ by
$$
  q a _q + (4-q) a_q' + 2\pi\,b_q = 0.
$$
Between consecutive blades, there is a similar inequality for $q\in\{1,\ldots,3\}$:
$$
\sum_{X\in B} \gamma_L(X)w(X) +\beta(e,X) > a_q' + b_q\,\op{azim}(B).
$$
When there are $q$ quarters and $4-q$ others, it follows that
\begin{equation}\label{eqn:4bl}
  \Gamma(Z) > q a_q + (4-q) a_q' + b_q\, (2\pi) = 0.
\end{equation}
There is a particular case that has been skipped.  In proving these inequalities, we have assumed that the situation is not  $q=3$ quarters and a $4$-cell of weight $1/2$. This particular case is treated in the next subsection. 


\subsection{four blades and three quarters}

The previous subsection excludes the case of $q=3$ quarters and one $4$-cell of weight $1/2$.  This case is considered in this subsection.  The entire purpose of the function $\beta$ is to make the inequality $\Gamma(Z)>0$ hold in this case.  The inequality is false without this correction term.

\begin{note} %XX
Details will be added about the case $q=3$ and a fourth cell a $4$-cell of weight $1/2$.
\end{note}
