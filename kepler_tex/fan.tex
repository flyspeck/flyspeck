...
\begin{lemma}\guid{}\rating{ZZ}\label{lemma:add-edge}
...
\end{array}
$$
Then $(V',E')$ is a fan.  Moreover, the hypermap $\op{hyp}(V',E')$ is isomorphic
to $H_{2k}$, where $k= \op{card}(F)$.
 Moreover, $\op{azim}(x)$ is the same
...
The set $V'$ is a subset of $V$, hence finite.  Every face has at least one dart, and this implies that $V'$ is nonempty.  As $E'\subset E$, the independence property of the edges is inherited from $E$.  Similarly, the intersection property of fans is inherited from $(V,E)$:
$$
C(\e)\cap C(\e') = C(\e\cap \e').
$$

The dart set $D'$ of $\op{hyp}(V',E')$ is naturally identified with $F\cup F'$, where
$F = \{(v,w) \in F\}$ and $F'=\{(w,v) \mid (v,w)\in F\}$.  Under this identification, $F$ is a face of $\op{hyp}(V',E')$.  The edge map $e'(v,w)= (w,v)$ is a bijection of $F$ onto $F'$.   Pick any $x_0\in F$ and
define a bijection from the disjoint union of two copies of the cyclic group $Z_k$ onto $D'$  by 
the pair of maps
$$
i \mapsto f^i x_0,\quad\text{ and } i\mapsto f^i e' x_0.
$$
This bijection extends to an isomorphism of hypermaps $H_{2k}$ to $\op{hyp}(V',E')$.
...
$$
where $x = (v,\rho v)$.  In particular $\op{azim}(x)\le\pi$ follow by the assumptions of the lemma.
\end{proof}
...
of $F$ of the form $y=(v,w)$.  Since $y$ leads into $U_F$, every neighborhood of
...
$$
\op{per}=\op{per}(V,E,F) = \sum_{i=0}^{k-1} \arc_V(0,\{\rho^i v,\rho^{i+1} v\}), 
$$
where  $k=\op{card}(F)$.
...
If Lemma~\ref{lemma:A} can be applied once, then it can often be applied repeatedly along a chain.  For example, the lemma implies
...
are met for $\{u,\rho^{-1} v,v\}$, giving the conclusion $\alpha(\rho^{-1} v)=\pi$.  Repeating, we obtain a chain
$$
...
Lemma~\ref{lemma:A} to $\{u,v,\rho u\}$ to conclude that $\alpha(v)=\pi$, and that there is a plane $A$ containing
$\{0,v,u,\rho u, \rho v,\rho^{-1} v\}$.    Iterate along the chain, as described above, to find that $V\subset A$.
...
The arcs making up the perimeter all lie in a common plane.  The permutation $\rho$ may be identified with the azimuth cycle around a line orthogonal to the plane.  The azimuth angles sum to $2\pi$ by Lemma~\ref{lemma:2pi-sum}.
\end{proof}
...
Then
\begin{itemize}
\item $\alpha(u) = \pi$, for all $u\in V\setminus \{v,w\}$;
\item $0< \alpha(v) = \alpha(w)\le \pi$;
\item $V\cap \op{aff}_+(\{0,v\},\rho v) = \{v,\rho v,\ldots, w\}$;
\item $V \cap \op{aff}_+(\{0,v\},\rho^{-1} v) = \{w,\rho w,\ldots v\}$; 
\item $0\le \beta(1)=\cdots =\beta(r-1) < \beta(r+1) =\cdots =\beta(k-1)\le\pi$.
\item $\op{per}(V,E,F) = 2\pi$.
\end{itemize}
\end{lemma}

\begin{proof}  Let $u\in V\setminus\{u,v\}$ be arbitrary.
Apply Lemma~\ref{lemma:A} to the set $\{u,v,w\}$ to find that $\alpha(u)=\pi$ and
that $\{u,v,w,\rho u,\rho^{-1} u\}$ belongs to a plane $A$.  Repeatedly apply 
Lemma~\ref{lemma:A}, as described above, to find that the plane (in fact half-plane)
is the same for $v,\rho v,\ldots,w$.  Similarly, all the vectors $w,\rho w,\ldots,v$ belong to a second half-plane.   
...
\begin{lemma}[generic geometry]\guid{EGHNAVX}\rating{ZZ}  \label{lemma:generic}
Let $(V,E,F)$ be a generic cyclic fan with hypermap $(D,e,n,f)$.  Fix $v_0\in V$.  For all $i$, set $v_i = \rho^i v_0$ and $\beta(i) = \op{azim}(0,v_0,v_1,v_i)$.
...
%
Pick an orthonormal frame and write the points $v_j$ in spherical coordinates $(r_j,\theta_j,\phi_j)$.  In an appropriate frame, $\phi_0=0$, and $\theta_i=\beta(i)$, for all $i$.  From $v_j\in \bWdart(x(v_0))$, it follows that $0\le\theta_j\le\theta_{k-1}\le\pi$ for all $j$.

For the first claim one may assume by induction that $0\le \beta(1)\le\cdots\le \beta(i)$.  The condition
$$
v_0\in \bWdart(x(v_i))
$$
reduces to a cross-product:
$$
...
$$
Under the genericity hypothesis, $\sin\phi_i\ne0$, $\sin\phi_{i+1}\ne0$ (when $0< i < k-1$).    These inequalities give $\theta_i\le\theta_{i+1}$ (with a small extra argument for the degenerate case $\theta_i,\theta_{i+1}\in\{0,\pi\}$).  The first conclusion follows by induction.
...
$V$ that appear in cyclic order (according to $\rho$ on $V$). 
...
$v_i = \rho^{j (i)}$, for some increasing function $j$ with range $\{0,\ldots,k-1\}$.)

\begin{proof} The first case to consider is $v(x)=v$.  For all $p\in C^0$, 
$$
0 < \op{azim}(0,v,\rho v,p) < \op{azim}(0,v,\rho v,\rho^{-1} v).
$$  
This implies $p\in \Wdart(x)$ as desired.  The case $v(x)=w$ is similar.

Now assume that $u=v(x)\ne v,w$.  
By Lemma~\ref{lemma:A},
$$
...
We have the following facts (by Lemma~\ref{lemma:A}):
$$
...
\begin{lemma}[slicing]\guid{EJRCFJD}\rating{ZZ}  Let $(V,E,F)$ be a cyclic fan with hypermap $H$.  Let $v,w\in V$ be independent.  Assume that $C^0\{v,w\}\subset \Wdart(x)$ for all darts $x\in F$.  Then
\begin{itemize}
\item $(V[v,w],E[v,w],F[v,w])$ and $(V[w,v],E[w,v],F[w,v])$ are cyclic fans.  
\item Let $H[v,w]$ and $H[w,v]$ be their hypermaps, respectively.  Let $\rho:V\to\ring{R}$ be any function.  Then
$$
\sum_{v\in V} \rho(v)\op{azim}(H,v) = \sum_{v\in V[v,w]}\rho(v)\op{azim}(H[v,w],v) + \sum_{v\in V[w,v]}\rho(v)\op{azim}(H[w,v],v).
$$
\end{itemize}
\end{lemma}
\indy{Index}{slice}
...
Finally, it remains to be seen that the fan is cyclic.  Lemma~\ref{lemma:localization} already shows that the hypermap is isomorphic to $H_{2k}$. and that $F[v,w]$ can be identified with a face.  The condition $V[v,w]\subset \bWdart(x)$ follows from the fact that the angles $\beta(i)$ are increasing in Lemmas~\ref{lemma:circular}, \ref{lemma:lunar}, \ref{lemma:generic}.
\end{proof}
...
In the definition of a deformation, it is not required that $(V(t),E(t),F(t))$ is a cyclic fan for all $t\in I$, although this will usually be the case.  Let $H=(D,e,n,f)$ be the hypermap of $(V,E,F)$.  Extend $\rho:V\to V$ gives $\varphi(\phi v,t)\in V(t)$, for every $v\in V$.  
...
$I = \{t \mid 0 \le t < 1\}$
such that the spherical coordinates $(r,\phi,\theta)$  of $\phi(u,t)$ 
$$
r(u),\quad \phi(u),\quad 
\begin{cases} 
...
\begin{lemma}\guid{JASDHOQ}\rating{ZZ} Let $I = \{t\mid 0\le t\le t_1\}$ or $I=\{t\mid 0\le t < t_1\}$.  Let $\phi$ be a deformation of the cyclic fan $(V,E,F)$ over
...
Let $(V,E,F)$ be a cyclic fan.  Then the perimeter $\op{per}$ of the fan is at most $2\pi$.
\end{lemma}
\indy{Index}{fan!cyclic}
...
$A_i=\{p\mid \cdot p = a_i\}$, for some $u_i\in \ring{R}^n$ and $a_i\in\ring{R}$. 
...
See \cite[]{webster:1994} for the proof of the final conclusion.
\end{proof}
...
\begin{lemma}\guid{AMHFNXP}\rating{ZZ} Let $P$ be a bounded polyhedron with $0$ as an interior point.  Let $(V_P,E_P)$ be the associated fan.  There is a bijection between the facets of $P$ and the topological components of $Y(V_P,E_P)$ given by 
$$
...
\begin{lemma}\guid{BSXAQBQ}\rating{ZZ} Let $P$ be a bounded polyhedron with $0$ as an interior point.  Let $\op{hyp}(V_P,E_P)$ be the associated hypermap.  For every dart $x$ in the hypermap, $\op{azim}(x) < \pi$.
\end{lemma}
\indy{Index}{polyhedron}
