...
% submitted version on March 14, 2011 was svn 2316.
...
Finally, continuity arguments may be used, as
...
this dimension is unbounded as a function of $a$.  Fortunately, we can
view an element $a$ for which this degree at $v$ is large as a
degeneration of elements $a'$ with small degrees:
$\deg_w({a'}^*\DIV)\le 1$ for all $w\in X$.  This follows the
principle that a polynomial with repeated roots is a degeneration of
polynomials with simple roots.  When the degrees are at most $1$, the
affine Springer fibers are have manageable complexity.
%, and as it grows, 
...
arguments~\cite{BBDG:1982}.  Let $S$ be a scheme over $\bar k$.  The
support $Z$ of a simple perverse sheaf on $S$ is a closed irreducible
subscheme of $S$.  There is a smooth open subscheme $U$ of $Z$ and a
local system ${\mathcal L}$ on $U$ such that the simple perverse sheaf
can be reconstructed as a middle extension of the local system on $U$:
\[
...
on a dense open $U$, then they are in fact equal on all of $S$.  More
generally, if $Z$ is an irreducible scheme of finite type over $k$, in
order to show that two pure complexes on $Z$ are equal in the
...
The purpose of the support theorem (Theorem~\ref{lemma:support}) and
...
%It is known that the codimension $r$ of $\nu^*\A_H$ in $\A_G$ is given
%by the formula
%\[
%r = \dim \A - \dim \A_H =  (\dim\,G - \dim\,H) \deg(D)/2.
%\]
...
are pure local systems of
  weight $n$.
\end{itemize}
The support theorem (\ref{lemma:support}) and the strategies of
Section~\ref{sec:continuity} are used to find the pure local systems.
...
Following the strategy of Section~\ref{sec:continuity}, 
