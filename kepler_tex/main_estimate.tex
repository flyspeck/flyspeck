...
unstable and poorly behaved in many ways.  Numerical experiments show
that we can insert edges $\{u,v\}$ of length up to $3.2$.  Beyond that
length, instability prevails.  In fact, experiments suggest that
...
First of all, the upright quarters in the $Q$-system
need to be brought under control. 
The brick wall analogy explains the purpose of quarters in the
$Q$-system.  The idea is that a quarter
transfers volume into the Voronoi cell where it lies flat from a Voronoi
...
Second, the combinatorics undergo a preliminary simplification.
...
Third, after reducing to a single simple face is made, we investigate
...
edge conventions, then
    $$
    Z(3,1)<\op{sovo}_0(S,\lambda_{oct}) 
      \approx 0.00898,\quad
