...
The conclusion of Lemma~\ref{lemma:face} \case{bijection} holds for any minimal counterexample:
...
to $H_{2k}$, where $k= \card(F)$.
 Finally, $\op{azim}(x)$ and $\Wdart(x)$  the same
...
The proof that the $\op{azim}(x)$ and $\Wdart(x)$ are the same with respect to the two fans is a consequence of their definitions:
$$
\begin{array}{lll}
\op{azim}(x) &= \op{azim}(\orz,\v,\rho \v,\rho^{-1} \v),\\
\Wdart(x) &= \Wdart(\orz,\v,\rho \v,\rho^{-1} \v).\\
\end{array}
$$
where $x = (\v,\rho \v)$.  
...
The wedge $\bWdart(x)$ is close and contains $\Wdart(x)$. Hence, the
closure $\bar U_F$ is contained in $\bWdart(x)$.  Let $\v\in V'$.  
%There is a dart of $F$ of the form $y=(\v,\rho \v)$.  Since $y$ leads into 
Since the dart $y=(\v,\rho\v)$ of $F$ leads into
...
Let $(V,E,F)$ be a cyclic fan.  Let $k=\card(F)$.  Assume that for some $0<r\le k-1$, the set $W=\{\v,\rho \v,\ldots,\rho^r \v\}\subset V$ is contained in a plane $A$ passing through $\orz$.  Let $\e$ be the unit normal to $A$ in the direction $\v\times \rho \v$.  Then 
the set $W$ is cyclic with respect to $\{\orz,\e\}$, and the azimuth cycle restricted to $W\setminus \{\rho^r \v\}$ is equal to $\rho$.
\end{lemma}
...
\claim{For all $0\le i \le r-1$, it will be shown that $\sigma \v_i = \v_{i+1}$.}
Otherwise, there is some 
$$\u \in (W\setminus \{\v_i,\v_{i+1}\}) \cap W(\orz,\e,\v_i,\v_{i+1}) \cap A.
%  ~~\subset~~ 
$$
