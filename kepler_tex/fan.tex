...
\begin{definition}[fan,~blade]\guid{CODE}  
...
% \begin{definition}[retro-fan]\guid{CODE}
...
\begin{remark}[verifying the fan properties]\guid{CODE}\label{remark:fan-verify}  
...
\begin{definition}[X,~Y]\guid{CODE}\label{def:XY}
...
\begin{lemma}[]\guid{CODE}\label{lemma:node-vertex}
...
\begin{definition}[leads~into]\guid{CODE}
For each dart $x$, 
...
\begin{definition}[fully surrounded]\guid{CODE}
...
\begin{definition}[conforming]\guid{CODE}\label{def:conforming}
...
\begin{definition}[$N(V,E)$]\guid{CODE}
If $(V,E)$ is a fan,  set
\begin{displaymath}
N(V,E) = \sum_F (k_F - 3),
\end{displaymath}
...
\begin{definition}[minimally non conforming fan]\guid{CODE}
...
\begin{remark}[reduction data]\guid{CODE}\label{remark:reduction}
...
\begin{definition}[affine~set,~affine~hull,~affine~dimension,~affinely~independent,~hyperplane]\guid{CODE}\label{def:affine}
...
\begin{definition}[relative interior,~closure,~relative boundary]\guid{CODE}
Let
...
\begin{definition}[face,~facet,~edge,~extreme~point]\guid{CODE}
...
\begin{remark}[convex background]\guid{CODE}\label{rem:convex-background}
...
\begin{remark}[affine background]\guid{CODE}\label{rem:affine-background}
...
\begin{definition}[polyhedron]\guid{CODE}
...
\begin{lemma}[]\guid{CODE}\label{lemma:aff-u}
...
\begin{lemma}[]\guid{CODE}\label{lemma:facet-partition}
...
\begin{lemma}[]\guid{CODE}\label{lemma:simplex-poly}
...
\begin{definition}[$(V_P,E_P)$]\guid{CODE}
