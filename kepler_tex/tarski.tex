...
\begin{definition}[$t_0$]\tlabel{tarski:t0}  Let $t_0=1.255$.
\end{definition}
...
\begin{lemma}\guid{RHUFIIB}\tlabel{tarski:rhosquare}\rating{40}
...
\begin{lemma}\guid{JVUNDLC}\rating{40}
...
\begin{lemma}\guid{RCEABUJ}\rating{40}
...
\begin{lemma}\guid{BXVMKNF}\rating{60}
...
\begin{lemma}\guid{SMWTDMU}\rating{80}
...
\begin{lemma}\guid{JITVRKC}\rating{100}
...
\begin{lemma}\guid{SGFCDZO}\rating{80}
...
\begin{lemma}\guid{ZPGPXNN} \tlabel{tarski:triangle-eq}\rating{40}
...
\begin{lemma}\guid{FAFKVLR}\rating{80}
...
\begin{lemma}\guid{CNXIFFC}\tlabel{tarski:coef-plane-sign}\rating{80}
Let $S=\{v_1,\ldots,v_4\}$ be
a set of four points in $\ring{R}^3$.  Suppose
that $\{v_1,\ldots,v_3\}$ is not a collinear
set. 
...
\begin{lemma}\guid{MYOQCBS}\rating{30}
Let $S=\{v_1,\ldots,v_4\}$ be
a set of four points in $\ring{R}^3$.  Suppose
that $\{v_1,\ldots,v_3\}$ is not a collinear
set. 
...
\begin{lemma}\guid{TXDIACY}\tlabel{tarski:convex-ball}\rating{40}
Let $S$ be a set of four points in $\ring{R}^3$.
Let $r>0$ and $v_0\in\ring{R}^3$.  If $S\subset B(v_0,r)$, then
$\op{conv}(S)\subset B(v_0,r)$.\endnote{A simplex lies in a ball if its vertices do, tarski:convex-ball}
\end{lemma}
...
\begin{lemma}\guid{CDEUSDF}\tlabel{tarski:circum2}\rating{80}
...
\begin{lemma}\guid{WSMRDKN}\tlabel{tarski:circum-acute}\rating{60}
...
\begin{lemma}\guid{BYOWBDF}\tlabel{tarski:eta:mono}\rating{60}
...
\begin{lemma}\guid{BFYVLKP}\rating{60}
...
\begin{lemma}\guid{WDOMZXH}\tlabel{tarski:1453}\rating{40}
If $2\le y\le\sqrt8$, then $\eta(y,2,2.51) < 1.453$.
\end{lemma}

\begin{lemma}\guid{ZEDIDCF}\tlabel{tarski:eta245}\rating{40}
...
\begin{lemma}\guid{NHSJMDH}\tlabel{tarski:eta696}\rating{80}
...
\begin{lemma}\guid{HVXIKHW}\tlabel{tarski:eta-min}\rating{40}
...
\begin{lemma}\guid{HMWTCNS}\tlabel{tarski:eta-root3}\rating{40}
Let $S=\{v_0,v_1,v_2\}$ be a set of three points in $\ring{R}^3$.
...
\begin{lemma}\guid{EZHSEQJ}\tlabel{tarski:cm4}\rating{80}
...
\endnote{Delta equals Cayley-Menger det, tarski:cm4}
...
\begin{lemma}\guid{KGFFYWQ}\rating{40}
Let $v_1,\ldots,v_4$ be four points
in $\ring{R}^3$.  Let $x_{ij} = |v_i-v_j|^2$.  Then
	$$\Delta(x_{12},x_{13},x_{14},x_{34},x_{24},x_{23})\ge0.$$
\end{lemma}

\begin{lemma}\guid{POLFLZY}\label{tarski:delta0}\rating{80}
Let $v_1,\ldots,v_4$ be four points
in $\ring{R}^3$.  Let $x_{ij} = |v_i-v_j|^2$.
...
\begin{lemma}\guid{MAEWNPU} \tlabel{tarski:x12}\rating{40}
$f$  is a quadratic
polynomial in $x_{34}$ with leading coefficient $-x_{12}$. 
\end{lemma}

\begin{lemma}\guid{AGBWHRD}\rating{40}
...
\begin{lemma}\guid{CMUDPKT}\rating{60}
Assume~\tref{asm:cm4}.  Let $x'_{34}$ be the smaller root
of $f$.  Then
...
\begin{definition}[$\condcross$]\tlabel{def:condC}
\index{Index}{condC}
We write
	$$
...
\begin{lemma}\guid{CXWOCGN} \tlabel{tarski:cross}\rating{250}
...
\begin{lemma}\guid{THADGSB}  \tlabel{tarski:cross-edge-generic}\rating{100}
...
\begin{lemma}\guid{ZZSBSIO}\tlabel{tarski:mid-Voronoi}\rating{40}
...
\begin{lemma}\guid{JGYWWBX} \tlabel{tarski:336}\rating{40}
There does not exist a set
$S=\{v_1,v_2,w_1,w_2\}$ of four distinct points
in $\ring{R}^3$ with the following properties.\endnote{Four points, no two crossing segments one at most $\sqrt8$, the other at most $3.2$. tarski:336}
\begin{itemize}
...
\begin{lemma}\guid{PAHFWSI}\tlabel{tarski:delta-2}\rating{40}
...
\begin{lemma}\guid{UVGVIXB}\tlabel{tarski:307}\rating{40}
...
\begin{lemma}\guid{PJFAYXI}\tlabel{tarski:22}\rating{40}
Let $S=\{v_1,v_2,w_1,w_2\}$ be a set of points
in $\ring{R}^3$ of cardinality four. Assume
that the distance between any pair of distinct
...
\begin{lemma}\guid{YXWIPMH}\tlabel{tarski:311}\rating{40}
...
\begin{lemma}\guid{PAATDXJ}\tlabel{tarski:3488}\rating{40}
Let $S=\{v_1,v_2,v_3,v_4\}$ be a set of four distinct points in $\ring{R}^3$.  Assume that
the distances $y_{ij}=|v_i-v_j|$ are given by
	$$
...
%\begin{lemma}\guid{LDGXZGT}\tlabel{tarski:flat-Q}\usage{ lemma:voronoi-truncation-over-Q  }\rating{80}
...
%\end{lemma}
%
%\begin{proof}  The minimum of $\Delta$ subject to these
...
\begin{definition}[$\condface$]\tlabel{tarski:condF}
\index{Index}{condF}
Let
	$$
...
\begin{lemma}\guid{YJHQPAL} \tlabel{tarski:face}\rating{200}
...
\begin{definition}[$\condskinny$]\tlabel{tarski:condS}
\index{Index}{condS}  
Let 
	$$
...
\begin{lemma}\guid{MPXSJDI}\tlabel{tarski:skinny-tri}\rating{200}
...
%% XX Mention interval arithmetic?
...
\begin{lemma}\guid{VMZBXSQ}\tlabel{tarski:beta:dcg-p129}\rating{120}
...
\begin{lemma}\guid{QNXDWNU}\tlabel{tarski:beta:B}\rating{80}
Let $S=\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that the pairwise distances are at least $2$.
Assume that\endnote{Beta lemma, with 2.23 and 2.77 parameters, for alphabet simplex of type B, tarski:beta:B}
   $$
...
\begin{lemma}\guid{PNUMEHN}\tlabel{tarski:beta:dcg-144a}\rating{80}
Let $S=\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
...
\begin{lemma}\guid{HLAHAUS} \tlabel{tarski:beta:dcg-144b}\rating{80}
Let $S=\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that pairs of points have separation at least $2$.
Assume that
...
\begin{lemma}\guid{RISDLIH} \tlabel{tarski:beta:dcg-144c}\rating{140}
Let $S=\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that pairs of points have separation at least $2$.
Assume that
$|v_1-v_2|=2$ and $|v_1-v_3|\ge3.2$.
...
\begin{lemma}\guid{LTCTBAN}\rating{80}
$f$  is a quadratic
...
\begin{lemma}\guid{GJWYYPS}\rating{80}
The discriminant of the quadratic polynomial $f$ is
	$$
...
\begin{definition}[f]\tlabel{def:fSS}
\index{Index}{f@$f[S,S']$}
...
\begin{lemma}\guid{PFDFWWV}\rating{40}
Let $\{v_1,v_2,v_3,v_5\}$ be a set of five
points in $\ring{R}^3$.  
...
\begin{lemma}\guid{GDLRUZB}\rating{40}
Let $\{v_1,v_2,v_3,v_5\}$ be a set of five
points in $\ring{R}^3$.  
...
\begin{lemma}\guid{KGSNYDS}\rating{40}
Let $\{v_1,v_2,v_3,v_5\}$ be a set of five
...
\begin{lemma}\guid{YOEHSIY} \tlabel{tarski:cm5-small}\rating{40}
Let $\{v_1,v_2,v_3,v_5\}$ be a set of five
...
\begin{lemma}\guid{QHSEWMI}\tlabel{tarski:pass-cone}\rating{60}
...
Then $w_2\in\op{cone}(w_1,\{v_1,v_2,v_3\})$.\endnote{Five points, An edge passes through a triangle.  Then the endpoint of the edge
lies in $\op{aff}_+(w_1,\{v_1,v_2,v_3\})$. tarski:pass-cone}
\end{lemma}
...
\newpage
\begin{lemma}\guid{DOUFCOI}\rating{80}
\tlabel{tarski:no-pass-sqrt2}\usage{DCG-[4.21]{ lemma:no-pass-sqrt2}}
%{\bf Lemma I 3.2.}
Suppose that  $S=\{v_1,v_2,v_3,w_1,w_2\}$
...
\begin{lemma}\guid{UQQVJON}\tlabel{tarski:line-tri}\rating{200}
...
\begin{lemma}\guid{FRCXQKB} \tlabel{tarski:qrtet-pair-pass}\usage{DCG-[4.22]{ lemma:qrtet-pair-pass}}\rating{40}
%{Lemma 1.4}
...
\begin{lemma}\guid{ZHBBLXP}\rating{80}
...
Statements of this form will be immediate corollaries of Lemma~\tref{tarski:line-tri}.\endnote{{\tt Enclosed Calculations:} tarski:EE (generic name), tarski:245, tarski:245bis, tarski:277, tarski:2t0-doesnt-pass-through, tarski:E:part4:1, tarski:enclosed-v, tarski:last:E, tarski:pass-anchor }

\newpage
\begin{lemma}\rating{230}\label{tarski:EE}
We have 
   $$
...
\begin{lemma}\guid{VNZLYWT}\tlabel{tarski:dcg-1220}\rating{240}
...
\begin{lemma}\guid{VAXNRNE} \tlabel{tarski:pass-makes-quarter}\usage{DCG-[4.34]{ tarski:pass-makes-quarter}}\rating{100}
%{Lemma 1.9}
\FIXX{I'm not sure the second part gets used.  Deleting it would simplify the proof.}\endnote{Five points, a segment of length $\le \sqrt8$ passes through
an anchor.  Then it creates a quarter, tarski:pass-makes-quarter}
...
\begin{lemma}\guid{GMOKOTN}\tlabel{tarski:dcg-p122}\rating{120}
% DCG part of proof of Lemma 11.23.
% DCG big gap on anchors.
\FIXX{I'm not sure if the $|v_1-v_3|\le\sqrt8$ gets used in proof.}
\endnote{Five points, part of the erasing with penalty 0.008 proof, 
tarski:dcg-p122}
Let $S=\{v_0,v_1,v_2,v_3,v_4\}$ be a set of five points in 
...
%\begin{lemma}\guid{XILKJJV}\rating{80}
...
%Lemma \tref{tarski:no-pass-sqrt2}.
%
...
\begin{lemma}\guid{EFXWFNQ}\tlabel{tarski:cone-avoidance}\dcg{Lemma~8.18}{79}\rating{120}
Let $S=\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
...
\begin{lemma}\guid{LOKDUSU}\tlabel{tarski:sqrt2-unobstructed}\usage{DCG-[5.20]{ lemma:sqrt2-unobstructed}}\rating{80}
%DCG p48, L.5.20.
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
...
% OLD VERSION:
%\begin{lemma}\guid{TLKHAUY} \tlabel{tarski:sqrt2-unobstructed}\rating{80}
%DCG p48, L.5.20.
...
\begin{lemma}\guid{FFSXOWD}\tlabel{tarski:unobstr-t0}\usage{DCG-[5.22]{ lemma:unobstr-t0}}\rating{80}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that the pairwise distances are at least $2$.  Assume
that\endnote{Four points, Ball $B(v_0,t_0)$ does not meet a barrier. tarski:unobstr-t0}
$$
   |v_1-v_2|\le 2.51,\quad |v_2-v_3|\le 2.51,\quad |v_1-v_3|<\sqrt8.
$$
...
%\begin{lemma}\guid{OXYYCVX}\rating{80}
...
\begin{lemma}\guid{ZODWCKJ} \tlabel{tarski:v-interior}\usage{DCG-[4.16]{ lemma:v-interior}}\rating{80}
...
\begin{lemma}\guid{EIYPZVL}\tlabel{tarski:anc-simplex-not-enc}\rating{120}
...
\begin{proof}  As in Lemma~\tref{tarski:v-interior}, 
...
\begin{lemma}\guid{VZETXZC}\tlabel{tarski:old372}\rating{60}
Let $S=\{w,v_1,v_2,v_3,v_4\}$ be a set of five points in $\ring{R}^3$.
Assume that the separation between each pair in $S$ is at least
$2$.  Assume that\endnote{Five points, An enclosed vertex of height at most $\sqrt8$ over a triangle creates a pair of qrtets.  (No enclosed over triangle lemma.)  tarski:old372}
  $$
...
\begin{lemma}\guid{OFGJQUS}\tlabel{tarski:mk-point}\rating{70}
...
\begin{lemma}\guid{LFYTDXC}\tlabel{tarski:rog-exist}\rating{40}
Let $S=\{v_1,v_2,v_3,v_4\}$ be a set of four points
in $\ring{R}^3$.  Assume that the points are not coplanar.
Let $x_{ij} = |v_i-v_j|^2$, for $1\le i < j\le 3$.
...
\begin{lemma}\guid{TIEEBHT}\tlabel{tarski:rog-ortho}\rating{60}
Let $S=\{v_1,v_2,v_3,v_4\}$ be a set of four points
...
\begin{lemma}\guid{POXDVXO}\tlabel{tarski:eta-ortho}\rating{40}
...
\begin{lemma}\guid{ECSEVNC}\rating{60}
...
\begin{lemma}\guid{SRGTIHY}\tlabel{tarski:coeff-sign}\rating{80}
Let $S=\{v_1,\ldots,v_5\}$ be
a set of five points in $\ring{R}^3$.  Suppose
that $\{v_1,\ldots,v_4\}$ is not a coplanar
...
\begin{lemma}\guid{ZRFMKPY}\rating{40}
Let $S=\{v_1,\ldots,v_5\}$ be
...
\begin{lemma}\guid{COEBRMF}\tlabel{tarski:consec-anchors}\rating{60}
Let $S=\{v_0,v_1,v_2,v_3,v_4\}$ be a set of five points in $\ring{R}^3$.
Assume that the pairwise separations are at least $2$.
Assume that\endnote{Five points, conditions on when two points $v_2$ $v_4$ are consecutive in the cyclic order around an upright diagonal $v_0,v_1$. tarski:consec-anchors}
...
\begin{lemma}\guid{JVDAFRS}\tlabel{tarski:over-under}\rating{40}
...
\begin{lemma}\guid{ARIKWRQ} \tlabel{tarski:hedra-tope}\rating{40}
...
\begin{lemma}\guid{MXHKOXR}\rating{40}
Let $S=\{v_1,\ldots, v_4\}$ be a set of four points
in $\ring{R}^3$.  Suppose that $S$ is not coplanar.
...
\begin{lemma}\guid{TYUNJLA}\tlabel{tarski:rog-lemma}\rating{40}
...
\begin{lemma}\guid{SHOGYBS}\tlabel{tarski:rho-sign}\rating{20}
Let $S=\{v_1,v_2,v_3,v_4\}$ be
...
\begin{lemma}\guid{VBVYGGT}\tlabel{tarski:circumcenter}\rating{80}
Let $S=\{v_1,v_2,v_3,v_4\}$ be a set of four points in $\ring{R}^3$.
...
\begin{lemma}\guid{GDRQXLG}\rating{60}
Let $S=\{v_1,v_2,v_3,v_4\}$ be a set of four points in $\ring{R}^3$.
Assume that $S$ is not a coplanar set.  
Let $x_{ij}=|v_i-v_j|^2$.
...
\begin{lemma}\guid{ZJEWPAP}\tlabel{tarski:eta-rad}\rating{50}
Let $S=\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
...
\begin{lemma}\guid{VSMPQYO} \tlabel{tarski:chi}\usage{DCG-[5.15]{ lemma:chi}}\rating{40}
Let $S=\{v_1,\ldots,v_4\}$ be a set of four points
in $\ring{R}^3$.  
...
\begin{lemma}\guid{PVLJZLA}\rating{40}
...
\begin{lemma}\guid{IAALCFJ} \tlabel{tarski:at-most-one-negative}\rating{80}
%proclaim{Lemma 2.1}
Let $S=\{v_1,v_2,v_3,v_4\}$ be a set of four points in $\ring{R}^3$.
Assume that 
   $$
...
\newpage
\begin{lemma}\guid{YOEEQPC}\rating{80}
\tlabel{tarski:neg-orient-quad}\usage{DCG-[5.16]{ lemma:neg-orient-quad}}
%\proclaim{Lemma 2.2}
Let $S=\{v_1,v_2,v_3,v_4\}$ be a set of four points in $\ring{R}^3$.
Assume that the pairwise distances are at least $2$.
Let $F=\{v_2,v_3,v_4\}$.\endnote{Four points, A negative orientation on a simplex, forces edges $\le 2.51$.  Quarter case.  tarski:neg-orient-quad}
...
\newpage
\begin{lemma}\guid{YJTLEGD}\rating{60}
\tlabel{tarski:neg-orient-tet}\usage{DCG-[5.17]{ lemma:neg-orient-tet}} Let $S=\{v_0,v_1,v_2,v_3\}$ be a
...
\newpage
\begin{lemma}\guid{NJBVVWG}\tlabel{tarski:sqrt2-chi+}\usage{DCG-[5.18]{lemma:sqrt2-chi+}}\rating{50}
%
...
% Lemma for GLQHFGH.
\begin{lemma}\guid{VUYPDCF}\tlabel{tarski:bounded-tri}\rating{110}
...
\begin{lemma}\guid{GLQHFGH}\tlabel{tarski:vor-bar}\rating{80}
Let $S=\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
...
\newpage
\begin{lemma}\guid{SGUCIKJ}\tlabel{tarski:vor-bar-sqrt2}\rating{40}
Let $\{v_1,v_2,v_3,v\}$ be a set of four points in $\ring{R}^3$. Assume that the pairwise
distances are at least $2$.
Assume that they are not collinear.\endnote{Four points, Voronoi, If the Voronoi cell meets a face hull, then the face has circumradius at least $\sqrt2$.  tarski:vor-bar-sqrt2}
Let $S=\{v_1,v_2,v_3\}$.
...
Lemma~\tref{tarski:sqrt2-chi+}.
\end{proof}

\begin{lemma}\guid{TCQPONA}\tlabel{tarski:vor-bar-tet}\rating{40}
Let $S=\{v_1,v_2,v_3\}$ be a
set of three points in $\ring{R}^3$.  Assume that\endnote{Four points, If a face hull meets a Voronoi cell, then distances are small. tarski:vor-bar-tet}
  $$
...
\begin{lemma}\guid{CEWWWDQ}\tlabel{tarski:vor-bar-quad}\rating{40}
Let $S=\{v,v_1,v_2,v_3\}$ be a
set of four points in $\ring{R}^3$.\endnote{Four points, If a Voronoi cell at a vertex of a simplex meets the opposite face, then distances are small.  tarski:vor-bar-quad}
...
\begin{lemma}\guid{CKLAKTB}\tlabel{tarski:decouple-orient}\rating{160}
...
\begin{lemma}\guid{XBNRPGQ}\tlabel{tarski:dcg-page82}\rating{100}
...
with respect to $S$ (by Lemma~\tref{tarski:sqrt2-chi+}). 
Thus, $c_S$ lies along
...
respect to $S$.  Again, by Lemma~\tref{tarski:sqrt2-chi+}, the
...
\begin{lemma}\guid{MITDERY}\tlabel{tarski:rog-in-S}\rating{80}
\FIXX{This was used in DCG Lemma~9.7, but is no longer.
...
\begin{lemma}\guid{BAJSVHC}\tlabel{tarski:split-lune}\rating{80}
...
%% Lemma for UREVUCX.
\begin{lemma}\guid{JGXXUKH}\tlabel{tarski:three-lune}\rating{100}
...
%% Helper for UREVUCX.
\begin{lemma}\guid{TQLZOUG}\tlabel{tarski:omega-cone-rog}\rating{80}
...
\begin{lemma}\guid{UREVUCX}\tlabel{tarski:omega-rog}\rating{80}
Suppose that $S=\{v_0,v_1,v_2,v_3\}$ is a set
of four points in $\ring{R}^3$.  Suppose that 
the orientations of $v_1$, $v_2$, and $v_3$ are positive with respect
to $S$.  Suppose that 
...
\begin{lemma}\guid{RKWVONN}\tlabel{tarski:tip-cone}\rating{80}
Suppose that $S=\{v_0,v_1,v_2,v_3\}$ is a set
of four points in $\ring{R}^3$.  Suppose that
the orientations of $v_1$, $v_2$, and $v_3$ are positive with respect
to $S$.  Suppose that\endnote{Four points,  The tip of a Voronoi cell lies in the cone over the qrtet or quarter or simplex. tarski:tip-cone}
  $$
  2 \le |v_i - v_j | < \sqrt8, \quad 0\le i < j \le 3.
  $$
Then
...
\begin{lemma}\guid{DKCSJPZ}\rating{80}
...
\begin{lemma}\guid{KMTAMFH}\tlabel{tarski:vor-bar-23}\rating{160}
...
\begin{lemma}\guid{JHOQMMR}\tlabel{tarski:eps-bigd-}\rating{120}
 Let $S=\{v_0,v_1,u_1,u_2\}$ be a set of four points
in $\ring{R}^3$.\endnote{Four points, if a blade meets a cone, then distances are small.  (For epsilon stuff).  tarski:eps-bigd-}
...
\begin{lemma}\guid{YFTQMLF}\tlabel{tarski:eps:fine:Rw}\rating{120}
\usage{DCG-[9.8]} % fine decomposition page 89.
 Let $S=\{v_0,v_1,u_1,u_2\}$ be a set of four points
in $\ring{R}^3$.\endnote{Four points, If a blade meets a Rogers, then distances are small.  For fitted crowns. tarski:eps:fine:Rw}
Suppose that all distances between pairs in $S$ are at least $2$.
Suppose all distances between pairs in $S$, except possibly $\{v_1,u_1\}$
$\{v_1,u_2\}$, are
less than $\sqrt8$.
...
\begin{lemma}\tlabel{tarski:fine:Rw:5}\usage{DCG-[9.8]}\rating{220}
...
%% Use in fine decomposition. Extracted
% Fine decomposition page 89.
\begin{lemma}\tlabel{tarski:fine:Rw:u}\usage{DCG-[9.8]}\rating{320}
 Let $S=\{v_0,v_1,w,u_1,u_2\}$ be a set of five points
in $\ring{R}^3$.  
Suppose that all distances between pairs in $S$ are at least $2$.
Suppose that 
...
$S'$ is a quarter.  By tarski\tarf{tarski:tip-cone}, we have
...
it follows that $x\in \op{aff}_-(\{v_0,v_1,w'\},u)$.  By tarski\tarf{tarski:tip-cone}, we have $x\in\op{cone}(u_1,\{v_0,v_1,w'\})$.
As above, $u_2\in\op{aff}_+^0(\{v_0,u_1\},\{v_1,w'\})$. 
It follows that we are in the third case of the lemma.\FIXX{Add Details.}
%By tarski\tarf{tarski:prev},
...
\begin{lemma}\guid{UJCUNAS}\tlabel{tarski:fine-Rw-split}\rating{80}
\usage{DCG-[9.8]} % Fine decomposition page 89.
 Let $S=\{v_0,v_1,w,u_1,u_2\}$ be a set of five points
in $\ring{R}^3$.\endnote{Five Points, If a Rogers meet a blade then strange things happen. tarski:fine-Rw-split}
Suppose that all distances between pairs in $S$ are at least $2$.
Suppose that 
   $$
   \begin{array}{rlrlrll}
...
\newpage
\begin{lemma}\guid{DVLHHMF}\tlabel{tarski:prev}\usage{DCG-[9.7]{lemma:prev}}\rating{160}
Let $S=\{v_0,v,w,w',u\}$ be a set of five points in $\ring{R}^3$.
Assume that all pairs of points are separated by distance at
least $2$. Assume that $2.51<|v_0-v|<\sqrt8$, $2.51<|w-w'|$, and\endnote{Five points, for fitted crowns, if a Rogers has a bad epsilon, then a segment crosses through a triangle, tarski:prev}
  $$
...
\begin{lemma}\guid{UIXOFDB}\tlabel{tarski:eps-inner}\usage{lemma:FC-no-over}\rating{80}
Let $S=\{v_0,v_1,v_1',w_1,w_2\}$ be a set of five points in $\ring{R}^3$.
Assume that the pairwise distances are at least $2$.  Assume
that\endnote{Five points, disjointness of a Rogers simplex with an rcone.  
For fitted crown  stuff.  tarski:eps-inner}
  $$\begin{array}{lll}
...
\begin{lemma}\guid{MJNUTQH}\tlabel{tarski:eps-outer}\usage{lemma:FC-no-over}\rating{140}
Let $S=\{v_0,v_1,v_2,w_1,w_2\}$ be a set of five points in $\ring{R}^3$.
Assume that the pairwise distances are at least $2$.  Assume
that\endnote{Five points: If two Rogers meet, then strange things happen.  For fitted crowns. tarski:eps-outer}
  $$\begin{array}{lll}
...
$$
By tarski\tarf{tarski:tip-cone}, we have
$$
...
\begin{lemma}\guid{UMMNOJN}\tlabel{tarski:decouple}\rating{80}
\FIXX{Revisit this lemma.  Move it right after 
 [CKLAKTB], Lemma~\tref{tarski:decouple-orient}}\endnote{Five points, a segment meets a blade, then a segment meets the convex hull of the three points giving blade, tarski:decouple}
...
\begin{lemma}\guid{BIQFYVG}\tlabel{tarski:back}\usage{DCG-[5.31]{ lemma:back}}\rating{80}
\FIXX{Revisit}
...
\begin{lemma}\guid{TEAULMK}\usage{DCG-[4.15]{ lemma:v-interior-alt}}\guid{GEWKDPE} \tlabel{tarski:v-interior-alt}\rating{80}
...
\begin{lemma}\guid{MPJEZGP}\tlabel{tarski:quad-types}\rating{80}
Let $S=\{v_0,v_1,v_2,v_3,v_4\}$ be a set of five points in
$\ring{R}^3$.
Assume that
   $$
...
\begin{lemma}\guid{GFVQUPP}\tlabel{tarski:d4}\rating{80}
...
\begin{lemma}\guid{TFKALQL}\tlabel{tarski:d4bis}\rating{80}
Let $S=\{v_0,v_1,v_2,v_3\}$ be a set of four
points in $\ring{R}^3$.  Suppose that pairwise separations
are at least $2$. 
Let $x_{ij}=|v_i-v_j|^2$.
...
\begin{lemma}\guid{AAGNQFL}\label{tarski:flat-then-convexq}\rating{60}
...
\begin{lemma}\guid{QOKQFRE}\rating{60}
Let $S=\{v_0,v_1,v_2,v_3,v_4\}$ be a set of
five points in $\ring{R}^3$.  Supose that the pairwise
distances are at least $2$.  Suppose that the pairwise
distances are at most $2.51$ for distinct $u,v\in S$, with
...
\begin{lemma}\guid{DFLUMBW}\rating{80}
% qcone well-defined.
...
\begin{lemma}\guid{HTYDGWI}  \tlabel{tarski:skew-quad}\usage{DCG-[4.30]{ lemma:skew-quad}}\rating{80}
%\proclaim{Lemma 1.6}
Let $\{v_0,v_1,v_2,v_3,v_4\}$ be a set of five points in
$\ring{R}^3$.  Suppose that the parwise separations are at
least $2$.\endnote{Five points, Quad, two diagonals of length at most $\sqrt8$ cross, then you have a quad cluster. tarski:skew-quad}
...
%\begin{lemma}\guid{XMSECYQ}\rating{80}
...
\begin{lemma}\guid{XLHACRX} \tlabel{tarski:oct-t0}\usage{DCG-[4.31]{ lemma:oct}}\rating{65}
%\proclaim{Lemma 1.7}
...
\begin{lemma}\guid{ZTFABJM}\tlabel{tarski:gc}\usage{DCG-[10.12]{ lemma:gc}}\rating{400}
...
% Lemma 2.2.
\begin{lemma}\guid{VTIVSIF}\tlabel{tarski:enclosed}\usage{DCG-[10.13]{ lemma:enclosed}}\rating{180}
...
\begin{lemma}\guid{TPXUMUZ}\tlabel{tarski:4circuit}\rating{160}
...
\begin{lemma}\guid{YWPHYZU} \tlabel{tarski:double-face}\usage{DCG-[4.32]{ lemma:double-face}}\rating{160}
%proclaim{Lemma 1.8}
Let $\{v_0,v_1,v_2,v_3,v_4,v_5\}$ be a set of six points in
$\ring{R}^3$.  Suppose that pairwise distances are at least $2$.\endnote{Six points, if a segment pases through two anchors, then it is longer than $\sqrt8$. tarski:double-face}
...
\begin{lemma}\guid{PYURAKS} \tlabel{tarski:single-enclosed}\usage{DCG-[4.33]{ lemma:single-enclosed}}\rating{80}
%proclaim{Lemma 1.11}
...
\begin{lemma}\guid{MLTDJJV}\tlabel{tarski:lin-sim-6-not}\rating{200}
...
\begin{lemma}\guid{WKMPKQS}\rating{80}
...
\begin{lemma}\guid{ZDKDXFM}\tlabel{tarski:lin-sim-6}\rating{80}
...
\begin{lemma}\guid{XHMHKIZ}\tlabel{tarski:lin-sim-5}\rating{60}
...
\begin{lemma}\guid{TIMIDQM}\tlabel{tarski:tri-tri-6}\rating{120}
...
By Lemma~\tref{tarski:no-pass-sqrt2},
...
\newpage
 ...
 % NEW, NOT IN DCG. 
\begin{lemma}\guid{KGTJGLX}\tlabel{tarski:tri-tri-5}\rating{80}
...
\begin{lemma}\guid{RTBONNT}\tlabel{tarski:tri-sim-7}\rating{120}
...
\begin{lemma}\guid{JMHCAKG}\tlabel{tarski:tri-sim-6}\rating{200}
...
\begin{lemma}\guid{UGQMJJA}\rating{160}\tlabel{tarski:tri-sim-5}
...
\begin{lemma}\guid{ZILQMDQ}\tlabel{tarski:sim-sim-8}\rating{80}
...
\begin{lemma}\guid{AQKANYN}\tlabel{tarski:sim-sim-7}\rating{160}
...
\begin{lemma}\guid{IHENCNY}\tlabel{tarski:sim-sim-6}\rating{160}
%% DCG isolated pair.
...
\begin{lemma}\guid{DWGDFMQ}\tlabel{tarski:sim-sim-5}\rating{80}
...
\begin{lemma}\guid{CZXNFAQ}\tlabel{tarski:sim-sim-qrtet}\rating{80}
...
\begin{lemma}\guid{BYUOUXO}\tlabel{tarski:qrtet-over}\usage{DCG-[4.35]{ lemma:qrtet-over}}\tlabel{tarski:sim-sim-qrtet-special}\rating{140}
Let $S_V=\{v_1,v_2,v_3,v_4\}$ be a set of four
points in $\ring{R}^3$.  Let $S_W=\{w_1,w_2,w_3,w_4\}$ be a set
of four points in $\ring{R}^3$.  Assume that that distances
between points in $S_V\cup S_W$ are at least $2$.  Assume that
...
This implies by Lemma~\tref{tarski:no-pass-sqrt2} that 
$\eta_V(v_2,v_3,v_4)\ge\sqrt2$.
...
\begin{lemma}\guid{ZYMHOWN} \tlabel{tarski:oct-over}\usage{DCG-[4.36]{ lemma:oct-over}}\rating{80}
...
\begin{lemma}\guid{OOHHKOI}\tlabel{tarski:adj-over}\usage{DCG-[4.37]{ lemma:adj-over}}\rating{80}
...
\begin{lemma}\guid{VXMZWGM}\tlabel{tarski:iso-over}\usage{DCG-[4.39]{ lemma:iso-over}}\rating{80}
If the interior of an isolated strict quarter $Q$ meets the
interior of another strict quarter, then the diagonal of $Q$ has
exactly three anchors.\endnote{If an isolated strict quarter meets the interior of another strict quarter, then three anchors.  Used for geometry of isolated pairs.  tarski:iso-over}
\end{lemma}
...
\begin{lemma}\guid{FAGLBKX}\rating{80}
    \oldlabel{3.8.2}
...
\begin{lemma}\guid{EANBDBV}\rating{80}
...
\begin{lemma}\guid{QMRAQFC}\tlabel{tarski:BCDE}\usage{Extracted from Volumes chapter, quoin
volume}\rating{80}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
...
\begin{lemma}\guid{RAJHSDQ}\tlabel{tarski:BCEF}\usage{Extracted from Volumes chapter, quoin volume}\rating{80}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that $\{v_0,v_1,v_2,v_3\}$ is not coplanar.  Let $p$
be the circumcenter of $\{v_0,v_1,v_2\}$, and $r$ its circumradius.  
Let $p'$ be the
...
\begin{lemma}\guid{XQXFRZY}\tlabel{tarski:ABCD}\usage{Extracted from Volumes chapter, quoin volume}\rating{80}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that $\{v_0,v_1,v_2,v_3\}$ is not coplanar.  Let $p$
be the circumcenter of $\{v_0,v_1,v_2\}$, and $r$ its circumradius.  
Let $p'$ be the
...
\begin{lemma}\guid{ZULMEGV}\tlabel{tarski:ABCE}\usage{Extracted from Volumes chapter, quoin volume}\rating{80}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that $\{v_0,v_1,v_2,v_3\}$ is not coplanar.  Let $p$
be the circumcenter of $\{v_0,v_1,v_2\}$, and $r$ its circumradius.  
Let $p'$ be the
...
\begin{lemma}\guid{WJFHKSY}\tlabel{tarski:wedge-union}\usage{lemma:sovo:truncRog}\rating{80}
Let $\{v_0,v_1,v_2,v_3,w\}$ be a set of five points in $\ring{R}^3$.
...
\begin{lemma}\guid{FKFJBPG}\tlabel{tarski:rogers-ball}\usage{lemma:sovo:truncRog}\rating{60}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
...
\begin{lemma}\guid{KRETJIK}\tlabel{tarski:rogers2}\usage{lemma:sovo:truncRog}\rating{80}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that the set is not coplanar.
...
\begin{lemma}\guid{KMQHURS}\tlabel{tarski:rcone-ball}\usage{lemma:sovo:truncRog}\rating{60}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that the set is not coplanar.
...
\begin{lemma}\guid{GBEJKWT}\tlabel{tarski:rogers-FR}\usage{lemma:sovo:truncRog}\rating{80}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that the set is not coplanar.
...
\begin{lemma}\guid{KCNGLUP}\tlabel{tarski:rogers-rad}\usage{lemma:sovo:truncRog}\rating{80}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
Assume that the set is not coplanar.
Let $\eta_V(v_0,v_1,v_2)\le b \le c$.  
Let $q_b$ (resp. $q_c$) be the point
...
\begin{lemma}\guid{WBHWSGD}\tlabel{tarski:CC}\usage{lemma:sovo:CC}\rating{80}
...
\begin{lemma}\guid{DJPAABI}\tlabel{tarski:CCbar}\usage{lemma:sovo:CC}\rating{80}
Let $\{v_0,v_1\}$ be a set of two points in $\ring{R}^3$>
let $t,\mu > 0$.  Set $y=|v_0-v_1|$
and $r=\op{cosarc}(y,t,\mu)$.
Let $C=\op{rcone}^0(v_0,v_1,r)$.
...
\begin{lemma}\guid{STPHWLD}\tlabel{tarski:rCCinvert-rad}\usage{sec:inverted}\rating{80}
...
\begin{lemma}\guid{JNNCRMJ}\label{tarski:2CCrad}\rating{80}
...
\begin{lemma}\guid{LIBCELL}\label{tarski:AA'}\usage{lemma:2tcc}\rating{80}
...
\begin{lemma}\guid{RJUGGRQ}\label{tarski:rcone2}\usage{lemma:2tcc}\rating{80}
Let $\{v_0,w_1,w_2\}$ be a set of three points in $\ring{R}^3$.
...
\begin{lemma}\guid{EFDFOTF}\label{tarski:RCFR}\usage{lemma:sovo:CR}\rating{80}
Let $\{v_0,v_1\}$ be a set of two points in $\ring{R}^3$.
...
\begin{lemma}\guid{TMDMILH}\label{tarski:dot-pos}\rating{40}
...
\begin{lemma}\guid{TDVIYHO}\label{tarski:miss-plane}\usage{lemma:dart-curve}\rating{80}
...
\begin{lemma}\guid{EZLYYDN}\label{tarski:subvertex}\usage{dist2,hypermap.tex}\rating{40}
...
\begin{lemma}\guid{NLZKMES}\tlabel{tarski:cone4.12.1}\rating{80}
%    \oldlabel{4.12.1}
Let $\{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
...
%\begin{lemma}\guid{WIRABFB}\tlabel{tarski:4.12.1}\rating{80}
%%    \oldlabel{4.12.1}
...
\newpage
\begin{lemma}\guid{CNISQSN}\tlabel{tarski:old4.12.2}\rating{80}
%%    \oldlabel{4.12.2}
...
\begin{lemma}\guid{OXAYQXU}\label{tarski:cone-meet}\rating{80}
Let $\{v_0,v,w\}$ be a set of three points in $\ring{R}^3$.  Let $t,\lambda,\lambda'>0$.  If \endnote{Three points, corner cells do not meet, tarski:cone-meet}
