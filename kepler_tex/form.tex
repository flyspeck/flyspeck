...
%Let $j\in\{1,\ldots,4\}$ be the number of quarters.
A quarter satisfies the inequality, for $j\in\{1,\ldots,4\}$:
$$
...
In the case $j\le 2$, we may assume that all the cells around the spine are $4$-cells.  Otherwise, we argue as follows.  Call a blade small if its two edges that are not spinal have length at most $2h_-$.  If there is a group $B$ of $2$- and $3$-cells along two small blades, then a calculation gives
$$
