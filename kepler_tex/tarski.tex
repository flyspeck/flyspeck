...
%% Next actions:
% expand macros for condS, condF, condA, condC.
% mark definitions at beginning.
% Finish filling in name, summary, tag fields.  Search for "tag{}" to find the next empty one.
% Separate out the Blueprint from this collection.
...
% Introduce some higher level abbreviations such as anchor, packing, t0, diagonal. Rewrite accordingly.
% Type out formal statements.
% Export to html.  (Automate the process.)
...
\def\filetarskiaway{\closeout\mywrite}
...
\marker{condC, condF, condS, ball, rcone, collinear, bis, rad, eta, beta, epsilon, arc, anchor, oct, rogers}{A term with the same name appears.}

\marker{ray, halfplane, blade, halfspace, lune, cone}{Involves $\op{aff}_+(S,S')$, with $(\#S,\#S')=(1,1),(2,1),(1,2),(3,1),(2,2),(1,3)$, respectively.}


\marker{segs-meet}{Involves the intersection of two segments.}
\marker{triseg-meet}{Involves the intersection of a segment with the convex hull of three points.}
\marker{aff-meet}{Involves the intersection of affine sets.}
\marker{inside}{Involves a point insides a region formed by a convex hull, a lune, or a cone.}
...
\subsection{Cayley-Menger}
\name{cm4}
\summary{Delta can be expressed as a Cayley-Menger determinant.}
\tag{cm4-delta, pt4}
\rating{80}
\guid{EZHSEQJ}
...
Define $\condA(v_1,\ldots,v_4,x_{ij})$ to be the following condition:
There exist
...
\begin{lemma}
Assume $\condA(v_1,\ldots,v_4,x_{ij})$.
...
\begin{tarski}
\name{def:condC}
\summary{Several lemmas rely on a technical hypothesis $\condcross$, which has been formlated as a definition.}
...
\name{condF}
\summary{Several lemmas rely on a technical hypothesis $\condface$, which has been separated out as a lemma.}
...
\name{beta:B}
\summary{}
%\endnote{Beta lemma, with 2.23 and 2.77 parameters, for alphabet simplex of type B, tarski:beta:B}
\tag{}
\rating{80}
\guid{QNXDWNU}
...
\begin{proof} \FIXX{Recheck.  Lemma wording was changed.} Assume for a contradiction that they meet at $x$.
We have
...
\name{unobstr-t0}
\summary{}
%\endnote{Four points, Ball $B(v_0,t_0)$ does not meet a barrier. tarski:unobstr-t0}
\tag{}
\rating{80}
\guid{FFSXOWD}
...
\name{old372}
\summary{}
%\endnote{Five points, An enclosed vertex of height at most $\sqrt8$ over a triangle creates a pair of qrtets.  (No enclosed over triangle lemma.)  tarski:old372}
\tag{}
\rating{60}
\guid{VZETXZC}
...
\begin{tarski}
\name{conv-halfspace}
\summary{In general, the convex hull of four point is the intersection of the four halfspaces determined by the four faces.}
...
\begin{proof} By Lemma~\tref{tarski:rho-ups},
...
\name{vor-bar-sqrt2}
\summary{}
%\endnote{Four points, Voronoi, If the Voronoi cell meets a face hull, then the face has circumradius at least $\sqrt2$.  tarski:vor-bar-sqrt2}
\tag{}
\rating{40}
\guid{SGUCIKJ}
...
\name{vor-bar-tet}
\summary{}
%\endnote{Four points, If a face hull meets a Voronoi cell, then distances are small. tarski:vor-bar-tet}
\tag{}
\rating{40}
\guid{TCQPONA}
...
\name{vor-bar-quad}
\summary{}
%\endnote{Four points, If a Voronoi cell at a vertex of a simplex meets the opposite face, then distances are small.  tarski:vor-bar-quad}
\tag{}
\rating{40}
\guid{CEWWWDQ}
...
\name{tip-cone}
\summary{}
%\endnote{Four points,  The tip of a Voronoi cell lies in the cone over the qrtet or quarter or simplex. tarski:tip-cone}
\tag{}
\rating{80}
\guid{RKWVONN}
...
\begin{lemma}\guid{TEAULMK}\usage{DCG-[4.15]{ lemma:v-interior-alt}} \tlabel{tarski:v-interior-alt}
