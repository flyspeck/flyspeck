...
\begin{remark}\guid{TIWWFYQ}\rz{100}
...
\begin{definition}[bisector]\guid{BGXHPKY}
\formaldef{$A$}{bis}
...
\begin{lemma}[]\guid{RHWVGNP}\rz{200}\label{lemma:V4} 
...
\begin{remark}\guid{ZHIQGGN}\rz{0}\label{remark:precise} 
The precise meaning of the
\newterm{sphere packing problem} or the \newterm{Kepler conjecture} 
...
\begin{lemma}[]\guid{KHEJKCI}\rz{150}\label{lemma:omega-face}  
...
\begin{lemma}[]\guid{IDBEZAL}\rz{250}\label{lemma:omega-facet} 
...
\begin{lemma}[]\guid{GLTVHUM}\rz{700} 
...
\begin{lemma}[]\guid{DUUNHOR}\rz{300}  \label{lemma:R-inter} 
...
\begin{lemma}[]\guid{QXSKIIT}\rz{500}\label{lemma:affine-system} 
...
\begin{lemma}[]\guid{OAPVION}\rz{100} 
...
\begin{lemma}[]\guid{XYOFCGX}\rz{500}\label{lemma:sqrt2-close} 
...
\begin{lemma}[]\guid{XNHPWAB}\rz{500}\label{lemma:v2} 
Let $V$ be a saturated packing.
...
\begin{lemma}[]\guid{YIFVQDV}\rz{500}   \label{lemma:perm-Vk} 
...
\begin{lemma}[]\guid{KSOQKWL}\rz{200} 
...
\begin{lemma}[]\guid{IVFICRK}\rz{300}\label{lemma:coset-bijection} 
...
\begin{lemma}[]\guid{WQPRRDY}\rz{450}\label{lemma:Rconv}  
...
are coarsely tuned, suitable for rough hewing.  Delaunay simplices articulate the
foreground; Voronoi cells recede into background.  The solution to the problem
lies in the right balance between foreground and background.  Too many
Delaunay simplices and the details overwhelm.  Too many Voronoi cells
and the estimates become too weak.  The central geometrical insights
of the original proof are expessed as rules that delineate foreground
...
\end{eqnarray*}
The issue is to prove it negligible.  In view of future applications, 
...
$M(h)-L(h)$ lying in the interval $[1.231,1.232]$.
...
\begin{proof} Consider a finite packing $ V=\{\u_1,\ldots, \u_N\}\subset \BB$ satisfying
...
that this enlarged set has cardinality less than $16$.  Set $h_i =
