...
Voronoi cells when the correction term $A$ is fcc-compatible.
...
\begin{note}%XX
Replace $\Delta_0/\pi =\Delta_1$ to simplify formulas.
\end{note}
...
\begin{proof} 
It is enough to show that $A(\lam_0)  = -\op{vol}(\Omega(\lam_0)) + 8 m_1 - \sum 8 m_2 M(h(\bl))$ is fcc-compatible and negligible.  
%The result then follows from Lemma~\ref{lemma:deltabound}.  
The function $A$ is fcc-compatible directly
...
The function $A_L$  is fcc-compatible directly
from equation~\eqn{eqn:km}
and Conjecture~\ref{conj:L12}.  

The theorem follows from a proof that $A_L$ is negligible.  More precisely, one needs
...
Details about global nonlinear optimization appear in the appendix.   The method is linear relaxation, which can be briefly described as follows.\footnote{Linear relaxation appears again later in the solution of significantly more difficult nonlinear optimzation problems.}  The domain $D$ is partitioned into finitely many subsets $D_1,\ldots, D_r$.   The positivity of $f$ is
...
Instead of the single inequality asserted by Lemma~\ref{lemma:cluster},  a collection of inequalities of the following form are established:
...
\begin{proof}  If $P=P(\Lam,g)$ is unbounded, pick $p\in P$ such that
...
$$
\begin{array}{lll}
4\pi &= \sum_i\op{area}(P_i)\\
     &\ge \sum_i A(a(h_i),k_i) \\
...
for the arcradius of this disk.  One can verify directly that~\cite[]{hales:2009:nonlinear} %% Linear lower bound on regular polygon (large disk) % cc:alin2
$$A(a',k) \ge c_0 + c_1 k + c_2 L(1) + c_3$$
where $c_3 = 0.85$.
Then 
$$
\begin{array}{lll}
4\pi &= \sum_i\op{area}(P_i)\\
     &\ge A(a',k_1)+\sum_{i>1} A(a(h_i),k_i) \\
