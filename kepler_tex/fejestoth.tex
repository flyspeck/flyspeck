...
\begin{lemma}\guid{VVEESTP}\rating{XX}  The strong dodecahedral conjecture implies the dodecahedral conjecture.
\end{lemma}
...
\begin{lemma}\guid{QQXNUBP}\rating{XX}  Two $D_k$-cells either intersect in a set of measure zero or coincide.  The union of the $D_k$-cells is $\Omega(\lambda_0)\cap B$.
\end{lemma}
...
\begin{lemma}\guid{HJDWBBR}\rating{XX} The surface area of $\Omega(\lam_0)$ is at least that
...
\begin{conjecture}[local inequality]\guid{WMEEWJQ}\rating{XX}  For any $D_k$-cell $X$
$$
...
\begin{lemma}\guid{GNNKBUK}\rating{XX}  The local inequality and the kissing number estimate
$$
\sum L(h) \le 12
$$
...
\begin{conjecture}\guid{BDEDUTL}\rating{XX}  Let $\Lambda$ be a packing with full contact.  Then for every vertex $\lambda\in\Lambda$, the arrangement of $12$ around that vertex is the kissing configuration of the face-centered cubic or hexagonal-close packing. 
\end{conjecture}
