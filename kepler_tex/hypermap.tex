...
\begin{summary}
  ...
  planar embedding.  The graph, endowed with a fixed embedding, is a
  \newterm{plane graph}.  A planar graph has too little structure for
  our purposes, because it does not specify a particular embedding.
  On the other hand, the plane graph has too much structure.  It is a
  topological structure where combinatorics alone should suffice.  A
  hypermap gives just the right amount of structure.  It is a purely
  combinatorial, but carries information that the planar graph lacks,
  by encoding the relations among nodes, edges, and faces.  This
  chapter is about hypermaps.

In the original proof of the Kepler conjecture, the basic
combinatorial structure was that of a \newterm{planar map}, as defined
by Tutte~\cite{unknown}.  Although planar maps appear throughout that
proof, they are lightweight object, in the sense that no significant
structural results are needed about them.

G. Gonthier makes hypermaps the fundamental combinatorial structure in
his formal proof of the four-color theorem~\cite{unknown}.  His formal
proof eliminates topological arguments about planarity in favor of
purely combinatorial arguments.  For example, the traditional proof of
the four-color theorem relies on the Jordan curve theorem, but
Gonthier's formal proof does not.  When I learned of Gonthier's work,
I significantly reorganized the proof.  I replaced planar maps with
hypermaps.  I  made hypermaps into a heavyweight object, in the
...
originally done topologically can now be done combinatorially.  This
...
The quick definition of hypermap is that it is a finite set together
...
to the operations such as contracting and edge in a graph or deleting
...
will be described heuristically in terms of drawing graphs in pencil
...
of the permutation $f$.  If $f^m=I_D$ for some $m$, then $m = k i$ for some
...
\begin{remark}[plane graphs as hypermaps]\guid{IVPJYAG}\label{rem:hypermap} A hypermap is an abstraction of
the concept of 
plane graph.  Place a dart at each angle of a plane graph.
One function, $f$, 
...
the data $(D,e,n,f)$ from the plane graph and discards the rest.

This construction of hypermaps from plane graphs is our primary 
reason to study hypermaps.  Many of the lemmas and proofs in this chapter have
are standard results about plane graphs, translated into the language of hypermaps.
...
\end{equation}
This \newterm{triality relation} shows that if $(e,n,f)$ give a hypermap, then so do
$(n,f,e)$ and $(f,e,n)$. 
 Because of these symmetries in the
defining relation, there will be multiple versions of theorems about
hypermaps, all obtained from one proof by symmetry.
Alternatively,  it is possible to define a hypermap as a finite set with
two permutations $e,n$, leaving $f$ to be derived from $e n f = I_D$, but the
triality symmetry would be lost in such a definition.
...
same orbit of the group generated by $S$.  Rather than rely on group theory,
we use the following two simple lemmas.
...
\claim{$P$ does not contain any $h_1h_2$-steps}.  Otherwise, a sublist
$[\ldots;u;h_1h_2u;\ldots]$ of $P$ can be expanded to a path
$[\ldots;u;h_2u;h_1u;\ldots]$ that contradicts the minimal property of
$P$.

This proves the first conclusion of the lemma.  Fix $h$ in a set of
...
and edges of graph, faces of a plane graph, and connectivity.
...
graphs, we will often depict hypermaps with the darts in a node
...
\begin{definition}[plain]\guid{HFRNMIU}
  ...
  \newterm{plain} (carefully note\footnote{This is a deliberate and
    dangerous play on the homophonous {\it plane} that makes this
...
\begin{displaymath}f^2 (e x) = f (f e n x) = f x.\end{displaymath} So
$e x = f x$.
...
transformations.  Standard operations on graphs such as the
contraction or deletion of an edge can be interpreted as series of
walkup transformations.

To focus on a dart $x$ in a hypermap, it can be useful to draw a
hexagon around $x$ and place the six darts $e x$, $f x$, $e^{-1} x$,
$n x$, $f^{-1} x$, $e x$, $n^{-1} x$ at its corners in
...
A \newterm{walkup} deletes a dart from a hypermap and reforms the
edge, node, and face maps to produce a hypermap on the reduced set of
darts.  Walkups come in three flavors: edge walkups, face walkups, and
node walkups.
...
$f x = e x = y$. So
\begin{displaymath}n y = n f x = n f e y = y,\end{displaymath} and $y$
is a fixed point of $n$, hence degenerate, contrary to assumption.
...
Later in the chapter, we give an algorithm that generates various
...
definition.  Intuitively, it corresponds to joining two vertices of a
graph by a polygonal segment, with a new node at each vertex of the
joining segment.

\begin{definition}[RDW, or reverse double
  walkup]\guid{TPEZAAM}\label{def:R}
...
Just as a graph can be planar or nonplanar, so a hypermap can be
planar or nonplanar.  A plane graph satisfies Euler's formula, which
relates the number of vertices, edges, and faces of the graph.
Euler's formula can be expressed in purely combinatorial terms, then
generalized to hypermaps.  A hypermap for which Euler's formula holds
is defined to be planar.  This definition is one of the most important
...
The Euler relation for graphs has many consequences.  One simple
consequence is that in a triangulation of the sphere, the number of
edges is equal to $3/2$ times the number of faces (because each face
has three edges, each edge borders two faces).  Here is another simple
...
We make a distinction between injective paths, which are paths that never
repeat a dart, and loops, which return to the initial dart in the trajectory.
...
kind of list, then a loop is a certain kind of cyclic list, as given in the following
definition.
...
The rest of this section develops the basic properties of M\"obius contours.
A M\"obius contour should be thought of as the simplest kind of nonplanar
contour path.  M\"obius contours turn out to be extremely useful,
because in practice, the best way to certify that a given hypermap is nonplanar
is produce a M\"obius contour in the hypermap  (Lemma~\ref{lemma:no-mobius}).
...
The final results in this section are somewhat  technical lemmas that will
...
class of darts in the originating hypermap.  We call the new hypermap a subquotient
rather than a quotient, because the equivalence relation
is defined on a {\it subset} of the darts of the originating
hypermap.  A subquotient hypermap is vaguely analogous to a subquotient in
group theory or a quotient space in topology.

Subquotients are used in the algorithm to generate planar hypermaps, described
later in the chapter.  The
originating hypermap is the hypermap we wish to construct and the subquotient
...
algorithm.  This material  will not be used outside this chapter.
...
Intuitively, the subquotient hypermap is represented as a graph whose
...
\claim{Finally, the function is one-one.}  Indeed, if the nodes of
...
Thus, $e'$ has order $2$ on the subquotient.
\end{proof}
...
there is at most one edge of $H$ that meets both of them.
\end{definition}
...
\begin{lemma}\guid{QQYVCFM}\rating{ZZ}\label{lemma:dih-iso}
...
%\begin{lemma}[]\guid{ZOKKAOI}\label{lemma:subquotient-planar}
...
%precisely the darts that belong to a dart in the subquotient.  This is
...
This final section of the chapter is more technical than other sections and
may be skipped without disrupting the flow of the book.

This section presents an algorithm that generates
all simple, plain, planar hypermaps satisfying certain general
conditions (Definition~\ref{def:restricted}).  The algorithm proceeds
by adding more and more edges and nodes to a dihedral hypermap by a
sequence of reverse double walkup transformations.

The algorithm itself is extremely elementary to describe in intuitive
...
retrace the edges of graph in pen.\footnote{Historically, this
  ...
  process in 1994.  The code was first implemented in Mathematica,
  then later in Java, and more recently by others in ML and
  Isabelle/HOL.}  We start the algorithm by picking any face $F_0$ of
the graph and tracing the edges in pen that form the simple closed
curve that surrounds it.  Next, we pick any node $v$ on that simple
...
formal proof.  A hypermap gives rigorous form to the pencil graph, and various
...
definition is idiosyncratic, taylored to our needs, and matched to our
...
the the face cannot be modified at any later stage of the algorithm.
...
$\hat\varphi\leftrightarrow\check\varphi$ will be used frequently in
this section.
...
%\begin{lemma}[subquotient isomorphism criterion]\guid{STKBEPH}\rating{100}
...
%bijection. Thus, the subquotient has no false faces.  The result
%follows.
%\end{proof}
%
...
\begin{lemma}[loop confinement]\guid{HQYMRTX}\rating{200} \label{lemma:yz}
  ...
  Let the natural number $m,p$ and darts $y,z$ be given by
...
%\claim{[$L$ is canonically false.]} In this case,
  ...
  true, which is contrary to assumption.  This proves the second
...
%  Under the bijection between the set of faces of a subquotient $H/{\cal
%    L}$ and the family ${\cal L}$ (of
%  Lemma~\ref{lemma:subquotient-bijection}), an $S$-flag on a subquotient
...
The proof can be organized into independent parts, according to the
...
disjoint from each other and disjont from the sets of darts of $L'\in
...
$\{y,f y,\ldots,f^p y\}$.  This set is either contained ini
...
transition from one subquotient to another is given by the transform of
...
terms the process of retracing a pencil drawing in pen.  At we
...
a pencil template as a diagraph.  Each directed edge encodes a
...
In heuristic terms, it only takes a finite number of pen strokes to retrace the pencil
drawing.  The completed pen drawing agrees with the initial pencil drawing.
...
%  Let $H$ be a restricted hypermap.  Fix a choice function $\op{ch}$
%  as above, and let $F$ be any face of $H$. Then associated with
%  $(\op{ch},F)$, there is a sequence of marked hypermaps ${\cal H}_i
%  = (H,{\cal L}_i,L_i,x_i)$ for $i=0,\ldots,k-1$ such that
%\begin{itemize}
...
%  subquotient $H/{\cal L}_k$ that is naturally isomorphic to $H$.
%\end{itemize}
%\end{lemma}
%
%\begin{proof}
...
%Lemma~\ref{lemma:all-dart}, the subquotient map $H\to H/{\cal M}$ is an
...
%constructs a subquotient of $H$ with more darts than the previous one and
%the number of steps is bounded by the cardinality of the dart set of
%$H$.
%\end{proof}
...
that does not require us to represent it first as a subquotient $H' =
...
possible completed pen drawings.  One of them will be the hidden
pencil drawing.
...
subset of $\Omega$ and such that $d$ is the maximum cardinality of a face.  A
...
\begin{definition}[extension]\guid{ZMVBANY}  
  ...
  $RDW(H,x,m,p,q)$.  Say that a boolean-valued function $\check\psi$
...
$A$ one may find all restricted hypermaps (for given $\Omega$ and $d$)
...
The proof will be presented below.  The proof is a matter of
...
Correlation consists in showing that for every pencil diagram we can
find a sequence of pen strokes obeying the Knaster-Tarski rules that
coincides with a sequence of pen strokes in the digraph. When we
correlate the strokes in this manner, it will follow that each pencil
diagram is isomorphic to a pen diagram in the Knaster-Tarski fixed
point set.  The final passage of this chapter makes this rigorous.
...
isomorphic to $\op{Dih}_{2d}$, hence also isomorphic to $H'$.  The
isomorphism can be chosen so that $\check\varphi$ pulls back to
$\check\varphi_{can}$ on the faces of $H/{\cal L}$, and so that $x'$
...
directed edge of the digraph, whose tail is correlated with
$(H'',m',\check\psi,x'')\in g(H',m,\check\varphi,x')\subset
...
%  to $D$ and whose greatest face cardinality is $d$.  It has a subquotient
...
% subquotient hypermap is a polygon $\op{Dih}_{2d}$.  When the the initial
% contour loop is chosen on a face of maximal cardinality, the natural number
% $d$ is an upper bound on cardinality of a face.
%
...
% process generates all restricted hypermaps with greatest face-cardinality
% $d$, with darts in $D$.
%
...
%  finite set $D$, and whose greatest face cardinality is $d$.
%\end{lemma}
