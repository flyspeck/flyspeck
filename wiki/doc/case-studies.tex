...
Flyspeck book\ednote{cite} and a Twelf formalization of the first chapter (Trigonometry\ednote{cite}).
...
either preloaded into the wiki or modeled
...
\ednote{Perhaps a feature matrix with a side to side
comparison of the two wikis would help?\\
a direct
comparison would have been helpful to illustrate more objectively the
advantages of the authors' own implementation SWiM.}
\ednote{ESWC: Is the ad-hoc nature of ontology authoring in Semantic MediaWiki important for mathematicians, or are they comfortable working in the existing ontology?  Is SWiM's triple-based browsing intuitive, or do working mathematicians prefer the more Wikipedia-based browse/query of Semantic MediaWiki?  Data on these types of questions would have dramatically bolstered the results. 
}

\subsection{Semantic MediaWiki 1.0}
\label{sec:smw-study}

\ednote{ESWC: It is reasonably well-known in the semantic wiki community (although I can't quickly find a published reference) that Semantic MediaWiki has a weak ontology import mechanism, and that its representations and query mechanisms are based on a fairly weak overall logic.  This isn't exactly news, although it is interesting to see an actual collaboration task like Flyspeck where this design is inadequate.}Semantic MediaWiki~\cite{KrSchVr:semwiki-reasoning07} is a semantic web extension
...
$\mathcal{EL}^{++}$~\cite{KrSchVr:semwiki-reasoning07}\ednote{Secondly,
uncommon terms, esp. in the DL domain should be briefly explained,
such as EL++, or Polleres' SPARQL rule transformation, which is
crucial for the benefit of SPARQL in this context. 
}\ednote{ESWC: Page 8: EL++ does support negation, just not *unrestricted* negation (you can express concept disjointness since you have gcis and the bottom concept, see: http://lat.inf.tu-dresden.de/~clu/papers/archive/ijcai05.pdf ).}, which, for example, does
...
failure~\cite{Polleres:SPARQL-Rules07}:\ednote{The advantages of more expressive
  querying capabilities in SWiM come (at least theoretically) with worse
  run-time performance. I suspect that this will not be a big issue since query
  performance is not the bottleneck here, however, a statment taking scale of
  the ontology into account would be nice.}\ednote{Secondly,
uncommon terms, esp. in the DL domain should be briefly explained,
such as EL++, or Polleres' SPARQL rule transformation, which is
crucial for the benefit of SPARQL in this context. 
}
