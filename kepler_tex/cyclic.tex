...
\item \case{cyclic hypermap} $H$ is isomorphic to $H_{2k}$, where $k =
\card(F)$;
...
\begin{proof} The map from the set of nodes to a face is a bijection
for the cyclic hypermap $H_{2k}$, hence it is a bijection for cyclic
fans.
...
$\v:F\to V$ and $\rho:V\to V$ for the bijections of the preceding
lemma.
\end{definition}
...
isomorphic to $H_{2k}$, where $k= \card(F)$.  Finally, the values
$\op{azim}(x)$ and $\Wdart(x)$ do not depend whether they computed
relative to $\op{hyp}(V,E)$ or to $\op{hyp}(V',E')$, for all $x\in
F$.
%That is, 
%\begin{displaymath}
%\begin{array}{lll}
...
C^0\{\v,\w\}\cap C^0_-\{\u\}\ne \emptyset.
\end{displaymath}
A cyclic fan is \newterm{lunar} with dependency $\v,\w$ if it is not
circular and if $\v$ and $\w$ are distinct and dependent elements of
$V$.
\end{definition}
\indy{Index}{generic}%
\indy{Index}{lunar}%
...
Assume that $\u$ and $\w\in\ring{R}^3$ are independent, as well as
$\u$ and $\v$.  Then $\op{azim}(\orz,\u,\v,\w)=\pi$ if and only if
...
\begin{lemma}[circular
geometry]\guid{KCHMAMG}\rating{ZZ}\label{lemma:circular}
...
\label{lemma:monotone}
  Let $(V,E,F)$ be a cyclic fan. Fix $\v_0\in V$.  Assume that $\v_0$
  and $\u$ are independent for all $\u\in V\setminus\{\v_0\}$.  For
...
Consider a lunar cyclic fan $(V,E,F)$ with dependent $\v,\w\in V$.
...
Let $(V,E,F)$ be a lunar cylic fan with dependency $\v,\w\in V$.  In
...
cyclic fan, one by one.  The verification of the property \case{V}
...
independent of $t$.  The independence property is equivalent to
...
\case{cyclic hypermap}~\case{face} The combinatorial properties of
...
remain dependent.  Thus, the cyclic fan remains lunar.
\end{proof}
...
\begin{proof} Each of the properties of a generic cyclic fan will be
examined in turn.

\case{V}: The set $V(t)$ is the image of $V$ and is therefore finite
and nonempty.  \case{origin} Since $\varphi$ is continuous and
$\orz\not\in V$, it follows that $\orz\not\in V(t)$ for sufficiently
small $t$.

\case{independence}: If $\v,\w$ are independent, then $\v(t)$ and
...
\case{face},~\case{cyclic hypermap}: The azimuth cycle on $E(\v(t))$
...
$\v_0$ are independent.  In the notation of
...
\claim{The \case{independence} property holds at $t=t_0$}.  Indeed, it
is enough to check independence when one of the points is
...
\claim{The line $L$ meets the segment $\op{conv}^0\{\v,-\rho\v\}$.}
...
independent. This gives independence.
...
\case{independence} property, \case{intersection} property, or the
...
\begin{definition}[fan~datum] Let $k\in\ring{N}$ and $I\subset
...
If $\v,\w\in V$ are independent, then $C\{\v,\w\} \subset
...
Let $(V,E,F)$ be a cyclic fan.  Let $\v,\w\in V$ be independent.
...
consider is $\v(x)=\v$.  For all $\p\in C^0\cap \bWdart(x)$,
\begin{displaymath}
...
imply $\p\in \Wdart(x)$ as desired.  The case $\v(x)=\w$ is similar.
...
$\{\v,\w\}\not\in E$.  Then $\v$ and $\w$ are independent, and
$C^0\{\v,\w\}\subset \Wdart(x)$ for all $x\in F$.
\end{lemma}

\begin{proof} By Remark~\ref{remark:fan-verify}, independence follows
...
$\v,\w\in V$ are independent and that $(V,E')=(V,E\cup
...
each $\u\in \{\v,\w\}$, assume that $\u$ is independent of all
elements of $V\setminus\{\u\}$.  Assume that $C^0\{\v,\w\}\subset
...
that $\v$ and $\w$ are independent.  (Note the similarity with
...
assume that $\u$ is independent of all elements of
$V\setminus\{\u\}$.
...
$\v,\w\in V$ be dependent.  By the previous claim, we may assume
...
of the two planes.  They are dependent elements of a lunar fan.
This is not generic.
\end{proof}

\subsection{irreducibility}

\begin{definition}[extremal,~minimal] Let $(V,E,F,G)$ be a special fan
...
\begin{definition}[irreducible] Let $(V,E,F,G)$ be a special fan and
