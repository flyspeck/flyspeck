...
\begin{wrapfigure}{r}{4.2cm}
  ...
  \vspace{-1.0cm}
\end{wrapfigure}
\ednote{ESWC: some mention of LaTeX is probably appropriate}
...
home in a semantic wiki\ednote{ESWC: Indeed, some of the advantages you claim
  for a wiki just aren't advantages of a *wiki* per se. E.g., on page 3, "It
  contains descriptive and motivating yet informal text that should be preserved
  for human understanding. This quasi-formal information would be difficult to
  present in a strictly formal setting of a proof assistant, but is at home in a
  semantic wiki." Clearly this isn't true. How a proof assistant handles informal commentary depends on the proof assistant software and how its setup. Conversely, there's no reason to think that a wiki is better at handling informal commentary. Consider literate programming sys!
 tems. They allow for the presentation of formal stuff (program code!!!) in a natural way with commentary (the paper). Consider the goals of Isar: http://isabelle.in.tum.de/Isar/
}.  The large number of lemmas, many independent
