...
\item Score all $3$-cells with $\eta > 1.34$ at $0.008\azim$, but keep the leaf, by \cc{cell3}{}.
\end{enumerate}
\end{remark}

\begin{definition}[small~face,~subcritical,~supercritical]
...
A \newterm{small face} is a triangle along the critical edge $y_1$ with one critical edge $y_1$ and two
...
\begin{lemma} There are at at least three $4$-cells (in every counterexample).
\end{lemma}
