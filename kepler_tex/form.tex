...
%Call the points of $ V$ {\it  vertices}.
%\indy{Index}{vertex}%
...
\indy{Index}{Voronoi cell} For each $\v\in  V$, the Voronoi cell%
$\Omega(\v)=\Omega( V,\v)$
...
Then 
\begin{nomerate}
\item \case{circumcenter} $\omega(\bu[j])$ is the circumcenter of the set $S_j =\{ \u_0,\ldots, \u_j\}$. 
...
\begin{lemma}\guid{RVFXXBU}\rating{400}  The $0,\ldots,4$-cells of the parameters $\bu\in  V(3)$
...
%Suppose the packing is weakly saturated in the sense that there does not exist $\v\not\in V$ such that $ V'= V\cup\{\v\}$ satisfies these same properties
...
g(h) = \arccos(h/2) - \pi/6.
$$
On the unit sphere,  consider the disks $D_i$ of radii $g(h_i)$, centered at $ \u_i/\normo{ \u_i}$.  These disks do not overlap; this follows from the easy inequality~\cite[]{hales:2009:nonlinear} %% Marchal disks are disjoint.  % cc:disks
$$
