...
Back in the realm of proof assistants, much recent research has gone
into the ``sledgehammer'' approach to theorem proving.  The idea is to
produce fully automated proofs of new theorems, by collecting together
all the thousands of theorems that have ever been proved in a given proof
assistant, feeding them all along with the desired new
theorem into a first-order theorem prover, and letting the first-order
theorem prover do its work.  To simplify, some shortcuts are made,
such as throwing away type information and higher-order logic when
converting to first-order.  These simplifications can create
inconsistencies that are ultimately harmless, because the answers
are rigorously checked later in the proof assistant.

J. Urban tells the story of applying machine learning algorithms to
filter out theorems that are unlikely to contribute to the proof of a
desired new theorem.  He went in one morning and found to his
amazement that with his new machine learning algorithm, he had obtained a
100\% success rate in fully automating the proofs of a new batch of
theorems.  On closer inspection, the machine learning algorithm had
learned that anything follows from a contradiction, had derived a
contradiction from the shortcuts, and exploited the contradiction for its
perfect success.
...
John Harrison, in a gem of a paper, asks ``Quis custodiet ipsos
custodes?''  who guards the guards~\cite{HaSelf}?  How do we prove the
correctness of the prover itself?  In that paper, he proves the
consistency of the HOL Light logic and the correctness of its
...
challenging research program (as well as a profound metynomy): to show
