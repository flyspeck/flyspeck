...
$$
\begin{array}{lll}
\Gamma(Z) &= \sum_{X\in Z} \gamma_L(X) w(X),\quad\hbox{ where }\\
\gamma_L(X) &= \gamma(X,L),
\end{array}
$$
...
$$
\sum_{X\in B} \gamma_L(X)w(X) \ge a + b\,\op{azim}(B),
...
$$
\sum_{X\in B} \gamma_L(X)w(X) > a_j' + b_j\,\op{azim}(B).
$$
We get
...
The following additional fact is used to compute the constant $a_j'$ when  $j=2$: some non-spinal edge of a blade has length greater than $2h_-$.   To see this, assume that all non-spinal edges have lengths in $[2,2h_-]$.  Calculations of the edge opposite the spine show that one of these edges has length at most $2h_-$, making an additional quarter ($j\ge 3$), contrary to  assumption.  
...
$\sum_i n_i \le (6N-12)$.
Summing over $i$, for $N\ge 15$, we get the follow
