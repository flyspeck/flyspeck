...
%% XX D-> D_dod
%% XX t_0 -> t_{dod}
%% XX 13 not a running assumption.
%% XX hypermap (D,e,n,f).
%% 0.178 or trgt.
%% need sqrt8 cutoff on distinguished for vertex adjustment argument. add by increasing lgth
%% XX dih-> azim at end
%% XX # for cardinality
...
We may repeat this construction until $G(\Lambda)$ becomes biconnected.  The process does not alter the volume of $\Omega_{trunc}(\Lambda,0)$.
...
The running
assumptions on $\Lambda$ remain in effect: $0\in\Lambda= \Lambda(0,2t_0)$, $G(\Lambda)$ is  biconnected.  This section make
...
%\subsubsection{type 1: concave vertex}  

% starting conditions, stopping, local config preservation, motion, monotonicity.
...
\item $|v|< 2.2$, 
%$v$ is not the only concave vertex of $U$,
and $|u-v|\ge 3.07$, $|v-w|\ge 3.07$.
\end{enumerate}
%% XX The condition on the "only concave vertex" is not in Version 2, but I think
%% it is necessary, because on page 29 of V2, the constant 1.94159 is used in the
%% one-concave vertex case.  Note that 1.94159+1.25841=3.2, not 3.07.
%% XX I think it is better to use the constant 3.07-t_0. This is done below.
...
Let $\Lambda$ be a packing satisfying $0\in\Lambda=\Lambda(0,2t_0)$.  The graph
$G(\Lambda)$ is planar.   We assume that $G(\Lambda)$ is biconnected. 
...
we have already described how a face of the hypermap gives a face of $G(\Lambda)$,
...
Let $H=(D,e,n,f)$ be a hypermap.  We say that a face of $H$ (that is, an orbit
of $D$ under $f$) is a triangle, quadrilateral, pentagon, etc. if the cardinality
of the orbit is $3$, $4$, $5$, respectively.  We say that two nodes are adjacent
...
of $D$ under $n$).  We say that
the node $v$ has type $(p,q,r)$, if the cardinality of $v$ is $p+q+r$ and if
there are $p$ triangular faces, $q$ quadrilateral faces, and $r$ other faces
that share a dart with $v$.  We call the cardinality of a node its degree.
...
XX Figure of 4-circuits.  \label{fig:quadtype}
...
There is an archive of structurally tame hypermaps in the file \cite{XX}.  This
...
computer code described in Section~\ref{XX}.  We refer the reader to that section
...
conjecture.  (Without loss of generality, we assume that $0\in\Lambda=\Lambda(0,2t_0)$;
...
Now we turn to an item by item discussion of the properties.  The first several
are elementary.
...
we obtain darts $x\ne y$ at the same node $v$; then $v$ is an articulation vertex
...
\subsubsection{face cardinality}

{\it The cardinality of each face of $H$ is at least $3$ and at most $7$.}

First we show that the cardinality is at least $3$.
Each dart has the form $x=(0,v,w,u)$.
By the definition of the face map, 
\begin{equation}\label{eqn:fx}
x = (0,v,w,u),\quad
f x = (0,w,\sigma_w^{-1}v,v),\quad
f^{-1} x = (0,u,v,\sigma_u v).
\end{equation}
By construction, $w,u\in E(v)$, which is nonempty by the connectedness of $G(\Lambda)$.

Also,  $\# E(v)>1$, for otherwise, if $E(v)=\{u\}$, then
$w=u$ and thus $f x$ and $f^{-1}x$ are two darts
of the same face and the same node, which by simplicity implies they are
equal:  $f x=f^{-1} x$.  Then $\sigma_u v = v$.  This implies
that $\# E(u)= \# E(v)=1$, so that $\{x,f x\}$ forms an 
orbit of $\tangle{e,n,f}$ on $D$.
Since there are at least $13$ nodes, this contradicts the fact that $D$ is
a single orbit under $\tangle{e,n,f}$.  Thus, $\# E(v)>1$.
It follows that $v,w,u$ are all distinct.  By the form of $x,fx$, and $f^{-1} x$
in (\ref{eqn:fx}), we see that these three darts are distinct.  Thus, each
...
The degree at node $v$ is equal to the cardinality of $E(v)$, the number of
edges of $G(\Lambda)$ with endpoint $v$.
We have just established that $\# E(v)>1$ for each node $v$.  Thus, the
degree is at least $2$.

Let $(p,q,r)$ be the type of node $v$.  If $r>0$, then the previous
property bounds the degree at five.  Assume $r=0$.  We defer the
proof in this case.  It is a corollary of the 
weight assignments property (Section~\ref{sec:pq}).
...
$\{v_0,v_i\}$ for $i=1,2,3,4$.  We have established that every node has degree
...
$$
If there is some face $F''\ne F'$  with at least four sides, then 
for $U''=U_{F''}$, we have by Theorem~\ref{thm:main},
$$
\mu(\Lambda) \ge 0.168 + 0.031 > \mu(\Lambda_{dod}).
$$
Now assume that every face other than $F'$ is a triangle.  There are at least
$8 = 13 - 5$ vertices in $\Lambda^*$ other than $v_0,v_1,\ldots,v_4$.  Pick any
four of them $w_1,\ldots,w_4$.  Let $F_1,\ldots,F_r$ be the triangles of $G(\Lambda)$
that meet
some node $w_i$.  Let $U_1,\ldots,U_r$ be the corresponding connected components. 
Another estimate \cite[Lemma~5.2]{arx} gives
$$
\sum_{i=1}^r \mu(\Lambda,U_i) \ge 4 (0.004).
$$
This gives
$$
...
{\it There exists an admissible weight assignment of total weight less than $0.178$.}
...
The condition that $w$ is admissible can then be expressed as two claims.
\begin{enumerate}
...
We review the argument from \cite[p.14]{arx}.
...
special $(p,q)$. Anything else is a geometric impossibility.  These pairs $(p,q)$
are listed in \cite[Lemma~6.1]{arx}.  In particular, $p\le 7$ and $q\le 5$.

Interval arithmetic methods establish a list of nonlinear inequalities between
$\op{azim}(U_F,v)$ and $\mu(\Lambda,U_F)$ when $F$ is a triangle or quadrilateral (XX give reference).
...
linear in these variables.  We obtain a system of linear inequalities. 
This is the method of linear relaxation.  A lower bound
on the left-hand side of (\ref{eqn:bp}) is the solution of the linear program
$$
\min \sum_F \optt{mu(F)}
$$
subject to 
...
Examining the list of constants $b(p,q)$ in the table, we find that this implies
