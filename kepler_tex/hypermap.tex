...
\begin{definition}[path,connected, combinatorial component]  A path from $x_0$ to $x_{k-1}$ is a list of darts $[x_0;p_1;\ldots;x_{k-1}]$ such that $x_{i+1} = h_i x_i$, where $h_i \in \{e,f,n,e^{-1},f^{-1},n^{-},I\}$ 
...
\begin{definition}[contour~path]  A contour path from $x_0$ to $x_{k-1}$ is a path  $[x_0;x_1;\ldots;x_{k-1}]$ such that $x_{i+1} = n^{-1} x_i$ or $f x_i$ for each $i<k$.  (That is, each step in the path is clockwise step around a node or a counterclockwise step around a face.)   If the contour path is injective on $\{0,\ldots,k-1\}$ and  $p_0 = p_k$, then it is a contour loop.
...
\begin{definition}[complementary~path,inverse~path] 
If $p$ is a contour path from $x$ to $y$, let the inverse path $p^{-1}$ from $y$ to $x$ be the path that replaces each $h$ step $[u;h u]$ with $[h u;h^2 u;\ldots;u]$, for $h=f,n^{-1}$.
%
Consider a plain hypermap.  Let $p$ be a path that does not visit any node twice.  If $p=[x;y;\ldots;z]$ is a path, we form a complementary loop $p^c$ as follows. Replace each maximal sublist
$$
[u;n^{-1} u; \ldots; n^{-k} u]
$$
with the sublist
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=70mm]{\pdfp/contour-comp.eps}
...
An articulation node of a hypermap is a node for which there exists two darts $x,y$, neither belonging to the node, such that every contour path from $x$ to $y$ passes through the given node. A connected hypermap is biconnected if it contains no articulation node.  
\end{definition}

\begin{lemma}\guid{MOORQUT}\rating{200} Let $H$ be a connected plain simple hypermap. Then the hypermap is biconnected.
\end{lemma}

\begin{proof} Let $x$ and $y$ be any darts, and let $N$ be any node such that $x,y\not\in N$.  We need to show how to get from $x$ to $y$ avoiding $N$.  Let $N = \{z_1,\ldots,z_r\}$, where $n^{-1} z_i = z_{i+1}$.  By connectedness, there exists a path from $x$ to $y$.
...
\begin{proof}  We prove the contrapositive.  The successor of $n x$ on $L$ is $x$. Suppose $P$ is $n x,f n x,\ldots,n y,y$.   Form contour path starting at $x$, then $n^{-1}$ steps to $L$, then follow $L$ to $y$, and on to $n x$.  Follow $P$ back to $n y$.  This is a M\"obius contour $x\ldots y\ldots n x\ldots n y$.
...
In this subsection, we assume that $H$ is a hypermap with the restrictive properties.
...
Our aim is to prove that every restricted hypermap with at most $V$ darts is generated by a particular algorithm.  The proof is a long induction argument.  We begin by showing how to go from one partially constructed hypermap to another more fully constructed hypermap.  The hypermap $H$ represents the fully constructed hypermap and two quotients $H/{\cal L}$ and $H/{\cal L'}$ represent the partially constructed hypermaps.  The algorithm involves the transition from the hypermap $H/{\cal L}$ to $H/{\cal L'}$.  This represents one step of the algorithm.

This section describes the transition from $H/{\cal L}$ to $H/{\cal L'}$.  The algorithm carries along various auxiliary data satisfying various assumptions, wich we now describe.

Let $H=(D,e,n,f)$ be a hypermap with normal family $\cal L$, quotient $H/{\cal L}$.  We assume that the quotient is simple.   Choose $F'$ be a false face of the quotient, corresponding to a contour loop $L$ of ${\cal L}$ and choose $x'\in F'$.  By the definition of $\emptyset$-flag, it  lies on an edge that meets a true face.   $x' = [\ldots;x]$.  As the face $F'$ is false in the canonical flag, there is a smallest $m\ge0$ such that $f^{m+1} x$ is followed by a $n^{-1}$ step in a loop of ${\cal L}$. Let $y = f^{m+1} x$. 


We assume that the canonical boolean function on $H'=H/{\cal L} = (D',e',n',f')$ is an $S$-flag.  Consider the darts $f^{j+1} y$.  There is a smallest $j\ge0$, such that $f^{j+1} y$ is a dart in some path of ${\cal L}$.  Set $z=f^{j+1} y$.  


\begin{example}  We give an illustrated example of this setup (Figure~\ref{fig:graph-gen}).   In the figure, the hypermap $H$ is represented as a planar graph.
...
\begin{remark} In summary, we have the following assumptions:
\begin{itemize}
\item $H$ is restricted.
\item $\cal L$ is normal.
\item $H'= H/{\cal L}$ is simple.
\item $F'$ is a false face of $H'$ under the canonical flag.
\item $e'x'$ lies in a true face.
\end{itemize}
\end{remark}
...
\begin{proof} 
For a contradiction, suppose $f^{j+1} y = f^k x$. Then also, $f^j y = f^{k-1} x$.  If $j>0$, then this goes against the minimality of $j$.  So $j=0$, and $y=f^{k-1} x = f^{m+1} x$.  Also, $k-1 < {m+1}$, which contradicts the minimality of ${m+1}$.  This proves the second claim of the lemma.
...
$$
which follows the contour loop $L$ from $n^{-1} y$ to $n z$, then complements the path of $L'$ from $y$ to $z$, traveling instead from $n z$ to $n^{-1} y$. The new nodes visited on the subpath of $L'$ from $y$ to $z$ do not contain any darts in any loop in ${\cal L}$.  (By the definition of normal family, every dart at a node or no dart at a node lies along some loop of ${\cal L}$.)  It follows that we obtain a normal family ${\cal L'}$  by eliminating $L$ and replacing it with $L'$ and $L''$.
...
\item ~[new edge] Write $y=f^{m+1} x$ where $m\ge0$ is as small as possible.  Pick $z$ on the same false face, where $z\ne f^k x$, $k\ne 1,\ldots,{m+1}$.  For each such $z$ and for each $j=0,\ldots,M-\#f$, construct a new edge from $y$ to $z$, with $j$ added nodes of degree $2$.  (The reverse double edge walkup splits the face into two.)  Replace $(x,y)$ with $(x,z)$.  If $z=x$, execute the first step [new true face] before returning the modified hypermap to the list.
\end{itemize}
