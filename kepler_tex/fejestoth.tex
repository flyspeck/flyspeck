...
This gives
$$\tau(F) > 4.02 + (2-n)\Delta_0 \ge 4.02 -4 \Delta_0 > \op{trgt}.$$
...
$$
The total number of darts is at most the total number of nodes, which is at most $12$.  The rightmost term is at least $\op{trgt}$ if $v> 1$. Thus, $v=1$.
...
We consider possible aggregates with $j(x')\ge 6$ and $d(r,s)<\op{trgt}$.
Following the classification of \cite[p.~126,~Fig.~12.1]{hales:2006:DCG}, and noting that $d(9,0) > \op{trgt}$, $d(6,2) > \op{trgt}$, we find that the set $D'$
...
$$
\begin{array}{lll}
d(8,0) + d(5,0) &> \op{trgt}\hbox{ or }\\
d(8,0) + 2 d(4,0) &> \op{trgt}\\
\end{array}
$$
\end{proof}
...
\item The cardinality of each face is at least three because there are no loops or multiple joins.  The cardinality of a face is at most $8$ because of the estimate $d(9,0)>\op{trgt}$.  
...
\item The total weight of the weight assignment is
$$
(4\pi - 20\Delta_0) < \op{trgt}.
$$
\item Let $v$ be a vertex of type $(p,q,0)$.  Let $A$ be the set of faces that meet the vertex $v$. Then 
$$
\tau_{F\in A}\tau(F) > d(4)~q.
$$
This gives the nonzero entries in the table of bounds $b(p,q,0)$.
\end{proof}
...
The hypermap does not contain any nodes of degree five or greater.
\end{lemma}
