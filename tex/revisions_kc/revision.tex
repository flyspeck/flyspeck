...
\end{center}
\caption{In fact, the situation of Fig~\ref{fig:biconnected} does not exist
...
\end{center}
\caption{The theorem is proved in various cases: two edges with
distinct endpoints, three or more edges, two edges, or a single edge
passing through the triangle $\{0,v,w\}$.}
\label{fig:cases}
\end{figure}

The following lemma shows that we cannot have two such edges $\{u_1,u_2\}$ and $\{u_1',u_2'\}$ with distinct endpoints.  From this it follows that there is an endpoint $u_2$ shared by every edge that passes through the
triangle $\{0,v,w\}$.  Lemma~\ref{lemma:three-edge} shows that there cannot be three or more edges.  Finally, the cases of one or two edges $\{u_1,u_2\}$ are treated.
...
$(w_1,w_2,w_3,_4,w_5)=(w,u_1,u_1',u_1'',v)$, so that setting
...
In particular, either $v$ and $w$ lie in different components of
the graph $\Gamma$ or in different components of $\Gamma\setminus\{a\}$,
for some articulation vertex $a$.
...
biconnected components of the graph $\Gamma$.  By Lemma~\ref{lemma:special}, there is no loop subregion along $\{u_1,u_2\}$ on the $v$-side.
...
The original text and replacement text appear in roman font.
Comments about the corrections appear in italic. 
