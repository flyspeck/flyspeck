...
%6915275259 (modified)
%5127197465 (never checked)
%474496219 (done in 1998  in kep_inequalities.ml)
%7710172071 (never checked)
...
The blueprint edition contains several introductory essays that give a gentle introduction
...
\begin{proof} Let $x=\|u-v\|$ and $y=\|u-w\|$.  Assume $x\ge y$.  As $u$ moves away from $v$, along a fixed line through $v$ and an initial position $u_0$, the region $X$ expands.  Thus, we may assume that $x=2.51$.   The boundary of $X$ is a polygonal curve consisting of line segments and concave arcs of circles.  The diameter is realized by the distance between two vertices $p_i(y)$ of the polygonal curve. We consider two cases according to $y\le 2.4$ and $y\ge 2.4$ because the structure of the boundary changes at $y=2.4$.  We calculate $\|p_i(y)-p_j(y)\|$ directly, checking in each case that the distances are less than $1.044$.
% See file sphereBook.nb, section A Revision of the Kepler Conjecture, lemma:diam.  These are non-rigorous Mathematica calculations, but since they are just 1 dimensional it would be a trivial matter to convert to interval arithmetic.  The maximum appears when y=2.51 between points p1,q1 distance 1.31*2/2.51 =1.04382...
\end{proof}
...
\item (C): $\|u_1-u_1'\|\ge 3.64,\quad \|u_2'-u_1\|\ge 2.91$;
\end{itemize}
...
% There is a note in the Mathematica code justifying that this does not decrease lengths.
...
% The picture is a line resting on a cylinder of radius r.  As r decreases, the dihedral angle decreases...
Then we have three rigidly determined simplices (A), (B), (C).  An explicit coordinate calculation of the distance between the two segments shows that the distance is greater than $1.044$ in each case.
\end{proof}
...
conditions A, B, C hold.  So without loss of generality, we may assume
that $\|u_2-u_1'\|<2.91$.  By Lemma~\ref{lemma:291}, this implies that $\{u_1',u_2\}$ does not pass through $\{0,v,w\}$.  By the conditions of the lemma, the segment $\{u_1,u_1'\}$ does not meet the plane $\{0,v,w\}$, so that $\{u_1,u_1'\}$ does not pass through $\{0,v,w\}$.  This means that the triangles $\{u_1,u_1',u_2\}$ is linked around $\{0,v,w\}$.  Some edge of $\{0,v,w\}$ passes through $\{u_1,u_1',u_2\}$.  Up to symmetry there are two cases: (1) $\{0,v\}$ passes through $\{u_1,u_1',u_2\}$, or (2) $\{v,w\}$ passes through $\{u_1,u_1',u_2\}$.
...
passes through $\{u_1,u_1',u_2\}$.  Otherwise, $u_2'$ lies in the convex hull of $\{0,u_1,u_1',u_2\}$ and a coordinate calculation shows that the upper bounds on the edges of the simplex $\{0,u_1,u_1',u_2\}$ are inconsistent with the lower bounds on the distances from $u_2'$ to the vertices of the simplex.  Since $\{0,u_2'\}$ passes through $\{u_1,u_1',u_2\}$ we may use geometric considerations to show that $\|u_2'\| > 2.51$.  This is contrary to the assumptions.

In the second case, $\{v,w\}$ passes through $\{u_1,u_1',u_2\}$ and through $\{u_1',u_2',u_2\}$.  Geometric considerations give $\|u_1-u_1'\|>2.85$ and $\|u_2-u_2'\|> 2.85$.  Assuming that B does not hold gives $\|u_2-u_2'\|< 2.91$.  The first paragraph of the proof now gives that $\{u_2,u_2',u_1\}$ links around the triangle $\{0,v,w\}$.  The edge $\{v,w\}$ does not pass through $\{u_2,u_2',u_1\}$.  Thus, we are in the first case, which has already been treated.
\end{proof}
...
edgel lengths show that the condition $\Delta>0$
...
unerased loop has at least four anchors.  Moreover, some anchored simplex around the upright diagonal is not an upright quarter (because of the edge $\{u_1,u_2\}$).  Geometric considerations based
on these constraints show that
...
%Either $p(w_1)$ is contained in the
...
that bound $0$ if each vertex has height at most $2.3$ (by \calc{5127197465}).
%
If any vertex has height greater than $2.3$ show that the $\op{vor}_0$-scored quad cluster scores
less than $-1.04\,\op{pt}$.  
For this, we may use the deformations of Lemmas~12.10 and 13.1.  We may also use \calc{474496219}, which shows that if the diagonal reaches $2\sqrt2$, each half is at most $0.009$.  
By these deformations and this calculation, the result now follows from \calc{7710172071}.}
%
