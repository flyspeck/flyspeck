...
a face, which is again a fan.  Special properties of the
...
We develop a theory of deformations of convex local fans, including
sufficient for the deformation of a convex local fan to remain a
convex local fan.
\end{summary}
...
\end{displaymath}
for all $\v\in V$.  This is the \newterm{interior} angle of the convex local fan at $\v$.  Also, write
\begin{displaymath}
...
\begin{lemma}[]\guid{CIZMRRH}\rating{ZZ} Every convex local fan is either
...
the projection is the identity map, 
and the dihedral angle is $\arc_V(\orz,\{\v,\rho\v\})$.
%
%\begin{displaymath}
...
simpler, but possibly more difficult to formalize.  The proof  is based on polar
...
\item \case{local~fan} $(V,E,F)$ is a convex local fan.
...
\newpage
\begin{lemma}[]\guid{CKQOWSA}\rating{1000}\label{lemma:std-fan} %older harder proof w/o computers was 1800.
...
%}
\begin{description}
\item \case{three points} Let $\{\v_1,\v_2,\v_3\}\subset \BB$ be a packing of three points.
...
\case{wedge} and \case{angle} have been built into the definition of a fan datum.  The azmuth cycle $\sigma(\p_i)$ on $E(\p_i) =
...
\begin{lemma}[]\guid{KFIIPLO}\rating{ZZ}\label{lemma:compact-fan}
...
To allow for more than one convex local fan $(V,E,F)$, we will extend the notation,
...
\begin{proof} Pick a sequence of fan data $\p$ such that the associated fan is a minimal fan and such that 
\begin{displaymath}
\min_{|i-j|>1} \norm{\p_i}{\p_j}
\end{displaymath}
...
\subsection{features}
  ...
  The aim is to show that the set of minimal fans is empty.  However,
...
\begin{lemma}\guid{JEJTVGB}\rating{ZZ}\label{lemma:empty-d}
\begin{displaymath}
\tau(V,E,F) \ge d (r,s)
\end{displaymath}
