...
Let $Y=Y(\orgn,V,E)$.
Assume that $\op{azim}(x)<\pi$
for all darts.  Fix a face $F$ of cardinality three and fix
$x\in F$.  Then
\begin{itemize}  
\item $U_F$ is equal to the intersection of the three half-spaces
$$H^0(i)=\op{aff}_-^0(\{\orgn,v(f^{i+1} x),v(f^{i+2}x)\},v(f^{i}x)),\quad i=0,1,2$$
\item if a dart $y$ leads into $U_F$, then $y\in F$.
\end{itemize}
\end{lemma}

\begin{proof} The intersection of two half-spaces, $H^0(1)\cap H^0(2)$ is
the wedge $W(x)$.   The set $C^0_t\subset W(x)$ sweep out precisely
...
blades $C(i)=C(\orgn,\{v(f^ix),v(f^{i+1}x)\})\subset X$.  Thus,
...
The set $X'$ is the disjoint union of $\op{aff}_+(\orgn,v(f^ix))$ and
the three blades $C^0(i)$.  These blades do not meet the vertices, hence
$v(y)=v(f^i x)$ for some $i$.  Thus, $y$ and $f^i x$ belong to the same
node.  The sets $W(y)$ and $W(f^i x)$ are disjoint for darts at the same
node, and this implies that $y=f^i x$.
\end{proof}
