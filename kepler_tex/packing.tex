...
\item If $A$ is convex, then $A = \op{conv}(A)$.
\end{itemize}
%
...
\begin{proof} Let $\bu = [\u_0;\ldots]$ and $\bv = [\v_0;\ldots]$.  The proof splits into
two cases, according to whether $\u_0=\v_0$.

\claim{[$\u_0\ne\v_0$]}.  In this case,
the intersection $R(\bu)\cap R(\bv)$ lies in
$\Omega(V, [\u_0;\v_0])\subset A(\u_0,\v_0)$ and the conclusion follows.  
Now assume that $\u_0=\v_0$.

Let $k+1$ be the
first index such that
\begin{displaymath}
\Omega(V,[\u_0;\ldots;\u_{k+1}]) \ne \Omega(V,[\v_0;\ldots;\v_{k+1}]).
%\op{conv}\{\omega( [\u_0]),\ldots,\omega( [\u_0;\ldots; \u_{k+1}])\}\ne
%\op{conv}\{\omega([\v_0]),\ldots,\omega([\v_0;\ldots;\v_{k+1}])\}.
\end{displaymath}

\claim{Such an index $k+1$ exists.}  Indeed, the definition of points $\omega(\trunc{\bu}{i})$
...
%\end{displaymath}
The intersection $R(\u)\cap R(\v)$ lies in the convex hull 
$C$ of $\{\omega([\u_0]),\ldots,\omega( [\u_0;\ldots; \u_k])\}$ and 
$\Omega'=\Omega(V,[\u_0;\ldots;\u_{k+1};\v_{k+1}])$.  
The set $\Omega'$ lies in a facet of $\Omega(V,\trunc{\bu}{k+1})$.  Hence the affine dimension
of $\Omega'$ is at most $3-(k+1)-1=1-k$.  In general, if a set $A$ has affine dimension $r$,
then the affine dimension of $\op{conv}(\{\p\}\cup A)$ is at most $r+1$.  It follows that
the affine dimension of $C$ is at most $(1-k) + (k+1) = 2$.
The intersection
...
We sketch a proof of this well-known result.
...
\item[(convex hull)]  The circumcenter of $W_j$ lies in the convex
hull of $W_j$.
