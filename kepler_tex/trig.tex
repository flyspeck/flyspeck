% This is the first foundational chapter.
% Author: Thomas Hales, 
...
level of the textbook \cite{unknown}. %XX 
...
Certain sentences in real arithmetic can be expressed with nothing more than the usual logical operations (the operators {\it and}, {\it or}, {\it implies}, {\it logical negation}),  the ring operations (addition, subtraction, and multiplication) for the real numbers, comparison ($(=)$ and $(>)$) of real numbers, the constants $0$ and $1$, real-valued variables, and quantifiers (universal and existential) over the real numbers.  Such sentences are said to belong to the Tarski arithmetic.  For example, the sentence
\begin{equation}\label{eqn:tarski}
\exists x.~x^7 - 4 x - 3 = 0 ~~\land~~ x > 0.
\end{equation}
falls within the Tarski arithmetic (after expanding the exponent $x^7$ as
$x\cdot x\cdot x\cdot x\cdot x\cdot x\cdot x$ and the constants $4=1+1+1+1$
and $3=1+1+1$).  
Starting with Tarski, researchers have developed algorithms to decide the truth of any sentence in the Tarski arithmetic \cite{unknown}. %XX
Although these algorithms are generally too slow to be of practical use,  it is
useful to identify such sentences.  In this book, we will assume that the reader
has the skill to solve particularly simple problems in the Tarski arithmetic such
as determining that the sentence \eqref{eqn:tarski} is true.
\end{background}
...
\footformal{DIFF\_SIN\, DIFF\_COS}
  ...
  \sin(x) = x - x^3/3! + x^5/5! \cdots.
    ...
    \end{equation}
By \hyperref[back:analysis]{real analysis}, convergence is absolute for every real number $x$.
...
function.  That is,
    \begin{displaymath}\cos(-x) = \cos(x),\quad\sin(-x) = -\sin(x).\end{displaymath}
\end{lemma}
\indy{Index}{function}%
...
By \hyperref[back:analysis]{real analysis}, the derivative of
    \begin{displaymath}(\cos(-x) - \cos(x))^2 + (\sin(-x) +\sin(x))^2\end{displaymath}
is identically zero.  Complete the proof as in the proof of Lemma~\ref{lemma:sin-add}.
\swallowed\end{proved}
...
\begin{proved}
The proof is an exercise in \hyperref[back:analysis]{real analysis}.
The \hyperref[eqn:cos']{derivative of $\sin$} is non-\hyperref[eqn:cospos]{negative}  between $0$ and $\pi/2$.
The value of \hyperref[eqn:cos0]{$\sin$ at $0$} is $0$.  It follows that $\sin$ is non-negative on
$[0,\pi/2]$.  It is enough to check
that the squares of the two sides are equal. 
Then $\sin^2(\pi/2) \hyperref[lemma:circle]{\,=\,}
{1-\cos^2(\pi/2)} \hyperref[eqn:cospi2]{\,=\,} 1$.
\swallowed\end{proved}
...
\begin{proved}
    ...
    \begin{displaymath}\sin(\pi/2 - x) = \sin(\pi/2)\cos(-x) + \cos(\pi/2)\sin(-x)\end{displaymath}
    and use
...
\begin{lemma}[triangle inequality]\guid{OIPLPTM}\formal{Jordan/metric\_spaces.ml:norm\_triangle}\label{lemma:triangle}
  \begin{displaymath}
  \normo{\u + \v} \le \normo{\u} + \normo{\v }.
  \end{displaymath}
...
\begin{definition}[convex hull]  If $V = \{\v_1,\v_2,\ldots,\v_n\}$ is a finite set
...
\begin{definition}[line,~collinear,~independent]	
Any set of the form $\op{aff}\{\v,\w\}$ is a \newterm{line} when $\v\ne \w$.  A set that is contained in a line is \newterm{collinear}.  If $\{\orz,\v,\w\}$ is not collinear, then $\v$ and $\w$ are said to be \newterm{independent} (or $\{\v,\w\}$ is said to be independent).
\end{definition}
\indy{Index}{line}%
\indy{Index}{collinear}%

\begin{definition}[plane, half plane, coplanar]\label{def:plane}	
An affine hull $A=\op{aff}\{\u,\v,\w\}$ is a \newterm{plane} when $\{\u,\v,\w\}$ is not collinear.   A set $\op{aff}(\{\u,\v\},\{\w\})$
is a \newterm{half-plane} when $\{\u,\v,\w\}$ is not collinear. A set that is contained in a plane is  \newterm{coplanar}.
\end{definition}
\indy{Index}{plane}%
...
\begin{definition}[half space] A set $\op{aff}_{\pm}(\{\u,\v,\w\},\{\v'\})$ is a \newterm{half-space},
...
Cayley and Menger found a formula for the square of  the determinant $D$ of the
matrix with rows $\v_1-\v_0$, $\ldots,$ $\v_n-\v_0$, 
for arbitrary vectors $\v_i\in\ring{R}^n$.   Set
...
\end{displaymath}
The
left-hand side is evidently a square and the polynomial on the right is
non-negative, whenever the variables $x_{ij}$ satisfy~\eqn{eqn:xij}
for some vectors $\v_1,\ldots,\v_4\in\ring{R}^3$.  
Moreover, $D$ and hence also $\Delta$ is positive when the set of four vectors is not
...
Very little matrix theory is required in this book.  The next lemma is a rare
exception.  Its proof requires various very basic facts about $3$ by $3$ matrices
and determinants.  The determinant of a product of two matrices is the product
of determinants.  The transpose of a matrix $A$ has the same determinant as $A$.
The determinant of a matrix $A$ is zero if and only if there exists a (row) vector $\u$
such that $\u\, A = \orz$.
\end{background}
...
Define 
    ...
    \end{displaymath}
The value of this function is the \newterm{angle} at $\u$ formed by $\v$ and $\w$.
...
\begin{lemma}  \guid{QRAAWFS}\rating{ZZ}\label{lemma:ups}
...
Let 
  \begin{displaymath}(y_1,\ldots,y_6) =(y_{01},y_{02},y_{03},y_{23},y_{13},y_{12}),
   \text{ where } y_{ij}=\norm{\v_i}{\v_j}.\end{displaymath}
Set
$x_i = y_i^2$.   
and
    \begin{displaymath}
...
where $\w_i = \v_i- \v_0$.
Let \begin{displaymath}\alpha_i =\dih_V(\{\v_0,\v_i\},\{\v_j,\v_k\})\end{displaymath}
where $\{i,j,k\}=\{1,2,3\}$.
...
By real analysis, the derivative of $f$ is zero, and the function $f$ is constant
along any segment in $\ring{R}^3$ along which $\Delta$ is positive.  The
remaining part of the proof constructs two segments 
along which $\Delta$ is positive.  The first connects $f(a,b,c)$ to $f(a,2,2)$ (provided
the variables are ordered appropriately).
The second connects  $(a,2,2)$ to $(2,2,2)$.  From this construction it follows
that $f(a,b,c)=f(2,2,2)$.  The last step is to evaluate the constant $f(2,2,2)$.  Arithmetic gives
$\Delta=4$, $2p= 2$,  $u_1=u_2=u_3 =0$, when $a=b=c=2$.  Finally,
    \begin{displaymath}f(a,b,c)= f(2,2,2) = -\pi/2 + 2\arctan(1) =0.\end{displaymath}


Let us return to the construction of the two segments.
...
\claim{$\Delta$ is positive on the linear segment from $(a,b,c)$ to $(a,2,2)$.}
Indeed, for $0\le t \le 1$, Tarski \hyperref[back:tarski]{arithmetic} gives
\begin{displaymath}
\begin{array}{lll}
...
\claim{$\Delta$ is positive on the linear segment from $(a,2,2)$ to $(2,2,2)$.}
Indeed, 
...
%
%Conversely, for $A$ any great circle there is a unique $C$ that inscribes the 
%great circles $D_i$, and $A$.  The perimeter of the triangle is the sum of the 
%distances from $\v$ to the points $C\cap D_i$.
...
%Let $\{\v_0,\v_1,\v_2\}\subset\ring{R}^3$ be a set that is not collinear.
A tuple $(\e_1,\e_2,\e_3)$ is \newterm{adapted} to $(\v_0,\v_1,\v_2)$ if
$\e_3 = (\v_1-\v_0)/\norm{\v_0}{\v_1}$ and
$\e_1\in\op{aff}_+^0(\{\v_0,\v_1\},\v_2)$.
% such that $\normo{\e_1}=1$ and $\e_1\cdot\e_3=0$;
%$\e_2 = \e_3\times \e_1$.
% The tuple $E=(\e_1,\e_2,\e_3)$ is \newterm{adapted} to $(\v_0,\v_1,\v_2)$.
\end{definition}
\indy{Index}{frame}%
\indy{Index}{adapted}%

\begin{lemma}[orthonormalization]\guid{QAUQIEC}\rating{ZZ}\label{lemma:frame}
...
Let
  \begin{displaymath}\gamma = \dih_V(\{\v_0,\v_1\},\{\w_1,\w_2\}).\end{displaymath}
  Then
    \begin{displaymath}
...
Let $\bar{\w_i} = (\v_1\cdot \v_1) \w_i - (\v_1\cdot \w_i) \v_1$.  
From the assumptions, $\bar{\w_1}\ne 0$.  Set $\e_1 = \bar{\w_1}/\normo{\bar{\w_1}}$.  Choose a unit vector
...
By definition, $\cos\gamma$ is the angle between $\bar{\w_1}$ and $\bar{\w_2}$.
Compute
   \begin{displaymath}
   \begin{array}{lll}
     \bar{\w_1} &= \normo{\bar{\w_1}} \e_1 \\
...
Then 
\begin{displaymath}\sin(\op{azim}(\orz,\v_1,\v_2,\v_3))\sim (\v_1 \times \v_2)\cdot \v_3.\end{displaymath}
\end{lemma}

\begin{proof}
The relation $\sim$ is an equivalence relation.
We may assume that  $\{\orz,\v_1,\v_2\}$ and $\{\orz,\v_1,\v_3\}$ are not  collinear sets, for otherwise both sides are zero. 
Let $(\e_1,\e_2,\e_3)$ be the positive frame adapted to $(\orz,\v_1,\v_2)$.
%\begin{displaymath}
%\begin{array}{rll}
%   \e_3 &= \v_1/\normo{\v_1}\\
...
%\end{array}
%\end{displaymath}
Write $\v_3= r\cos\theta\, \e_1 + r\sin\theta \, \e_2 + h\, \e_3$ in cylindrical coordinates, where $\theta = \op{azim}(\orz,\v_1,\v_2,\v_3)$.
Then by the explicit formulas for the positive frame,
\begin{displaymath}
\begin{array}{rll}
...
The following lemma identifies the zenith angle $\phi$. 
As the zenith angle is easily expressed in terms
...
\rating{0}
\formalauthor{Nguyen Quang Truong}
Let $(\v_0,\v_1)$ be an ordered pair of distinct points in $\ring{R}^3$.
Let $\v_2\ne \v_0$.  Set $\phi = \arc_V(\v_0,\{\v_2,\v_1\})\in[0,\pi]$.
Let $\e_3$ be the unit vector $(\v_1-\v_0)/\norm{\v_1}{\v_0}$.  Let $r = \norm{\v_2}{\v_0}$.
Then $\v_2$
can be expressed in the form
   ...
   r\cos\phi\, \e_3,
   \end{displaymath}
where $\bar{\v}_2\cdot \e_3 = 0$.
The angle $\phi$ is called the \newterm{zenith} angle (or \newterm{latitude}) of $\v_2$ along $(\v_0,\v_1)$.
\indy{Index}{zenith}%
...
Assume that $\{\v_0,\v_1,\v_2\}\subset\ring{R}^3$ % and $\{\v_0,\v_1,\p\}$ 
is not a collinear set.
Let $(\e_1,\e_2,\e_3)$ be the positive frame adapted to $(\v_0,\v_1,\v_2)$.
  Then for any $\p$,
   \begin{equation}
...
\begin{displaymath}\p = \v_0 + r'\cos\theta\,\e_1 + r'\sin\theta\,\e_2 + h\, \e_3,\end{displaymath}
for some $h$ and $r'=\normo{\p-\v_0-h\,\e_3}\ge0$.  
The zenith angle puts $\p$ in the form
\begin{displaymath}\p = \v_0 + r'\cos\theta\,\e_1 + r'\sin\theta\,\e_2 + r\cos\phi\, \e_3,\end{displaymath}
where
\begin{displaymath}
\begin{array}{lll}
r^2 &= \norm{\p}{\v_0}^2\\ 
...
\end{array}
\end{displaymath}
Since $\sin\phi$ is non-negative, this implies that $r'=r\sin\phi$, as desired.
\end{proof}

\begin{definition}[spherical coordinates]\label{def:sph}
Equation~\eqref{eqn:sph} is called the spherical coordinate representation of
$\p$ with respect to $(\v_0,\v_1,\v_2)$.   
\end{definition}



(This book follows the variable
naming conventions $(\theta,\phi)$ of American calculus textbooks, 
which reverses the international scientific notation.)



%Any triple $(x,y,z)$ has spherical coordinates.
%The radial component is $r = \sqrt{x^2+y^2+z^2}$.  
%In the degenerate case when $r=0$, Equations~\eqref{eqn:spherical} 
...
%Let $n = (\w-\v) \times (\u-\v)$.
%Then the zenith angle of any $\p\ne \v$ in the plane $\op{aff}\{\v,\w,\u\}$,
...
Let $V=\{\v_1,\ldots,\v_k\}$ be a finite set of
nonzero points in the plane, with
polar coordinates $\v_i = (r_i\cos\theta_i,r_i\sin\theta_i)$.
...
polar coordinates.  Write $\v_i \prec \v_j$ if
...
There is a cyclic permutation $\sigma:V\to V$ which sends
$\v\in V$ to the next larger element with respect to this order,
or back to the first element if $\v$ is the largest.
Call $\sigma$ the \newterm{polar cycle}
of the set $V$.
...
Let \begin{displaymath}\theta_{ji} = \theta_i - \theta_j + 2\pi k_{ji},\end{displaymath}
...
\begin{definition}[cyclic set] Let $(\v_0,\v_1)$ be an ordered pair of distinct points in
$\ring{R}^3$.
...
a cyclic set $V$ with respect to $(\v_0,\v_1)$ does not depend
on the choice of $\p\in\ring{R}^3$ (such that $\{\v_0,\v_1,\p\}$ is non-collinear).
\end{lemma}
\indy{Index}{azimuth cycle}%
\indy{Index}{cyclic set}%
