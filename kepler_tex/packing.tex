...
have a common facet, and so forth.  Beware of the notation: 
 the index $k$ is the codimension of $\Omega(V,\bu)$ in $\ring{R}^3$,
...
The point is defined whenever $\Omega(V, \bu[j+1])$ is
...
R_2 &= \{\p \in R(\bu)\cap \Omega(V,\bu[1]) \mid \norm{ \u_0}{\p}=\norm{ \u_1}{\p} =\sqrt2\}.
\end{array}
\end{displaymath}
\bigskip
...
$\op{conv}(\{\xi(\bv[2])\}\cup R(\bv[2]))$ as $\bv[2]$ runs over
