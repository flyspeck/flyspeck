...
the techniques of Remark~\ref{remark:fan-verify}.
...
\begin{proof} Let $x = (\v,\rho\v)\in F$.  The first conclusion follows from the first part of Lemma~\ref{lemma:coplanar}.  
...
is an (relative) open condition that holds for small $t$.  This half-plane deforms into the bounding half-plane of $\Wdart(F,\v_0(t))$.  Hence $\u = \v_r\in \Wdart(F,\v_0(t))$ for sufficiently small $t$.
...
$$
This is increasing in the perimeter.    For sufficiently small $t$,  it remains a generic cyclic fan (Lemma~\ref{lemma:fan-open}).
Eventually, for some smallest $t=t_0$ a non-generic cyclic fan is reached (and the perimeter has increased to $2\pi$) or some $\angle(\varphi(\u,t_0))=\pi$.  In the latter  case,  the minimality condition fails, and the result follows.  If a non-generic cyclic fan is reached the result follows from previous cases.  A fan cannot be reached at $t=t_0$ that is not cyclic, because the conditions for a fan to be cyclic are closed conditions.  Finally, if the deformation fails to be a fan at time $t=t_0$, then either the \case{independence} or \case{intersection} property fails.  

The \case{independence} property does not fail.  The plane through $\{\orz,\v,\rho\v\}$ meets the plane through $\{\orz,\rho^{-2}\v,\rho^{-1}\v\}$ along a line through the origin.  Let $\varphi(\v,t_1)\ne\orz$ lie on that line, for some $t_1>0$.  At this point,
...
The \case{intersection} property does not fail. XX  (The fan at time $t<t_0$ is
in $W(\orz,\v,\rho\v,\rho^{-1}\v)$, which prevents the expansion at later time from meeting a blade.)

Thus, the deformation is a cyclic fan at $t=t_0$, and the perimeter is at most $2\pi$.
\end{proof}
