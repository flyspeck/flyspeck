...
/*
It depends on legacy_simplex, and I want to remove the dependency
of recurse.cc on legacy_simplex so that I can use autogenerated library.
...
int f206A(int depth,double xx[6],double zz[6],cellOption options) {
  ...
  const Function F = 
    ( (s.mode==numerical_data::strategy::n1) ? legacy_simplex::num1   :
      ((s.mode ==numerical_data::strategy::n1m) ? legacy_simplex::num1 * mone  :
       ((s.mode == numerical_data::strategy::n2m) ? legacy_simplex::num2 * mone :
	(assert(s.mode == numerical_data::strategy::merge),
	 legacy_simplex::num2 * malpha * mone + 
	 legacy_simplex::num1 * alpha) ) ) );
