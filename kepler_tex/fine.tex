...
%of those with a vertex at $v$.\indy{Index}{Qv@$\CalQ(\Lambda,v)$}
...
\begin{lemma}\guid{ZKYMYWJ} \tlabel{lemma:voronoi-truncation-over-Q}\rating{0}\oldrating{100}\formalauthor{Nguyen Quang Truoung, 9/2008}
...
%If $x$ lies in the \indy{Index}{Voronoi cell} Voronoi cell at $v_0$, 
...
By Tarski arithmetic\tarf{vor-bar-tet}\tarf{vor-bar-quad},
...
then $Q$ too is in
the $Q$-system.  By Tarski arithmetic\tarf{pass-cone},  $x\in\op{aff}_+(v_0,\{v_1,v_2,v_3\})$.  
...
%to Tarski arithmetic\tarf{flat-Q}. 
The rest is clear.
\end{proof}
...
\begin{lemma}\guid{FOCUDTG}\tlabel{lemma:VC-Omega}\rating{100}
...
$w\ne v_0$.  
% By Tarski arithmetic\tarf{unobstr-t0}, 
From $x\not\in Y$, it follows that the point $v_0$ is
unobstructed  at $x$ by Tarski arithmetic\tarf{unobstr-t0}.  
Thus, $|x-w|< |x-v_0|\le t_0$.  By
Tarski arithmetic\tarf{unobstr-t0} again, $w$ is unobstructed at $x$, so
...
\begin{lemma}\guid{OVOAHCG} [Decoupling Lemma]\tlabel{lemma:V-cell-local}\rating{100}
...
By Tarski arithmetic\tarf{decouple}, 
...
$F$ is a barrier.  Again, by Tarski arithmetic\tarf{decouple},
...
\indy{Greek}{zzeta@$\eta_{V0}$}
\end{definition}

%By Tarski arithmetic\tarf{1453}, 
%if $h\le\sqrt2$, then $\eta(2h,2,2t_0)\le \eta(2,2t_0,\sqrt2) <
%1.453$.\indy{Greek}{ZZZZ1.453@1.453}
...
\begin{lemma}\guid{BVFXVOM}\tlabel{lemma:fitted-q1q2}\rating{100}
...
If $\alpha=\op{azim}(v_0,v,w_1,w_2)\ge\pi$, we argue as follows.
By construction $\op{azim}(v_0,v,w_1,q_1)$, $\op{azim}(v_0,v,q_2,w_2)\le\pi/2$.
Then
$$
 \op{azim}(v_0,v,w_1,q_1)\le\pi/2\le\alpha - \pi/2 \le
 \alpha-\op{azim}(v_0,v,q_2,w_2)=\op{azim}(v_0,v,w_1,q_2).
$$

Now assume $\op{azim}(v_0,v,w_1,w_2) < \pi$.
Let $S=\{v_0,v,w_1,w_2\}$.
The inequality $2.77 \le |w_1-w_2|$ implies that the simplex $S$
has positive orientation at $w_1$ and $w_2$ (by Tarski arithmetic\tarf{neg-orient-quad}). 
...
%in the wedge $W$.
%\indy{Greek}{zzdelta@$\bigd(v,W)$}
%\indy{Greek}{zzDelta@$\FC(v,W)$}
%\end{definition}
%
...
\begin{lemma}\guid{XKZWPNE}\tlabel{lemma:meet-normal}\rating{100}
...
Say $w_2\ne u_1,u_2$.  If $\op{aff}_+(v_0,\{v,w_2\})$ meets $F$, then by Tarski arithmetic\tarf{pass-makes-quarter},
...
So $w_2\in\op{aff}_+(v_0,\{v,u_1,u_2\})$.  By Tarski arithmetic\tarf{XX},
...
\begin{lemma}\guid{ZHEVXFD}\tlabel{lemma:new-anchor}\rating{80}
Let $(\Lambda,v_0)$ be a centered packing
...
We have $|w'-u|\le 2t_0$ by Tarski arithmetic\tarf{pass-makes-quarter}.  
Thus, $\{v_0,v,w',u\}$
...
\begin{lemma}\guid{QURYBSL}\tlabel{lemma:FC-}\rating{80}  
...
Tarski arithmetic\tarf{eps-bigd-}
...
an upright quarter.  By Tarski arithmetic\tarf{consec-anchors}, $u_1$ and $u_2$
...
\begin{lemma}\guid{CGNBSFX}\tlabel{lemma:fine-Rw}\rating{50}
...
except with one Tarski arithmetic\tarf{eps:fine:Rw} calculation 
substituted for 
another\tarf{eps-bigd-}.
\end{proof}
...
%\begin{lemma}\guid{QVPJLAK}\tlabel{lemma:fine:barrier}  
...
%$\{v_0,v,w\}$ is a barrier.  By Tarski arithmetic\tarf{tip-cone}, we have
...
%$w'$ between $u$ and $w$.  By Tarski arithmetic\tarf{prev},
...
\begin{lemma}\guid{HDDIOWP}\tlabel{lemma:fine-Rw:5}\rating{300}
...
Tarski arithmetic\tarf{fine-Rw-split} enumerates six different possible
configurations of points $\{v_0,v,w,u_1,u_2\}$.  We discuss each
in turn.

In the first three cases, 
%
%Assume that $\epsilon_0=w$.  By Tarski arithmetic\tarf{fine:Rw:5},
...
Lemma~\ref{lemma:fine-Rw:5}.\FIXX{This is fine-Rw:5}
...
In a quarter, its two anchors are consecutive, by Tarski arithmetic\tarf{consec-anchors}.  Thus,
...
\begin{lemma}\guid{NVCHDMC}\tlabel{lemma:delta-tri}\rating{200}
...
%
%\begin{lemma}\guid{FSHBKTW}\tlabel{lemma:delta-flat}
%Let $(\Lambda,v_0)$ be a centered packing.
...
%\end{proof}
%
%\begin{lemma}\guid{KURAGPW}\tlabel{lemma:delta-upright}
...
\begin{lemma}\guid{RTPCNDU}\tlabel{lemma:FC-unobstr}\dcg{Lemma~9.12}{92}\rating{400}
...
By Tarski arithmetic\tarf{tip-cone},
...
By Tarski arithmetic\tarf{tip-cone}, the segment $\op{conv}\{u_1,y\}$
...
\begin{lemma}\guid{OTPJFDT}\tlabel{lemma:FC-VC}\rating{200}  
Let $(\Lambda,v_0)$ be a centered packing.
...
result now follows from Tarski arithmetic\tarf{decouple}.

If $\rad_V(S) <\eta_{V0}(v,v_0)$, then $u$ and $w_1$ are anchors.  In
this case, the result follows from Tarski arithmetic\tarf{prev}.\FIXX{Check reference}
...
\begin{lemma}\guid{XEPIHTG}\tlabel{lemma:FC-no-over}\rating{400}
...
positive.  The circumradius of $S$ satisfies
    ...
    $$
By Tarski arithmetic\tarf{eps-inner}, 
...
By Tarski arithmetic\tarf{eps-outer}, if these sets meet the points fall into
...
Tarski arithmetic\tarf{prev} are met.  This gives that $Q=\{v_0,v,w',w''\}$ is an upright
...
%\eta_{V0}(u,v_0)$, then Tarski arithmetic\tarf{prev} implies that each point
...
$$
%
 \indy{Index}{labels!edge}
 \indy{Index}{first!edge}
 \indy{Index}{edge convention}
\end{definition}
...
\begin{definition}[enclosed] \label{def:enclosed}\indy{Index}{enclosed}
...
%
 \indy{Index}{vertex!enclosed}\indy{Index}{enclosed}
\end{definition}
...
\begin{lemma}\guid{WKFXGOH}\tlabel{lemma:2.77}\rating{200}
...
By Tarski arithmetic\tarfE{enclosed-v}, an
...
Tarski arithmetic\tarfE{pass-anchor}, $v_1$ and $v_2$ are anchors of
...
$2t_0$ by Tarski arithmetic\tarf{double-face}. If $\{v_0,w\}$ is 
an upright diagonal, the quarter takes the form
$\{v_0,w,v_1,v_3\}$, or $\{v_0,w,v_2,v_3\}$ for some $v_3$, by
Tarski arithmetic\tarf{double-face}. If both of these are quarters, then
...
with $|v_i-v_0|,|v_j-v_0|\le 2t_0$ by Tarski arithmetic\tarf{skew-quad}.  The
only possibility is that the edge $\{v_4,v_5\}$ crosses the two
edges with endpoint $v_1$, with $|v_1-v_0|\ge2t_0$ in case $\SC$.  But
this too is impossible by Tarski arithmetic\tarf{double-face}.
...
\begin{lemma}\guid{EJMNANQ}\tlabel{lemma:no-overlap}\rating{200}
...
so $S$ has type $\SA$. By Tarski arithmetic\tarfE{277}, we have
...
\begin{lemma}\guid{DVQVUEF}\tlabel{lemma:typeA-Omega}\rating{100}
...
% ...
By Tarski arithmetic\tarf{vor-bar-sqrt2}, $x$ and $v_0$ lie on the
...
By Tarski arithmetic\tarf{vor-bar-sqrt2}, $\op{conv}\{v_1,v_2,v_3\}$ 
separates
...
\begin{lemma}\guid{CBCGXQH}\tlabel{lemma:typeA-crown}\rating{50}
...
\begin{lemma}\guid{UUBEDBC}\tlabel{lemma:typeB-Omega}\rating{100} 
...
\begin{lemma}\guid{ENLMBGV}\tlabel{lemma:typeB-rcone}\rating{20}
 Let $S=\{v_0,v_1,v_2,v_3\}$ be a simplex of type $\SB$ in a centered packing $(\Lambda,v_0)$.
Then  $\op{rcone}^0(v_0,v_1,|v-v_0|/2.77)$ does not meet the
$\op{cone}(v_0,\{v_2,v_3\})$.
\end{lemma}

\begin{proof} This is Tarski arithmetic\tarf{beta:B}.
\end{proof}

\begin{lemma}\guid{QMRCLTD}\tlabel{lemma:typeB-crown}\rating{50} 
...
\begin{lemma}\guid{NOJVOSP}\tlabel{lemma:typeC-Omega}\rating{100}
...
\begin{lemma}\guid{YPHKFPC}\tlabel{lemma:typeC-crown}\rating{50}
Let $(\Lambda,v_0)$ be a centered packing with crown tuple $(v_0,v,w_1,w_2)$.
...
\begin{lemma}\guid{WAIOBCW}\tlabel{lemma:typeD-anchor}\rating{50}
...
\begin{lemma}\guid{HAYPLOG}\tlabel{lemma:C'Q}\rating{50}
The flat quarter along the face $\{v_0,v_1,v_2\}$ is
in the $Q$-system.
\end{lemma}

\begin{proof}
By Tarski arithmetic\tarfE{245}, there cannot be an enclosed vertex
of height at most $\sqrt2$. 
So nothing is enclosed over the flat quarter.
By Tarski arithmetic\tarfE{245bis}, there cannot be an edge of length
...
\begin{lemma}\guid{POWGHIV}\tlabel{lemma:VC-no-conv}\rating{50}
...
By Tarski arithmetic\tarf{vor-bar-quad}, this implies that 
...
\begin{lemma}\guid{CSOQOWG}\tlabel{lemma:typeD}\rating{50}
...
\begin{lemma}\guid{QBPLLTF}\tlabel{lemma:fine-2}\rating{50}
...
$S$.  Thus,  $V_S(t_S) \subset \op{conv}(S)$.  By Tarski arithmetic\tarf{convex-ball}, 
...
\begin{lemma}\guid{PBHCTOO}\tlabel{lemma:crown-2}\rating{50}
...
\begin{lemma}\guid{UETPGXL}\tlabel{lemma:R-subset}\rating{20}
...
\begin{lemma}\guid{OYWBMHJ}\tlabel{lemma:pack-fan}\rating{20}
...
\begin{proof}
Use Tarski arithmetic\tarfE{2t0-doesnt-pass-through}.
\end{proof}
...
\begin{lemma}\guid{HKKVYTF}\tlabel{lemma:Q-in-region}\rating{60}
...
%\begin{lemma}\guid{GSGLZWH}\tlabel{lemma:graph-compat}
...
\indy{Index}{triangle}\indy{Index}{quadrilateral}\indy{Index}{exceptional}
\end{definition}
...
%\begin{lemma}\guid{IZZSNKV}\tlabel{lemma:FC-standard}
...
\begin{lemma}\guid{IHDUARW}\label{lemma:tri-class}  
...
By Tarski arithmetic\tarf{EE}, there
...
%\end{definition}
%
% \indy{Index}{cluster!standard}
% \indy{Index}{cluster!quad}

\begin{lemma}\guid{MXVTOXQ}\tlabel{lemma:quad-classify}
  ...
  \item (flat) $\{v_0,v_1,v_2,v_3\}$ and $\{v_0,v_1,v_3,v_4\}$ are flat
   ...
   \item (pure) We have $|v_1-v_3|>\sqrt8$, $|v_2-v_4|>\sqrt8$ and
    ...
    \item (mixed) We have $|v_1-v_3|>\sqrt8$, $|v_2-v_4|>\sqrt8$, there
      ...
      \indy{Index}{pure}\indy{Index}{mixed}
\end{itemize}
\end{lemma}
...
\begin{lemma}\guid{JLXKOHL}\tlabel{lemma:quad-fan}\rating{60}\usage{Assembly pent-prism}
...
\begin{lemma}\guid{RQNFDFG}\tlabel{lemma:pure-sqrt2}
...
\begin{lemma}\guid{EEPIGGK} Let $(\Lambda,v_0)$ be  a centered packing.
...
It uses a result of Tarski arithmetic\tarf{enclosed} 
...
\begin{lemma}\guid{ULPPVFT}\tlabel{lemma:Omega-U-meas}\rating{80}
Let $(\Lambda,v_0)$ be a centered packing.  Let $(v_0,V,E)$ be a fan
...
\begin{lemma}\guid{ZIRXULI}\tlabel{lemma:VCM-fan}\rating{60}
...
\begin{lemma}\guid{AZSPGPD}\tlabel{lemma:VCM-alphabet}\rating{60}
...
\begin{lemma}\guid{MPXGINF}\tlabel{lemma:Omega-fan-measure}\rating{100}
...
\begin{lemma}\guid{OQCYHRF}\tlabel{lemma:VCM-VC}\rating{40}
Let $(\Lambda,v_0)$ be a centered packing.
...
Tarski arithmetic\tarf{beta:dcg-144a}.  
...
The result now follows from Tarski arithmetic\tarf{beta:dcg-144b} when
$y_1\ge 2.2$ and from Tarski arithmetic\tarf{beta:dcg-144c} when
$y_1\le 2.2$.
\end{proof}
...
\begin{lemma}\guid{NTMPSSA}\label{lemma:omega-simp}\FIXX{Move to volumes}  Let $S = \{v_0,v_1,v_2,v_3\}$ be a set of four points in $\ring{R}^3$.
...
\begin{proof} By Tarski arithmetic\tarf{omega-rog}, the set $\Omega(v_1,S)$ is a union of
...
(The denominator is always nonzero when $S$ is not coplanar by Tarski arithmetic\tarf{ups0}\tarf{delta0}.)
The following appeared as Claim~\ref{claim:volan}.


\begin{lemma}\guid{EAFHFFN}\tlabel{lemma:volan}  %%Cf. claim:volan
...
Set
      ...
      $$
\indy{Index}{vzorR@$\op{svR}$} \indy{Greek}{zzsigmaR@$\sigma$}
\end{definition}

\begin{lemma}\guid{DORMNYA}\tlabel{lemma:sigma-sum}
...
\begin{lemma}\guid{CVSWDIM}\tlabel{lemma:R'}
...
will be of particular importance.
    Set $A(h) = (1-h/t_0) (\phi(h,t_0)-\phi(t_0,t_0))$.\indy{Index}{A}
...
\begin{lemma}\guid{QTXANGU}\tlabel{lemma:sigma-tau}
    %\proclaim{Lemma 3.2}
...
\begin{lemma}\guid{MCZYGZL}\tlabel{lemma:squander-contravene}
