...
The parameters $a,b,c$ lie in $\leftclosed2/\hm,4\rightclosed$ and satisfy $\Delta(4,4,4,a^2,b^2,c^2)\ge0$.
...
Calculation~\ref{calc:cc:d2a} shows that the function
...
%As a consequence, the minimum occurs when a new flat node forms.   Then eliminate any case with four consecutive nodes that are not flat (after treating the quadrilateral with four edges $y_{ij}=2$ as a special case).
...
\begin{calculation}\guid{}\rating{ZZ}\label{calc:Lexell}
%See Mathematica numerical calculation.
Let
$$
g(s;a,b,c,e_1,e_2,e_3) = \dih(2,2,2,a+s,b,c) e_1 + \dih(2,2,2,b,c,a+s) e_2 + \dih(2,2,2,c,a+s,b) e_3.
$$
Let primes denote derivatives with respect to the variable $s$.
Assume that
$e_i\in\leftclosed1,1+\sol_0/\pi\rightclosed$,  that
$a,b,c\in\leftclosed2/\hm,4\rightclosed$, and that $\Delta(4,4,4,a^2,b^2,c^2)\ge0$.
Then
$$
g'(0;a,b,c,e_1,e_2,e_3)^2 - g''(0;a,b,c,e_1,e_2,e_3) > 0.
$$
\end{calculation}

\begin{calculation}\guid{}\rating{ZZ}\label{calc:2der}
...
$$
where  $\sol_0=3\arccos(1/3)-\pi$, and
$$
\begin{array}{lll}
...
\begin{calculation}\guid{}\rating{ZZ}\label{calc:cc:d2a}
...
g'(0)=0,
$
then $g''(0)<0$.\footnote{The function $g(t)$ may fail to be differentiable at $t=0$ for parameters $y_1,\ldots,y_6$ for which $\Delta(y_1^2,\ldots,y_6^2)=0$.  Thus, it is necessary to work on the noncompact domain $\Delta>0$ for this inequality.  It can be compactified as follows.  The derivatives $g^{(i)}(0)$ can be replaced by limiting values of
$\Delta((y_1+t)^2,\ldots,y_6^2)^k g^{(i)}(t)$ as $t$ tends to zero, for the smallest positive exponent $k$ that gives a finite limit.}
