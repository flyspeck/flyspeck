...
%
% % Listing styles
\lstset{float=htb,columns=flexible,frame=lines,basicstyle=\scriptsize,
...
theories have been implemented.\ednote{@CL, maybe insert graph of the ontology?}  Relevant
...
OMDoc-specific calculus will have to be used, which is currently in development\ednote{To
  do: Check this!}.
...
collections (like OpenMath content dictionaries) in the system.\ednote{This is honest and
  really sufficient IMHO, but maybe sounds a bit weak -- what do you think? --CL} For
...
using the SIOC ontology~\cite{SIOC:web}, SWiM can also support the coordination of the
project.  OMDoc markup and mathematical formulae in discussion pages are not yet
...
ontologies loaded into the wiki\footnote{Types of OMDoc links are automatically extracted
  from the markup; see above.}.  That means that the annotations required by Flyspeck are
possible, but not in an ad-hoc way, which we would have found useful in the prototyping
phase.  Instead, one would have to create or import a formal ontology first (SWiM supports
basic ontology editing), and then one would be able to annotate documents using terms from
that ontology.
...
queries are easy to express in SPARQL; consider the query about lemmas about a certain
topic that have not been proven so far:
