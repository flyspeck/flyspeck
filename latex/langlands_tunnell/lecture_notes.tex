...
\begin{theorem}[Tychonoff]  The product of family of compact topological
spaces is compact.
\end{theorem}
...
corresond with $(a_{p_1^{i_1}},\ldots,a_{p_k^{i_k}})$ under this isomorphism.
\end{proof}
...
\begin{exercise}  Show that every neighborhood of $0$ in $\ring{Z}_p$ contains
...
\begin{theorem} The groups $\op{Gal}(K/F)$ is isomorphic to $\lim_{\leftarrow}\op{Gal}(E/F)$
...
Define $\sigma x = \sigma_{E_1} x$.  If $x\in {E_2}\in I$, it follows from the compatibility
...
\begin{proof}  Let $E_2\in I$ be a finite extension of $F$.  If we show
...
component $E_2\in I$ and $\op{Gal}(K/E)$ at every other factor.
\end{proof}
...
Then $\sigma'x \ne x$, and $x\not\in E$.  This proves the reverse inclusion.
\end{proof}
