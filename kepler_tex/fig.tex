...
\input /tmp/x.txt  % generate by tikz.ml : Tikz.gen_out.
...
%\def\tinydot#1{\draw[fill=black] (#1) node [inner sep=0.5pt,shape=circle,fill=black] {}}
...
% not recommended: it converts everything to points, and units get messed up.
...
It can be constructed by placing an octahedral pyramid along each facet
...
HCP pattern is rotated 60 degrees with respect to the FCC pattern.  In the HCP
pattern,
there is a uniquely determined plane of reflectional symmetry, 
containing six of the
twelve points.  The FCC pattern is a cuboctahedron.
}
{
[scale=1.8]
\begin{scope}
...
\tikzfig{sanskrit}{\guid{KSOEMIZ} 
  Derivation of Sanskrit formula.  A cannonball packing can be
...
the facets), 
giving density $4(4\pi/3)/\sqrt8^3 = \pi/\sqrt{18}$.
}
{
[scale=0.95]
\autoTCFVGTS
...
\def\figODGBUWK{ \tikzfig{marchal-intro}{\guid{ODGBUWK} Marchal
    cells of a two-dimensional sphere packing. } 
{ 
[scale=2.7] 
\autoBWEYURN
} }
...
\def\figWMWCTGH{
\tikzfig{quot}{\guid{WMWCTGH} When the hypermap comes from a plane graph,
we may depict the normal family as a collection of
 shaded loops alongside the edges of
the graph.  The subquotient is the hypermap associated with the traversed
...
This marked hypermap is described in (\ref{ex:graph-gen}).  As usual, the angles
of a plane graph are the darts of the hypermap, and the permuations $e,n,f$ are
...
Starting in the first frame with the marked hypermap described in (\ref{ex:graph-gen}),
...
{
%[scale=1.0,z={(0.0,-0.7)},y={(0.0,0.7)},x={(1,0.0)}] % Marchal polyhedron of a regular dodec.
[scale=1.0,z={(0,0)}]
\def\an{10}
\begin{scope}
...
For simplicity, we illustrate the two-dimensional analogue of the Marchal partition.
...
\def\figQTCGYTB{
\tikzfig{reflex}{\guid{QTCGYTB} A nonreflexive
...
\def\figWKUYEXM{
\tikzfig{slice-torser}{\guid{WKUYEXM} Given $p,q\in I$, the slice $I[p,q]$ follows
