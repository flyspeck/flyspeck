...
to study than the Voronoi cell itself.  The truncation parameter must be at least
the radius of the circumradius $\sqrt{3}\tan{\pi/5}\approx 1.258$ of the regular dodecahedron 
because the optimal Voronoi cell must not be truncated at all.  The truncation parameter $1.26$
that we use in the proof of the Kepler conjecture is well-suited for the strong dodecahedral
conjecture as well.
\end{remark}
...
\[
  D_4(\bu)=\bigcup \  \{ R(\v)\cap B\mid {\omega( \bu)
=\omega(\bv),  \v\in  \bV(3),\ \trunc{\v}{0}= \trunc{\u}{0}} \}.
\]
...
\begin{definition}[$D_3$]
  ...
  $b(\bu)< 1.26$.  If $h(\bu) \ge 1.3$ and $b(\bu)< 1.26$, there exists a
  unique point $\xi$ on the segment from $\omega( \trunc{\bu}{2})$ to
  $\omega(\bu)$ such that $\xi$ has distance  $1.3$ from $ \u_0$.  Let
\[
