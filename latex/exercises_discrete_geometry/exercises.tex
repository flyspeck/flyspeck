...
Call a nonempty packing $V$ in $\ring{R}^3$ in which every $\v\in V$ has
distance $2$ from twelve other $\w\in V$ a \fullterm{packing with full
...
\begin{proof}
  ...
  \claim{We claim that $(V,E^+(V))$.} Indeed, it is checked by
...
\begin{remark}
This section was moved from the book {\it Dense Sphere Packings} to the
exercises on May 4, 2012 because of a minor error in the statement
``The nonreflexive node adjacent to a reflexive node\dots can be deformed.''
in the proof of the main estimate.  There may be other nodes around that
prevent the nonreflexive node from moving in the way described.  This
seems implausible, but it must be ruled out.
\end{remark}
