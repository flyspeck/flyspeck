...
\begin{definition}[wedge,~W]\index{wedge}\tlabel{def:tar:wedge}
...
%% Lemma for UREVUCX.
\begin{lemma}\guid{JGXXUKH}\tlabel{tarsk:three-lune}\rating{100}
...
\begin{lemma}\guid{MJNUTQH}\tlabel{tarski:eps-outer}\usage{lemma:FC-no-over}\rating{320}
...
the first case holds.  Thus, we assume that
$$
     H=\op{aff}^0_-(\{v_0,v_1,w_1\},w_2) = \op{aff}^0_-(\{v_0,v_1,w_1\},p_2);
     $$
The dihedral angle $\dih_V(\{v_0,w_1\},\{v_1,w_2\}) < \pi/2$ (justified by
the arguments of Section~\ref{sec:trig}),\FIXX{Counterexample:
...
Our assumption gives  $x \in R_1 \subset H$.  Also, $R_2 = \op{rog}^0(v_0,w_2,\ldots)$
is convex, contains $x$, and extends to $y=(v_0+w_2)/2$.  The plane
...
\begin{lemma}\guid{ZDKDXFM}\tlabel{tarski:lim-sim-6}\rating{80}
...
\begin{definition}[mask]\index{masked}
