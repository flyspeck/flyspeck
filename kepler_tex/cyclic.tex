...
\claim{The \case{independence} property holds}.  Indeed, the set $\{\orz,\rho\v,\v,\rho^i\v\}$ is not coplanar for $i=-1,-2$, because otherwise some interior angle is $\pi$, by Lemma~\ref{lemma:A}.   
...
Before the time $t_1$ at which $\{\orz,\rho\v,\varphi(\v,t)\}$ is collinear, there is an earlier time $t_2$ at which $\varphi(\v,t_2)\in L$.  
