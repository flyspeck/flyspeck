...
\] 
for all $\v\in V$.  This is the \newterm{interior} angle of the nonreflexive local fan at $\v$.  Also, write
\[ 
...
\end{cases}
\] 
%This deformation $(V(t),E(t),F(t))$ is a nonreflexive local fan for all $t\in I$.
...
remain parallel.  Thus, the nonreflexive local fan remains lunar.
\end{proof}
...
This  result adapts easily  to a nonreflexive local fan, which also has
 a perimeter.  This section proves the bound $2\pi$ on the perimeter of a
nonreflexive local fan (Lemma~\ref{lemma:convex-hyp}).  A great circle has
perimeter $2\pi$.  The proof follows the classical argument, which deforms the nonreflexive local fan in a way that flattens out the interior angles and increases
the perimeter.  Eventually a circular nonreflexive local fan or lunar nonreflexive local fan is reached.  Both of these extremal cases have perimeter $2\pi$. 
...
proof requires two other estimates that are similar in nature to the
main estimate.  These secondary estimates will be given at the end of the chapter.

%To give a uniform proof of the main estimate and
%secondary estimates, we collect the parameters that are estimate
%specific into a \newterm{profile}.  The first basic
%profile contains the parameters that are relevant for the main
%estimate.
...
\item \case{local~fan} $(V,E,F)$ is a nonreflexive local fan (Definition~\ref{def:convex-local}).
\item \case{subset} $S\subset E$.
\item \case{s~norm} If $\{\v,\w\}\in S$, then $\norm{\v}{\w}=2\hm$.
\item \case{e~norm} If $\{\v,\w\}\in E\setminus S$, then $\norm{\v}{\w}\le 2\hm$.
...
The proof of the following lemma is deferred until Lemma~\ref{lemma:std-fan}.

\begin{lemma*}\label{deferred:std-fan}
...
Assume the deferred result~\ref{deferred:std-fan}.
...
\claim{$(V(\p),E(\p),F(\p))$ is a nonreflexive local fan.} The properties
...
Let $(V,E,F,S)$ be a special fan.  Let $\v,\w\in V$ be
...
Let $(V,E,F,S)$ be a special fan (with parameters $r$ and $s$).  Pick
...
\begin{lemma}[generic]\guid{RRAJQBH}\rating{750}\label{lemma:circular-nonmin}
Every minimal fan is generic.
\end{lemma}

\begin{proof}
  By the classification of nonreflexive local fans, it is enough to show
...
nonreflexive local fans $(V,E,F)$.  Recall that a deformation is continuous
...
special fan.}  The deformed value is a nonreflexive local fan for small positive
...
Let $(V,E,F)$ be a nonreflexive local fan.  If $V$ is contained in a plane
...
Every nongeneric nonreflexive local fan $(V,E,F)$ has at least three nonflat
nodes in $V$.
\end{lemma}
...
\begin{lemma}[feature list]\guid{IDOTAPN}\rating{50}\label{lemma:feature}\label{l:f}
No special fan $(V,E,F,S)$ has all of the following features:
\begin{enumerate}\wasitemize 
\bigskip
\centerline{\bf{extremality}}
\smallskip
\item \case{extreme~edge~\ref{l:ee}} For every $\v\in V$ and every $\w\in E(\v)$, the edge
$\{\v,\w\}\in E$ is $S$-extremal.
\item \case{minimal~node~\ref{l:mn}} For every $\v\in V$, either $\normo{\v}=2$
  or there exists $\w\in E(\v)$ such that $\{\v,\w\}$ is $S$-minimal.
\item \case{s~minimal~\ref{l:sm}} Either $r+2s = 6$, or every edge of $E$ is $S$-minimal.

%%
\bigskip\centerline{\bf{general flat node}}\smallskip
\item \case{flat~exists~\ref{l:fe}} For every $\v\in V$, at least one of $\v_1,\v_2,\v_3,\v_4$ is flat, or  $r+s\le 4$ and
$\norm{\u}{\w}=2$ for all $\{\u,\w\}\in E$.
\item \case{no~triple~flat~\ref{l:ntf}} For every $\v\in V$, at least one of $\v_0$, $\v_1$, and $\v_2$
is not flat.
\item \case{flat~count~\ref{l:fc}} There are at least three nonflat nodes of $V$.

%%
\bigskip\centerline{\bf{single flat node}}\smallskip
\item \case{balance~\ref{l:b}} For every $\v\in V$, if $\v$ is flat and if $\{\u,\v\},\{\w,\v\}\in
E\setminus S$, then
\[ 
\norm{\u}{\v} = \norm{\w}{\v}.
\] 
\item \case{minimal~node~flat~\ref{l:mnf}} 
For every $\{\u,\v\}\in E$, if $\v$ is flat, then $\{\u,\v\}$ is $S$-minimal 
or $\normo{\v}=2$.


%%
\bigskip\centerline{\bf{adjacent flat nodes}}\smallskip
\item \case{s~flat~\ref{l:sf}} For every $\v\in V$, if $\v_1$ and $\v_2$ are both flat, then 
\[ S\cap
\{\{\v_0,\v_1\},\{\v_1,\v_2\},\{\v_2,\v_3\}\} =
\emptyset.\] 
\item \case{flat~middle~\ref{l:fm}} For every $\v\in V$, if $\v_1$ and $\v_2$ are both flat, then
\[ 
\norm{\v_1}{\v_2} = 2.
\] 
%\item \case{flat~extremal~\ref{l:fext}} ( deprecated on May 11, 2011.)
%For every $\v\in V$, if $\v_1$ and $\v_2$
...
%%
\bigskip\centerline{\bf{cardinality}}\smallskip
\item \case{card~\ref{l:c}} 
Let      $s=\card(S)$ and $r=\card(E) - s = \card(F)-s$.  Then
\[ 0\le s \le 3\textand 3-s \le r \le 6 -
2s.\] 

\item \case{triangle~free~\ref{l:tf}} $\op{card}(F) \ne 3$.
\item \case{quadrilateral~free~\ref{l:qf}} $\op{card}(F) \ne 4$.
\item \case{pentagon~free~\ref{l:pf}} $\op{card}(F) \ne 5$.
\item \case{hexagon~free~\ref{l:hf}} $\op{card}(F) \ne 6$.
\end{enumerate}\wasitemize 
\end{lemma}
...
Next, to contradict the existence of minimal fans,
we give a long series of lemmas that show that every minimal fan
...
\begin{lemma}[card]\guid{FPITROS}\rating{50}\label{l:c}
Every minimal fan $(V,E,F,S)$ has the feature \case{card~\ref{l:f}}.
\end{lemma}
...
\begin{lemma}[no~triple~flat]\guid{TESVAFW}\rating{80}\label{l:ntf}
Every minimal fan $(V,E,F,S)$ has the feature \case{no~triple~flat~\ref{l:f}}.
\end{lemma}

\begin{proof}  Three flat nodes produces a long arc
\[ 
  \arc_V(0,\{\v_0,\v_4\}) 
...
\begin{lemma}[flat~count]\guid{GOKZLRP}\rating{40}\label{l:fc}
Every minimal fan $(V,E,F,S)$ has the feature \case{flat~count~\ref{l:f}}.
\end{lemma}
...
\begin{lemma}[s~flat]\guid{SDCCMGA}\rating{100}\label{l:sf}
Every minimal fan $(V,E,F,S)$ has the feature \case{s~flat~\ref{l:f}}.
\end{lemma}
...
&\le\arc_V(\v_0,\v_3).
\end{align*}
where $y=\normo{\v_0}$.  The strict inequality is a \cc{SDCCMGA}{}.
This is a contradiction.
\end{proof}

\begin{lemma}[extreme~edge]\guid{BAWWPPB}\rating{900}\label{l:ee}
Every minimal fan $(V,E,F,S)$ has the feature \case{extreme~edge~\ref{l:f}}.
\end{lemma}
...
%We may restrict $a$ further so that $a\le 3.8$.

%If $g$ has a local minimum at $s=0$, then
The function
\begin{equation}\label{eqn:g''}
G(a) = (16-a^2)^2a^2(\Delta g'(a)^2 - 0.01\Delta^{3/2} g''(a))
\end{equation}
...
polynomial and has isolated roots.  A 
\cc{UPONLFY}{   %old: 2065952723}{
This is a
  calculation of derivatives.  There are degeneracies when $a$ approaches
  $4$ that are treated in the same way as the degeneracies in
  Lemma~\ref{lemma:hexagon-free}.}  shows that $G(a)$ is nonnegative
for all parameters $b,c,e_1,e_2,e_3$ in the domain.  It follows from
this inequality that every interior critical point of $g$ is a local
maximum.  Indeed, the dominant term
%\footnote{We may exclude the point $a=4$, because at this endpoint of
%  the domain, we have $\Delta=-4 (b^2+c^2-16)^2\le0$, 
%which is contrary to assumption.}
of $G$ near an interior critical point is 
\[
-0.01(16-a^2)^2a^2\Delta^{3/2}g''(a)
\] 
so that
...
\begin{lemma}[minimal~node]\guid{PZFSEVR}\rating{200}\label{l:mn}
Every minimal fan $(V,E,F,S)$ has the feature \case{minimal~node~\ref{l:f}}.
\end{lemma}
...
\begin{lemma}[minimal~node~flat]\guid{JLXMEJN}\rating{350}\label{l:mnf}
Every minimal fan $(V,E,F,S)$ has the feature \case{minimal~node~flat~\ref{l:f}}.
...
\begin{lemma}[balance]\guid{TPKKQOL}\rating{350}\label{l:b}
Every minimal fan $(V,E,F,S)$ has the feature \case{balance~\ref{l:f}}.
...
\begin{lemma}[flat~middle]\guid{CFJSRQH}\rating{450}\label{l:fm}
Every minimal fan $(V,E,F,S)$ has property
\case{flat~middle~\ref{l:f}} at every $\v\in V$.
\end{lemma}
...
2\arc(2\hm,2,2\hm)+\arc(2,2,2\hm) > \pi.
\end{equation}
This is not generic.
%\begin{equation}\label{eqn:3side}
...
%\begin{lemma}[flat~extremal]\guid{OUCPLRI}\label{l:fext}%\rating{1600}
%(deprecated on May 11, 2011)
%Every minimal fan $(V,E,F,S)$ has the feature \case{flat~extremal~\ref{l:f}}.
%\end{lemma}
...
%A \cc{OUCPLRI}{This is a second derivative calculation.} shows that the second derivative of each
...
\begin{lemma}[s~minimal]\guid{HJXLQOT}\rating{100} \label{l:sm}
Every minimal fan $(V,E,F,S)$ has the feature \case{s~minimal~\ref{l:f}}.
\end{lemma}
...
\begin{lemma}[triangle~free]\guid{OMKYNLT}\rating{200}\label{lemma:triangle-free}\label{l:tf}
Every minimal fan $(V,E,F,S)$ has the feature \case{triangle~free~\ref{l:f}}.
\end{lemma}
...
$S$-minimality.  A \cc{OMKYNLT}{This is the triangle case of the main inequality.} shows that
...
\begin{lemma}[flat~exists]\guid{CMBZAOZ}\rating{500}\label{lemma:flat-exists}\label{l:fe}
Every minimal fan $(V,E,F,S)$ has the feature \case{flat~exists~\ref{l:f}}.
...
By property \case{extreme~edge}, each edge $\norm{\v_i}{\v_{i+1}}$ is
$2$ or $2\hm$. Set $y_i = \normo{\v_i}$ and $y_{ij} =
\norm{\v_i}{\v_j}$.
We may also
...
by a \cc{2986512815}{}, the function $\tau$ as a function of
...
\begin{lemma}[quadrilateral~free]\guid{JNTEFVP}\rating{550}\label{lemma:quadrilateral-free}\label{l:qf}
Every minimal fan $(V,E,F,S)$ has the feature \case{quadrilateral~free~\ref{l:f}}.
\end{lemma}
...
$\tau(V,E,F) \ge d(4,0)$ is established by a \cc{JNTEFVP}{There is a
  diagonal of length at most $\sqrt8$.}.  The space of configurations
is five-dimensional.

By property \case{flat~count}, one node is flat.  The parameter $s$
runs over three cases $s=0,1,2$.  A \cc{JNTEFVP}{There is a diagonal
  of length at most $\sqrt8$.}  gives $\tau(V,E,F)\ge d(4-s,s)$ in
each case.  The space of configurations is four-dimensional.
\end{proof}

\begin{lemma}[pentagon~free]\guid{PQFYWHW}\rating{650}\label{lemma:pentagon-free}\label{l:pf}
Every minimal fan $(V,E,F,S)$ has the feature \case{pentagon~free~\ref{l:f}}.
\end{lemma}
...
may be $0$ or $1$.  A \cc{PQFYWHW}{} in each of
these cases gives $\tau(V,E,F)\ge d(5-s,s)$.  The space of
configurations is five-dimensional when the flat nodes are not
adjacent.  
%Because of \case{flat~extremal}, the space of
%configurations is only four-dimensional when the flat nodes are
%adjacent.
\end{proof}

\begin{lemma}[hexagon~free]\guid{GYQVFXJ}\rating{750}
\label{lemma:hexagon-free}\label{l:hf}
Every minimal fan $(V,E,F,S)$ has the feature \case{hexagon~free~\ref{l:f}}.
\end{lemma}
...
\end{enumerate}\wasitemize 
A \cc{GYQVFXJ}{} in each case give the inequality
...
are seven, seven, six, and five respectively.
  ...
  the dihedral angle formula can contain a term $0/0$.  The cited source gives further
details about how to estimate hexagons in a small neighborhood of these nongeneric
fans.
\end{proof}  
...
\begin{lemma}[emptiness]\guid{LPQUDGF}\rating{50}\label{lemma:min-empty}  
...
%\ifverbose{\footnote{Praise to the emptiness that blanks out existence. --Rumi} %-- Rumi % The essential Rumi page 21.
%\indy{Index}{Rumi}%
%}
\end{proof}

The following secondary estimates are needed in 
the linear programs that appear in Section~\ref{sec:tsp}.
The following is a variant of the definition of special fan
in which there there are two significant lengths ($2\hm$ and $\sqrt8$) rather than the single length $2\hm$.  Note in particular that every diagonal of a secondary fan has length
at least $\sqrt8$.

\begin{definition}[secondary fan]\guid{TYCXTDF}
A \newterm{secondary fan} is a tuple 
\[(V,E,F,S_1,S_2),\] where
\begin{enumerate}
\item \case{packing} $V$ is a packing.  That is, for every $\v,\w\in
V$, if $\norm{\v}{\w}<2$, then $\v=\w$.
\item \case{annulus} $V\subset \BB$.
\item \case{local~fan} $(V,E,F)$ is a nonreflexive local fan (Definition~\ref{def:convex-local}).
\item \case{subset} $S_1\subset E$, $S_2\subset E$, and $S_1\cap S_2=\emptyset$.
\item \case{s$_1$~norm} If $\{\v,\w\}\in S_1$, then $2\hm\le\norm{\v}{\w}\le \sqrt8$.
\item \case{s$_2$~norm} If $\{\v,\w\}\in S_2$, then $\norm{\v}{\w}=\sqrt8$.
\item \case{e~norm} If $\{\v,\w\}\in E\setminus (S_1\cup S_2)$, then $\norm{\v}{\w}\le 2\hm$.
\item \case{diagonal} For all distinct elements $\v,\w\in V$, if
$\{\v,\w\}\not\in E$, then \[ \norm{\v}{\w}\ge
\sqrt8.\] 
\item \case{card} %$k=\card(F)$,
Let      $s_1=\card(S_1)$, $s_2=\card(S_2)$, and $r=\card(E) - s_1-s_2$.  Then
\[ 0\le s_1\le 1,~~ 0\le s_2 \le 2,\textand 3-(s_1+s_2) \le r \le 5 -
2(s_1+s_2).\] 
\end{enumerate}
The constants $r$, $s_1$, and $s_2$ are called the \newterm{parameters} of the secondary fan.
\end{definition}


\begin{lemma}[secondary estimate]\guid{ZHPXLTX}
Let $(V,E,F,S_1,S_2)$ be a secondary fan with parameters
$(r,s_1,s_2)=(5,0,0)$.  Then
\[\tau(V,E,F)\ge 0.696.\] % was 0.751.\]
Let $(V,E,F,S_1,S_2)$ be a secondary fan with parameters
$(r,s_1,s_2)=(3,1,0)$.  Then
\[\tau(V,E,F)\ge 0.477.\] % was 0.492.\]
\end{lemma}

\begin{proof}
The proof of the secondary estimate follows exactly the same lines of reasoning as the main estimate.  The following 
constants are used in the proof:
% XX use second batch:
\begin{align*}
d'(5,0,0) &=0.696\\ % was  0.751~
d'(3,1,0) &=0.477\\ % was  0.492~
d'(3,0,1) &=0.568\\ % was  0.623~
d'(2,0,1) &=0.128\\ % was  0.128~
d'(1,1,1) &=0.349\\ % was  0.364~
d'(1,0,2) &=0.440. % was  0.495~
\end{align*}
The constants are additive:
\begin{align*}
d'(5,0,0) &= d'(3,0,1) + d'(2,0,1)\\
d'(3,0,1) &= d'(1,0,2) + d'(2,0,1)\\
d'(3,1,0) &= d'(1,1,1) + d'(2,0,1).
\end{align*}
By considering a minimal counterexample, as in the case of special fans, the proof reduces to a secondary fan with the following parameters $(r,s_1,s_2)$ and constraints:
\begin{enumerate}
\item parameters $(1,0,2)$,
\item parameters $(2,0,1)$,
\item parameters $(1,1,1)$,
\item parameters $(5,0,0)$ and
two flat nodes (possibly adjacent),
\item parameters $(4,1,0)$
and one flat node.
\end{enumerate}
These cases have been checked by a \cc{ZHPXLTX}{}.
\end{proof}
...
The next chapter completes the proof of the Kepler conjecture.  The
following definitions are the only ones needed in the next chapter for
that proof: nonreflexive local fan $(V,E,F)$, special fan $(V,E,F,S)$, the
parameters $(r,s)$ of a special fan, $d(r,s)$, $\tau(V,E,F)$, and
perimeter.  Here $(V,E)$ is a fan, $F$ is a face of the hypermap of
$(V,E)$, and $S$ is a subset of $E$.  Also, $(r,s)$ is an ordered pair
of natural numbers, and $d(r,s)$ is given as a table of real numbers.
The function $\tau$ measures the looseness of a packing $V$ around a
face $F$.  The perimeter is a real number associated with a nonreflexive local fan $(V,E,F)$.
...
the upper bound of $2\pi$ for the perimeter of a geodesically convex spherical polygon.  The main estimate is $\tau(V,E,F)\ge d(r,s)$ for
