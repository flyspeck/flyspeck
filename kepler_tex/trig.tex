...
\indy{Index}{cosine!derivative}
    ...
    \frac{d \cos(x)}{dx} = -\sin(x),\qquad \frac{ d \sin(x)}{dx} = \cos(x).
    \end{equation}
...
\indy{Notation}{r@$r$ (polar coordinate)}
\indy{Notation}{xy@$(x,y)$ (Cartesian point)}
\indy{Notation}{ZZphi@$\phi$ (angle)}
...
This section reviews vector geometry in $\ring{R}^N$, including products (cross and dot), inequalities (triangle and Cauchy-Schwarz), and hulls (convex and affine).
...
\indy{Notation}{dotproduct@$u\cdot v$ (dot product)}
...
indicate the semialgebraic conditions of strict $(<)$ and weak $(\le)$ inequality.
...
\begin{definition}[collinear]  A set $S$ is {\it collinear} if there exists
...
$\{u,v,w\}$ not collinear, are called {\it half-planes}.
\end{definition}
\indy{Index}{plane}
\indy{Index}{half-plane}
\indy{Notation}{A@$A$ (plane)}

\begin{definition}[coplanar] A set $S$ is  {\it coplanar} if there exists
a plane that contains every point of $S$.
\indy{Index}{coplanar}
\end{definition}

\begin{definition}[half space] A set $\op{aff}_{\pm}(\{u,v,w\},\{v'\})$,
with $\{u,v,w,v'\}$ not coplanar, is called a {\it half-space}.  Under the substitution of
...
Following this brief digression into vector geometry, this section
...
so forth.  By the case of equality in Cauchy-Schwarz
...
\end{lemma}
\indy{Notation}{Zgamma@$\gamma$!angle}
\indy{Index}{arc}
...
Similarly, the angle does not depend on the listed order of the two half-planes.  The definition projects to a plane orthogonal to the line, carrying the half-planes to rays meeting at a point, and the identifies the dihedral angle with the angle between the rays.
...
The spherical law of cosines is the most fundamental identity of spherical trigonometry.   As a matter of fact, although  spherical
...
are related to those $a',b',\ldots$ of $A'B'C'$ by
   ...
   $$
and so forth.
\indy{Notation}{ABC@$ABC$ (triangle)}}  
One
...
\indy{Notation}{p@$p$ (trig expression)}
...
To check that points on different Lexell circles give different solid angles,  any convenient point on the circle will do.  For example, there is an isosceles triangle $b=c$.  Under this specialization, the left-hand-side of equation \eqn{eqn:Deltap} has derivative
$$
...
\indy{Notation}{G@$G$ (great circle)}
...
\indy{Index}{Girard's formula}
\indy{Notation}{A@$A$ (triangle area)}
\indy{Notation}{ZZalpha@$\alpha$ (angle)}
\indy{Notation}{ZZbeta@$\beta$ (angle)}
\indy{Notation}{ZZgamma@$\gamma$ (angle)}
...
$0\le\theta < 2\pi$.
\indy{Notation}{r@$r$!polar coordinate}
\indy{Notation}{theta@$\theta$!polar coordinate}
...
\indy{Notation}{r@$r$!spherical coordinate}
\indy{Notation}{ZZtheta@$\theta$!azimuth angle}
\indy{Notation}{ZZphi@$\phi$!zenith angle}
...
\indy{Index}{coordinate systems!polar coordinates}
\indy{Index}{polar coordinates}
\indy{Notation}{r@$r$ (polar coordinates)}
\indy{Notation}{ZZtheta@$\theta$ (polar coordinates)}
