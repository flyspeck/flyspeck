...
OMDoc specification in this ontology is not totally complete, though most
...
\begin{figure}
  ...
  \begin{tikzpicture}[set style={{default}+=[xscale=.5,yscale=.47,font=\normalsize\sffamily]},default]
    ...
    \draw[-open triangle 60] (d) -- node[left=1ex] {$\sqsubseteq$} (s);
    \draw[-open triangle 60] (a) -- (s);
    \draw[-open triangle 60] (p) -- node[right=2ex] {$\sqsubseteq$} (s);
