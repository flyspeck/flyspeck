...
% watch out for variable overwrite with macros, as in setellipseplane.  It overwrites \vx,\vy, creates havoc, etc.
...
\def\figHFFTUNW{
\tikzfig{fig:rogers-omega}{\guid{HFFTUNW} The points $\omega_j$ are constructed
...
\def\figBUGZBTW{
\tikzfig{fig:rogers-random}{\guid{BUGZBTW} The Rogers partition of a packing.
...
\def\figNOCHOTB{
\tikzfig{fig:rogers-sqrt2}{\guid{NOCHOTB} There are eight two-dimensional Rogers simplices
of diameter $\sqrt2$
that meet at the center of a square.
Whenever the diameter of the Rogers simplex
is less than $\sqrt2$, it is one of exactly six that meet at an extreme point of a Voronoi cell.
...
\def\figYAJOTSL{
\tikzfig{fig:rogers-fact}{\guid{YAJOTSL} In dimension $k$, the union of $(k+1)!$ 
...
\def\figBWEYURN{
\tikzfig{fig:marchal-2d}{\guid{BWEYURN} The Marchal partition.
