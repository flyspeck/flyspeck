...
$M_2(h)\ge M(h)$ except when $h\in [h_-,h_+]$.  The aim of this section is to prove a variant of Theorem~\ref{theorem:mk} that uses the function $M_2$ rather than $M$.  For this, we need to combine cells into larger groups, called supercells.
...
\begin{theorem}[supercell estimate]\label{lemma:superineq} 
...
by inequality (\ref{eqn:vole}).

In supercells, we replace  with Theorem~\ref{lemma:superineq}.
...
\begin{corollary} Conjecture~\ref{conj:L12} implies the
Kepler conjecture.
\end{corollary}
...
We call a $4$-cell a {\it quarter}, when it has exactly one
...
angle\footnote{XX} at most $2.8$.  Assume a quarter with $\gamma_2(X) <0$;
...
the convex polyhedron (Lemma~\ref{XX}).  By the Lemma~\ref{lemma:dart-upper}, 
$\sum_i n_i \le (6N-12)$.
