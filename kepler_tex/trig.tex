...
basic topology.  It has the definitions of trigonometric
...
While the proof assistant had adequate command of general concepts of
...
trigonometry and analytic geometry than the proof assistant, which, after all, is still
in its teens.
...
The cosine and sine functions are defined\footnote{This is how the the trigonometric functions
...
theorem \cite{unknown}.  The cosine and sine are now defined in the system as the real an complex
...
two functions are necessarily equal.  This observation is the starting
point of a powerful method to prove many functional identities,
including trigonometric identities.  This method can be developed
...
The next two lemmas take this approach to trigonometric identity
proving.  We do not strive to give a fully automated proof.  We are
content to certify a trigonometric identity with a function $f$ that
...
\begin{tidbit}
Incidentally, this trigonometric identity recently tried to crash through the gates of physics.
Two robotics experts, Schmidt and Lipson,  wrote a computer program that
automatically discovers Hamiltons and Lagrangians from
raw experimental data.  Discover magazine reported that this program can 
discover the same laws in hours that Newton took decades to find~\cite{discover-2009}.  
However, one of the primary challenges of their project was to keep out purely mathematical
identities such as $\sin^2(x)+\cos^2(x)=1$, which may try to pass as
a  conservation law with physical significance~\cite{lipson}.
\end{tidbit}
...
the function $\atn$.   It is the angular argument of $(x,y)$ with the
branch cut along the negative axis.  That is, $x = r\cos\theta$,
$y=r\sin\theta$, for some $r\ge0$, where $\theta=\atn(x,y)$.  This definition avoids all
the messy cases of Definition~\ref{def:atn}.
...
because its domain is the entire field of real numbers, its range is bounded, and
its derivative is a rational function.  
Wherever angles appear in this book, the arctangent is apt to appear as well.

Other inverse trigonometric functions will generally be reduced to the
arctangent.  
This section defines the $\arccos$ function and show how
...
\begin{proved}
The quick and sloppy justification is simply that 
...
\begin{eqnarray}\label{eqn:dot-law}
...
$n+1$ points in $\ring{R}^n$ is based on the same method as the $3$ by
$3$ case; the identity~\eqn{eqn:dot-law} gives a rewrite rule for each
...
cosines, and a beautiful formula due to Euler and Lagrange for the area of a
spherical triangle.
...
\begin{tidbit}
  The formalist versus the intuitive geometer.  I recall the argument
  I had as a young student with my analytic geometry teacher over this
  definition of angle.  I had an intuitive notion of angle and wanted
  to know that the definition conformed to my notions.  My teacher
  insisted that mathematicians are under no obligation to justify a
  definition.  Mathematical consistency suffices.  If I go back to put
  my schoolboy intuition into words, I wanted to know that angle is
  invariant under isometries of $\ring{R}^n$, and that when $\u,\v,\w$
  are mapped by an isometry into a fixed plane, this definition of
  angle in $\ring{R}^n$ agrees with the definition I already knew for
  the angle in a plane.  Of course, that is precisely what this
  definition accomplishes, even if I did not have the words then
  to express it.  I accepted the norm (Definition~\ref{def:norm})
  without objection, because it made sense to me as successive
  applications of the Pythagorean theorem in the plane:
\[
\normo{\u}^2 = u_0^2 + v_0^2, ~~ v_0^2 = u_1^2 + v_1^2, \ldots,
~~ v_{n-2}^2 = u_{n-1}^2 + v_{n-1}^2,\qquad v_{n-1}=0.
\]
\end{tidbit}
...
triangle.  Because of formula \eqn{eqn:ups}, Heron's classical formula
...
determined by two points $\{\v_0,\v_1\}$ on the delimiting line and
two points $\v_2$ and $\v_3$ on the two half-planes.
...
The dihedral angle is degenerate and will not be used when $\w_1 =
...
\begin{proof} The proof is an exercise based on previously established
  ...
  $\w_2/\normo{\w_2}$, $\w_3/\normo{\w_3}$, $\w_1/\normo{\w_1}$,
  respectively.  Hence assume without loss of generality that
  $\normo{\w_2}=\normo{\w_3}=\normo{\w_1}=1$.
...
The expression $\alpha_1+\alpha_2+\alpha_3-\pi$ is \newterm{Girard's
  formula} (known first to T. Harriot) for the area of a spherical
triangle with angles $\alpha_1$, $\alpha_2$, $\alpha_3$.  We return to
this formula in the next chapter~\eqn{eqn:girard}, when area and
volume of treated.  The following lemma gives an alternative formula
for the area, discovered by Euler and Lagrange.  We give the proof
here because the formula is a purely trigonometric identity that is
 proved without any reference its interpretation as an area.
% \indy{Index}{Girard, A.}%
...
why there should exist a formula of this general form.  
...
In practice, it is easier to carry out the details of the proof by a slightly
different strategy.  We can check that
the derivatives of the two sides of the identity are equal as rational functions.
The domain is connected, and from this it follows that
the two sides differ by at most a constant.  By calculating a particular test value, we
see that the two sides are precisely equal.
...
\begin{proof} It follows by basic vector arithmetic that
  ...
  $\{\v_0,\v_1,\v_2\}$.  The choices vectors $\e_1$ and $\e_2$ are
...
We break with tradition by making $h$ the coefficient of the frame
vector $\e_1$ (rather than $\e_3$).  We make a similar break with
tradition in the definition of spherical coordinates.  The reason for
this can be traced to the ordering of the arguments in the definition
of dihedral angle.  We have found it useful to group the arguments in
pairs $\dih_V(\{\v_0,\v_1\},\{\v_2,\v_3\})$ to emphasize that the
dihedral angle is invariant under the symmetries
$\v_0\leftrightarrow\v_1$ and $\v_2\leftrightarrow\v_3$.  By pairing
the arguments in this way, the axis along which the dihedral angle is
measured is the line $\op{aff}\{\v_0,\v_1\}$.  For internal
consistency throughout the book, the directional axis of the dihedral
angle, $\w_1 = \v_1-\v_0$, gives the axis $\e_1$ of the cylindrical
coordinates.  \indy{Index}{coordinate systems}%
...
is the angle between $\bar{\w_2}$ and $\bar{\w_3}$.  Compute
\begin{displaymath}
\begin{array}{lll}
\bar{\w_2} &= \normo{\bar{\w_2}} \e_2 \\
...
\end{array}
\end{displaymath}
Since $\sin\phi$, $r$, and $r'$ are non-negative, this implies that $r'=r\sin\phi$, as
desired.
\end{proof}
...
Formal proofs have been constructed of all of the major results in this chapter.  In fact,
many of the formal proofs of results that appear early in the chapter are part of standard distribution
of the {\tt HOL Light} proof assistant.
Jason Rute, during the summer before starting graduate work at Carnegie Mellon University in 2008,
constructed the formal proof of some of the theorems in this chapter.  Most of the remaining work
was carried out by Nguyen Quang Truong in 2009.    He also constructed the formal proofs of
a large collection of lemmas in elementary geometry, including basic facts about the functions
$\ups$ and $\Delta$.
Euler's theorem was formized by Vu Khac Ky and
Trieu Thi Diep.
...
$\ups$ and $\Delta$ are polynomials of three and six variables,
respectively.  They appear repeatedly in formulas for angle, area, and
volume.  The function $\arc_V(\u,\{\v,\w\})$ gives the angle at point
$\u$ of a triangle with vertices $\u,\v,\w$.  The function
$\arc(a,b,c)$ is the angle opposite $c$ of a triangle with sides
$a,b,c$.  The function $\dih_V$ is the dihedral angle of a simplex,
expressed as a function of its four vertices.  The function $\dih$ is
the dihedral angle of a simplex, expressed as a function of its six
edges.

The cylindrical coordinates of a point in $\ring{R}^3$ are
$(r,\theta,h)$.  The spherical coordinates are $(r,\theta,\phi)$.  The
angle $\theta$ is called the azimuth angle.  It is determined by four
