...
\begin{lemma}\label{lemma:affhull-center}
...
\item%[(distinctness)]  
The points $\omega(\trunc{\bu}{j})$, for
...
\omega(\trunc{\bu}{i+1})$.  Otherwise, equality holds, and the
...
\claim{The sequence $h(\trunc{\bu}{j})$ is
increasing in $j$.}  Indeed, this has just been established as part of the previous claim.
\end{proof}
...
Let $\op{Sym}(k+1)$ be the \newterm{group} of all permutations on
$k+1$ letters.  Let $\bu = ( \u_0,\ldots, \u_k)\in \bV(k)$.  For any
...
$r^2$. Thus,
\begin{displaymath}
T_1'= \sum_{ \u\in  V(\p,r)} \op{vol}(\Omega(V, \u)) 
\ge \sum_{X\subset B(\p,r)} \op{vol}(X) + c_1 r^2 = T_1 + c_1 r^2.
\end{displaymath}
...
Hence
\begin{displaymath}
T_2' = -\sum_{ V(\p,r)} 8 m_1 = 
-\sum_{X\subset B(\p,r)}\left(\frac{2m_1}\pi\right) \op{tsol(X)} + c_2 r^2 = T_2 + c_2 r^2.
\end{displaymath}
