...
\begin{calculation}\label{calc:marchal}\guid{HJKDESR}\guid{1025009205}\guid{3564312720}\rating{ZZ}
% old \guid{WJDLOCM}
% 
...
\begin{calculation}\label{calc:cc:disks}\guid{4198769118}\rating{ZZ} 
% was guid 8550443271
Let
\[ 
g(h) = \arccos(h/2) - \pi/6.
...
The local inequality \guid{PWVDMPT} has separate cases for $D_k$-cells for
$k=1,2,3,$.
...
The $D_3$-cell (ID-6938212390) has domain $y_i\in[2,\sqrt8]$ and
truncation $r=\sqrt2$, but the result holds for much smaller
truncation.  We can restrict to $y_i\in [2,2.7]$ by decreasing the
truncation to $1.35$ and breaking the $D_3$-cell into $1$-, $2$-cells.

In the $D_4$-cell inequality (ID-9627800748), the domain is
$y_i\in[2,\sqrt8]$, but we can restrict to $y_i\in[2,2.7]$ as follows.
If some edge has length greater than $2.7$, the simplex has
circumradius at least $1.35$.  Truncate at $1.35$, breaking the
