...
The cosine and sine functions are defined\footnote{This is how the the
  trigonometric functions were originally defined in the proof
  assistant {\tt HOL Light}.  More recently, complex analysis has been
  developed in {\tt HOL Light} sufficient for the analytic proof of
  ...
  the exponential function $e^{i x}$.  To simplify the exposition, we
  present the original definitions.} by their infinite series:%
...
The function $\tan$ is strictly increasing and one-one on the domain
...
$\tan$ is \hyperref[lemma:tan-monotone]{one-one}:
\begin{displaymath}
...
According to the formalist,
  the a definition requires no justification.  Mathematical
  consistency suffices.  As Hilbert famously said,
``One must be able to say at all times -- instead of points, straight lines, and planes -- tables, beer mugs, and chairs.''
The intuitive geometer asks for more:
does the definition conform to
  intuitive notions of angle?  To put raw intuition into words,
  angle should be invariant under isometries of $\ring{R}^n$, and when
  $\u,\v,\w$ are mapped by an isometry into a fixed plane, this
  definition of angle in $\ring{R}^n$ should agree with the accepted
  definition in the plane.   That is precisely what this definition
  accomplishes.  The norm (Definition~\ref{def:norm}) also extends the
  intuition of the plane, obtained as it is by successive applications
  of the Pythagorean theorem in two dimensions.  
\[
...
that the projection to the plane is one-one.  The following
