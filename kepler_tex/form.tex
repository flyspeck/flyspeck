...
Fix $2\le r\le R$.
Let $(\Lambda,\mu)$ be a pair where 
\begin{itemize}
  \item $\Lam$ is a finite packing, 
  \item $0,\mu\in\Lam$,
  \item $\normo{\lam}\le R$ for all $\lam\in\Lam$,
  \item $\norm{\lam}{\mu}\ge r$ for all $\mu\ne\lam\in\Lam$.
\end{itemize}
Suppose the set is saturated in the sense that there does not exist $\lam\not\in\Lambda$ such that $(\Lam\cup\{\lam\},\mu)$ satisfies these same properties.  Let $\Lam^*=\Lam\setminus\{0\}$.
...
$\normo{p} > c(\lam) R/2$ for all $\lam\in\Lam^*$.  Let $q =  R p/\normo{p}$.
Then $R=\normo{q}$.  By the saturation hypothesis, there exists $\lam\in\Lam^*$ such that $\norm{q}{\lam}<r$.    Then,
$$
\begin{array}{lll}
