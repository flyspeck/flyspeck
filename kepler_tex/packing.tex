...
%   $\Omega(\v)$ is used, rather than $\Omega(\v)$.  The truncation at
...
\begin{proof} 
Since the sets $\Omega(\bu[j])$ satisfy \eqn{eqn:omega-dim}, it
...
\begin{proof} 
It is enough to show that $G( \u_0) = -\op{vol}(\Omega( \u_0)) + 8
...
than $\sqrt2$.  Call such faces \newterm{blades}.  (There is no
relationship between the blades here and those of
Chapter~\ref{sec:fan}.)  \indy{Index}{edge!spine}%
...
blades along the critical edge.

\claim{It suffices to prove Lemma~\ref{lemma:cluster} when the cluster
contains at least one quarter and at least two blades.} Indeed, by
...
quarter is flanked by two blades.
...
consecutive blades.  $B$ is either a singleton set containing a
$4$-cell, or a set of three cells: a $2$-cell and two adjacent
$3$-cells.  Write $\op{azim}(B)$ for the azimuth angle formed by the
two blades.  By Calculation~\ref{calc:cc:5bl}, the cells between two
consecutive blades satisfy an
inequality: % five or more blades along a spine are found
% at~[ZTGIJCF].  % cc:5bl:
...
In the case of three and four blades, the proof relies to an even
...
contains the number of blades, variables representing the edges and
the azimuth angles between consecutive blades.
...
partition $D$ into subsets $D_i$: Are there three blades or four?  Is
...
$\gamma_L$ on a given quarter negative? Does a particular blade have a
...
but one case.  This is the case of four blades, three quarters, and
...
= y_1(X_k)$ for all $j,k\in\{1,2,3,4\}$.  Finally, each blade flanks
two different simplices; the edge lengths of the blade must agree.
...
A four-bladed cluster is an octahedron.  The set of octahedra is a
...
a $6$-dimensional object, parameterized by $6$ edges.  The four-bladed
