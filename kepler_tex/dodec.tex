...
% 50 page limit
% 13 should not be a running assumption. Add it as a special assumption where needed.
...
by placing a point of $\Lambda$ at the center of each face of a regular dodecahedron (of inradius $2$).
...
Lower bounds on the volume $X$ of a Voronoi cell can be expressed
as upper bounds on the density $4\pi M/(3X)$ of packings of congruent balls
...
of the Dodecahedral conjecture, this article points  out
...
point in this proof, a somewhat related collection of graph
appears.
...
Some terms have slightly redefined, when doing so is harmless:
{\it distinguished}.
...
is a simple polygon.  This again involves constructing an auxiliary 
...
Inductively, build a set of distinguished edges as follows.
...
Then
$$
w(y) + \sum_{x\in X} w(x) \ge t_m  + m' 0.016.
$$
...
hold.
\begin{enumerate}
\item $H$ is a plain, planar hypermap.
...
verification that $w$ is a Voronoi weight, breaks into separate lemmas for each properties in the definition.  The article \cite{arx} devotes
...
\mu(\Lambda,U_F) + \sum_{F'\in X} \mu(\Lambda,U_{F'} \ge t_m  + m' 0.016.
\end{equation}
\item If the node of $x$ has type $(p,q,0)$ and degree $m=p+q$, then
