...
\begin{figure}[htb]
  \centering
  \myincludegraphics{\pdfp/dart.eps}
...
\begin{figure}[htb]
  \centering
  \myincludegraphics{\pdfp/hypermap-ex.eps}
...
\begin{figure}[htb]
  \centering
  \myincludegraphics{\pdfp/dart-fix.eps}
...
\begin{figure}[htb]
  \centering
  \myincludegraphics{\pdfp/walk.eps}
...
\begin{figure}[htb]
  \centering
  \myincludegraphics{\pdfp/walkdegen.eps}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=120mm]{\pdfp/double-node-walkup.eps}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=120mm]{\pdfp/double-face-walkup.eps}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=100mm]{\pdfp/double-edge-walkup.eps}
...
\begin{figure}[htb]
  \centering
  \myincludegraphics{\pdfp/double_edge.eps}
...
\begin{figure}[htb]
  \centering
  \myincludegraphics{\pdfp/mobius.eps}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=80mm]{\pdfp/violate-jct.eps}
...
In this subsection, we assume that $H$ is a hypermap with restrictive properties.
...
This section describes the transition from $H/{\cal L}$ to $H/{\cal M}$.  The algorithm carries along various auxiliary data satisfying various assumptions, wich we now enumerate.

\begin{remark}[context]\label{enum:context}
\begin{enumerate}
\item Assume $H=(D,e,n,f)$ is a restricted hypermap with normal family $\cal L$.
...
\item Assume $x'\in F(L)$ is a quotient dart such that $e'x'$ lies in a true face.  Write $x' = [\ldots;x]$, for some $x\in D$. 
\item As the face $F(L)$ is false, there is a smallest $m\ge0$ such that $f^{m+1} x$ is followed by an $n^{-1}$ step. Let $y = f^{m+1} x$.  Let $S = S(x,{\cal L}) = \{f^i x \mid 1 \le i\le m\}$.   The set $S$ maps bijectively to a set of darts in the quotient, which we also denote by $S$. 
\item Assume that the canonical boolean function on $H'$ is an $S$-flag.
\item There is a smallest $p\ge0$, such that $f^{p+1} y$ belongs to some loop of ${\cal L}$.  Set $z=f^{p+1} y$.  
\end{enumerate}
\end{remark}
...
\begin{lemma}\guid{HQYMRTX}\rating{200}  $z$ is on the contour loop $L$ and is not equal to $f^k x$, for $0 < k \le {m+1}$.
\end{lemma}
...
{\bf Case 1: $e'z'$ lies in a true face.}  The dart $f e x$ is interior, true, and lies at the same node as $x$.  By the definition of $S$-flag there is a path from $f e x$ to $e' z'$.  However, $e'z'$ is interior, and this contradicts Lemma~\ref{lemma:contour-interior-exterior}.
...
\begin{figure}[htb]
  \centering
  \myincludegraphics{\pdfp/L1L2.eps}
...
${\cal M}$ is a normal family, the noncanonical function $\varphi$ is an $S_{\cal M}$-flag, and the assumptions on the context are all satisfied (with respect to  $\varphi$).
\end{lemma}

\begin{proof} 
The new nodes visited on the subpath of $L_1$ from $y$ to $z$ do not contain any darts in any loop in ${\cal L}$.  (By the definition of normal family, every dart at a node or no dart at a node lies along some loop of ${\cal L}$.)  It follows that we obtain a normal family ${\cal M}$  by eliminating $L$ and replacing it with $L_1$ and $L_2$.
...
Finally, every loop visits at least two nodes.  This is true for $L_1$ and $L_2$ because they visit the nodes of $y$ and $z$.  It is true of the other loops because they belong to the normal family ${\cal L}$.

Next we show that the function $\varphi$ is an $S_{\cal M}$-flag. The contour loops and values of the canonical function do not change on ${\cal L}\setminus \{L\}$.  As $L$ corresponds to a false face, the two defining properties of a flag hold for ${\cal L}\setminus \{L\}$.  The faces $L_1,L_2$ might be either true or false.  Each has an edge shared with a true face.  Also, the darts in each can follow the loop ($L_1$ or $L_2$) until reaching that edge, so if the face is true, its darts can be connected to any other true dart.  Thus, the canonical function is a flag.

To check simplicity of the quotient, we must check that each of the contour paths in ${\cal M}$ never returns to a node after leaving it.  This is true of ${\cal L}\setminus\{L\}$ by assumption and true of $L_1$ and $L_2$ by construction.
\end{proof}


We have various possibilities for $H/{\cal M} = (D_{\cal M},e_{\cal M},\ldots)$.
\begin{enumerate}
\item  The canonical function might be true on every face.  By Lemma~\ref{XX}, the quotient map $H\to H/{\cal M}$ is an isomorphism.  The iteration terminates.
...
Repeating this construction, starting with $H/{\cal M}$ and using the same initial dart $x$, with each successive iteration, the natural number $m$ increases, until eventually, $x$ lies on a true face.  When this occurs, the number of true faces increases by one.  By Lemma~\ref{lemma:all-dart}, the process terminates when the number of true faces is sufficiently large, and the quotient $H/{\cal L}$ becomes isomorphic to $H$.
...
\item ~[new edge] Write $y=f^{m+1} x$ where $m\ge0$ is as small as possible.  Pick $z$ on the same false face, where $z\ne f^k x$, $k\ne 1,\ldots,{m+1}$.  For each such $z$ and for each $p=0,\ldots,M-\#f$, construct a new edge from $y$ to $z$, with $p$ added nodes of degree $2$.  (The reverse double edge walkup splits the face into two.)  Replace $(x,y)$ with $(x,z)$.  If $z=x$, execute the first step [new true face] before returning the modified hypermap to the list.
\end{itemize}
