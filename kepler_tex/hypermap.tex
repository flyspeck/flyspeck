...
\begin{lemma}\guid{QLPBIKV}\rating{50}\label{lemma:er} The relation on the set of darts $x\sim y$ when there is a path from $x$ to $y$ is an equivalence relation.
...
\begin{lemma}\guid{QZTPGJV}\rating{50} An injective contour path from $x$ to $y$ can be constructed from a contour path from $x$ to $y$ by dropping some darts from the path.
\end{lemma}
...
\begin{lemma}[Knaster-Tarski]\guid{EAOGWLE}\rating{XX}   Let $X$ be a set.  Let $f:P(X)\to P(X)$ be a function from the powerset of $X$ to itself.  Assume that $f$ is monotonic in the sense that whenever $Y\subset Z\subset X$, it follows that
...
\indy{Notation}{P@$P(\cdot)$ (powerset)}
