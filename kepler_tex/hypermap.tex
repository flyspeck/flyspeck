...
three functions $e,n,f:D\to D$ whose composition is the identity:
    $$e\ocirc n\ocirc f = I.$$
The elements of $D$ are called {\it darts}.  The functions $e,n$ and $f$
...
\begin{definition}[plain,~planar] A hypermap is {\it plain} (carefully note the spelling!) when
$e$ is an involution on $D$ (that is, $e^2 = I$).  A hypermap
is {\it planar} (note the spelling!) when the Euler relation holds:
    $$\# n + \# e + \# f = \# D + 2\, \#c.$$
\indy{Index}{plain}%
\indy{Index}{planar}%
\end{definition}
...
A walkup $W_h$ at $x$ {\it merges} when the walkup joins the orbit of $h$ through $x$ with another orbit.  
It {\it splits} when the walkup splits the orbit at $x$ into two orbits.
\indy{Index}{split}%
\indy{Index}{merge}%
\indy{Index}{orbit}%
\end{definition}
...
{\it isomorphic} when there is a bijection $\varphi:D\to D'$ such that
...
In other words, by starting with the {\it seed} hypermaps in $A$ one may find all restricted hypermaps (for given $D$ and $d$) by applying the function $f$ repeatedly:
$$
