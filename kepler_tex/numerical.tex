...
of unity properties
\[
a  =  \sum_{i=1}^n a B_{i,n}(x) \le \sum_{i=1}^n a_i B_{i,n}(x) = p(x).
\]
...
\]
and linear upper bounds to nonpolynomial functions in two variables:
\begin{equation}\label{eqn:sL}
\atn(\sqrt{x},y) < L_i(x,y),
\end{equation}
for all $(x,y)$ in the range of the polynomials $B_i,C_i$, for some
affine functions $L_i(x,y) = r_i x + s_i y + t_i$.  The polynomial
...
the entire rectangular domain, one can recursively cover the domain by
smaller rectangles until two such inequalities can be found on each
smaller rectangle.

The material in this section is based
on the work of R. Zumkeller, who has 
developed Bernstein polynomial methods
in the context of the Flyspeck project~\cite{unknown}, \cite{unknown}.
